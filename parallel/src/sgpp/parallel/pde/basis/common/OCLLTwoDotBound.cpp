...
#include <sgpp/parallel/pde/basis/common/OCLLTwoDotBound.hpp>
...
void CompileLTwoDotBound(int id, std::string kernel_src, cl_kernel* kernel) {
  ...
  for (size_t d_inner = 0; d_inner < dims; d_inner++) {
    ...
    stream_program_src << "element *= (l2dot(\
             ...
             j_levelreg,j_indexreg,j_level_intreg,ptrLcl_q[" << d_inner <<
                       "]));" << std::endl;
  }
  ...
  if (err != CL_SUCCESS) {
    std::cout << "OCL Error: compileLTwoDotBound OpenCL Build Error. Error Code: "
              << err << std::endl;
...
void SetArgumentsLTwoDotBound() {
  ...
  for (unsigned int i = 0; i < num_devices; ++i) {
    ciErrNum |= clSetKernelArg(LTwoDotBoundKernel[i], counter++, sizeof(cl_mem),
...
Timing PrintGFLOPSLTwoDotBound() {
  ...
  if (!isFirstTimeLTwoDotBound) {
    std::cout << "Overall Runtime of AllReduceLTwoDotBound: " <<
              LTwoDotBoundAllReduceTime << std::endl;

    std::cout << "Time for GPU-calculation of LTwoDotBound: " << LTwoDotBoundTime <<
              std::endl;
  ...
  if (!isFirstTimeLTwoDotBound) {
    double LTwoDotBoundGFLOPS = ((numberOfFlopBound * CounterLTwoDotBound) /
                                 LTwoDotBoundTime) * (1e-9);
    double LTwoDotBoundOPS = StorDInnersize * StorDBound * (dimsD * (20.0 + dimsD));
    double LTwoDotBoundGOPS = ((LTwoDotBoundOPS * CounterLTwoDotBound) /
                               LTwoDotBoundTime) * (1e-9);

...
// fermiUseRegister = 1;
  ...
  stream_program_src << "REAL element = alphaTemp["
                     << "k"
                     << "];" << std::endl;
  ...
  if (err != CL_SUCCESS) {
    std::cout << "OCL Error: compileLTwoDotBound OpenCL Build Error. Error Code: " << err
              << std::endl;
...
Timing PrintGFLOPSLTwoDotBound() {
  ...
  if (!isFirstTimeLTwoDotBound) {
    std::cout << "Overall Runtime of AllReduceLTwoDotBound: " << LTwoDotBoundAllReduceTime
              << std::endl;

    std::cout << "Time for GPU-calculation of LTwoDotBound: " << LTwoDotBoundTime << std::endl;
    //        std::cout << "Number of Calls of LTwoDotBound             : " << CounterLTwoDotBound
    //        << std::endl;
  }
  ...
  if (!isFirstTimeLTwoDotBound) {
    double LTwoDotBoundGFLOPS =
        ((numberOfFlopBound * CounterLTwoDotBound) / LTwoDotBoundTime) * (1e-9);

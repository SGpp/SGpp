...
void SetArgumentsLTwoDotLaplaceBound() {
  ...
  for (unsigned int i = 0; i < num_devices; ++i) {
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceBoundKernel[i], counter++, sizeof(cl_mem),
...
Timing PrintGFLOPSLTwoDotLaplaceBound() {
  ...
  if (!isFirstTimeLTwoDotLaplaceBound) {
    std::cout << "Overall Runtime of AllReduceLTwoDotLaplaceBound: "
              << LTwoDotLaplaceBoundAllReduceTime << std::endl;
    std::cout << "Time for GPU-calculation of LTwoDotLaplaceBound: " << LTwoDotLaplaceBoundExecTime
              << std::endl;
//        std::cout << "Number of Calls of LTwoDotLaplaceBound             : " <<
//        CounterLTwoDotLaplaceBound << std::endl;
#ifdef USE_MPI

    for (size_t i = 0; i < MOD; i++) {
      //      std::cout << "Time for GPU-calculation of LTwoDotLaplaceBound["<<i<<"]: "<<
      //      LTwoDotLaplaceBoundAll[i] << std::endl;
    }
  ...
  if (!isFirstTimeLTwoDotLaplaceBound) {
    double LTwoDotLaplaceBoundGFLOPS =
        ((numberOfFlop * CounterLTwoDotLaplaceBound) / LTwoDotLaplaceBoundExecTime) * (1e-9);
    double LTwoDotLaplaceBoundOPS = StorDInnersize * StorDBound * (dimsD * (26.0));
    double LTwoDotLaplaceBoundGOPS =
        ((LTwoDotLaplaceBoundOPS * CounterLTwoDotLaplaceBound) / LTwoDotLaplaceBoundExecTime) *
        (1e-9);
    //    std::cout << "GFLOPS for LTwoDotLaplaceBound : " << LTwoDotLaplaceBoundGFLOPS <<
    //    std::endl;
    return (
        Timing(LTwoDotLaplaceBoundGFLOPS, LTwoDotLaplaceBoundGOPS, LTwoDotLaplaceBoundExecTime));
  }
...
std::string LTwoDotLaplaceBoundHeader() {
  ...
  stream_program_src
      ...
      << std::endl;
  stream_program_src << "			 		__global  REAL* ptrIndex, 	"
                     << std::endl;
  stream_program_src << "					__global  REAL* ptrLevel_int, 	"
                     << std::endl;
  stream_program_src << "				 	__global  REAL* ptrParResult, 	"
                     << std::endl;
  stream_program_src << "			 		__global  REAL* ptrAlpha, 	"
                     << std::endl;
  stream_program_src << "					__constant  REAL* ptrLcl_q,	"
                     << std::endl;
  stream_program_src << "				 	__constant  REAL* ptrLambda, 	"
                     << std::endl;
  stream_program_src << "	    				ulong overallMultOffset,	"
                     << std::endl;
...
void CompileLTwoDotLaplaceBound(int id, std::string kernel_src, cl_kernel* kernel) {
  ...
  if (keplerUseRegister) {
    ...
    for (size_t d_inner = 0; d_inner < dims; d_inner++) {
      stream_program_src << typeREAL2 << "i_level_" << d_inner << " =         ptrLevel["
                         << d_inner * storageInnerSizePaddedBound << "+ (get_global_id(1))*"
                         << LSIZE << "];" << std::endl;
  ...
  typeREAL = "REAL ";
  stream_program_src << typeREAL << "alphaTempReg = alphaTemp["
                     << "k"
                     << "];" << std::endl;
  ...
  if (err != CL_SUCCESS) {
    std::cout << "OCL Error: compileLTwoDotLaplaceBoundKernel OpenCL Build Error. Error Code: "
              << err << std::endl;

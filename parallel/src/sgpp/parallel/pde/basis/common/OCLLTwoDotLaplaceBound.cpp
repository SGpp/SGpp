...
void SetArgumentsLTwoDotLaplaceBound() {
  ...
  for (unsigned int i = 0; i < num_devices; ++i) {
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceBoundKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrLevelBound[i]);
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceBoundKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrLevelIndexLevelintconBound[i]);

    ciErrNum |= clSetKernelArg(LTwoDotLaplaceBoundKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrIndexBound[i]);
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceBoundKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrLevel_intBound[i]);
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceBoundKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrParResultBound[i]);
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceBoundKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrAlphaBound[i]);
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceBoundKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrLcl_qBound[i]);
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceBoundKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrLambdaBound[i]);
...
Timing PrintGFLOPSLTwoDotLaplaceBound() {
  ...
  if (!isFirstTimeLTwoDotLaplaceBound) {
    ...
    std::cout << "Time for GPU-calculation of LTwoDotLaplaceBound: " <<
              LTwoDotLaplaceBoundExecTime << std::endl;
  ...
  double numberOfFlop = StorDInnersize * StorDBound * (dimsD *
                        (29.0 + dimsD) + 2);

  if (!isFirstTimeLTwoDotLaplaceBound) {
    double LTwoDotLaplaceBoundGFLOPS = ((numberOfFlop *
                                         CounterLTwoDotLaplaceBound) / LTwoDotLaplaceBoundExecTime) * (1e-9);
    double LTwoDotLaplaceBoundOPS = StorDInnersize * StorDBound * (dimsD * (26.0));
    double LTwoDotLaplaceBoundGOPS = ((LTwoDotLaplaceBoundOPS *
                                       CounterLTwoDotLaplaceBound) / LTwoDotLaplaceBoundExecTime) * (1e-9);
    //    std::cout << "GFLOPS for LTwoDotLaplaceBound : " << LTwoDotLaplaceBoundGFLOPS << std::endl;
    return (Timing(LTwoDotLaplaceBoundGFLOPS, LTwoDotLaplaceBoundGOPS,
                   LTwoDotLaplaceBoundExecTime));
  }
...
std::string LTwoDotLaplaceBoundHeader() {
  ...
  stream_program_src <<
                     "__kernel void multKernel(	__global  REAL* ptrLevel,       "  << std::endl;
...
void CompileLTwoDotLaplaceBound(int id, std::string kernel_src,
  ...
  stream_program_src << "	 __local REAL alphaTemp[" << LSIZE << "];	"  <<
                     std::endl;
  ...
  for (unsigned d_outer = 0; d_outer < dims; d_outer++) {
    ...
    if (d_outer < dcon) {
      ...
      stream_program_src << "res += TimestepCoeff*ptrLambda[" << d_outer <<
                         "] * element* gradTemp[" << d_outer* LSIZE << " + get_local_id(0)];" <<
                         std::endl;
    } else {

      stream_program_src << "res += TimestepCoeff*ptrLambda[" << d_outer <<
                         "] * element* gradreg_" << d_outer << ";" << std::endl;
    }
  }
  ...
  if ((dims - 1) < dcon) {
    ...
    stream_program_src << "res += element * (l2dotreg_" << (dims - 1) << "); " <<
                       std::endl;
  }
  ...
  if (err != CL_SUCCESS) {
    std::cout <<
              "OCL Error: compileLTwoDotLaplaceBoundKernel OpenCL Build Error. Error Code: "
              << err << std::endl;

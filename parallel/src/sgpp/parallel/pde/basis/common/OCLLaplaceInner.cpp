...
void CompileLaplaceInner(int id, std::string kernel_src, cl_kernel* kernel) {
  ...
  if (err != CL_SUCCESS) {
    std::cout << "OCL Error: compileMultKernel2DIndexesJInReg2 OpenCL Build Error. Error Code: "
              << err << std::endl;
...
Timing PrintGFLOPSLaplaceInner() {
  ...
  double LaplaceInnerTime = (MultTimeLaplaceInner + ReduTimeLaplaceInner) * (1e-9);
  LaplaceInnerTime = LaplaceInnerExecTime;

  if (!isFirstTimeLaplaceInner) {
    std::cout << "Overall Runtime of AllReduceLaplaceInner: " << LaplaceInnerAllReduceTime
              << std::endl;
    std::cout << "Time for GPU-calculation of LaplaceInner: " << LaplaceInnerTime << std::endl;

    //    for(size_t i = 0; i < 186; i++) {
    //      std::cout << "Time for GPU-calculation of LaplaceInner["<<i<<"]: " <<
    //      LaplaceInnerExecAll[i] << std::endl;
  ...
  if (!isFirstTimeLaplaceInner) {
    ...
    double LaplaceInnerGOPS = ((LaplaceInnerOPS * CounterLaplaceInner) / LaplaceInnerTime) * (1e-9);

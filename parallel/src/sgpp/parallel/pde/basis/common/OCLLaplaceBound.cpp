  ...
  namespace parallel {
    namespace oclpdekernels {
      ...
      std::string BoundGradientFunction() {
        ...
        stream_program_src << "  ulong doGrad = (ulong)((i_level == j_level) && (i_index == j_index) && (i_level != 1.0));"  << std::endl;
        stream_program_src << "  grad = select(0.0, i_level * 2.0 * lcl_q_inv, doGrad);		"  << std::endl;
      ...
      std::string LaplaceBoundHeader() {
        std::stringstream stream_program_src;
        stream_program_src << "__kernel void multKernelBound(	__global  REAL* ptrLevel,       "  << std::endl;
      ...
      void CompileLaplaceBound(int id, std::string kernel_src, cl_kernel* kernel) {
        ...
        if (err != CL_SUCCESS) {
          std::cout << "OCL Error: compileMultKernel2DBound2Indexes OpenCL Build Error. Error Code: " << err << std::endl;
      ...
      Timing PrintGFLOPSLaplaceBound() {
        ...
        if (!isFirstTimeLaplaceBound) {
          std::cout << "Overall Runtime of AllReduceLaplaceBound: " << LaplaceBoundAllReduceTime << std::endl;

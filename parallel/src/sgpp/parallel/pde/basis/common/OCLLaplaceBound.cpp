...
std::string BoundGradientFunction() {
  ...
  stream_program_src << "  ulong doGrad = (ulong)((i_level == j_level) && (i_index == j_index) && "
                        "(i_level != 1.0));"
                     << std::endl;
  stream_program_src << "  grad = select(0.0, i_level * 2.0 * lcl_q_inv, doGrad);		"
                     << std::endl;
  stream_program_src << "  return grad;							"
                     << std::endl;
  stream_program_src << "}								"
                     << std::endl;
...
std::string LaplaceBoundHeader() {
  std::stringstream stream_program_src;
  stream_program_src << "__kernel void multKernelBound(	__global  REAL* ptrLevel,       "
                     << std::endl;
  stream_program_src
...
void CompileLaplaceBound(int id, std::string kernel_src, cl_kernel* kernel) {
  ...
  if (err != CL_SUCCESS) {
    std::cout << "OCL Error: compileMultKernel2DBound2Indexes OpenCL Build Error. Error Code: "
              << err << std::endl;
...
Timing PrintGFLOPSLaplaceBound() {
  ...
  double LaplaceBoundTime = (MultTimeLaplaceBound + ReduTimeLaplaceBound) * (1e-9);
  LaplaceBoundTime = LaplaceBoundExecTime;

  if (!isFirstTimeLaplaceBound) {
    std::cout << "Overall Runtime of AllReduceLaplaceBound: " << LaplaceBoundAllReduceTime
              << std::endl;
    std::cout << "Time for GPU-calculation of LaplaceBound: " << LaplaceBoundTime << std::endl;
  }

  double numberOfFlopBound = StorDInnersize * StorDBound * (dimsD * (28.0 + dimsD));

  if (!isFirstTimeLaplaceBound) {
    double LaplaceBoundGFLOPS =
        ((numberOfFlopBound * CounterLaplaceBound) / LaplaceBoundTime) * (1e-9);
    double LaplaceBoundOPS = StorDInnersize * StorDBound * (dimsD * (26.0));
    double LaplaceBoundGOPS = ((LaplaceBoundOPS * CounterLaplaceBound) / LaplaceBoundTime) * (1e-9);

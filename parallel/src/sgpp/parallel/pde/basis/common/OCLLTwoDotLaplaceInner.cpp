...
void SetArgumentsLTwoDotLaplaceInner() {
  ...
  for (unsigned int i = 0; i < num_devices; ++i) {
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceInnerKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrLevelInner[i]);
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceInnerKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrLevelIndexLevelintconInner[i]);

    ciErrNum |= clSetKernelArg(LTwoDotLaplaceInnerKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrIndexInner[i]);
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceInnerKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrLevel_intInner[i]);

    ciErrNum |= clSetKernelArg(LTwoDotLaplaceInnerKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrAlphaInner[i]);
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceInnerKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrParResultInner[i]);
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceInnerKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrLcl_qInner[i]);
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceInnerKernel[i], counter++,
                               sizeof(cl_mem), (void*) &d_ptrLambdaInner[i]);
...
Timing PrintGFLOPSLTwoDotLaplaceInner() {
  ...
  if (!isFirstTimeLTwoDotLaplaceInner) {
    ...
    std::cout << "Time for GPU-calculation of LTwoDotLaplaceInner: " << totalTime <<
              std::endl;
#ifdef USE_MPI
  ...
  if (!isFirstTimeLTwoDotLaplaceInner) {
    double LTwoDotLaplaceInnerGFLOPS = ((numberOfFlop *
                                         CounterLTwoDotLaplaceInner) / totalTime) * (1e-9);
    double LTwoDotLaplaceInnerOPS = StorD * StorD * (dimsD * (16.0));
    double LTwoDotLaplaceInnerGOPS = ((LTwoDotLaplaceInnerOPS *
                                       CounterLTwoDotLaplaceInner) / totalTime) * (1e-9);
...
std::string LTwoDotLaplaceInnerHeader() {
  ...
  stream_program_src <<
                     "                        	ulong ConstantMemoryOffset,   			"  << std::endl;
  stream_program_src <<
...
void CompileLTwoDotLaplaceInner(int id, std::string kernel_src,
                                cl_kernel* kernel) {
  ...
  typeREAL = "REAL ";
  stream_program_src << typeREAL << "alphaReg = alphaTemp[" << "k" << "];" <<
                     std::endl;
  ...
  if (err != CL_SUCCESS) {
    std::cout <<
              "OCL Error: compileLTwoDotLaplaceinner OpenCL Build Error. Error Code: " << err
              << std::endl;

...
void SetArgumentsLTwoDotLaplaceInner() {
  ...
  for (unsigned int i = 0; i < num_devices; ++i) {
    ciErrNum |= clSetKernelArg(LTwoDotLaplaceInnerKernel[i], counter++, sizeof(cl_mem),
...
Timing PrintGFLOPSLTwoDotLaplaceInner() {
  ...
  if (!isFirstTimeLTwoDotLaplaceInner) {
    std::cout << "Overall Runtime of AllReduceLTwoDotLaplaceInner: "
              << LTwoDotLaplaceInnerAllReduceTime << std::endl;
//    std::cout << "Time for GPU-calculation of LTwoDotLaplaceInnerRedu: " <<
//    LTwoDotLaplaceInnerExecTimeFirst*(1e-9) << std::endl;
//    std::cout << "Time for GPU-calculation of LTwoDotLaplaceInnerRedu2: " <<
//    LTwoDotLaplaceInnerExecTime*(1e-9) << std::endl;
//    std::cout << "Time for GPU-calculation of LTwoDotLaplaceInnerProfilingAcc: " <<
//    LTwoDotLaplaceInnerProfilingAcc*(1e-9) <<  std::endl;
//    std::cout << "Time for GPU-calculation of LTwoDotLaplaceInnerProfilingWait: " <<
//    LTwoDotLaplaceInnerProfilingWait*(1e-9) <<  std::endl;
#ifdef USE_MPI

    for (size_t i = 0; i < MOD; i++) {
      //      std::cout << "Time for GPU-calculation of LTwoDotLaplaceInner["<<i<<"]: "<<
      //      LTwoDotLaplaceInnerAll[i] << std::endl;
      //      std::cout << "   Profiling["<<i<<"]: " << LTwoDotLaplaceInnerProfiling[i]*(1e-9) ;
      //      std::cout << "   Waiting["<<i<<"]: " << LTwoDotLaplaceInnerWaiting[i] *(1e-9) <<
      //      std::endl;
    }
    ...
    //    std::cout << "Time for GPU-calculation of LTwoDotLaplaceInnerRest:  " <<
    //    LTwoDotLaplaceInnerExecTime << std::endl;
    std::cout << "Time for GPU-calculation of LTwoDotLaplaceInner: " << totalTime << std::endl;
#ifdef USE_MPI
//        std::cout << "Number of Calls of LTwoDotLaplaceInnerFirst         : " <<
//        CounterLTwoDotLaplaceInnerFirst << std::endl;
//        std::cout << "Number of Calls of LTwoDotLaplaceInnerRest         : " <<
//        (CounterLTwoDotLaplaceInnerRest-CounterLTwoDotLaplaceInnerLast) << std::endl;
//        std::cout << "Number of Calls of LTwoDotLaplaceInnerLast         : " <<
//        CounterLTwoDotLaplaceInnerLast << std::endl;

//        std::cout << "Number of Calls of LTwoDotLaplaceInner             : " <<
//        CounterLTwoDotLaplaceInner << std::endl;

//    std::cout << "Time for Single GPU-calculation of LTwoDotLaplaceInnerFirst: " <<
//    LTwoDotLaplaceInnerExecTimeFirst/CounterLTwoDotLaplaceInnerFirst << std::endl;
//    std::cout << "Time for Single GPU-calculation of LTwoDotLaplaceInnerRest : " <<
//    LTwoDotLaplaceInnerExecTime/(CounterLTwoDotLaplaceInnerRest-CounterLTwoDotLaplaceInnerLast) <<
//    std::endl;
//    std::cout << "Time for Single GPU-calculation of LTwoDotLaplaceInnerLast : " <<
//    LTwoDotLaplaceInnerExecTimeLast/CounterLTwoDotLaplaceInnerLast << std::endl;
#endif
  }
  ...
  if (!isFirstTimeLTwoDotLaplaceInner) {
    double LTwoDotLaplaceInnerGFLOPS =
        ((numberOfFlop * CounterLTwoDotLaplaceInner) / totalTime) * (1e-9);
    double LTwoDotLaplaceInnerOPS = StorD * StorD * (dimsD * (16.0));
    double LTwoDotLaplaceInnerGOPS =
        ((LTwoDotLaplaceInnerOPS * CounterLTwoDotLaplaceInner) / totalTime) * (1e-9);

    //    std::cout << "GFLOPS for LTwoDotLaplaceInner : " << LTwoDotLaplaceInnerGFLOPS <<
    //    std::endl;
...
void CompileLTwoDotLaplaceInner(int id, std::string kernel_src, cl_kernel* kernel) {
  ...
  typeREAL = "REAL ";
  stream_program_src << typeREAL << "alphaReg = alphaTemp["
                     << "k"
                     << "];" << std::endl;
  ...
  for (unsigned d_outer = 0; d_outer < dims; d_outer++) {
    ...
    if (d_outer < dcon) {
      stream_program_src << "res += TimestepCoeff*ptrLambda[" << d_outer << "] * element* gradTemp["
                         << d_outer * LSIZE << " + get_local_id(0)];" << std::endl;
    } else {
      stream_program_src << "res += TimestepCoeff*ptrLambda[" << d_outer << "] * element* gradreg_"
                         << d_outer << ";" << std::endl;
    }
  }

  if ((dims - 1) < dcon) {
    stream_program_src << "res += element * l2dotTemp[" << (dims - 1) * LSIZE
                       << " + get_local_id(0)]; " << std::endl;
  } else {
  ...
  if (err != CL_SUCCESS) {
    std::cout << "OCL Error: compileLTwoDotLaplaceinner OpenCL Build Error. Error Code: " << err
              << std::endl;

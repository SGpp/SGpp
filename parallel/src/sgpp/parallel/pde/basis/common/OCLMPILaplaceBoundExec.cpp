...
void ExecLaplaceBound(REAL* ptrAlpha, REAL* ptrResult, REAL* lcl_q, REAL* lcl_q_inv, REAL* ptrLevel,
...

  {
    ...
    for (size_t overallMultOffset = 0; overallMultOffset < multglobal;
      ...
      for (unsigned int i = 0; i < num_devices; i++) {
        ...
        ciErrNum |=
            clSetKernelArg(LaplaceBoundKernel[i], 8, sizeof(cl_ulong), (void*)&overallMultOffset2);
      ...
      for (size_t ConstantMemoryOffset = 0; ConstantMemoryOffset < constantglobal;
        ...
        for (unsigned int i = 0; i < num_devices; i++) {
          ...
          ciErrNum = clEnqueueNDRangeKernel(command_queue[i], LaplaceBoundKernel[i], 2, 0,
  ...
  if (num_devices > 1) {
    ...
    for (unsigned int i = 0; i < num_devices; i++) {
      clEnqueueReadBuffer(command_queue[i], d_ptrResultBound[i], CL_FALSE, 0,
                          storageInnerSizeBound * sizeof(REAL), ptrResultPinnedBound, 0, NULL,
                          &GPUDone[i]);
    }
...
void OCLPDEKernels::RunOCLKernelLaplaceBound(sgpp::base::DataVector& alpha,
  ...
  if (MPISizeListBound[myrank] != 0) {
    myStopwatch->start();
    ExecLaplaceBound(alpha.getPointer(), result.getPointer(), lcl_q, lcl_q_inv, ptrLevel, ptrIndex,
                     ptrLevel_int, argStorageSize, argStorageDim, MPIOffsetListBound[myrank],

...
void ExecLTwoDotInner(REAL* ptrAlpha, REAL* ptrResult, REAL* lcl_q, size_t MPIOffset,
...

  {
    ...
    for (size_t overallMultOffset = 0; overallMultOffset < multglobal;
      ...
      for (unsigned int i = 0; i < num_devices; i++) {
        ...
        ciErrNum |=
            clSetKernelArg(LTwoDotInnerKernel[i], 7, sizeof(cl_ulong), (void*)&overallMultOffset2);
      ...
      for (size_t ConstantMemoryOffset = 0; ConstantMemoryOffset < constantglobal;
        ...
        for (unsigned int i = 0; i < num_devices; i++) {
          ...
          ciErrNum = clEnqueueNDRangeKernel(command_queue[i], LTwoDotInnerKernel[i], 2, 0,
...
void OCLPDEKernels::RunOCLKernelLTwoDotInner(SGPP::base::DataVector& alpha,
  ...
  if (MPISizeListInner[myrank] != 0) {
    myStopwatch->start();
    ExecLTwoDotInner(alpha.getPointer(), result.getPointer(), lcl_q, MPIOffsetListInner[myrank],

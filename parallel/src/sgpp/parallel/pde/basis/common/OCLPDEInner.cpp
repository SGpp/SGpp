...
cl_mem d_ptrLevelIndexLevelintconInner[NUMDEVS];  // constant memory buffer holding all three
                                                  // components
...
std::string ReduceInnerKernelStr() {
  ...
  stream_program_src << "__kernel void ReduceInnerKernel(__global  REAL* ptrResult,    "
                     << std::endl;
  stream_program_src << "                                __global  REAL* ptrParResult, "
                     << std::endl;
  stream_program_src << "                		 ulong overallParOffset,      "
                     << std::endl;
  stream_program_src << "                		 ulong num_groups)      " << std::endl;
  stream_program_src << "{							       "
                     << std::endl;
  stream_program_src << "unsigned j = get_global_id(0);				       "
                     << std::endl;
  stream_program_src << "REAL res = 0.0;					       " << std::endl;
  stream_program_src << "for (unsigned k = 0; k < num_groups; k++) {		       "
                     << std::endl;
  stream_program_src << "	res += ptrParResult[k*get_global_size(0) + j];         "
                     << std::endl;
  stream_program_src << "}							       "
                     << std::endl;
...
void CompileReduceInner(int id, std::string kernel_src, cl_kernel* kernel) {
  ...
  int ierr = snprintf(buildOptions, sizeof(buildOptions),
                      ...
                      storageSize, storageSizePadded, num_groups, 0L, dims);
  ...
  if (err != CL_SUCCESS) {
    std::cout << "OCL Error: compileReduceInner OpenCL Build Error. Error Code: " << err
              << std::endl;
...
std::string InnerLTwoDotFunction() {
  ...
  stream_program_src << "ulong overlap = (max(q, (i2d * in_l2d -in_l2d)) < min(p, (i2d * in_l2d + "
                        "in_l2d)));			"
                     << std::endl;
  stream_program_src << "double temp_res = ((0.5*in_l1d) * (- fabs((l2d *q -i2d)) - fabs((l2d * p "
                        "- i2d))) + in_l1d);	"
                     << std::endl;
#else
  stream_program_src << "double q = fma(i1d, in_l1d, -in_l1d); //(i1d-1)*in_l1d;		"
                     << std::endl;
  stream_program_src << "double p = fma(i1d, in_l1d,  in_l1d); //(i1d+1)*in_l1d;		"
                     << std::endl;
  stream_program_src << "ulong overlap = (max(q, fma(i2d, in_l2d, -in_l2d)) < min(p, fma(i2d, "
                        "in_l2d,in_l2d)));			"
                     << std::endl;
  stream_program_src << "double temp_res = fma((0.5*in_l1d), (- fabs(fma(l2d,q,-i2d)) - "
                        "fabs(fma(l2d,p,-i2d))), in_l1d);	"
                     << std::endl;
#endif
  stream_program_src << "double res_two = select(0.0,temp_res,overlap); // Now mask result	"
                        "				"
                     << std::endl;
  stream_program_src << "return (select(res_two, res_one, (ulong)(lid == ljd)))*lcl_q;		"
                        "				"
                     << std::endl;
  stream_program_src << "}" << std::endl;
...
void SetBuffersInner(REAL* ptrLevel, REAL* ptrIndex, REAL* ptrLevel_int, size_t localStorageSize,
  ...
  constant_mem_size_noboundary =
      constant_mem_size_noboundary - lcl_q_size * sizeof(REAL) - lambda_size * sizeof(REAL);
  alphaend_size = pad;
#ifdef USE_MPI
  int myrank;
  ...
  for (size_t i = 0; i < num_devices; ++i) {
    ...
    d_ptrLevelIndexLevelintconInner[i] = clCreateBuffer(
        context, CL_MEM_READ_ONLY, constant_buffer_size_noboundary * sizeof(REAL), NULL, &ciErrNum);
    ...
    ptrAlphaPinnedInner =
        (REAL*)clEnqueueMapBuffer(command_queue[0], d_ptrAlphaPinnedInner[0], CL_TRUE, CL_MAP_WRITE,
                                  0, alpha_size * sizeof(REAL), 0, NULL, NULL, &ciErrNum);
    ...
    d_ptrResultInner[i] =
        clCreateBuffer(context, CL_MEM_READ_WRITE, result_size * sizeof(REAL), NULL, &ciErrNum);
    oclCheckErr(ciErrNum, "clCreateBuffer ptrResult");

    ptrResultPinnedInner =
        (REAL*)clEnqueueMapBuffer(command_queue[0], d_ptrResultPinnedInner[0], CL_TRUE, CL_MAP_READ,
                                  0, pinnedresult_size * sizeof(REAL), 0, NULL, NULL, &ciErrNum);
    ...
    d_ptrLcl_qInner[i] =
        clCreateBuffer(context, CL_MEM_READ_ONLY, lcl_q_size * sizeof(REAL), NULL, &ciErrNum);
...
void CleanUpInner() {
  if (!isFirstTimeLaplaceInner || !isFirstTimeLTwoDotInner || !isFirstTimeLTwoDotLaplaceInner) {
    cl_int ciErrNum = CL_SUCCESS;

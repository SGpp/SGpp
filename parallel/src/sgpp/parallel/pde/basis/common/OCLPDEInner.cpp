...
void transposer(REAL* sink, REAL* source, size_t dim1, size_t dim2pad,
                size_t dim2) {
...
std::string ReduceInnerKernelStr() {
  ...
  stream_program_src <<
                     "__kernel void ReduceInnerKernel(__global  REAL* ptrResult,    "  << std::endl;
  stream_program_src <<
                     "                                __global  REAL* ptrParResult, "  << std::endl;
  stream_program_src << "                		 ulong overallParOffset,      "  <<
                     std::endl;
  stream_program_src << "                		 ulong num_groups)      "  <<
                     std::endl;
...
void CompileReduceInner(int id, std::string kernel_src, cl_kernel* kernel) {
  ...
  if (err != CL_SUCCESS) {
    std::cout << "OCL Error: compileReduceInner OpenCL Build Error. Error Code: " <<
              err << std::endl;
...
std::string InnerLTwoDotFunction() {
  ...
  stream_program_src <<
                     ...
                     << std::endl;
  stream_program_src <<
                     "double temp_res = ((0.5*in_l1d) * (- fabs((l2d *q -i2d)) - fabs((l2d * p - i2d))) + in_l1d);	"
                     << std::endl;
#else
  stream_program_src <<
                     "double q = fma(i1d, in_l1d, -in_l1d); //(i1d-1)*in_l1d;		" << std::endl;
  stream_program_src <<
                     "double p = fma(i1d, in_l1d,  in_l1d); //(i1d+1)*in_l1d;		" << std::endl;
  stream_program_src <<
...
void SetBuffersInner(REAL* ptrLevel,
  ...
  constant_mem_size_noboundary = constant_mem_size_noboundary - lcl_q_size *
                                 sizeof(REAL)
  ...
  transposer(ptrLevel_intTInner, ptrLevel_int, dims, storageSizePadded,
             storageSize);
  ...
  ptrLevelIndexLevelintInner = (REAL*)calloc(level_size + index_size +
  ...
  for (size_t i = 0; i < storageSize; i++) {
    for (size_t d = 0; d < dims; d++) {
      ...
      three_d += 1;
      ptrLevelIndexLevelintInner[i * threedims + three_d] = ptrLevel_int[i * dims +
          d];
      three_d += 1;
    }
  ...
  par_result_max_size = std::min(par_result_max_size,
  ...
  constant_buffer_iterations_noboundary = constant_buffer_size_noboundary /
                                          (3 * dims);
  ...
  for (size_t i = 0; i < num_devices; ++i) {
    ...
    d_ptrAlphaPinnedInner[i] = clCreateBuffer(context,
    ...
    //RED
    d_ptrResultPinnedInner[i] = clCreateBuffer(context,
...
void SetUpMPIInner() {
  ...
  SGPP::parallel::PartitioningTool::calcDistribution(data_size, nproz,
      MPISizeListInner, MPIOffsetListInner, minimum_size);

  if (myrank == 0) {
    std::cout << "nproz " << nproz << std::endl;

    for (size_t i = 0; i < nproz; i++) {
      std::cout << "MPIOffsetListInner[" << i << "] = " << MPIOffsetListInner[i] <<
                std::endl;
      std::cout << "MPISizeListInner[" << i << "] = " << MPISizeListInner[i] <<
                std::endl;
    }
  }

...
void OperationLaplaceVectorizedLinearBoundary::reset() { init_grid_storage(); }
...
double OperationLaplaceVectorizedLinearBoundary::gradient_dirichlet(size_t i, size_t j,
                                                                    size_t dim) {
  double grad;
...
#if defined(__MIC__)
void OperationLaplaceVectorizedLinearBoundary::mult_dirichlet_mic(size_t process_i_start,
...
void OperationLaplaceVectorizedLinearBoundary::mult_dirichlet(SGPP::base::DataVector& alpha,
      ...
      for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
        ...
        for (size_t dim = 0; dim < max_dims; dim++) {
          ...
          __m128d mm_doGrad = _mm_and_pd(
              ...
              _mm_and_pd(_mm_cmpeq_pd(mm_iid, mm_ijd), _mm_cmpneq_pd(mm_lid, mm_one)));  // +3
      ...
      size_t i = i_boundary_filtered[ii];
      {
        for (size_t jj = 0; jj < this->storage->getSize(); jj++) {
          ...
          for (size_t d_outer = 0; d_outer < this->storage->getDimension(); d_outer++) {
            ...
            (result_boundary_filtered_->getPointer()[ii]) +=
                (this->lambda_->get(d_outer) * element);
          }
        }
      }
    }
...
void OperationLaplaceVectorizedLinearBoundary::mult(SGPP::base::DataVector& alpha,
                                                    SGPP::base::DataVector& result) {
  result.setAll(0.0);

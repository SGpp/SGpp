...
OperationLTwoDotProductVectorizedLinearBoundary::
    ~OperationLTwoDotProductVectorizedLinearBoundary() {
  ...
  double bandwidth = all_iterations *
                     ...
                     all_time;
  std::cout << "IN OPERATOR : LTDOT BOUNDARY, GFLOPS :" << gflops
...
void OperationLTwoDotProductVectorizedLinearBoundary::reset() { init_grid_storage(); }

void OperationLTwoDotProductVectorizedLinearBoundary::init_constants() {
  all_time = 0.0;
  all_iterations = 0.0;
...
double OperationLTwoDotProductVectorizedLinearBoundary::l2dot_dirichlet(size_t i, size_t j,
                                                                        size_t dim) {
      ...
      for (size_t ii = thr_start; ii < thr_end; ii++) {
        ...
        for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
          ...
          for (size_t dim = 0; dim < max_dims; dim++) {
            ...
            __m512d mm_lid =
                _mm512_extload_pd(level_boundary_filtered_ptr_ + i_idx, _MM_UPCONV_PD_NONE,
                                  _MM_BROADCAST_1X8, _MM_HINT_NONE);
            __m512d mm_iid =
                _mm512_extload_pd(index_boundary_filtered_ptr_ + i_idx, _MM_UPCONV_PD_NONE,
                                  _MM_BROADCAST_1X8, _MM_HINT_NONE);
            ...
            __m512d mm_val = _mm512_mask_blend_pd(mm_selector, mm_res_one, mm_res_two);  // +1
      ...
      for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
        ...
        for (size_t dim = 0; dim < max_dims; dim++) {
          ...
          __m256d mm_temp_res = _mm256_blendv_pd(
              mm_temp_res_inner,
              _mm256_add_pd(
                  _mm256_and_pd(mm_temp_res_leftbound,
                                _mm256_cmp_pd(mm_i2d, _mm256_setzero_pd(), _CMP_EQ_OQ)),
                  _mm256_and_pd(mm_temp_res_rightbound, _mm256_cmp_pd(mm_i2d, mm_one, _CMP_EQ_OQ))),
              _mm256_cmp_pd(mm_l2d, mm_one, _CMP_EQ_OQ));  // +7
          ...
          __m256d mm_temp_res2 = _mm256_blendv_pd(
              mm_temp_res_inner2,
              _mm256_add_pd(_mm256_and_pd(mm_temp_res_leftbound2,
                                          _mm256_cmp_pd(mm_i2d2, _mm256_setzero_pd(), _CMP_EQ_OQ)),
                            _mm256_and_pd(mm_temp_res_rightbound2,
      ...
      for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
        ...
        for (size_t dim = 0; dim < max_dims; dim++) {
          ...
          __m128d mm_temp_res = _mm_blendv_pd(
              mm_temp_res_inner,
              _mm_add_pd(_mm_and_pd(mm_temp_res_leftbound, _mm_cmpeq_pd(mm_i2d, _mm_setzero_pd())),
                         _mm_and_pd(mm_temp_res_rightbound, _mm_cmpeq_pd(mm_i2d, mm_one))),
              _mm_cmpeq_pd(mm_l2d, mm_one));  // +7
          ...
          mm_element = _mm_mul_pd(mm_element, mm_val);                          // 1 flop
          ...
          __m128d mm_temp_res2 = _mm_blendv_pd(
              mm_temp_res_inner2,
              _mm_add_pd(
                  _mm_and_pd(mm_temp_res_leftbound2, _mm_cmpeq_pd(mm_i2d2, _mm_setzero_pd())),
                  _mm_and_pd(mm_temp_res_rightbound2, _mm_cmpeq_pd(mm_i2d2, mm_one))),
...
/*
   ...
   (int)result_boundary_filtered_->getSize(), MPI_DOUBLE,

...
#endif
{
  std::cout << "IN CONSTRUCTOR: OperationLTwoDotProductVectorizedLinearBoundary"
            << std::endl;
  init_constants();
  init_grid_storage();
}
...
OperationLTwoDotProductVectorizedLinearBoundary::~OperationLTwoDotProductVectorizedLinearBoundary() {
  ...
  double flop = (double) (23 * storage->dim() ) * (double) (
                  storage->size() * all_i_size[process_index]);
...
double OperationLTwoDotProductVectorizedLinearBoundary::l2dot_dirichlet(
      ...
      for (size_t ii = thr_start; ii < thr_end; ii++) {
        ...
        for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
          ...
          for (size_t dim = 0; dim < max_dims; dim++) {
            ...
            __m512d mm_val = _mm512_mask_blend_pd(mm_selector, mm_res_one,
                                                  mm_res_two);  // +1
        ...
        for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
          ...
          for (size_t dim = 0; dim < max_dims; dim++) {
            ...
            __m256d mm_temp_res = _mm256_blendv_pd(mm_temp_res_inner,
                                                   _mm256_add_pd(_mm256_and_pd(mm_temp_res_leftbound, _mm256_cmp_pd(mm_i2d,
            ...
            __m256d mm_temp_res2 = _mm256_blendv_pd(mm_temp_res_inner2,
                                                    _mm256_add_pd(_mm256_and_pd(mm_temp_res_leftbound2, _mm256_cmp_pd(mm_i2d2,
                                                        _mm256_setzero_pd(), _CMP_EQ_OQ)),
                                                        _mm256_and_pd(mm_temp_res_rightbound2, _mm256_cmp_pd(mm_i2d2, mm_one,
                                                            _CMP_EQ_OQ))
                                                                 ),
        ...
        for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
          ...
          for (size_t dim = 0; dim < max_dims; dim++) {
            ...
            __m128d mm_temp_res = _mm_blendv_pd(mm_temp_res_inner,
                                                _mm_add_pd(_mm_and_pd(mm_temp_res_leftbound, _mm_cmpeq_pd(mm_i2d,
                                                    _mm_setzero_pd())),
            ...
            __m128d mm_temp_res2 = _mm_blendv_pd(mm_temp_res_inner2,
                                                 _mm_add_pd(_mm_and_pd(mm_temp_res_leftbound2, _mm_cmpeq_pd(mm_i2d2,
                                                     _mm_setzero_pd())),
...
void OperationLTwoDotProductVectorizedLinearBoundary::mult(
  SGPP::base::DataVector& alpha, SGPP::base::DataVector& result) {
  result.setAll(0.0);

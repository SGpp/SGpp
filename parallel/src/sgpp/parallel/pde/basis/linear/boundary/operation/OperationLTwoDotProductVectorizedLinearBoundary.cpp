...
namespace SGPP {
  namespace parallel {
    ...
    OperationLTwoDotProductVectorizedLinearBoundary::OperationLTwoDotProductVectorizedLinearBoundary(SGPP::base::GridStorage* storage)
      :
      storage(storage),
      level_(NULL),
      level_int_(NULL),
    ...
    void OperationLTwoDotProductVectorizedLinearBoundary::reset() {
      init_grid_storage();
    }
    ...
    void OperationLTwoDotProductVectorizedLinearBoundary::init_grid_storage() {
      ...
      level_boundary_filtered_ = new SGPP::base::DataMatrix((size_t) i_boundary_filtered.size(), level_->getNcols());

      if (level_int_boundary_filtered_)
        delete level_int_boundary_filtered_;

      level_int_boundary_filtered_ = new SGPP::base::DataMatrix((size_t) i_boundary_filtered.size(), level_int_->getNcols());

      if (index_boundary_filtered_)
        delete index_boundary_filtered_;

      index_boundary_filtered_ = new SGPP::base::DataMatrix((size_t) i_boundary_filtered.size(), index_->getNcols());
...
#if defined(__MIC__)
      storage->getLevelIndexArraysForEvalTLBOptimized(*(this->level_), *(this->index_), SGPP::parallel::MIC, BLOCK_LENGTH);
      storage->getLevelForIntegralTLBOptimized(*(this->level_int_), SGPP::parallel::MIC, BLOCK_LENGTH);
#elif defined(__SSE4_2__) || defined(__AVX__)
      storage->getLevelIndexArraysForEvalTLBOptimized(*(this->level_), *(this->index_), SGPP::parallel::X86SIMD, BLOCK_LENGTH);
      storage->getLevelForIntegralTLBOptimized(*(this->level_int_), SGPP::parallel::X86SIMD, BLOCK_LENGTH);
#else
      storage->getLevelIndexArraysForEval(*(this->level_), *(this->index_));
      storage->getLevelForIntegral(*(this->level_int_));
#endif
      ...
      #pragma omp parallel
      {
        ...
        gradient_temp[omp_get_thread_num()] = new SGPP::base::DataVector(VECTOR_SIZE * this->storage->dim() * REG_BCOUNT);
        l2dot_temp[omp_get_thread_num()] = new SGPP::base::DataVector(VECTOR_SIZE * this->storage->dim() * REG_BCOUNT);
      }
      ...
      SGPP::parallel::DMVectorizationPaddingAssistant::padDataset(*(this->operation_result_matrix_), vectorizationType);
      ...
      #pragma omp parallel
      {
        ...
        size_t thr_start;
        size_t thr_end;
        SGPP::parallel::PartitioningTool::getOpenMPPartitionSegment(0, result_matrix_rows, &thr_start, &thr_end);
    ...
    double OperationLTwoDotProductVectorizedLinearBoundary::l2dot_dirichlet(size_t i, size_t j, size_t dim) {
    ...
    void OperationLTwoDotProductVectorizedLinearBoundary::mult_dirichlet(SGPP::base::DataVector& alpha, SGPP::base::DataVector& result) {
      ...
      memcpy(alpha_padded_->getPointer(), alpha.getPointer(), original_size * sizeof(double));
        ...
        #pragma omp parallel
        {
          ...
          size_t thr_start;
          size_t thr_end;
          SGPP::parallel::PartitioningTool::getOpenMPPartitionSegment(process_i_start, process_i_end, &thr_start, &thr_end);
          ...
          for (size_t ii = thr_start; ii < thr_end; ii++) {
            ...
            for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
              ...
              for (size_t dim = 0; dim < max_dims; dim++) {
                ...
                __m512d mm_lid = _mm512_extload_pd(level_boundary_filtered_ptr_ + i_idx, _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);
                __m512d mm_iid = _mm512_extload_pd(index_boundary_filtered_ptr_ + i_idx, _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);
            ...
            for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
              ...
              for (size_t dim = 0; dim < max_dims; dim++) {
                ...
                __m256d mm_temp_res = _mm256_blendv_pd(mm_temp_res_inner,
                                                       _mm256_add_pd(_mm256_and_pd(mm_temp_res_leftbound, _mm256_cmp_pd(mm_i2d, _mm256_setzero_pd(), _CMP_EQ_OQ)),
                ...
                __m256d mm_temp_res2 = _mm256_blendv_pd(mm_temp_res_inner2,
                                                        _mm256_add_pd(_mm256_and_pd(mm_temp_res_leftbound2, _mm256_cmp_pd(mm_i2d2, _mm256_setzero_pd(), _CMP_EQ_OQ)),
                                                            _mm256_and_pd(mm_temp_res_rightbound2, _mm256_cmp_pd(mm_i2d2, mm_one, _CMP_EQ_OQ))
                                                                     ),
            ...
            for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
              ...
              for (size_t dim = 0; dim < max_dims; dim++) {
                ...
                __m128d mm_temp_res2 = _mm_blendv_pd(mm_temp_res_inner2,
                                                     _mm_add_pd(_mm_and_pd(mm_temp_res_leftbound2, _mm_cmpeq_pd(mm_i2d2, _mm_setzero_pd())),
...
#pragma prefetch
    ...
    void OperationLTwoDotProductVectorizedLinearBoundary::mult(SGPP::base::DataVector& alpha, SGPP::base::DataVector& result) {
      result.setAll(0.0);
      ...
      if (dirichlet) {
        ...
        throw new SGPP::base::operation_exception("OperationLaplaceVectorizedLinearBoundary::mult : This method is only available on grids with Dirichlet boundaries in all dimensions!");
      }
    }

...
OperationLTwoDotLaplaceVectorizedLinearBoundary::
    ~OperationLTwoDotLaplaceVectorizedLinearBoundary() {
  double flop = static_cast<double>((28) * storage->getDimension() +
                                    storage->getDimension() * storage->getDimension()) *
                static_cast<double>(storage->getSize() * all_i_size[process_index]);
...
void OperationLTwoDotLaplaceVectorizedLinearBoundary::reset() { init_grid_storage(); }
...
void OperationLTwoDotLaplaceVectorizedLinearBoundary::init_grid_storage() {
  ...
  for (size_t i = 0; i < this->storage->getSize(); i++) {
    ...
    if (i_boundary == false) i_boundary_filtered.push_back(i);
  }
  ...
  for (int i = 0; i < process_count; ++i) {
    ...
    int process_portion =
        (i == process_count - 1)
            ? static_cast<int>(this->i_boundary_filtered.size() - i * single_process_portion)
  ...
  for (int i = 0; i < process_count; ++i) {
    ...
    int process_send_size = (process_index == process_count - 1)
                                ? static_cast<int>(this->i_boundary_filtered.size() -
                                                   single_process_portion * process_index)
                                : static_cast<int>(single_process_portion);

    process_send_size =
        std::min<int>(process_send_size, static_cast<int>(this->i_boundary_filtered.size() -
                                                          single_process_portion * process_index));
    ...
    int process_recv_size =
        (i == process_count - 1)
            ? static_cast<int>(this->i_boundary_filtered.size() - single_process_portion * i)
            : static_cast<int>(single_process_portion);
...
#pragma omp barrier
  ...
  size_t result_matrix_cols = storage->getSize();  // (size_t) (ceil((double)storage->getSize() /
  ...
  if (matrix_needed_size_bytes > matrix_max_size_gb * 1024 * 1024 * 1024) {
    ...
    snprintf(exception_string, sizeof(exception_string)
            "OperationLaplaceVectorizedLinearBoundary::init : More memory (= %i GB) needed to "
            "store the operation matrix, Please set the SGPP_PDE_MATRIX_SIZE_GB environment "
            "variable!",
...
double OperationLTwoDotLaplaceVectorizedLinearBoundary::gradient_dirichlet(size_t i, size_t j,
                                                                           size_t dim) {
  double grad;
  ...
  bool doGrad =
      ((i_level_grad == j_level_grad) && (i_index_grad == j_index_grad) && (i_level_grad != 1));
...
double OperationLTwoDotLaplaceVectorizedLinearBoundary::l2dot_dirichlet(size_t i, size_t j,
                                                                        size_t dim) {
  ...
  double temp_res = (temp_res_inner * (l2d != 1)) +
                    temp_res_leftbound * ((l2d == 1) && (i2d == 0)) +
                    temp_res_rightbound * ((l2d == 1) && (i2d == 1));
  temp_res *= (0.5 * in_l1d);
...
#if defined(__MIC__)
void OperationLTwoDotLaplaceVectorizedLinearBoundary::mult_dirichlet_mic(size_t process_i_start,
                                                                         size_t process_i_end) {
  {
    std::size_t padded_size = this->level_->getNcols();
    ...
    __m512d mm_timestep_coeff =
        _mm512_extload_pd(&TimestepCoeff, _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);
    size_t thr_start;
    size_t thr_end;
      ...
      for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
        ...
        for (size_t dim = 0; dim < max_dims; dim++) {
          ...
          __m512d mm_lcl_q_inv = _mm512_extload_pd(lcl_q_inv_temp_ptr_ + dim, _MM_UPCONV_PD_NONE,
                                                   _MM_BROADCAST_1X8, _MM_HINT_NONE);
          // std::cout << ii << " " << level_boundary_filtered_->getNrows() << dim << " " <<
          // level_boundary_filtered_->getNcols() << std::endl;
          ...
          __mmask8 mm_doGrad = (__mmask8)(
              _mm512_kand(_mm512_cmp_pd_mask(mm_lid, mm_ljd, _MM_CMPINT_EQ),
          ...
          __m512d mm_in_lid =
              _mm512_extload_pd(level_int_boundary_filtered_ptr_ + i_idx, _MM_UPCONV_PD_NONE,
                                _MM_BROADCAST_1X8, _MM_HINT_NONE);
          __m512d mm_in_ljd = _mm512_load_pd(temp_level_int_ptr);
          __m512d mm_res_one = _mm512_mask_mul_pd(
              mm_zero, (__mmask8)(_mm512_kand(_mm512_cmp_pd_mask(mm_iid, mm_ijd, _MM_CMPINT_EQ),
                                              _mm512_cmp_pd_mask(mm_ljd, mm_one, _MM_CMPINT_NE))),
              mm_two_thirds, mm_in_lid);
          ...
          __mmask8 mm_overlap = _mm512_cmp_pd_mask(
              ...
              _MM_CMPINT_LT);  // 6+1
          __m512d mm_temp_res_inner = _mm512_sub_pd(
              _mm512_sub_pd(
                  mm_two,
          ...
          __m512d mm_temp_res = _mm512_mask_blend_pd(
              _mm512_cmp_pd_mask(mm_l2d, mm_one, _MM_CMPINT_EQ), mm_temp_res_inner,
          ...
          __m512d mm_res_one2 = _mm512_mask_mul_pd(
              mm_zero, (__mmask8)(_mm512_kand(_mm512_cmp_pd_mask(mm_iid, mm_ijd2, _MM_CMPINT_EQ),
                                              _mm512_cmp_pd_mask(mm_ljd2, mm_one, _MM_CMPINT_NE))),
              mm_two_thirds, mm_in_lid);
          ...
          __m512d mm_temp_res_inner2 = _mm512_sub_pd(
              _mm512_sub_pd(
                  mm_two,
          ...
          __m512d mm_temp_res2 = _mm512_mask_blend_pd(
              _mm512_cmp_pd_mask(mm_l2d2, mm_one, _MM_CMPINT_EQ), mm_temp_res_inner2,
              _mm512_mask_blend_pd(_mm512_cmp_pd_mask(mm_i2d2, mm_one, _MM_CMPINT_EQ),
                                   mm_temp_res_leftbound2, mm_temp_res_rightbound2));
          mm_temp_res2 =
              _mm512_mul_pd(mm_temp_res2, _mm512_mul_pd(mm_half, mm_in_l1d2));  // 2 flops
        ...
        __m512d mm_temp2 =
            _mm512_load_pd(l2dot_temp_ptr + temp_cols + VECTOR_SIZE * (max_dims - 1));
...
void OperationLTwoDotLaplaceVectorizedLinearBoundary::mult_dirichlet(
...
#pragma omp parallel
    mult_dirichlet_mic(process_i_start, process_i_end);
    ...
    for (size_t ii = thr_start; ii < thr_end; ii++) {
      ...
      for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
        ...
        for (size_t dim = 0; dim < max_dims; dim++) {
          ...
          __m256d mm_doGrad =
              _mm256_and_pd(_mm256_cmp_pd(mm_lid, mm_ljd, _CMP_EQ_OQ),
          ...
          __m256d mm_res_one = _mm256_mul_pd(
              mm_two_thirds,
              _mm256_and_pd(mm_in_lid,
                            _mm256_and_pd(_mm256_cmp_pd(mm_iid, mm_ijd, _CMP_EQ_OQ),
                                          _mm256_cmp_pd(mm_ljd, mm_one, _CMP_NEQ_OQ))));  // 1+2
          ...
          __m256d mm_temp_res_inner = _mm256_sub_pd(
              _mm256_sub_pd(
                  mm_two,
          ...
          __m256d mm_temp_res = _mm256_blendv_pd(
              mm_temp_res_inner, _mm256_blendv_pd(mm_temp_res_leftbound, mm_temp_res_rightbound,
          ...
          __m256d mm_res_one2 = _mm256_mul_pd(
              mm_two_thirds,
              _mm256_and_pd(mm_in_lid, _mm256_and_pd(_mm256_cmp_pd(mm_iid, mm_ijd2, _CMP_EQ_OQ),
                                                     _mm256_cmp_pd(mm_ljd2, mm_one,
                                                                   _CMP_NEQ_OQ))));  // 2 // +1
          ...
          __m256d mm_temp_res_inner2 = _mm256_sub_pd(
              _mm256_sub_pd(
                  mm_two,
          ...
          __m256d mm_temp_res2 = _mm256_blendv_pd(
              mm_temp_res_inner2, _mm256_blendv_pd(mm_temp_res_leftbound2, mm_temp_res_rightbound2,
        ...
        __m256d mm_temp2 =
            _mm256_load_pd(l2dot_temp_ptr + temp_cols + VECTOR_SIZE * (max_dims - 1));
    ...
    for (size_t ii = thr_start; ii < thr_end; ii++) {
      ...
      for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
        ...
        for (size_t dim = 0; dim < max_dims; dim++) {
          ...
          __m128d mm_doGrad = _mm_and_pd(
              _mm_cmpeq_pd(mm_lid, mm_ljd),
              _mm_and_pd(_mm_cmpeq_pd(mm_iid, mm_ijd), _mm_cmpneq_pd(mm_lid, mm_one)));  // +5
          ...
          __m128d mm_res_one =
              _mm_mul_pd(mm_two_thirds,
                         _mm_and_pd(mm_in_lid, _mm_and_pd(_mm_cmpeq_pd(mm_iid, mm_ijd),
                                                          _mm_cmpneq_pd(mm_ljd, mm_one))));  // 1+2
          ...
          __m128d mm_temp_res_inner = _mm_sub_pd(
              _mm_sub_pd(mm_two,
          ...
          __m128d mm_temp_res = _mm_blendv_pd(
              mm_temp_res_inner, _mm_blendv_pd(mm_temp_res_leftbound, mm_temp_res_rightbound,
          ...
          __m128d mm_doGrad2 = _mm_and_pd(
              _mm_cmpeq_pd(mm_lid, mm_ljd2),
              _mm_and_pd(_mm_cmpeq_pd(mm_iid, mm_ijd2), _mm_cmpneq_pd(mm_lid, mm_one)));  // 1 // +2
          ...
          __m128d mm_res_one2 = _mm_mul_pd(
              mm_two_thirds, _mm_and_pd(mm_in_lid, _mm_and_pd(_mm_cmpeq_pd(mm_iid, mm_ijd2),
                                                              _mm_cmpneq_pd(mm_ljd2,
                                                                            mm_one))));  // 2 // +1
          ...
          __m128d mm_temp_res_inner2 = _mm_sub_pd(
              _mm_sub_pd(mm_two,
          ...
          __m128d mm_temp_res2 = _mm_blendv_pd(
              mm_temp_res_inner2, _mm_blendv_pd(mm_temp_res_leftbound2, mm_temp_res_rightbound2,
...
#pragma omp parallel
  {
    ...
    for (size_t i = thr_start; i < thr_end; i++) {
      ...
      for (size_t j = 0; j < this->storage->getSize(); j++) {
        ...
        for (size_t d_outer = 0; d_outer < this->storage->getDimension(); d_outer++) {
          ...
          temp += this->TimestepCoeff * (this->lambda_->get(d_outer)) * element *
    ...
    /*
      ...
      (gradient_temp_ptr[d_inner] * (d_outer == d_inner)));
                    ...
                    (result_boundary_filtered_->getPointer()[ii]) += (this->lambda_->get(d_outer)) *
      element;
                  }
                }
            }
...
#pragma prefetch
  ...
  /*
   MPI_Alltoallv(result_boundary_filtered_ptr, send_size.data(), send_start.data(), MPI_DOUBLE,
           ...
           MPI_COMM_WORLD);
  */
  MPI_Allgatherv(MPI_IN_PLACE, send_size[0], MPI_DOUBLE, result_boundary_filtered_ptr,
                 recv_size.data(), recv_start.data(), MPI_DOUBLE, MPI_COMM_WORLD);
/*
MPI_Allreduce(MPI_IN_PLACE, result_boundary_filtered_ptr,
static_cast<int>result_boundary_filtered_->getSize(),
MPI_DOUBLE,
...
void OperationLTwoDotLaplaceVectorizedLinearBoundary::mult(sgpp::base::DataVector& alpha,
  ...
  if (dirichlet) {
    ...
    throw sgpp::base::operation_exception(
        "OperationLTwoDotLaplaceVectorizedLinearBoundary::mult : This method is only available on "

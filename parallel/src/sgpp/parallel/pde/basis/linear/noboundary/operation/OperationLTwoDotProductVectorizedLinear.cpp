...
#endif
{
  std::cout << "IN CONSTRUSTOR: OperationLTwoDotProductVectorizedLinear" <<
            std::endl;
  init_constants();
  init_grid_storage();
}
...
OperationLTwoDotProductVectorizedLinear::~OperationLTwoDotProductVectorizedLinear() {
  ...
  double flop = (double) (23 * storage->getDimension() ) * (double) (
                  storage->getSize() * storage->getSize());
  double gflops = (all_iterations * flop / all_time) / 1000000000;
  double bandwidth = all_iterations * sizeof(double) * (double) (
                       storage->getSize() * storage->getSize()) / all_time ;
  std::cout << "IN OPERATOR : LTDOT, GFLOPS :" << gflops << " BANDWIDTH :" <<
...
void OperationLTwoDotProductVectorizedLinear::init_grid_storage() {
  ...
  if (matrix_needed_size_bytes > matrix_max_size_gb * 1024 * 1024 * 1024) {
    ...
    sprintf(exception_string,
            "OperationLTwoDotProductVectorizedLinear::init : More memory (= %i GB) needed to store the operation matrix, Please set the SGPP_PDE_MATRIX_SIZE_GB environment variable!",
...
double OperationLTwoDotProductVectorizedLinear::l2dot(size_t i, size_t j,
    size_t dim) {
...
void OperationLTwoDotProductVectorizedLinear::mult(SGPP::base::DataVector&

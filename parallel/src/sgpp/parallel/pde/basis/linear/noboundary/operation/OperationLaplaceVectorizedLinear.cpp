...
OperationLaplaceVectorizedLinear::~OperationLaplaceVectorizedLinear() {
  ...
  std::cout << "IN OPERATOR : LAPLACE, GFLOPS :" << gflops << " BANDWIDTH :" <<
            bandwidth / (1000000000.0) << " GB/s" << " ITERATIONS :" << all_iterations <<
            " TIME :" << all_time << std::endl;
  //std::cout << gflops <<" " << bandwidth << " " << all_time << " " << all_iterations <<" "<< storage->getSize() << '\n';
  delete this->level_;
...
void OperationLaplaceVectorizedLinear::init_grid_storage() {
  ...
  if (matrix_needed_size_bytes > matrix_max_size_gb * 1024 * 1024 * 1024) {
    ...
    sprintf(exception_string,
            "OperationLaplaceVectorizedLinear::init : More memory (= %i GB) needed to store the operation matrix, Please set the SGPP_PDE_MATRIX_SIZE_GB environment variable!",
...
double OperationLaplaceVectorizedLinear::gradient(size_t i, size_t j,
    size_t dim) {
  double grad;

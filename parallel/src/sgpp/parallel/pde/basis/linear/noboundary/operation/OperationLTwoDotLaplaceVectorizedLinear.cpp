...
namespace SGPP {
  namespace parallel {
    ...
    OperationLTwoDotLaplaceVectorizedLinear::OperationLTwoDotLaplaceVectorizedLinear(SGPP::base::GridStorage* storage)
      : storage(storage),
        level_(NULL),
        level_int_(NULL),
        index_(NULL),
    ...
    OperationLTwoDotLaplaceVectorizedLinear::OperationLTwoDotLaplaceVectorizedLinear(SGPP::base::GridStorage* storage, SGPP::base::DataVector& lambda)
      : storage(storage),
        level_(NULL),
        level_int_(NULL),
        index_(NULL),
    ...
    OperationLTwoDotLaplaceVectorizedLinear::~OperationLTwoDotLaplaceVectorizedLinear() {
      double flop = (double) (26 * storage->dim() + storage->dim() * storage->dim()) * (double) (storage->size() * storage->size());
    ...
    void OperationLTwoDotLaplaceVectorizedLinear::reset() {
      init_grid_storage();
    }
    ...
    void OperationLTwoDotLaplaceVectorizedLinear::init_grid_storage() {
      if (this->level_)
        delete this->level_;
      ...
      if (matrix_needed_size_bytes > matrix_max_size_gb * 1024 * 1024 * 1024) {
        ...
        sprintf(exception_string, "OperationLTwoDotLaplaceVectorizedLinear::init : More memory (= %i GB) needed to store the operation matrix, Please set the SGPP_PDE_MATRIX_SIZE_GB environment variable!", (int) matrix_needed_size_gb);
    ...
    double OperationLTwoDotLaplaceVectorizedLinear::gradient(size_t i, size_t j, size_t dim) {
      double grad;
      ...
      return (grad * *(this->lcl_q_inv_->getPointer() + dim));
    }

    double OperationLTwoDotLaplaceVectorizedLinear::l2dot(size_t i, size_t j, size_t dim) {
...
#define ALEX
    ...
    void OperationLTwoDotLaplaceVectorizedLinear::mult(SGPP::base::DataVector& alpha, SGPP::base::DataVector& result) {
      result.setAll(0.0);

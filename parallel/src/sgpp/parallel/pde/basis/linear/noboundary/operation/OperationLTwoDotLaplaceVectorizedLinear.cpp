...
OperationLTwoDotLaplaceVectorizedLinear::~OperationLTwoDotLaplaceVectorizedLinear() {
  ...
  double bandwidth = all_iterations *
                     ...
                     all_time;
  std::cout << "IN OPERATOR : COMBINED, GFLOPS :" << gflops
...
void OperationLTwoDotLaplaceVectorizedLinear::reset() { init_grid_storage(); }
...
#pragma omp barrier
  ...
  if (matrix_needed_size_bytes > matrix_max_size_gb * 1024 * 1024 * 1024) {
    ...
    snprintf(exception_string, sizeof(exception_string)
            "OperationLTwoDotLaplaceVectorizedLinear::init : More memory (= %i GB) needed to store "
...
#ifdef __MIC__
void OperationLTwoDotLaplaceVectorizedLinear::mult_mic(size_t process_i_start, size_t process_i_end,
...
void OperationLTwoDotLaplaceVectorizedLinear::mult(SGPP::base::DataVector& alpha,
                                                   SGPP::base::DataVector& result) {
  result.setAll(0.0);
...
#pragma omp parallel
    mult_mic(process_i_start, process_i_end, result);
#elif defined(__SSE4_2__) && defined(__AVX__)
...
/*
MPI_Allreduce(MPI_IN_PLACE, result_ptr, static_cast<int>result.getSize(), MPI_DOUBLE,

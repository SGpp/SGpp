...
#endif
{
  std::cout << "IN CONSTRUSTOR: OperationLTwoDotLaplaceVectorizedLinear" <<
            std::endl;
  init_constants();
  init_grid_storage();
...
void OperationLTwoDotLaplaceVectorizedLinear::init_grid_storage() {
  ...
  if (matrix_needed_size_bytes > matrix_max_size_gb * 1024 * 1024 * 1024) {
    ...
    sprintf(exception_string,
            "OperationLTwoDotLaplaceVectorizedLinear::init : More memory (= %i GB) needed to store the operation matrix, Please set the SGPP_PDE_MATRIX_SIZE_GB environment variable!",
...
double OperationLTwoDotLaplaceVectorizedLinear::gradient(size_t i, size_t j,
    size_t dim) {
  double grad;
  ...
  bool doGrad = ((i_level_grad == j_level_grad)
                 && (i_index_grad == j_index_grad));
  grad = i_level_grad * 2.0 * static_cast<double>(static_cast<int>(doGrad));

  return (grad** (this->lcl_q_inv_->getPointer() + dim));
}

double OperationLTwoDotLaplaceVectorizedLinear::l2dot(size_t i, size_t j,
    size_t dim) {
...
void OperationLTwoDotLaplaceVectorizedLinear::mult(SGPP::base::DataVector&
  ...
  memcpy(alpha_padded_->getPointer(), alpha.getPointer(),
    ...
    #pragma omp parallel
    {
      ...
      __m512d mm_timestep_coeff = _mm512_extload_pd(&TimestepCoeff,
...
#pragma prefetch(temp_level_ptr, temp_index_ptr, alpha_padded_temp_ptr_)
          ...
          for (size_t dim = 0; dim < max_dims; dim++) {
            ...
            __m512d mm_in_lid = _mm512_extload_pd(level_int_ptr_ + i_idx,
                                                  _MM_UPCONV_PD_NONE, _MM_BROADCAST_1X8, _MM_HINT_NONE);
            __m512d mm_in_ljd = _mm512_load_pd(temp_level_int_ptr);
            __m512d mm_res_one = _mm512_mask_mul_pd(mm_zero, _mm512_cmp_pd_mask(mm_iid,
                                                    mm_ijd, _MM_CMPINT_EQ ), mm_two_thirds, mm_in_lid); //1+2
            ...
            __mmask8 mm_overlap = _mm512_cmp_pd_mask(_mm512_max_pd(mm_q,
            ...
            __m512d mm_res_one2 = _mm512_mask_mul_pd(mm_zero, _mm512_cmp_pd_mask(mm_iid,
                                  mm_ijd2, _MM_CMPINT_EQ ), mm_two_thirds, mm_in_lid); //1+2
            ...
            __mmask8 mm_overlap2 = _mm512_cmp_pd_mask(_mm512_max_pd(mm_q2,
          ...
          __m512d mm_temp2 = _mm512_load_pd(l2dot_temp_ptr + temp_cols + VECTOR_SIZE *
                                            (max_dims - 1));
        ...
        for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
          ...
          for (size_t dim = 0; dim < max_dims; dim++) {
            ...
            __m256d mm_res_one = _mm256_mul_pd(mm_two_thirds, _mm256_and_pd(mm_in_lid,
                                               _mm256_cmp_pd(mm_iid, mm_ijd, _CMP_EQ_OQ))); //1+2
            ...
            __m256d mm_res_one2 = _mm256_mul_pd(mm_two_thirds, _mm256_and_pd(mm_in_lid,
                                                _mm256_cmp_pd(mm_iid, mm_ijd2, _CMP_EQ_OQ))); //2 // +1
          ...
          __m256d mm_temp2 = _mm256_load_pd(l2dot_temp_ptr + temp_cols + VECTOR_SIZE *
                                            (max_dims - 1));
        ...
        for (size_t j = 0; j < padded_size; j += VECTOR_SIZE * REG_BCOUNT) {
          ...
          for (size_t dim = 0; dim < max_dims; dim++) {
            ...
            __m128d mm_res_one = _mm_mul_pd(mm_two_thirds, _mm_and_pd(mm_in_lid,
                                            _mm_cmpeq_pd(mm_iid, mm_ijd))); //1+2
            ...
            __m128d mm_res_one2 = _mm_mul_pd(mm_two_thirds, _mm_and_pd(mm_in_lid,
                                             _mm_cmpeq_pd(mm_iid, mm_ijd2))); //2 // +1
          ...
          __m128d mm_temp2 = _mm_load_pd(l2dot_temp_ptr + temp_cols + VECTOR_SIZE *
                                         (max_dims - 1));

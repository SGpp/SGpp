...
DynamicTwoPartitionAutoTuning::DynamicTwoPartitionAutoTuning(size_t problemSize,
    size_t partition2Divider, size_t retune_cycles) :
...
void DynamicTwoPartitionAutoTuning::autoTune() {
  ...
  if (((_tuneCounter % _retune) == 0)) {
    if (_timePartition1 != 0 && _timePartition2 != 0) {
      double partition1_element_time = static_cast<double>(_sizePartition1) /
                                       static_cast<double>(_timePartition1);
      double partition2_element_time = static_cast<double>(_problemSize -
                                       _sizePartition1) / static_cast<double>(_timePartition2);
    ...
    if (_partition2_speedup == 0 && _timePartition1 / _timePartition2 > 3) {
      _partition2_speedup = 2.0 * static_cast<double>(_partition2Divider) /
                            static_cast<double>(_problemSize - _partition2Divider);
    }
  }

  double normalized_workingset = static_cast<double>(_problemSize) /
                                 (_partition2_speedup + 0.93);

  partition2 = static_cast<size_t>(normalized_workingset * _partition2_speedup);

  if (partition2 < _partition2Divider /
      2) {   // don't use an accelerator for too small chunks
    _sizePartition1 = _problemSize;
    return;
  }

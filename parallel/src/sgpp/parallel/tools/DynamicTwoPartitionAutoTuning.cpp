...
void DynamicTwoPartitionAutoTuning::autoTune() {
  ...
  if (((_tuneCounter % _retune) == 0)) {
    if (_timePartition1 != 0 && _timePartition2 != 0) {
      double partition1_element_time =
          static_cast<double>(_sizePartition1) / static_cast<double>(_timePartition1);
      double partition2_element_time = static_cast<double>(_problemSize - _sizePartition1) /
                                       static_cast<double>(_timePartition2);
    ...
    // can try again with a small chunksize for partition2. The speedup assigned inside the if
    // results in a size of

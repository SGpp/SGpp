...
TwoPartitionAutoTuning::TwoPartitionAutoTuning(size_t problemSize,
...
void TwoPartitionAutoTuning::doTune() {
  ...
  std::cout << "AUTOTUNING-PARTITION-SIZES (" << std::setw(
              8) << _problemSize << "):"
  ...
  std::cout << " Time2: " << std::setprecision(3) << _timePartition2
            ...
            << std::setprecision(1)
            << " (" << 100.0 * (double)(_problemSize - getPartition1Size()) /
            (double)_problemSize << "%)" << std::endl;
  std::cout.flags(f);
  std::cout.precision(prec);
}

void TwoPartitionAutoTuning::setExecutionTimes(double timePartition1,
    double timePartition2) {

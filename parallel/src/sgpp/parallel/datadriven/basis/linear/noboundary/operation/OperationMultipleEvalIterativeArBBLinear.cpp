...
#ifdef ARBB_ARRAY
    time = myArBBKernels2D->multArBB(ptrAlpha, ptrData, ptrLevel, ptrIndex,
                                     ptrResult, result_size, storageSize, dims);
#else
    time = myArBBKernels->multArBB(ptrAlpha, ptrData, ptrLevel, ptrIndex, ptrResult,
                                   result_size, storageSize, dims);
#endif
  } else if (this->dataset_->getNcols() == 4) {
#ifdef ARBB_ARRAY
    time = myArBBKernels4D->multArBB(ptrAlpha, ptrData, ptrLevel, ptrIndex,
                                     ptrResult, result_size, storageSize, dims);
#else
    time = myArBBKernels->multArBB(ptrAlpha, ptrData, ptrLevel, ptrIndex, ptrResult,
                                   result_size, storageSize, dims);
...
#ifdef ARBB_ARRAY
    time = myArBBKernels5D->multArBB(ptrAlpha, ptrData, ptrLevel, ptrIndex,
                                     ptrResult, result_size, storageSize, dims);
#else
    time = myArBBKernels->multArBB(ptrAlpha, ptrData, ptrLevel, ptrIndex, ptrResult,
                                   result_size, storageSize, dims);
...
#ifdef ARBB_ARRAY
    time = myArBBKernels10D->multArBB(ptrAlpha, ptrData, ptrLevel, ptrIndex,
                                      ptrResult, result_size, storageSize, dims);
#else
    time = myArBBKernels->multArBB(ptrAlpha, ptrData, ptrLevel, ptrIndex, ptrResult,
                                   result_size, storageSize, dims);
...
double OperationMultipleEvalIterativeArBBLinear::multTransposeVectorized(
  ...
  if (this->dataset_->getNrows() % 16 != 0
      || soruceSize != this->dataset_->getNrows()) {
  ...
  if (this->dataset_->getNcols() == 2) {
#ifdef ARBB_ARRAY
    time = myArBBKernels2D->multTransArBB(ptrSource, ptrData, ptrLevel, ptrIndex,
                                          ptrGlobalResult, soruceSize, storageSize, dims);
#else
    time = myArBBKernels->multTransArBB(ptrSource, ptrData, ptrLevel, ptrIndex,
                                        ptrGlobalResult, soruceSize, storageSize, dims);
#endif
  } else if (this->dataset_->getNcols() == 4) {
#ifdef ARBB_ARRAY
    time = myArBBKernels4D->multTransArBB(ptrSource, ptrData, ptrLevel, ptrIndex,
                                          ptrGlobalResult, soruceSize, storageSize, dims);
#else
    time = myArBBKernels->multTransArBB(ptrSource, ptrData, ptrLevel, ptrIndex,
                                        ptrGlobalResult, soruceSize, storageSize, dims);
...
#ifdef ARBB_ARRAY
    time = myArBBKernels5D->multTransArBB(ptrSource, ptrData, ptrLevel, ptrIndex,
                                          ptrGlobalResult, soruceSize, storageSize, dims);
#else
    time = myArBBKernels->multTransArBB(ptrSource, ptrData, ptrLevel, ptrIndex,
                                        ptrGlobalResult, soruceSize, storageSize, dims);
...
#ifdef ARBB_ARRAY
    time = myArBBKernels10D->multTransArBB(ptrSource, ptrData, ptrLevel, ptrIndex,
                                           ptrGlobalResult, soruceSize, storageSize, dims);
#else
    time = myArBBKernels->multTransArBB(ptrSource, ptrData, ptrLevel, ptrIndex,
                                        ptrGlobalResult, soruceSize, storageSize, dims);

...
class OCLKernelBase {
 ...
 public:
  cl_int createMultTrans(size_t dims, size_t local_workgroup_size, cl_context context,
    ...
    return OCLKernelBase::buildKernel(program_src, "multTransOCL", context, num_devices, device_ids,
                                      kernel);
  }

  cl_int createMult(size_t dims, size_t local_workgroup_size, cl_context context,
                    size_t num_devices, cl_device_id* device_ids, cl_kernel* kernel) {
    std::string program_src = generateSourceMult(dims, local_workgroup_size);
    return OCLKernelBase::buildKernel(program_src, "multOCL", context, num_devices, device_ids,
                                      kernel);
  }

 private:
  /**
   * @brief buildKernel builds the program that is represented by @a program_src and creates @a
   * num_devices kernel objects
   * that are stored into the array @a kernel (must be already allocated with at least @a
   * num_devices )
   *
   ...
   * @param kernel already allocated array: the resulting kernels are put into this array, one for
   * each device (=> at least num_devices entries)
   * @return
   */

...
class OCLKernelBase {
 ...
 public:
  cl_int createMultTrans(size_t dims, size_t local_workgroup_size,
    ...
    return OCLKernelBase::buildKernel(program_src, "multTransOCL", context,
  ...
  cl_int createMult(size_t dims, size_t local_workgroup_size, cl_context context,
    ...
    return OCLKernelBase::buildKernel(program_src, "multOCL", context, num_devices,
                                      device_ids, kernel);
  }
 ...
 private:
   ...
   * @return
   */
  static inline cl_int buildKernel(const std::string& program_src,
    ...
    cl_program program = clCreateProgramWithSource(context, 1, &kernel_src, NULL,
                         &err);

    if (err != CL_SUCCESS) {
      std::cout << "OCL Error: Failed to create program! Error Code: " << err <<
                std::endl;
    ...
    for (size_t i = 0; i < num_devices; i++) {
      ...
      if (err != CL_SUCCESS) {
        std::cout << "OCL Error: Failed to create kernel! Error Code: " << err <<
                  std::endl;
        return err;
      }
    }

...
#ifndef SPOCLKERNELIMPL_HPP
#define SPOCLKERNELIMPL_HPP
 ...
 public:
  ...
  inline void initOCLBuffers(sgpp::base::DataMatrixSP* level, sgpp::base::DataMatrixSP* index,
    ...
    if (level != NULL && clLevel[0] == NULL) {
      for (size_t i = 0; i < num_devices; i++) {
        clLevel[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                    sizeof(float) * storageSize, level->getPointer(), NULL);
      }
    }

    if (index != NULL && clIndex[0] == NULL) {
      for (size_t i = 0; i < num_devices; i++) {
        clIndex[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                    sizeof(float) * storageSize, index->getPointer(), NULL);
      }
    }

    if (mask != NULL && clMask[0] == NULL) {
      for (size_t i = 0; i < num_devices; i++) {
        clMask[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                   sizeof(float) * storageSize, mask->getPointer(), NULL);
      }
    }

    if (offset != NULL && clOffset[0] == NULL) {
      for (size_t i = 0; i < num_devices; i++) {
        clOffset[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                     sizeof(float) * storageSize, offset->getPointer(), NULL);
      }
    }
    ...
    if (clData[0] ==
      ...
      for (size_t i = 0; i < num_devices; i++) {
        clData[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                   sizeof(float) * dataset->getSize(), dataset->getPointer(), NULL);
      }
    }
  }
  ...
  inline void initParams(sgpp::base::DataVectorSP& grid, sgpp::base::DataVectorSP& tmp) {
    if (clPinnedGrid == NULL) {
      ...
      pinnedGrid = reinterpret_cast<float*>(clEnqueueMapBuffer(command_queue[0], clPinnedGrid,
    ...
    for (size_t i = 0; i < num_devices; i++) {
      clEnqueueWriteBuffer(command_queue[i], clDevGrid[i], CL_TRUE, 0,
                           sizeof(float) * grid.getSize(), pinnedGrid, 0, NULL, NULL);
    }
    ...
    if (clPinnedTmp == NULL) {
      ...
      pinnedTmp = reinterpret_cast<float*>(clEnqueueMapBuffer(command_queue[0], clPinnedTmp,
                                                              CL_TRUE, CL_MAP_READ | CL_MAP_WRITE,
                                                              0, mem_size, 0, NULL, NULL, NULL));
    }
    ...
    for (size_t i = 0; i < num_devices; i++) {
      clEnqueueWriteBuffer(command_queue[i], clDevTmp[i], CL_TRUE, 0, sizeof(float) * tmp.getSize(),
                           pinnedTmp, 0, NULL, NULL);
    }
  }
  ...
  double multImpl(sgpp::base::DataMatrixSP* level, sgpp::base::DataMatrixSP* index,
    ...
    for (size_t i = 0; i < num_devices; i++) {
      ...
      if (rangeSize > 0) {
        ...
        if (err != CL_SUCCESS) {
          std::cout << "OCL Error: Failed to enqueue kernel command! Error Code: " << err
                    << std::endl;
          return 0.0;
        }
      }
    ...
    for (size_t i = 0; i < num_devices; i++) {
      ...
      if (rangeSize > 0) {
        ...
        err = clEnqueueReadBuffer(command_queue[i], clDevTmp[i], CL_FALSE, sizeof(float) * offset,
                                  sizeof(float) * rangeSize, &(pinnedTmp[offset]), 0, NULL,
                                  &(GPUDone[i]));
  ...
  double multTransposeImpl(sgpp::base::DataMatrixSP* level, sgpp::base::DataMatrixSP* index,
    ...
    if (kernel_multTrans[0] == NULL) {
      OCLBasisType basis;
      basis.createMultTrans(dims, ocl_local_size, context, num_devices, device_ids,
                            kernel_multTrans);
    }
    ...
    for (size_t i = 0; i < num_devices; i++) {
      ...
      if (rangeSize > 0) {
        ...
        err = clEnqueueReadBuffer(command_queue[i], clDevGrid[i], CL_FALSE, sizeof(float) * offset,
                                  sizeof(float) * rangeSize, &(pinnedGrid[offset]), 0, NULL,
                                  &(GPUDone[i]));

        ...
        inline void initOCLBuffers(
          ...
          if (level != NULL && clLevel[0] == NULL) {
            for (size_t i = 0; i < num_devices; i++) {
              clLevel[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * storageSize, level->getPointer(), NULL);
            }
          }

          if (index != NULL && clIndex[0] == NULL) {
            for (size_t i = 0; i < num_devices; i++) {
              clIndex[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * storageSize, index->getPointer(), NULL);
            }
          }

          if (mask != NULL && clMask[0] == NULL) {
            for (size_t i = 0; i < num_devices; i++) {
              clMask[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * storageSize, mask->getPointer(), NULL);
            }
          }

          if (offset != NULL && clOffset[0] == NULL) {
            for (size_t i = 0; i < num_devices; i++) {
              clOffset[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * storageSize, offset->getPointer(), NULL);
            }
          }

          if (clData[0] == NULL) { // use first element as indicator if data has been already copied to device
            for (size_t i = 0; i < num_devices; i++) {
              clData[i] = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * dataset->getSize(), dataset->getPointer(), NULL);
            }
          }
        }

...
class SPOCLKernelImpl: public OCLKernelImplBase {
 ...
 public:
  ...
  inline void initParams(SGPP::base::DataVectorSP& grid,
                         SGPP::base::DataVectorSP& tmp) {
    if (clPinnedGrid == NULL) {
      ...
      pinnedGrid = (float*) clEnqueueMapBuffer(command_queue[0], clPinnedGrid,
    ...
    if (clPinnedTmp == NULL) {
      ...
      pinnedTmp = (float*) clEnqueueMapBuffer(command_queue[0], clPinnedTmp, CL_TRUE,
    ...
    for (size_t i = 0; i < num_devices; i++) {
      clEnqueueWriteBuffer(command_queue[i], clDevTmp[i], CL_TRUE, 0,
                           sizeof(float) * tmp.getSize(), pinnedTmp, 0, NULL, NULL);
    }
  }
    ...
    for (size_t i = 0; i < num_devices; i++) {
      ...
      if (rangeSize > 0) {
        ...
        err = clEnqueueReadBuffer(command_queue[i], clDevTmp[i], CL_FALSE,
                                  sizeof(float) * offset, sizeof(float) * rangeSize, &(pinnedTmp[offset]), 0,
                                  NULL, &(GPUDone[i]));
    ...
    for (size_t i = 0; i < num_devices; i++) {
      ...
      if (rangeSize > 0) {
        ...
        err = clEnqueueReadBuffer(command_queue[i], clDevGrid[i], CL_FALSE,
                                  sizeof(float) * offset, sizeof(float) * rangeSize, &(pinnedGrid[offset]), 0,
                                  NULL, &(GPUDone[i]));

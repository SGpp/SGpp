...
#pragma offload_transfer target(mic)
    ...
    if ((KernelImplementation::getChunkDataPoints() % SPX86SimdKernelBase::getChunkDataPoints()) !=
        0) {
      std::stringstream s;
      s << "For MIC symmetric execution: the chunksize of the MIC Kernel ("
        ...
        << SPX86SimdKernelBase::getChunkDataPoints() << ")!" << std::endl;
...
#endif
#endif
    std::cout << "Chunksize of MIC Kernel is " << KernelImplementation::getChunkDataPoints()
              << std::endl;
...
#pragma omp single
    ...
    // WARNING only works efficiently if there are more threads on the host than coprocessors in the
    // system atm
    size_t start;
    size_t end;
    ...
    for (size_t d = start; d < end; d++) {
      ...
      PartitioningTool::getPartitionSegment(
          start_index_data, end_index_data, micsp::number_mic_devices, d, &start_index_data_micdev,
          &end_index_data_micdev, KernelImplementation::getChunkDataPoints());
...
#pragma omp \
    parallel  // this parallel section is run on mic, using the hardwarethreads available there
        {
          ...
          PartitioningTool::getOpenMPPartitionSegment(
              ...
              &end_index_data_mic_thread, KernelImplementation::getChunkDataPoints());
          KernelImplementation::multImpl(
      ...
      micsp::transferResultMult(start_index_data_micdev, mychunk, d,
    ...
    size_t end_index_data_mic_thread;
    PartitioningTool::getOpenMPPartitionSegment(
        start_index_data, end_index_data, &start_index_data_mic_thread, &end_index_data_mic_thread,
        KernelImplementation::getChunkDataPoints());
    KernelImplementation::multImpl(
        micsp::ptrLevel, micsp::ptrIndex, micsp::ptrMask, micsp::ptrOffset, micsp::ptrData,
        alpha.getPointer(), result.getPointer(), result_size, dims, start_index_grid,
        end_index_grid, start_index_data_mic_thread, end_index_data_mic_thread);
#endif
  }

  static inline void multTranspose(sgpp::base::DataMatrixSP* level, sgpp::base::DataMatrixSP* index,
                                   sgpp::base::DataMatrixSP* mask, sgpp::base::DataMatrixSP* offset,
    ...
    parallel  // this parallel section is run on mic, using the hardwarethreads available there
        {
          ...
          KernelImplementation::multTransposeImpl(
              micsp::ptrLevel, micsp::ptrIndex, micsp::ptrMask, micsp::ptrOffset, micsp::ptrData,
              micsp::ptrDataMic, micsp::ptrAlphaMic, source_size, dims, start_index_grid_mic_thread,
              end_index_grid_mic_thread, start_index_data_micdev, end_index_data_micdev);
        }
      }

      if (micsp::multicard_multtrans_fast) {
        if (d > 0) {
          micsp::transferResultMultTrans(start_index_grid_micdev,
                                         end_index_grid_micdev - start_index_grid_micdev, d,
                                         micsp::tempgrid[d]);
        } else {
          micsp::transferResultMultTrans(start_index_grid_micdev,
                                         end_index_grid_micdev - start_index_grid_micdev, d,
...
#pragma omp barrier
    ...
    size_t end_index_grid_mic_thread;
    PartitioningTool::getOpenMPPartitionSegment(start_index_grid, end_index_grid,
                                                &start_index_grid_mic_thread,
                                                &end_index_grid_mic_thread, 1);
    KernelImplementation::multTransposeImpl(
        micsp::ptrLevel, micsp::ptrIndex, micsp::ptrMask, micsp::ptrOffset, micsp::ptrData,
        source.getPointer(), result.getPointer(), source_size, dims, start_index_grid_mic_thread,

...
#pragma offload_transfer target(mic)
          ...
          if ((KernelImplementation::getChunkDataPoints() % X86SimdKernelBase::getChunkDataPoints()) != 0) {
            std::stringstream s;
            s << "For MIC symmetric execution: the chunksize of the MIC Kernel (" << KernelImplementation::getChunkDataPoints() << ") needs to be a multiple of the chunksize of the CPU Kernel (" << X86SimdKernelBase::getChunkDataPoints() << ")!";
        ...
        static inline void mult(
          ...
          size_t start;
          size_t end;
          PartitioningTool::getOpenMPPartitionSegment(0, mic::number_mic_devices, &start, &end, 1);

          for (size_t d = start; d < end; d++) {
            size_t start_index_data_micdev;
            size_t end_index_data_micdev;
            PartitioningTool::getPartitionSegment(start_index_data, end_index_data, mic::number_mic_devices, d, &start_index_data_micdev, &end_index_data_micdev, KernelImplementation::getChunkDataPoints());
  ...
  in(result_size,dims, start_index_grid, end_index_grid, start_index_data_micdev, end_index_data_micdev)
            {
              ...
              #pragma omp parallel // this parallel section is run on mic, using the hardwarethreads available there
              {
                ...
                KernelImplementation::multImpl(mic::ptrLevel, mic::ptrIndex, mic::ptrMask, mic::ptrOffset, mic::ptrData, mic::ptrAlphaMic, mic::ptrDataMic,
                                               result_size, dims, start_index_grid, end_index_grid, start_index_data_mic_thread, end_index_data_mic_thread);
              }
            }
            mic::transferResultMult(start_index_data_micdev, mychunk, d, result.getPointer()); // workaround intel compiler bug
          }
          ...
          PartitioningTool::getOpenMPPartitionSegment(start_index_data, end_index_data,
              &start_index_data_mic_thread, &end_index_data_mic_thread, KernelImplementation::getChunkDataPoints());
          KernelImplementation::multImpl(mic::ptrLevel, mic::ptrIndex, mic::ptrMask, mic::ptrOffset, mic::ptrData, alpha.getPointer(), result.getPointer(),
                                         result_size, dims, start_index_grid, end_index_grid, start_index_data_mic_thread, end_index_data_mic_thread);
#endif
        }
        ...
        static inline void multTranspose(
          ...
          for (size_t d = start; d < end; d++) { //  multi device offload (done in parallel via OpenMP on host)
            ...
            if (mic::multicard_multtrans_fast) {
              ...
              PartitioningTool::getPartitionSegment(start_index_data, end_index_data, mic::number_mic_devices, d,
              ...
              PartitioningTool::getPartitionSegment(start_index_grid, end_index_grid, mic::number_mic_devices, d,
                                                    &start_index_grid_micdev, &end_index_grid_micdev, 1);
            }

            // transfer only the data that is needed for this coprocessor
            mic::transferInputMultTrans(start_index_data_micdev, end_index_data_micdev - start_index_data_micdev, source.getPointer(),
  ...
  in(source_size,dims, start_index_grid_micdev, end_index_grid_micdev, start_index_data_micdev, end_index_data_micdev, d)
            {
              ...
              #pragma omp parallel // this parallel section is run on mic, using the hardwarethreads available there
              {
                ...
                KernelImplementation::multTransposeImpl(mic::ptrLevel, mic::ptrIndex, mic::ptrMask, mic::ptrOffset, mic::ptrData, mic::ptrDataMic, mic::ptrAlphaMic,
            ...
            if (mic::multicard_multtrans_fast) {
              if (d > 0) {
                mic::transferResultMultTrans(start_index_grid_micdev, end_index_grid_micdev - start_index_grid_micdev, d, mic::tempgrid[d]);
              } else {
                mic::transferResultMultTrans(start_index_grid_micdev, end_index_grid_micdev - start_index_grid_micdev, d, result.getPointer());
              }
            } else {
              mic::transferResultMultTrans(start_index_grid_micdev, end_index_grid_micdev - start_index_grid_micdev, d, result.getPointer());
            }
          ...
          PartitioningTool::getOpenMPPartitionSegment(start_index_grid, end_index_grid,
              &start_index_grid_mic_thread, &end_index_grid_mic_thread, 1);
          KernelImplementation::multTransposeImpl(mic::ptrLevel, mic::ptrIndex, mic::ptrMask, mic::ptrOffset, mic::ptrData, source.getPointer(), result.getPointer(),

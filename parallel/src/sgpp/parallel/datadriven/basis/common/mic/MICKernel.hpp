...
#pragma offload_transfer target(mic)
    ...
    if ((KernelImplementation::getChunkDataPoints() % X86SimdKernelBase::getChunkDataPoints()) !=
        0) {
      std::stringstream s;
...
#pragma omp single
    ...
    for (size_t d = start; d < end; d++) {
      ...
      PartitioningTool::getPartitionSegment(
          start_index_data, end_index_data, mic::number_mic_devices, d, &start_index_data_micdev,
...
#pragma omp \
    parallel  // this parallel section is run on mic, using the hardwarethreads available there
        {
          ...
          KernelImplementation::multImpl(mic::ptrLevel, mic::ptrIndex, mic::ptrMask, mic::ptrOffset,
    ...
    PartitioningTool::getOpenMPPartitionSegment(
        ...
        KernelImplementation::getChunkDataPoints());
    KernelImplementation::multImpl(mic::ptrLevel, mic::ptrIndex, mic::ptrMask, mic::ptrOffset,
                                   mic::ptrData, alpha.getPointer(), result.getPointer(),
                                   result_size, dims, start_index_grid, end_index_grid,
                                   start_index_data_mic_thread, end_index_data_mic_thread);
#endif
  }
    ...
    for (size_t d = start; d < end;
      ...
      if (mic::multicard_multtrans_fast) {
        ...
        PartitioningTool::getPartitionSegment(start_index_grid, end_index_grid,
                                              mic::number_mic_devices, d, &start_index_grid_micdev,
                                              &end_index_grid_micdev, 1);
      }

      // transfer only the data that is needed for this coprocessor
      mic::transferInputMultTrans(

...
void arbb_multTrans(const arbb::dense<arbb::array<fp_Type, 2>, 1>& Data,
                    const arbb::dense<arbb::array<fp_Type, 2>, 1>& Level,
                    const arbb::dense<arbb::array<fp_Type, 2>, 1>& Index,
                    const arbb::dense<fp_Type>& source, arbb::dense<fp_Type>& result) {
  struct local {
    static void evalGridPointTrans(const arbb::dense<arbb::array<fp_Type, 2>, 1>& Data,
                                   const arbb::array<fp_Type, 2>& Level,
                                   const arbb::array<fp_Type, 2>& Index,
...
template <typename fp_Type>
void arbb_multTrans_mod(const arbb::dense<arbb::array<fp_Type, 2>, 1>& Data,
                        const arbb::dense<arbb::array<fp_Type, 2>, 1>& Level,
                        const arbb::dense<arbb::array<fp_Type, 2>, 1>& Index,
                        const arbb::dense<fp_Type>& source, arbb::dense<fp_Type>& result) {
  struct local {
    static void evalGridPointTrans(const arbb::dense<arbb::array<fp_Type, 2>, 1>& Data,
      ...
      _for(arbb::usize i = 0, i < Data.num_cols(), i++) {
        ...
        for (size_t l = 0; l < 2; l++) {
          _if(Level[l] == 2.0) {}
          _else_if(Index[l] == 1.0) { tmp *= arbb::max(2.0 - (Level[l] * curData[l]), 0.0); }
          _else_if(Index[l] == (Level[l] - 1.0)) {
            tmp *= arbb::max((Level[l] * curData[l]) - Index[l] + 1.0, 0.0);
          }
          _else { tmp *= arbb::max((1.0 - arbb::abs(((Level[l] * curData[l]) - Index[l]))), 0.0); }
          _end_if;
        }
...
template <typename fp_Type>
void arbb_mult(const arbb::dense<arbb::array<fp_Type, 2>, 1>& Data,
               const arbb::dense<arbb::array<fp_Type, 2>, 1>& Level,
               const arbb::dense<arbb::array<fp_Type, 2>, 1>& Index,
               const arbb::dense<fp_Type>& alpha, arbb::dense<fp_Type>& result) {
  struct local {
    static void evalGridPoint(const arbb::array<fp_Type, 2>& DataPoint,
                              const arbb::dense<arbb::array<fp_Type, 2>, 1>& Level,
                              const arbb::dense<arbb::array<fp_Type, 2>, 1>& Index,
...
template <typename fp_Type>
void arbb_mult_mod(const arbb::dense<arbb::array<fp_Type, 2>, 1>& Data,
                   const arbb::dense<arbb::array<fp_Type, 2>, 1>& Level,
                   const arbb::dense<arbb::array<fp_Type, 2>, 1>& Index,
                   const arbb::dense<fp_Type>& alpha, arbb::dense<fp_Type>& result) {
  struct local {
...
double ArBBKernels2D::multTransArBB(double* ptrSource, double* ptrData, double* ptrLevel,
  ...
  try {
    ...
    arbb::call(&(arbb_multTrans<arbb::f64>))(ArBB_Data_2D, ArBB_Level_2D, ArBB_Index_2D,
...
double ArBBKernels2D::multArBB(double* ptrAlpha, double* ptrData, double* ptrLevel,
  ...
  try {
    ...
    arbb::call(&(arbb_mult<arbb::f64>))(ArBB_Data_2D, ArBB_Level_2D, ArBB_Index_2D, ArBB_alpha,
                                        ArBB_result);
  } catch (const std::exception& e) {
    std::cout << "Error using Intel ArBB: " << e.what() << std::endl;
  }
...
double ArBBKernels2D::multTransSPArBB(float* ptrSource, float* ptrData, float* ptrLevel,
  ...
  try {
    ...
    arbb::call(&(arbb_multTrans<arbb::f32>))(ArBB_DataSP_2D, ArBB_LevelSP_2D, ArBB_IndexSP_2D,
                                             ArBB_source, ArBB_result);
  } catch (const std::exception& e) {
    std::cout << "Error using Intel ArBB: " << e.what() << std::endl;
  }
...
double ArBBKernels2D::multSPArBB(float* ptrAlpha, float* ptrData, float* ptrLevel, float* ptrIndex,
  ...
  try {
    ...
    arbb::call(&(arbb_mult<arbb::f32>))(ArBB_DataSP_2D, ArBB_LevelSP_2D, ArBB_IndexSP_2D,
...
double ArBBKernels2D::multModTransArBB(double* ptrSource, double* ptrData, double* ptrLevel,
                                       double* ptrIndex, double* ptrGlobalResult, size_t sourceSize,
                                       size_t storageSize, size_t dims) {
  double time = 0.0;
  ...
  try {
    ...
    arbb::call(&(arbb_multTrans_mod<arbb::f64>))(ArBB_Data_2D, ArBB_Level_2D, ArBB_Index_2D,
                                                 ArBB_source, ArBB_result);
  } catch (const std::exception& e) {
    std::cout << "Error using Intel ArBB: " << e.what() << std::endl;
  }
...
double ArBBKernels2D::multModArBB(double* ptrAlpha, double* ptrData, double* ptrLevel,
                                  double* ptrIndex, double* ptrResult, size_t result_size,
                                  size_t storageSize, size_t dims) {
  double time = 0.0;
  ...
  try {
    ...
    arbb::call(&(arbb_mult_mod<arbb::f64>))(ArBB_Data_2D, ArBB_Level_2D, ArBB_Index_2D, ArBB_alpha,
                                            ArBB_result);
  } catch (const std::exception& e) {
    std::cout << "Error using Intel ArBB: " << e.what() << std::endl;
  }
...
double ArBBKernels2D::multModTransSPArBB(float* ptrSource, float* ptrData, float* ptrLevel,
                                         float* ptrIndex, float* ptrGlobalResult, size_t sourceSize,
                                         size_t storageSize, size_t dims) {
  double time = 0.0;
  ...
  try {
    ...
    arbb::call(&(arbb_multTrans_mod<arbb::f32>))(ArBB_DataSP_2D, ArBB_LevelSP_2D, ArBB_IndexSP_2D,
                                                 ArBB_source, ArBB_result);
  } catch (const std::exception& e) {
    std::cout << "Error using Intel ArBB: " << e.what() << std::endl;
  }
...
double ArBBKernels2D::multModSPArBB(float* ptrAlpha, float* ptrData, float* ptrLevel,
                                    float* ptrIndex, float* ptrResult, size_t result_size,
                                    size_t storageSize, size_t dims) {
  double time = 0.0;
  ...
  try {
    ...
    arbb::call(&(arbb_mult_mod<arbb::f32>))(ArBB_DataSP_2D, ArBB_LevelSP_2D, ArBB_IndexSP_2D,
                                            ArBB_alpha, ArBB_result);
  } catch (const std::exception& e) {
    std::cout << "Error using Intel ArBB: " << e.what() << std::endl;
  }

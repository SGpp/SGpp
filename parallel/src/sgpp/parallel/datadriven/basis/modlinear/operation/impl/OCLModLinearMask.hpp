 ...
 private:
  virtual std::string generateSourceMult(size_t dims, size_t local_workgroup_size) {
    ...
    stream_program_src << "	__local " << getType<real_type>::asString() << " locIndex["
                       << dims * local_workgroup_size << "];" << std::endl;
    stream_program_src << "	__local " << getType<real_type>::asString() << " locMask["
                       << dims * local_workgroup_size << "];" << std::endl;
    stream_program_src << "	__local " << getType<real_type>::asString() << " locOffset["
                       << dims * local_workgroup_size << "];" << std::endl;
    ...
    for (size_t d = 0; d < dims; d++) {
      ...
      stream_program_src << "			abs = as_" << getType<real_type>::asString()
                         << "(as_" << getType<real_type>::intAsString() << "(index_calc) | as_"
    ...
    for (size_t d = 0; d < dims; d++) {
      stream_program_src << "		eval = ((ptrLevel[(m*" << dims << ")+" << d << "]) * (data_"
                         << d << "));" << std::endl;
      ...
      stream_program_src << "		abs = as_" << getType<real_type>::asString() << "(as_"
                         ...
                         << "]));" << std::endl;
      stream_program_src << "		last = ptrOffset[(m*" << dims << ")+" << d << "] + abs;"
                         << std::endl;
      stream_program_src << "		localSupport = fmax(last, 0.0"
                         << getType<real_type>::constSuffix() << ");" << std::endl;
  ...
  virtual std::string generateSourceMultTrans(size_t dims, size_t local_workgroup_size) {
    ...
    for (size_t d = 0; d < dims; d++) {
      ...
      stream_program_src << "			abs = as_" << getType<real_type>::asString()
                         << "(as_" << getType<real_type>::intAsString() << "(index_calc) | as_"
                         << getType<real_type>::intAsString() << "(mask_" << d << "));"
                         << std::endl;

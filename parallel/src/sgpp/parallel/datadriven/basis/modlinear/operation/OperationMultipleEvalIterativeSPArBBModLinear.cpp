...
double OperationMultipleEvalIterativeSPArBBModLinear::multVectorized(
...
#ifdef ARBB_ARRAY
    time = myArBBKernels2D->multModSPArBB(ptrAlpha, ptrData, ptrLevel, ptrIndex,
                                          ptrResult, result_size, storageSize, dims);
#else
    time = myArBBKernels->multModSPArBB(ptrAlpha, ptrData, ptrLevel, ptrIndex,
                                        ptrResult, result_size, storageSize, dims);
#endif
  }
...
double OperationMultipleEvalIterativeSPArBBModLinear::multTransposeVectorized(
  ...
  if (this->dataset_->getNcols() == 2) {
#ifdef ARBB_ARRAY
    time = myArBBKernels2D->multModTransSPArBB(ptrSource, ptrData, ptrLevel,
           ptrIndex, ptrGlobalResult, source_size, storageSize, dims);
#else
    time = myArBBKernels->multModTransSPArBB(ptrSource, ptrData, ptrLevel, ptrIndex,

 ...
 private:
  virtual std::string generateSourceMult(size_t dims,
    ...
    for (size_t d = 0; d < dims; d++) {
      ...
      stream_program_src << "				curSupport *= max(2.0" <<
                         getType<real_type>::constSuffix() << " - ( (locLevel[(k*" << dims << ")+" << d
                         << "]) * (data_" << d << ") ), 0.0" << getType<real_type>::constSuffix() <<
                         ") ;" << std::endl;
      stream_program_src << "			}" << std::endl;
      stream_program_src << "			else if ((locIndex[(k*" << dims << ")+" << d <<
    ...
    for (size_t d = 0; d < dims; d++) {
      ...
      stream_program_src << "		{" << std::endl;
      stream_program_src << "			curSupport *= max(2.0" <<
                         getType<real_type>::constSuffix() << " - ( (ptrLevel[(m*" << dims << ")+" << d
                         << "]) * (data_" << d << ") ), 0.0" << getType<real_type>::constSuffix() <<
                         ") ;" << std::endl;
      stream_program_src << "		}" << std::endl;
      stream_program_src << "		else if ((ptrIndex[(m*" << dims << ")+" << d <<
  ...
  virtual std::string generateSourceMultTrans(size_t dims,
    ...
    for (size_t d = 0; d < dims; d++) {
...
#ifdef USEOCL_LOCAL_MEMORY
      stream_program_src << "				curSupport *= max(2.0" <<
                         getType<real_type>::constSuffix() << " - ( (level_" << d << ") * (locData[(" <<
                         d << "*" << local_workgroup_size << ")+k]) ), 0.0" <<
                         getType<real_type>::constSuffix() << ") ;" << std::endl;
#else
      stream_program_src << "				curSupport *= max(2.0" <<
                         getType<real_type>::constSuffix() << " - ( (level_" << d << ") * (ptrData[(" <<
                         d << "*sourceSize)+k]) ), 0.0" << getType<real_type>::constSuffix() << ") ;" <<
                         std::endl;
#endif
      stream_program_src << "			}" << std::endl;
      ...
      stream_program_src << "			{" << std::endl;
#ifdef USEOCL_LOCAL_MEMORY
      stream_program_src << "				curSupport *= max(( (level_" << d << ") * (locData[("
                         << d << "*" << local_workgroup_size << ")+k]) ) - (index_" << d << ") + 1.0" <<
                         getType<real_type>::constSuffix() << ", 0.0" <<
                         getType<real_type>::constSuffix() << ");" << std::endl;
#else
      stream_program_src << "				curSupport *= max(( (level_" << d << ") * (ptrData[("
                         << d << "*sourceSize)+k]) ) - (index_" << d << ") + 1.0, 0.0);" << std::endl;
#endif
      stream_program_src << "			}" << std::endl;
      stream_program_src << "			else " << std::endl;
      stream_program_src << "			{" << std::endl;
      ...
      stream_program_src << "				curSupport *= max(1.0" <<
                         ...
                         ") * (locData[(" << d << "*" << local_workgroup_size << ")+k]) ) - (index_" << d
                         << ") ), 0.0" << getType<real_type>::constSuffix() << ");" << std::endl;
#else
      stream_program_src << "				curSupport *= max(1.0" <<
                         getType<real_type>::constSuffix() << " - fabs( ( (level_" << d <<
                         ") * (ptrData[(" << d << "*sourceSize)+k]) ) - (index_" << d << ") ), 0.0" <<
                         getType<real_type>::constSuffix() << ");" << std::endl;
#endif
      stream_program_src << "			}" << std::endl;
    }
    ...
    for (size_t i = start_index_data; i < end_index_data; i++) {
      ...
      for (size_t d = 0; d < dims; d++) {
        if (ptrLevel[(j * dims) + d] == 2.0) {
          ...
          curSupport *= std::max<double>(2.0 - ((ptrLevel[(j * dims) + d]) *
                                                (ptrData[(d * source_size) + i])), 0.0);
        } else if (ptrIndex[(j * dims) + d] == (ptrLevel[(j * dims) + d] - 1.0)) {
          curSupport *= std::max<double>(((ptrLevel[(j * dims) + d]) *
                                          (ptrData[(d * source_size) + i])) - ptrIndex[(j * dims) + d] + 1.0, 0.0);
        } else {
          curSupport *= std::max<double>(1.0 - fabs( ((ptrLevel[(j * dims) + d]) *
                                         (ptrData[(d * source_size) + i])) - ptrIndex[(j * dims) + d] ), 0.0);
        }
      }
    ...
    for (size_t i = start_index_data; i < end_index_data; i++) {
      ...
      for (size_t d = 0; d < dims; d++) {
        if (ptrLevel[(j * dims) + d] == 2.0f) {
          ...
          curSupport *= std::max<float>(2.0f - ((ptrLevel[(j * dims) + d]) *
                                                (ptrData[(d * source_size) + i])), 0.0f);
        } else if (ptrIndex[(j * dims) + d] == (ptrLevel[(j * dims) + d] - 1.0f)) {
          curSupport *= std::max<float>(((ptrLevel[(j * dims) + d]) *
                                         (ptrData[(d * source_size) + i])) - ptrIndex[(j * dims) + d] + 1.0f, 0.0f);
        } else {
          curSupport *= std::max<float>(1.0f - (float)fabs( ((ptrLevel[(j * dims) + d])
                                        * (ptrData[(d * source_size) + i])) - ptrIndex[(j * dims) + d] ), 0.0f);
        }
      }

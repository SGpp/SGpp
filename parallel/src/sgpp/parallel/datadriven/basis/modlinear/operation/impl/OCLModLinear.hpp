 ...
 private:
  virtual std::string generateSourceMult(size_t dims, size_t local_workgroup_size) {
    ...
    for (size_t d = 0; d < dims; d++) {
      ...
      stream_program_src << "{" << std::endl;
      stream_program_src << "curSupport *= 1.0" << getType<real_type>::constSuffix() << ";"
                         << std::endl;
      stream_program_src << "}" << std::endl;
      stream_program_src << "else if ((locIndex[(k*" << dims << ")+" << d << "]) == 1.0"
                         << getType<real_type>::constSuffix() << ")" << std::endl;
      stream_program_src << "{" << std::endl;
      stream_program_src << "curSupport *= max(2.0" << getType<real_type>::constSuffix()
                         << " - ( (locLevel[(k*" << dims << ")+" << d << "]) * (data_" << d
                         << ") ), 0.0" << getType<real_type>::constSuffix() << ") ;" << std::endl;
      stream_program_src << "}" << std::endl;
      stream_program_src << "else if ((locIndex[(k*" << dims << ")+" << d << "]) == ((locLevel[(k*"
                         << dims << ")+" << d << "]) - 1.0" << getType<real_type>::constSuffix()
                         << ") )" << std::endl;
      stream_program_src << "{" << std::endl;
      stream_program_src << "curSupport *= max(( (locLevel[(k*" << dims << ")+" << d
                         << "]) * (data_" << d << ") ) - (locIndex[(k*" << dims << ")+" << d
                         << "]) + 1.0" << getType<real_type>::constSuffix() << ", 0.0"
                         << getType<real_type>::constSuffix() << ");" << std::endl;
      stream_program_src << "}" << std::endl;
      stream_program_src << "else " << std::endl;
      stream_program_src << "{" << std::endl;
      stream_program_src << "curSupport *= max(1.0" << getType<real_type>::constSuffix()
                         << " - fabs( ( (locLevel[(k*" << dims << ")+" << d << "]) * (data_" << d
                         << ") ) - (locIndex[(k*" << dims << ")+" << d << "]) ), 0.0"
                         << getType<real_type>::constSuffix() << ");" << std::endl;
      stream_program_src << "}" << std::endl;
    }
    ...
    for (size_t d = 0; d < dims; d++) {
      ...
      stream_program_src << "else if ((ptrIndex[(m*" << dims << ")+" << d << "]) == ((ptrLevel[(m*"
                         ...
                         << ") )" << std::endl;
      stream_program_src << "{" << std::endl;
      stream_program_src << "curSupport *= max(( (ptrLevel[(m*" << dims << ")+" << d
                         << "]) * (data_" << d << ") ) - (ptrIndex[(m*" << dims << ")+" << d
                         << "]) + 1.0" << getType<real_type>::constSuffix() << ", 0.0"
                         << getType<real_type>::constSuffix() << ");" << std::endl;
      stream_program_src << "}" << std::endl;
      stream_program_src << "else " << std::endl;
      stream_program_src << "{" << std::endl;
      stream_program_src << "curSupport *= max(1.0" << getType<real_type>::constSuffix()
                         << " - fabs( ( (ptrLevel[(m*" << dims << ")+" << d << "]) * (data_" << d
                         << ") ) - (ptrIndex[(m*" << dims << ")+" << d << "]) ), 0.0"
                         << getType<real_type>::constSuffix() << ");" << std::endl;
      stream_program_src << "}" << std::endl;
    }
  ...
  virtual std::string generateSourceMultTrans(size_t dims, size_t local_workgroup_size) {
    ...
    for (size_t d = 0; d < dims; d++) {
      stream_program_src << "if ((level_" << d << ") == 2.0" << getType<real_type>::constSuffix()
                         << ")" << std::endl;
      stream_program_src << "{" << std::endl;
      stream_program_src << "curSupport *= 1.0" << getType<real_type>::constSuffix() << ";"
                         << std::endl;
      ...
      stream_program_src << "curSupport *= max(2.0" << getType<real_type>::constSuffix()
                         << " - ( (level_" << d << ") * (locData[(" << d << "*"
                         << local_workgroup_size << ")+k]) ), 0.0"
                         << getType<real_type>::constSuffix() << ") ;" << std::endl;
#else
      stream_program_src << "curSupport *= max(2.0" << getType<real_type>::constSuffix()
                         << " - ( (level_" << d << ") * (ptrData[(" << d
                         << "*sourceSize)+k]) ), 0.0" << getType<real_type>::constSuffix() << ") ;"
                         << std::endl;
#endif
      stream_program_src << "}" << std::endl;
      ...
      stream_program_src << "curSupport *= max(( (level_" << d << ") * (locData[(" << d << "*"
                         << local_workgroup_size << ")+k]) ) - (index_" << d << ") + 1.0"
      ...
      stream_program_src << "curSupport *= max(1.0" << getType<real_type>::constSuffix()
                         ...
                         << local_workgroup_size << ")+k]) ) - (index_" << d << ") ), 0.0"
                         << getType<real_type>::constSuffix() << ");" << std::endl;
#else
      stream_program_src << "curSupport *= max(1.0" << getType<real_type>::constSuffix()

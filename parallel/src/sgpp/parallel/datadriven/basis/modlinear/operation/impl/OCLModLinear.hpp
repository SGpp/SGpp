...
class OCLModLinear : public OCLKernelBase {
 ...
 private:
  virtual std::string generateSourceMult(size_t dims, size_t local_workgroup_size) {
    ...
    stream_program_src << "__attribute__((reqd_work_group_size(" << local_workgroup_size
                       << ", 1, 1)))" << std::endl;
    stream_program_src << "void multOCL(__global const " << getType<real_type>::asString()
                       << "* ptrLevel," << std::endl;
    stream_program_src << "           __global const " << getType<real_type>::asString()
                       << "* ptrIndex," << std::endl;
    stream_program_src << "           __global const " << getType<real_type>::asString()
                       << "* ptrMask,"
    ...
    stream_program_src << "           __global const " << getType<real_type>::asString()
                       << "* ptrAlpha," << std::endl;
    stream_program_src << "           __global       " << getType<real_type>::asString()
                       << "* ptrResult," << std::endl;
    ...
    stream_program_src << "	__local " << getType<real_type>::asString() << " locLevel["
                       << dims * local_workgroup_size << "];" << std::endl;
    stream_program_src << "	__local " << getType<real_type>::asString() << " locIndex["
                       << dims * local_workgroup_size << "];" << std::endl;
    stream_program_src << "	__local " << getType<real_type>::asString() << " locAlpha["
    ...
    stream_program_src << " uint fastChunkSizeGrid = (chunkSizeGrid / " << local_workgroup_size
                       << ") * " << local_workgroup_size << ";" << std::endl;
    ...
    stream_program_src << std::endl;
    stream_program_src << "		for(int k = 0; k < " << local_workgroup_size << "; k++)"
                       << std::endl;
    stream_program_src << "		{" << std::endl;
    stream_program_src << "			curSupport = locAlpha[k];" << std::endl
                       << std::endl;

    for (size_t d = 0; d < dims; d++) {
      stream_program_src << "			if ((locLevel[(k*" << dims << ")+" << d
                         << "]) == 2.0" << getType<real_type>::constSuffix() << ")" << std::endl;
      stream_program_src << "			{" << std::endl;
      stream_program_src << "				curSupport *= 1.0"
                         << getType<real_type>::constSuffix() << ";" << std::endl;
      stream_program_src << "			}" << std::endl;
      stream_program_src << "			else if ((locIndex[(k*" << dims << ")+" << d
                         << "]) == 1.0" << getType<real_type>::constSuffix() << ")" << std::endl;
      stream_program_src << "			{" << std::endl;
      stream_program_src << "				curSupport *= max(2.0"
                         << getType<real_type>::constSuffix() << " - ( (locLevel[(k*" << dims
                         << ")+" << d << "]) * (data_" << d << ") ), 0.0"
                         << getType<real_type>::constSuffix() << ") ;" << std::endl;
      stream_program_src << "			}" << std::endl;
      stream_program_src << "			else if ((locIndex[(k*" << dims << ")+" << d
                         << "]) == ((locLevel[(k*" << dims << ")+" << d << "]) - 1.0"
                         << getType<real_type>::constSuffix() << ") )" << std::endl;
      stream_program_src << "			{" << std::endl;
      stream_program_src << "				curSupport *= max(( (locLevel[(k*" << dims
                         << ")+" << d << "]) * (data_" << d << ") ) - (locIndex[(k*" << dims << ")+"
                         << d << "]) + 1.0" << getType<real_type>::constSuffix() << ", 0.0"
                         << getType<real_type>::constSuffix() << ");" << std::endl;
      stream_program_src << "			}" << std::endl;
      stream_program_src << "			else " << std::endl;
      stream_program_src << "			{" << std::endl;
      stream_program_src << "				curSupport *= max(1.0"
                         << getType<real_type>::constSuffix() << " - fabs( ( (locLevel[(k*" << dims
                         << ")+" << d << "]) * (data_" << d << ") ) - (locIndex[(k*" << dims << ")+"
                         << d << "]) ), 0.0" << getType<real_type>::constSuffix() << ");"
                         << std::endl;
      stream_program_src << "			}" << std::endl;
    }
    ...
    stream_program_src << "	}" << std::endl;
    stream_program_src << std::endl;
    stream_program_src << "	// Iterate over all grid points (slow ones, without cache)"
                       << std::endl;
    stream_program_src << " for(int m = start_grid + fastChunkSizeGrid; m < end_grid; m++)"
                       << std::endl;
    stream_program_src << "	{" << std::endl;
    ...
    for (size_t d = 0; d < dims; d++) {
      ...
      stream_program_src << "		{" << std::endl;
      stream_program_src << "			curSupport *= max(2.0"
                         << getType<real_type>::constSuffix() << " - ( (ptrLevel[(m*" << dims
                         << ")+" << d << "]) * (data_" << d << ") ), 0.0"
                         << getType<real_type>::constSuffix() << ") ;" << std::endl;
      stream_program_src << "		}" << std::endl;
      stream_program_src << "		else if ((ptrIndex[(m*" << dims << ")+" << d
                         << "]) == ((ptrLevel[(m*" << dims << ")+" << d << "]) - 1.0"
                         << getType<real_type>::constSuffix() << ") )" << std::endl;
      stream_program_src << "		{" << std::endl;
      stream_program_src << "			curSupport *= max(( (ptrLevel[(m*" << dims << ")+"
                         << d << "]) * (data_" << d << ") ) - (ptrIndex[(m*" << dims << ")+" << d
                         << "]) + 1.0" << getType<real_type>::constSuffix() << ", 0.0"
                         << getType<real_type>::constSuffix() << ");" << std::endl;
      stream_program_src << "		}" << std::endl;
      stream_program_src << "		else " << std::endl;
      stream_program_src << "		{" << std::endl;
      stream_program_src << "			curSupport *= max(1.0"
                         << getType<real_type>::constSuffix() << " - fabs( ( (ptrLevel[(m*" << dims
                         << ")+" << d << "]) * (data_" << d << ") ) - (ptrIndex[(m*" << dims << ")+"
                         << d << "]) ), 0.0" << getType<real_type>::constSuffix() << ");"
                         << std::endl;
      stream_program_src << "		}" << std::endl;
    }
  ...
  virtual std::string generateSourceMultTrans(size_t dims, size_t local_workgroup_size) {
    ...
    stream_program_src << "__attribute__((reqd_work_group_size(" << local_workgroup_size
                       << ", 1, 1)))" << std::endl;
    stream_program_src << "void multTransOCL(__global const " << getType<real_type>::asString()
                       << "* ptrLevel," << std::endl;
    stream_program_src << "           __global const " << getType<real_type>::asString()
                       << "* ptrIndex," << std::endl;
    stream_program_src << "           __global const " << getType<real_type>::asString()
    ...
    stream_program_src << "	" << getType<real_type>::asString()
                       ...
                       << std::endl;
#ifdef USEOCL_LOCAL_MEMORY
    stream_program_src << "	__local " << getType<real_type>::asString() << " locData["
                       << dims * local_workgroup_size << "];" << std::endl;
    stream_program_src << "	__local " << getType<real_type>::asString() << " locSource["
                       << local_workgroup_size << "];" << std::endl
                       << std::endl;
#endif
    ...
    stream_program_src << " for(int i = start_data; i < end_data; i+=" << local_workgroup_size
                       << ")" << std::endl;
    stream_program_src << "	{" << std::endl;

    for (size_t d = 0; d < dims; d++) {
      stream_program_src << "		locData[(" << d << "*" << local_workgroup_size
                         << ")+(localIdx)] = ptrData[(" << d << "*sourceSize)+(localIdx+i)];"
                         << std::endl;
    }
    ...
    stream_program_src << "		for(int k = 0; k < " << local_workgroup_size << "; k++)"
                       << std::endl;
    stream_program_src << "		{" << std::endl;
    stream_program_src << "			curSupport = locSource[k];" << std::endl
                       << std::endl;
#else
    ...
    for (size_t d = 0; d < dims; d++) {
...
#ifdef USEOCL_LOCAL_MEMORY
      stream_program_src << "				curSupport *= max(2.0"
                         << getType<real_type>::constSuffix() << " - ( (level_" << d
                         << ") * (locData[(" << d << "*" << local_workgroup_size << ")+k]) ), 0.0"
                         << getType<real_type>::constSuffix() << ") ;" << std::endl;
#else
      stream_program_src << "				curSupport *= max(2.0"
                         << getType<real_type>::constSuffix() << " - ( (level_" << d
                         << ") * (ptrData[(" << d << "*sourceSize)+k]) ), 0.0"
                         << getType<real_type>::constSuffix() << ") ;" << std::endl;
#endif
      stream_program_src << "			}" << std::endl;
      stream_program_src << "			else if ((index_" << d << ") == ((level_" << d
                         << ") - 1.0" << getType<real_type>::constSuffix() << ") )" << std::endl;
      stream_program_src << "			{" << std::endl;
#ifdef USEOCL_LOCAL_MEMORY
      stream_program_src << "				curSupport *= max(( (level_" << d
                         << ") * (locData[(" << d << "*" << local_workgroup_size
                         << ")+k]) ) - (index_" << d << ") + 1.0"
                         << getType<real_type>::constSuffix() << ", 0.0"
                         << getType<real_type>::constSuffix() << ");" << std::endl;
#else
      stream_program_src << "				curSupport *= max(( (level_" << d
                         << ") * (ptrData[(" << d << "*sourceSize)+k]) ) - (index_" << d
                         << ") + 1.0, 0.0);" << std::endl;
#endif
      stream_program_src << "			}" << std::endl;
      stream_program_src << "			else " << std::endl;
...
#ifdef USEOCL_LOCAL_MEMORY
      stream_program_src << "				curSupport *= max(1.0"
                         << getType<real_type>::constSuffix() << " - fabs( ( (level_" << d
                         << ") * (locData[(" << d << "*" << local_workgroup_size
                         << ")+k]) ) - (index_" << d << ") ), 0.0"
                         << getType<real_type>::constSuffix() << ");" << std::endl;
#else
      stream_program_src << "				curSupport *= max(1.0"
                         << getType<real_type>::constSuffix() << " - fabs( ( (level_" << d
                         << ") * (ptrData[(" << d << "*sourceSize)+k]) ) - (index_" << d
                         << ") ), 0.0" << getType<real_type>::constSuffix() << ");" << std::endl;
#endif
      stream_program_src << "			}" << std::endl;
    }
...
inline void OCLModLinear<double>::multDefault(
    ...
    SGPP::base::DataMatrix* /*mask*/,    // unused for this specialization
    SGPP::base::DataMatrix* /*offset*/,  // unused for this specialization
    SGPP::base::DataMatrix* dataset, SGPP::base::DataVector& alpha, SGPP::base::DataVector& result,
...
inline void OCLModLinear<double>::multTransposeDefault(
    ...
    SGPP::base::DataMatrix* /*mask*/,    // unused for this specialization
    SGPP::base::DataMatrix* /*offset*/,  // unused for this specialization
    SGPP::base::DataMatrix* dataset, SGPP::base::DataVector& source, SGPP::base::DataVector& result,
    const size_t start_index_grid, const size_t end_index_grid, const size_t start_index_data,
    const size_t end_index_data) {
  double* ptrLevel = level->getPointer();
  double* ptrIndex = index->getPointer();
    ...
    for (size_t i = start_index_data; i < end_index_data; i++) {
      ...
      for (size_t d = 0; d < dims; d++) {
        if (ptrLevel[(j * dims) + d] == 2.0) {
          ...
          curSupport *= std::max<double>(
              2.0 - ((ptrLevel[(j * dims) + d]) * (ptrData[(d * source_size) + i])), 0.0);
        } else if (ptrIndex[(j * dims) + d] == (ptrLevel[(j * dims) + d] - 1.0)) {
          curSupport *=
              std::max<double>(((ptrLevel[(j * dims) + d]) * (ptrData[(d * source_size) + i])) -
                                   ptrIndex[(j * dims) + d] + 1.0,
                               0.0);
        } else {
          curSupport *= std::max<double>(
              1.0 - fabs(((ptrLevel[(j * dims) + d]) * (ptrData[(d * source_size) + i])) -
                         ptrIndex[(j * dims) + d]),
              0.0);
        }
      }
    ...
    for (size_t i = start_index_data; i < end_index_data; i++) {
      ...
      for (size_t d = 0; d < dims; d++) {
        if (ptrLevel[(j * dims) + d] == 2.0f) {
          ...
          curSupport *= std::max<float>(
              2.0f - ((ptrLevel[(j * dims) + d]) * (ptrData[(d * source_size) + i])), 0.0f);
        } else if (ptrIndex[(j * dims) + d] == (ptrLevel[(j * dims) + d] - 1.0f)) {
          curSupport *=
              std::max<float>(((ptrLevel[(j * dims) + d]) * (ptrData[(d * source_size) + i])) -
                                  ptrIndex[(j * dims) + d] + 1.0f,
                              0.0f);
        } else {
          curSupport *= std::max<float>(
              1.0f - static_cast<float>(
                         fabs(((ptrLevel[(j * dims) + d]) * (ptrData[(d * source_size) + i])) -
                              ptrIndex[(j * dims) + d])),
              0.0f);
        }
      }

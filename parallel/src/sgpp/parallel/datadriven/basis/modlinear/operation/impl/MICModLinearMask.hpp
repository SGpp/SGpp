...
class MICModLinearMask : public MICKernelBase {
 public:
  ...
  static inline void multImpl(double* ptrLevel, double* ptrIndex, double* ptrMask,
                              double* ptrOffset, double* ptrData, double* ptrAlpha,
                              double* ptrResult, size_t result_size, size_t dims,
      ...
      for (size_t j = start_index_grid; j < end_index_grid; j++) {
        ...
        for (size_t d = 0; d < dims - 1; d++) {
          ...
          __m512d mask = _mm512_extload_pd(&(ptrMask[(j * dims) + d]), _MM_UPCONV_PD_NONE,
                                           _MM_BROADCAST_1X8, _MM_HINT_NONE);
          __m512d offset = _mm512_extload_pd(&(ptrOffset[(j * dims) + d]), _MM_UPCONV_PD_NONE,
                                             _MM_BROADCAST_1X8, _MM_HINT_NONE);
  ...
  static inline void multTransposeImpl(double* ptrLevel, double* ptrIndex, double* ptrMask,
                                       double* ptrOffset, double* ptrData, double* ptrSource,
                                       double* ptrResult, size_t sourceSize, size_t dims,
                                       const size_t start_index_grid, const size_t end_index_grid,
                                       const size_t start_index_data, const size_t end_index_data) {
#ifdef __MIC__

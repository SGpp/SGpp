...
datadriven::DMSystemMatrixBase* DMSystemMatrixMPITypeFactory::createDMSystemMatrixMPIType(
    base::Grid& grid, base::DataMatrix& trainDataset, double lambda, VectorizationType vecType,
    MPIType mpiType) {
  ...
  switch (mpiType) {
    case MPIAllreduce:
      ...
      result = new sgpp::parallel::DMSystemMatrixVectorizedIdentityAllreduce<KernelImplementation>(
          grid, trainDataset, lambda, vecType);
      break;

    case MPIAlltoallv:
      parallelizationType = "Alltoallv";
      result = new sgpp::parallel::DMSystemMatrixVectorizedIdentityMPI(grid, trainDataset, lambda,
                                                                       vecType);
      break;

    case MPIAsync:
      parallelizationType = "Asynchronous Communication";
      result = new sgpp::parallel::DMSystemMatrixVectorizedIdentityAsyncMPI<KernelImplementation>(
          grid, trainDataset, lambda, vecType);
      break;
    ...
    case MPITrueAsync:
      ...
      result =
          new sgpp::parallel::DMSystemMatrixVectorizedIdentityTrueAsyncMPI<KernelImplementation>(
              grid, trainDataset, lambda, vecType);
      break;
    ...
    case MPIOnesided:
      ...
      result =
          new sgpp::parallel::DMSystemMatrixVectorizedIdentityOnesidedMPI<KernelImplementation>(
              grid, trainDataset, lambda, vecType);
      break;

    case MPIBigdata:
      parallelizationType = "MPI Bigdata";
      result = new sgpp::parallel::DMSystemMatrixVectorizedIdentityBigdataAllreduce<
    ...
    case MPINone:
      ...
      result =
          new sgpp::parallel::DMSystemMatrixVectorizedIdentity(grid, trainDataset, lambda, vecType);
      break;
...
datadriven::DMSystemMatrixBase* DMSystemMatrixMPITypeFactory::getDMSystemMatrix(
  ...
  if (grid.getType() == sgpp::base::GridType::Linear ||
    ...
    if (vecType == parallel::X86SIMD) {
      return createDMSystemMatrixMPIType<CPUKernel<X86SimdLinear> >(grid, trainDataset, lambda,
                                                                    vecType, mpiType);

#ifdef USEOCL
    } else if (vecType == parallel::OpenCL) {
      return createDMSystemMatrixMPIType<OCLKernel<OCLLinear<double> > >(grid, trainDataset, lambda,
                                                                         vecType, mpiType);
      ...
      return createDMSystemMatrixMPIType<MICKernel<MICLinear> >(grid, trainDataset, lambda, vecType,
                                                                mpiType);
      ...
      return new sgpp::parallel::DMSystemMatrixVectorizedIdentityMPI(grid, trainDataset, lambda,
                                                                     vecType);
    }
  } else if (grid.getType() == sgpp::base::GridType::ModLinear) {
    if (vecType == parallel::X86SIMD) {
      if (strcmp(modlinear_mode, "orig") == 0) {
        return createDMSystemMatrixMPIType<CPUKernel<X86SimdModLinear> >(grid, trainDataset, lambda,
                                                                         vecType, mpiType);
        ...
        throw base::factory_exception(
            "MPITypeFactory: SGPP_MODLINEAR_EVAL must be 'mask' or 'orig'.");
      }
      ...
      if (strcmp(modlinear_mode, "orig") == 0) {
        return createDMSystemMatrixMPIType<
            OCLCPUHybridKernel<X86SimdModLinear, OCLModLinear<double> > >(grid, trainDataset,
                                                                          lambda, vecType, mpiType);
      } else if (strcmp(modlinear_mode, "mask") == 0) {
        return createDMSystemMatrixMPIType<
            OCLCPUHybridKernel<X86SimdModLinearMask, OCLModLinearMask<double> > >(
            grid, trainDataset, lambda, vecType, mpiType);
      } else {
        throw base::factory_exception(
            "MPITypeFactory: SGPP_MODLINEAR_EVAL must be 'mask' or 'orig'.");
      ...
      if (strcmp(modlinear_mode, "orig") == 0) {
        return createDMSystemMatrixMPIType<MICKernel<MICModLinear> >(grid, trainDataset, lambda,
                                                                     vecType, mpiType);
      } else if (strcmp(modlinear_mode, "mask") == 0) {
        return createDMSystemMatrixMPIType<MICKernel<MICModLinearMask> >(grid, trainDataset, lambda,
                                                                         vecType, mpiType);
      } else {
        throw base::factory_exception(
            "MPITypeFactory: SGPP_MODLINEAR_EVAL must be 'mask' or 'orig'.");
      ...
      if (strcmp(modlinear_mode, "orig") == 0) {
        ...
        return createDMSystemMatrixMPIType<
            MICCPUHybridKernel<X86SimdModLinearMask, MICModLinearMask> >(grid, trainDataset, lambda,
                                                                         vecType, mpiType);
      } else {
        throw base::factory_exception(
            "MPITypeFactory: SGPP_MODLINEAR_EVAL must be 'mask' or 'orig'.");
    ...
    throw base::factory_exception(
        "MPITypeFactory: OperationMultipleEvalVectorized is not implemented for this grid type.");
  }
}
}  // namespace parallel
}  // namespace sgpp

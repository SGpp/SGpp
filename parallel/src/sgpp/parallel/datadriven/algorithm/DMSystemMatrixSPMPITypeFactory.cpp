...
datadriven::DMSystemMatrixBaseSP* DMSystemMatrixSPMPITypeFactory::createDMSystemMatrixMPITypeSP(
    base::Grid& grid, base::DataMatrixSP& trainDataset, float lambda, VectorizationType vecType,
    MPIType mpiType) {
...
datadriven::DMSystemMatrixBaseSP* DMSystemMatrixSPMPITypeFactory::getDMSystemMatrixSP(
    base::Grid& grid, base::DataMatrixSP& trainDataset, float lambda, VectorizationType vecType,
    MPIType mpiType) {
  ...
  if (strcmp(grid.getType(), "linear") == 0 || strcmp(grid.getType(), "linearL0Boundary") == 0 ||
    ...
    if (vecType == parallel::X86SIMD) {
      ...
      return createDMSystemMatrixMPITypeSP<
          SPOCLCPUHybridKernel<SPX86SimdLinear, OCLLinear<float> > >(grid, trainDataset, lambda,
                                                                     vecType, mpiType);

#endif
#ifdef USEMIC
    } else if (vecType == parallel::MIC) {
      return createDMSystemMatrixMPITypeSP<SPMICKernel<SPMICLinear> >(grid, trainDataset, lambda,
                                                                      vecType, mpiType);
    ...
    if (vecType == parallel::X86SIMD) {
      ...
      if (strcmp(modlinear_mode, "orig") == 0) {
        return createDMSystemMatrixMPITypeSP<
            SPOCLCPUHybridKernel<SPX86SimdModLinear, OCLModLinear<float> > >(
            grid, trainDataset, lambda, vecType, mpiType);
      } else if (strcmp(modlinear_mode, "mask") == 0) {
        return createDMSystemMatrixMPITypeSP<
            SPOCLCPUHybridKernel<SPX86SimdModLinearMask, OCLModLinearMask<float> > >(
            grid, trainDataset, lambda, vecType, mpiType);
      } else {
        throw base::factory_exception(
            "MPITypeFactory: SGPP_MODLINEAR_EVAL must be 'mask' or 'orig'.");
      ...
      if (strcmp(modlinear_mode, "orig") == 0) {
        return createDMSystemMatrixMPITypeSP<
            SPMICCPUHybridKernel<SPX86SimdModLinear, SPMICModLinear> >(grid, trainDataset, lambda,
                                                                       vecType, mpiType);
      } else if (strcmp(modlinear_mode, "mask") == 0) {
        return createDMSystemMatrixMPITypeSP<
            SPMICCPUHybridKernel<SPX86SimdModLinearMask, SPMICModLinearMask> >(
            grid, trainDataset, lambda, vecType, mpiType);
      } else {
        throw base::factory_exception(
            "MPITypeFactory: SGPP_MODLINEAR_EVAL must be 'mask' or 'orig'.");

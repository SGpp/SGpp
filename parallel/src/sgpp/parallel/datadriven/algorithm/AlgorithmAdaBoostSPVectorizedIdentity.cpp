...
AlgorithmAdaBoostSPVectorizedIdentity::AlgorithmAdaBoostSPVectorizedIdentity(
  ...
  VectorizationType vecMode, size_t mode) : AlgorithmAdaBoostBase(SparseGrid,
        gridType, static_cast<SGPP::base::HashGenerator::level_t>(gridLevel), trainData,
  ...
  if (vecMode != X86SIMD && vecMode != OpenCL && vecMode != ArBB
    ...
    throw SGPP::base::operation_exception("AlgorithmAdaBoostVectorizedIdentity : Only X86SIMD or OCL or ArBB or HYBRID_X86SIMD_OCL are supported vector extensions!");
  }
...
void AlgorithmAdaBoostSPVectorizedIdentity::alphaSolver(double& lambda,
  ...
  SGPP::base::PrecisionConverter::convertDataVectorToDataVectorSP(*this->classes,
      classesSP);
  SGPP::base::PrecisionConverter::convertDataVectorToDataVectorSP(alpha, alphaSP);
  SGPP::base::PrecisionConverter::convertDataVectorToDataVectorSP(weight,
      weightSP);
  SGPP::base::PrecisionConverter::convertDataMatrixToDataMatrixSP(*this->data,
      dataSP);

  DMWeightMatrixSPVectorizedIdentity WMatrix(*this->grid, dataSP,
      static_cast<float>(lambda), weightSP, this->vecMode);
  ...
  if (final) {
    ...
    SGPP::solver::ConjugateGradientsSP myCG(this->imax,
                                            static_cast<float>(this->epsilon));
    myCG.solve(WMatrix, alphaSP, rhsSP, false, false, -1.0);
  }

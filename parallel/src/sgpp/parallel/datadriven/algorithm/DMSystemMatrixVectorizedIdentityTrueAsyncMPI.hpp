...
class DMSystemMatrixVectorizedIdentityTrueAsyncMPI : public
 ...
 public:
   ...
   */
  DMSystemMatrixVectorizedIdentityTrueAsyncMPI(SGPP::base::Grid& SparseGrid,
    ...
    SGPP::parallel::PartitioningTool::calcDistribution(
      ...
      _mpi_data_offsets, SGPP::parallel::DMVectorizationPaddingAssistant::getVecWidth(
        this->vecMode_));
  ...
  virtual void mult(SGPP::base::DataVector& alpha,
    ...
    MPI_Request[mpi_size]; //allocating a little more than necessary, otherwise complicated index computations needed
    ...
    myGlobalMPIComm->IrecvFromAll(ptrTemp, 1, _mpi_data_sizes, _mpi_data_offsets,
                                  tagsData, dataRecvReqs);

    this->myTimer_->start();

    size_t dataProcessChunkStart = _mpi_data_offsets[mpi_myrank];
    size_t dataProcessChunkEnd = dataProcessChunkStart +
                                 _mpi_data_sizes[mpi_myrank];
    size_t gridProcessChunkStart = _mpi_grid_offsets[mpi_myrank];
    size_t gridProcessChunkEnd = gridProcessChunkStart +
                                 _mpi_grid_sizes[mpi_myrank];
    int idx;
    #pragma omp parallel
    {
      this->kernel_.mult(
      ...
      #pragma omp master
      {
        SGPP::parallel::myGlobalMPIComm->IsendToAll(&ptrTemp[dataProcessChunkStart],
            _mpi_data_sizes[mpi_myrank],
  ...
  virtual void generateb(SGPP::base::DataVector& classes,
    ...
    this->computeTimeMultTrans_ += this->myTimer_->stop();
    SGPP::parallel::myGlobalMPIComm->dataVectorAllToAll(b, _mpi_grid_offsets,
        _mpi_grid_sizes);
    this->completeTimeMultTrans_ += this->myTimer_->stop();
  }
  ...
  virtual void rebuildLevelAndIndex() {
    ...
    SGPP::parallel::PartitioningTool::calcDistribution(this->storage_->size(),

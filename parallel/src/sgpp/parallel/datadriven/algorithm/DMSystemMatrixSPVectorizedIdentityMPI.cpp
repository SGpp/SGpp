...
DMSystemMatrixSPVectorizedIdentityMPI::DMSystemMatrixSPVectorizedIdentityMPI(
  ...
  : DMSystemMatrixBaseSP(trainData, lambda), vecMode_(vecMode),
    numTrainingInstances_(0), numPatchedTrainingInstances_(0), m_grid(SparseGrid) {
  ...
  SGPP::parallel::PartitioningTool::calcDistribution(m_grid.getStorage()->size(),
      mpi_size, _mpi_grid_sizes, _mpi_grid_offsets, 1);
  SGPP::parallel::PartitioningTool::calcDistribution(
    this->numPatchedTrainingInstances_, mpi_size, _mpi_data_sizes,
    _mpi_data_offsets,
    SGPP::parallel::DMVectorizationPaddingAssistant::getVecWidthSP(this->vecMode_));

  this->B_ = SGPP::op_factory::createOperationMultipleEvalVectorizedSP(m_grid,
...
DMSystemMatrixSPVectorizedIdentityMPI::~DMSystemMatrixSPVectorizedIdentityMPI() {
  ...
  double* allwaittime = new
  double [SGPP::parallel::myGlobalMPIComm->getNumRanks()];
  double* allcomptimemultTrans = new
  double [SGPP::parallel::myGlobalMPIComm->getNumRanks()];
  double* allcomptimemult = new
  double [SGPP::parallel::myGlobalMPIComm->getNumRanks()];
  MPI_Gather(&waitting_time, 1, MPI_DOUBLE, allwaittime, 1, MPI_DOUBLE, 0,
             MPI_COMM_WORLD);
  MPI_Gather(&computeTimeMultTrans_, 1, MPI_DOUBLE, allcomptimemultTrans, 1,
             MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Gather(&computeTimeMult_, 1, MPI_DOUBLE, allcomptimemult, 1, MPI_DOUBLE, 0,
             MPI_COMM_WORLD);

  if (SGPP::parallel::myGlobalMPIComm->getMyRank() == 0) {
    for (size_t i = 0; i < SGPP::parallel::myGlobalMPIComm->getNumRanks(); i++) {
      std::cout << "waiting time for process " << i << " is: " << allwaittime[i] <<
                std::endl;
    }

    for (size_t i = 0; i < SGPP::parallel::myGlobalMPIComm->getNumRanks(); i++) {
      std::cout << "comp time multtrans for process " << i << " is: " <<
                allcomptimemultTrans[i] << std::endl;
    }

    for (size_t i = 0; i < SGPP::parallel::myGlobalMPIComm->getNumRanks(); i++) {
      std::cout << "comp time mult for process " << i << " is: " << allcomptimemult[i]
                << std::endl;
    }
  }
...
void DMSystemMatrixSPVectorizedIdentityMPI::mult(SGPP::base::DataVectorSP&
    alpha, SGPP::base::DataVectorSP& result) {
...
void DMSystemMatrixSPVectorizedIdentityMPI::generateb(SGPP::base::DataVectorSP&
...
void DMSystemMatrixSPVectorizedIdentityMPI::rebuildLevelAndIndex() {
  SGPP::parallel::PartitioningTool::calcDistribution(m_grid.getStorage()->size(),
      SGPP::parallel::myGlobalMPIComm->getNumRanks(), _mpi_grid_sizes,
      _mpi_grid_offsets, 1);
  size_t mpi_rank = SGPP::parallel::myGlobalMPIComm->getMyRank();
...
void DMSystemMatrixSPVectorizedIdentityMPI::multVec(base::DataVectorSP& alpha,
  ...
  SGPP::parallel::myGlobalMPIComm->dataVectorSPAllToAll(result, _mpi_data_offsets,
      _mpi_data_sizes);
...
void DMSystemMatrixSPVectorizedIdentityMPI::multTransposeVec(
  base::DataVectorSP& source, base::DataVectorSP& result) {
  this->myTimer_->start();
  this->computeTimeMultTrans_ += this->B_->multTransposeVectorized(source,
                                 result);

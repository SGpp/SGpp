...
void HestonSolver::refineInitialGridWithPayoff(DataVector& alpha,
...
void HestonSolver::refineInitialGridWithPayoffToMaxLevel(DataVector& alpha,
    float_t strike, std::string payoffType, float_t dStrikeDistance,
...
void HestonSolver::setStochasticData(DataVector& thetas_arg,
                                     DataVector& kappas_arg, DataVector& volvols_arg, DataMatrix& hMatrix_arg,
                                     float_t r) {
...
void HestonSolver::solveCrankNicolson(size_t numTimesteps, float_t timestepsize,
                                      size_t maxCGIterations, float_t epsilonCG, DataVector& alpha, size_t NumImEul) {
  if (this->bGridConstructed && this->bStochasticDataAlloc) {
    ...
    if (this->tBoundaryType == "Dirichlet") {
      ...
      myHestonSystem = new HestonParabolicPDESolverSystemEuroAmer(*this->myGrid,
          alpha, *this->thetas, *this->volvols, *this->kappas, *this->hMatrix, this->r,
          timestepsize, "CrNic", this->dStrike, this->payoffType, this->useLogTransform,
    ...
    std::cout << "Average Grid size: " << static_cast<float_t>
              (myHestonSystem->getSumGridPointsComplete()) / static_cast<float_t>
              (numTimesteps) << std::endl;
    std::cout << "Average Grid size (Inner): " << static_cast<float_t>
              (myHestonSystem->getSumGridPointsInner()) / static_cast<float_t>
...
void HestonSolver::initGridWithPayoff(DataVector& alpha, float_t strike,
...
float_t HestonSolver::get1DEuroCallPayoffValue(float_t assetValue,
    float_t strike) {
...
void HestonSolver::solveImplicitEuler(size_t numTimesteps, float_t timestepsize,
                                      size_t maxCGIterations, float_t epsilonCG, SGPP::base::DataVector& alpha,
                                      bool verbose, bool generateAnimation, size_t numEvalsAnimation) {
...
void HestonSolver::setEnableCoarseningData(std::string adaptSolveMode,
...
size_t HestonSolver::getGridPointsAtMoney(std::string payoffType,
    ...
    if (this->bGridConstructed) {
      for (size_t i = 0; i < this->myGrid->getStorage()->size(); i++) {
        ...
        if (payoffType == "std_euro_call" || payoffType == "std_euro_put"
          ...
          if ( ((stockPriceSum / static_cast<float_t>(this->dim)) < (strike - eps))
               || ((stockPriceSum / static_cast<float_t>(this->dim)) > (strike + eps)) ) {
            isAtMoney = false;
          }
        } else {
...
void HestonSolver::initCartesianGridWithPayoff(DataVector& alpha,
    ...
    for (size_t i = 0; i < this->myGrid->getStorage()->size(); i++) {
      ...
      if (payoffType == "std_euro_call") {
        ...
        if (!curPoint->isInnerPoint()
          ...
          alpha[i] =
            dblFuncValues[0]; //(this->myBoundingBox->getBoundary(0).rightBoundary-strike)/(this->myBoundingBox->getBoundary(0).rightBoundary)*dblFuncValues[0];
          ...
          float_t normalPayoff = std::max<float_t>(((tmp / static_cast<float_t>
                                 (numAssets)) - strike), 0.0);
          float_t vRange = this->myBoundingBox->getBoundary(1).rightBoundary -
                           this->myBoundingBox->getBoundary(1).leftBoundary;
          float_t sPayoffDiff = dblFuncValues[0] - normalPayoff;
          alpha[i] = normalPayoff + ((dblFuncValues[1] - this->myBoundingBox->getBoundary(
                                        1).leftBoundary) / vRange) * sPayoffDiff;
        } else {
          // Payoff function
          alpha[i] = std::max<float_t>(((tmp / static_cast<float_t>(numAssets)) - strike),
                                       0.0);
        }
        ...
        if (!curPoint->isInnerPoint()) {
          if (numAssets == 1
            ...
            float_t constantB = (strike * pow(exp(this->myBoundingBox->getBoundary(
                                                    1).leftBoundary - this->myBoundingBox->getBoundary(1).rightBoundary),
                                              20.0)) / (1 - exp(this->myBoundingBox->getBoundary(1).leftBoundary -
                                                  this->myBoundingBox->getBoundary(1).rightBoundary));
            float_t constantA = strike + constantB;
            alpha[i] = constantA * pow(exp(dblFuncValues[1] -
                                           this->myBoundingBox->getBoundary(1).rightBoundary), 20.0) - constantB;
          } else {
            ...
            payoffFuncVal = std::max<float_t>(strike - payoffFuncVal /
                                              (static_cast<float_t>(numAssets)), 0.0);
            ...
            alpha[i] = std::max<float_t>(payoffFuncVal + volFraction *
                                         (maxVolFuncVal - payoffFuncVal), 0.0);
          }
        } else {
          ...
          alpha[i] = std::max<float_t>(strike - ((tmp / static_cast<float_t>(numAssets))),
                                       0.0);
        }
      } else {
...
void HestonSolver::initLogTransformedGridWithPayoff(DataVector& alpha,
    float_t strike, std::string payoffType) {
  float_t tmp;
    ...
    for (size_t i = 0; i < this->myGrid->getStorage()->size(); i++) {
      ...
      if (payoffType == "std_euro_call") {
        ...
        if (!curPoint->isInnerPoint()) {
          ...
          alpha[i] = std::max<float_t>((sumStockPrices - strike * kMultiplier) /
                                       static_cast<float_t>(numAssets), 0.0);
        } else {
          // Non-boundary point. Just set the payoff value.
          tmp = 0.0;
...
float_t HestonSolver::evalOption(std::vector<float_t>& eval_point,
  ...
  if (this->useLogTransform) {
    for (size_t i = 0; i < eval_point.size();
         i = i + 2) { // Here we know that the variance dimension is not log-transformed, so we shouldn't log its value
...
float_t EvaluateHestonClosedFormIntegralFunction(float_t phi, float_t xi,
    float_t theta, float_t kappa, float_t rho, float_t r, float_t T, float_t K,
    float_t S, float_t v, int type) {
  float_t a, b, u, x;
  ...
  C = (r * phi * C * T) + (a / float_t(pow(xi,
                                       2.0))) * ((b - rho * xi * phi * C + d) * T - float_t(2.0) * log( (float_t(
                                           1.0) - g * exp(d * T)) / (float_t(1.0) - g)  ));

  // Build D
  complex<float_t> D;
  D = std::complex<float_t>(0.0, 1.0);
  D = ((b - rho * xi * phi * D + d) / float_t(pow(xi,
       2.0))) * ((float_t(1.0) - exp(d * T)) / (float_t(1.0) - g * exp(d * T)));
  ...
  realArgument = (exp(float_t(0.0) - realArgument * phi * float_t(log(
                        K))) * f) / (realArgument * phi);

  return real(realArgument);
}

void HestonSolver::EvaluateHestonExactSurface(DataVector& alpha,
    float_t maturity) {
  if (!this->bGridConstructed)
  ...
  for (size_t i = 0; i < this->myGrid->getStorage()->size(); i++) {
    ...
    alpha[i] = EvaluateHestonPriceExact(dblFuncValues[0], dblFuncValues[1],
                                        this->volvols->get(0), this->thetas->get(0), this->kappas->get(0),
...
void HestonSolver::EvaluateHestonExactSurfacePut(DataVector& alpha,
    float_t maturity) {
  if (!this->bGridConstructed)
    throw new application_exception("HestonSolver::EvaluateHestonPriceExact : The grid wasn't initialized before!");
  ...
  for (size_t i = 0; i < this->myGrid->getStorage()->size(); i++) {
    ...
    alpha[i] = EvaluateHestonPriceExactPut(dblFuncValues[0], dblFuncValues[1],
                                           this->volvols->get(0), this->thetas->get(0), this->kappas->get(0),
                                           this->hMatrix->get(0, 1), this->r, maturity, this->dStrike) ;
    delete dblFuncValues;
  }
...
void HestonSolver::CompareHestonBs1d(float_t maturity, float_t v) {
  ...
  boundaries1d[0].rightBoundary = this->myBoundingBox->getBoundary(
                                    0).rightBoundary;
  boundaries1d[0].bDirichletLeft = this->myBoundingBox->getBoundary(
                                     0).bDirichletLeft;
  boundaries1d[0].bDirichletRight = this->myBoundingBox->getBoundary(
                                      0).bDirichletRight;
  ...
  SGPP::base::DataVector* alphaHeston = new SGPP::base::DataVector(
    grid1d->getSize());
...
void HestonSolver::EvaluateHestonExact1d(DataVector& alpha, Grid* grid1d,
    BoundingBox* boundingBox1d, float_t maturity, float_t v) {
  float_t tmp;

  for (size_t i = 0; i < grid1d->getStorage()->size(); i++) {
    std::string coords = grid1d->getStorage()->get(i)->getCoordsStringBB(
                           *boundingBox1d);
    std::stringstream coordsStream(coords);
    ...
    alpha[i] = EvaluateHestonPriceExact(dblFuncValues[0], v, this->volvols->get(0),
                                        this->thetas->get(0), this->kappas->get(0), this->hMatrix->get(0, 1), this->r,
                                        maturity, this->dStrike) ;
    delete dblFuncValues;
  }

  OperationHierarchisation* myHierarchisation =
    SGPP::op_factory::createOperationHierarchisation(*grid1d);
  myHierarchisation->doHierarchisation(alpha);
  delete myHierarchisation;
}

void HestonSolver::EvaluateBsExact1d(DataVector& alpha, Grid* grid1d,
                                     BoundingBox* boundingBox1d, float_t maturity, float_t sigma) {
  float_t tmp;

  SGPP::finance::BlackScholesSolver* myBSSolver = new
  SGPP::finance::BlackScholesSolver(false);
  ...
  for (size_t i = 0; i < grid1d->getStorage()->size(); i++) {
    ...
    alpha[i] = myBSSolver->getAnalyticSolution1D(dblFuncValues[0], true, maturity,
               sigma, this->r, this->dStrike);
    delete dblFuncValues;
  }
...
float_t HestonSolver::EvaluateHestonPriceExact(float_t S, float_t v, float_t xi,
    float_t theta, float_t kappa, float_t rho, float_t r, float_t T, float_t K) {
  float_t int1 = 0.5 + (1.0 / M_PI) * GaussLobattoInt(0.001, 1000.0, 1e-10,
                 100000, xi, theta, kappa, rho, r, T, K, S, v, 1);
  float_t int2 = 0.5 + (1.0 / M_PI) * GaussLobattoInt(0.001, 1000.0, 1e-10,
                 100000, xi, theta, kappa, rho, r, T, K, S, v, 2);
  return S * int1 - K * exp((-1.0) * r * T) * int2;
}

float_t HestonSolver::EvaluateHestonPriceExact(float_t S, float_t v,
    float_t maturity) {
  return EvaluateHestonPriceExact(S, v, this->volvols->get(0),
                                  this->thetas->get(0), this->kappas->get(0), this->hMatrix->get(0, 1), this->r,
                                  maturity, this->dStrike);
}

float_t HestonSolver::EvaluateHestonPriceExactPut(float_t S, float_t v,
    float_t xi, float_t theta, float_t kappa, float_t rho, float_t r, float_t T,
    float_t K) {
  float_t int1 = 0.5 + (1.0 / M_PI) * GaussLobattoInt(0.001, 1000.0, 1e-10,
                 100000, xi, theta, kappa, rho, r, T, K, S, v, 1);
  float_t int2 = 0.5 + (1.0 / M_PI) * GaussLobattoInt(0.001, 1000.0, 1e-10,
...
float_t HestonSolver::EvaluateHestonPriceExactPut(float_t S, float_t v,
    float_t maturity) {
  return EvaluateHestonPriceExactPut(S, v, this->volvols->get(0),
                                     this->thetas->get(0), this->kappas->get(0), this->hMatrix->get(0, 1), this->r,
                                     maturity, this->dStrike);
}

void HestonSolver::GetBsExactSolution(SGPP::base::DataVector& alphaBS,
                                      float_t maturity) {
  SGPP::finance::BlackScholesSolver* myBSSolver = new
  SGPP::finance::BlackScholesSolver(false);
  ...
  for (size_t i = 0; i < this->myGridStorage->size(); i++) {
    ...
    if (this->useLogTransform)
      alphaBS[i] = myBSSolver->getAnalyticSolution1D(exp(S), true, maturity, sqrt(v),
                   this->r, this->dStrike);
    else
      alphaBS[i] = myBSSolver->getAnalyticSolution1D(S, true, maturity, sqrt(v),
                   this->r, this->dStrike);
  }
...
void HestonSolver::CompareHestonBsExact(SGPP::base::DataVector& alpha,
                                        float_t maturity) {
...
void HestonSolver::CompareHestonNumericToBsExact(SGPP::base::DataVector&
    alphaHestonNumeric, SGPP::base::DataVector& alphaBS,
...
float_t HestonSolver::GaussLobattoIntStep(
  ...
  const float_t fmll = EvaluateHestonClosedFormIntegralFunction(mll, xi, theta,
                       kappa, rho, r, T, K, S, v, type);
  const float_t fml = EvaluateHestonClosedFormIntegralFunction(ml, xi, theta,
                      kappa, rho, r, T, K, S, v, type);
  const float_t fm  = EvaluateHestonClosedFormIntegralFunction(m, xi, theta,
                      kappa, rho, r, T, K, S, v, type);
  const float_t fmr = EvaluateHestonClosedFormIntegralFunction(mr, xi, theta,
                      kappa, rho, r, T, K, S, v, type);
  const float_t fmrr = EvaluateHestonClosedFormIntegralFunction(mrr, xi, theta,
                       kappa, rho, r, T, K, S, v, type);
  neval += 5;
  ...
  if (dist == acc || mll <= a || b <= mrr) {
    ...
    return  GaussLobattoIntStep(a, mll, fa, fmll, neval, maxeval, acc, xi, theta,
                                kappa, rho, r, T, K, S, v, type)
            + GaussLobattoIntStep(mll, ml, fmll, fml, neval, maxeval, acc, xi, theta, kappa,
                                  rho, r, T, K, S, v, type)
            + GaussLobattoIntStep(ml, m, fml, fm, neval, maxeval, acc, xi, theta, kappa,
                                  rho, r, T, K, S, v, type)
            + GaussLobattoIntStep(m, mr, fm, fmr, neval, maxeval, acc, xi, theta, kappa,
                                  rho, r, T, K, S, v, type)
            + GaussLobattoIntStep(mr, mrr, fmr, fmrr, neval, maxeval, acc, xi, theta, kappa,
                                  rho, r, T, K, S, v, type)
            + GaussLobattoIntStep(mrr, b, fmrr, fb, neval, maxeval, acc, xi, theta, kappa,
                                  rho, r, T, K, S, v, type);
...
float_t HestonSolver::GaussLobattoInt(float_t a, float_t b,
  ...
  return GaussLobattoIntStep(a, b,
                             EvaluateHestonClosedFormIntegralFunction(a, xi, theta, kappa, rho, r, T, K, S,
                                 v, type), EvaluateHestonClosedFormIntegralFunction(b, xi, theta, kappa, rho, r,
                                     T, K, S, v, type),
                             neval,
                             maxeval,
...
void HestonSolver::CompareHestonSolutionToExact(SGPP::base::DataVector*
    solution, SGPP::base::DataVector* exact, std::string filename,
  ...
  if (myGrid->getStorage()->dim() == 2) {
    ...
    float_t inc_x = ((dimOne.rightBoundary - dimOne.leftBoundary) / float_t(
                       PointsPerDimension - 1));
    float_t inc_y = ((dimTwo.rightBoundary - dimTwo.leftBoundary) / float_t(
                       PointsPerDimension - 1));
    ...
    for (size_t i = 0; i < points; i++) {
      for (size_t j = 0; j < points; j++) {
        ...
        fileout << (offset_x + ((float_t)(i))*inc_x) << " " << (offset_y + ((float_t)(
                  j))*inc_y) << " " << (myEval->eval(*solution, point) - myEval->eval(*exact,
                                        point)) << std::endl;
      }
...
float_t HestonSolver::EvalSinglePoint1Asset(float_t s, float_t v,
    DataVector& alphaVec) {
  OperationEval* myEval = SGPP::op_factory::createOperationEval(*myGrid);
  std::vector<float_t> point;
  point.push_back(s);

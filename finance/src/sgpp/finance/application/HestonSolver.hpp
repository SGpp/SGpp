...
class HestonSolver : public sgpp::pde::ParabolicPDESolver {
 protected:
  ...
  /// Identifies if the Heston Equation should be solved using log-transformed stock-price
  /// coordinates. The variance coordinates are always linear.
 ...
 public:
   ...
   * the Heston Equation
   */
  explicit HestonSolver(bool useLogTransform = true);
   ...
   * some statistical data about the underlying (long-run variance, vol of vol, mean reversion
   * rate). This function allows you to set this data.
   *
   ...
   * @param rhos correlation matrix. Size needs to be twice the number of assets to handle the two
  ...
  /**
   * Concrete implementation of the Crank-Nicholson solver. Optional use of implicit euler as a
   * 'kickstart'.
   *
   * @param numTimesteps the number of timesteps to use. The product of this and the timestepsize
   * parameter must be equal to the maturity T.
   * @param timestepsize size of the timestep to take. Assumed constant.
   * @param maxCGIterations the maximum number of iterations to be undertaken by the CG algorithm
   * before aborting.
   ...
   *
   * @return number of iterations needed to solve the Heston Equation, if called before solving 0 is
   * returned
   */
   ...
   * closed-form Heston surface for a call option.
   *
   ...
   * stochastic data already saved in this instance.
   *
   * @param S stock price
   * @param v variance
   */
   ...
   * the stochastic data already saved in this instance.
   *
   * @param S stock price
   * @param v variance
   */
   ...
   * Black-Scholes surface on the current grid.
   *
   ...
   * functions.
   * Adapted from the implementation found at
   * http://www.bnikolic.co.uk/nqm/1dinteg/gausslobatto.html
   *
   * @param a lower integration limit
   * @param b upper integration limit
   * @param fa value of the function at the lower limit (used to save an evaluation when refinement
   * is used)
   * @param fb value of the function at the upper limit (used to save an evaluation when refinement
   * is used)
   ...
   * @param abstol absolute tolerance - integration stops when the error estimate is smaller than
   * this
   * @param maxeval maximum of evaluations to make. If this number of evaluations is made without
   * reaching the required accuracy, an exception of type std::runtime_error is thrown.
   * @param xi Heston parameter
   * @param theta Heston parameter
   * @param kappa Heston parameter
   * @param rho Heston parameter
  ...
  /**
   * Uses an evaluation operator to get the interpolated option value for a particular s and v based
   * on the given numerical solution.
   *
   ...
   * of the variance values. Vanilla option only.
   *
   ...
   * Black-Scholes solution on the current grid.
   *

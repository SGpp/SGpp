...
BlackScholesPATParabolicPDESolverSystem::BlackScholesPATParabolicPDESolverSystem(
  ...
  for (size_t i = 0; i < this->BSalgoDims.size(); i++) {
    ...
    if (dimCount > 1) {
      throw SGPP::base::algorithm_exception("BlackScholesPATParabolicPDESolverSystem::BlackScholesPATParabolicPDESolverSystem : There is minimum one float_td algorithmic dimension!");
    }
  }
...
void BlackScholesPATParabolicPDESolverSystem::applyLOperator(
...
void BlackScholesPATParabolicPDESolverSystem::applyMassMatrix(
  SGPP::base::DataVector& alpha, SGPP::base::DataVector& result) {
  SGPP::base::DataVector temp(alpha.getSize());
  result.setAll(0.0);
...
void BlackScholesPATParabolicPDESolverSystem::coarsenAndRefine(
  bool isLastTimestep) {

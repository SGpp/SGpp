...
BlackScholesPATParabolicPDESolverSystemEuroAmerParallelOMP::BlackScholesPATParabolicPDESolverSystemEuroAmerParallelOMP(
  ...
  BlackScholesPATParabolicPDESolverSystemEuroAmer(SparseGrid, alpha, lambda,
      eigenvecs, mu_hat, TimestepSize, OperationMode, dStrike, option_type, r,
      useCoarsen, coarsenThreshold, adaptSolveMode, numCoarsenPoints, refineThreshold,
      refineMode, refineMaxLevel), rhs_corrector(NULL) {
}
...
void BlackScholesPATParabolicPDESolverSystemEuroAmerParallelOMP::applyLOperatorInner(
  ...
  ((SGPP::pde::UpDownOneOpDimEnhanced*)(
     this->OpLaplaceInner))->multParallelBuildingBlock(alpha, result);
  result.mult(-0.5);
#else
...
#ifdef USE_ENHANCED_UPDOWN
  ((SGPP::pde::UpDownOneOpDimEnhanced*)(
     this->OpLaplaceBound))->multParallelBuildingBlock(alpha, result);
  result.mult(-0.5);
#else
  std::vector<size_t> algoDims =
...
BlackScholesPATParabolicPDESolverSystemEuroAmerParallelOMP::generateRHS() {
  ...
  if (this->useCoarsen == true || this->nExecTimesteps == 0
      || this->bnewODESolver == true  || this->option_type == "std_amer_put") {
    ...
    this->rhs_corrector = new SGPP::base::DataVector(

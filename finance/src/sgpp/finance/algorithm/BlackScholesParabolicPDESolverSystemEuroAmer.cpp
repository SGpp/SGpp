...
BlackScholesParabolicPDESolverSystemEuroAmer::BlackScholesParabolicPDESolverSystemEuroAmer(
  ...
  SGPP::base::GridIndex::level_type xLevel) {
  ...
  if (this->BSalgoDims.size() != this->BoundGrid->getStorage()->dim()) {
    throw SGPP::base::algorithm_exception("BlackScholesParabolicPDESolverSystemEuropean::BlackScholesParabolicPDESolverSystemEuropean : Number of algorithmic dimensions is not equal to the number of grid's dimensions.");
  }
  ...
  if (this->BoundGrid->getStorage()->dim() != this->mus->getSize()
    ...
    throw SGPP::base::algorithm_exception("BlackScholesParabolicPDESolverSystemEuropean::BlackScholesParabolicPDESolverSystemEuropean : Dimension of mu and sigma parameters don't match the grid's dimensions!");
  }
  ...
  if (this->BoundGrid->getStorage()->dim() != this->rhos->getNrows()
    ...
    throw SGPP::base::algorithm_exception("BlackScholesParabolicPDESolverSystemEuropean::BlackScholesParabolicPDESolverSystemEuropean : Row or col of rho parameter don't match the grid's dimensions!");
  }
  ...
  for (size_t i = 0; i < this->BSalgoDims.size(); i++) {
    if (this->BSalgoDims[i] >= this->BoundGrid->getStorage()->dim()) {
      throw SGPP::base::algorithm_exception("BlackScholesParabolicPDESolverSystemEuropean::BlackScholesParabolicPDESolverSystemEuropean : Minimum one algorithmic dimension is not inside the grid's dimensions!");
    }
  }
  ...
  if (bLogTransform == false) {
    ...
    this->OpDeltaInner = SGPP::op_factory::createOperationDelta(*this->InnerGrid,
                         *this->deltaCoef);
    this->OpGammaInner = SGPP::op_factory::createOperationGamma(*this->InnerGrid,
                         *this->gammaCoef);
  ...
  // create needed operations that are different in case of a log-transformed Black-Scholoes equation
    ...
    this->OpDeltaInner = SGPP::op_factory::createOperationDeltaLog(*this->InnerGrid,
                         *this->deltaCoef);
    this->OpGammaInner = SGPP::op_factory::createOperationGammaLog(*this->InnerGrid,
                         *this->gammaCoef);
  }
  ...
  SGPP::base::DimensionBoundary[grid_bb->getDimensions()];

  for (size_t d = 0; d < grid_bb->getDimensions(); d++) {
    if (bLogTransform == true) {
      float_t interval_width = exp(grid_bb->getBoundary(d).rightBoundary) - exp(
                                 grid_bb->getBoundary(d).leftBoundary);
      float_t hedge_offset = (interval_width - (interval_width *
                              HEDGE_WIDTH_PERCENT)) / 2.0;
      myBoundaries[d].leftBoundary = exp(grid_bb->getBoundary(
                                           d).leftBoundary) + hedge_offset;
      myBoundaries[d].rightBoundary = exp(grid_bb->getBoundary(
                                            d).rightBoundary) - hedge_offset;
    } else {
      float_t hedge_offset = (grid_bb->getIntervalWidth(d) -
                              (grid_bb->getIntervalWidth(d) * HEDGE_WIDTH_PERCENT)) / 2.0;
      myBoundaries[d].leftBoundary = grid_bb->getBoundary(d).leftBoundary +
                                     hedge_offset;
      myBoundaries[d].rightBoundary = grid_bb->getBoundary(d).rightBoundary -
                                      hedge_offset;
    }
  ...
  SGPP::base::BoundingBox* myHedgeBB = new SGPP::base::BoundingBox(
    grid_bb->getDimensions(), myBoundaries);
  // hedging
  myHedge = new SGPP::finance::Hedging(*myHedgeBB, HEDGE_POINTS_PER_DIM,
                                       HEDGE_EPS, bLogTransform);
...
void BlackScholesParabolicPDESolverSystemEuroAmer::coarsenAndRefine(
  bool isLastTimestep) {
  ...
  myHedge->calc_hedging(*this->BoundGrid, *this->alpha_complete,
                        filename_ext.str());
#endif
}

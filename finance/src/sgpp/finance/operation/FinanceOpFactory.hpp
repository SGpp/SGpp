 ...
 * \f$ \int_{\Omega} S_i S_j \frac{\partial u(\vec{s}}{\partial S_i} \frac{\partial
 * v(\vec{s}}{\partial S_j} d \vec{s}\f$
 *
 * @param grid Grid which is to be used
 * @param coef Reference to a DataMatrix object that contains the constant coeffecients of this
 * bilinear form
 ...
 * This operation calculates the following bilinear form which is
 * needed to solve the Black Scholes equation:
 * \f$ \int_{\Omega} \frac{\partial u(\vec{s}}{\partial S_i} \frac{\partial v(\vec{s}}{\partial S_j}
 * d \vec{s}\f$
 *
 * @param grid Grid which is to be used
 * @param coef Reference to a DataMatrix object that contains the constant coeffecients of this

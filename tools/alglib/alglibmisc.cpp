     ...
     ae_state *_state)
{
    ...
    ae_assert(normtype>=0&&normtype<=3, "KDTreeBuild: incorrect NormType", _state);
     ...
     ae_state *_state)
{
    ...
    ae_assert(normtype>=0&&normtype<=3, "KDTreeBuildTagged: incorrect NormType", _state);
    ...
    if( kdt->nodes.ptr.p_int[offs]>0 )
    {
        ...
        for(i=i1; i<=i2-1; i++)
        {
            ...
            if( kdt->normtype==3 )
            {
                ...
                for(j=0; j<=nx-1; j++)
                {
                    x = kdt->xy.ptr.pp_double[i][j];
                    y = kdt->x.ptr.p_double[j];

                    dist1 = ae_sqr(x-y, _state);
                    dist2 = ae_sqr(1-std::abs(x-y), _state);

                    ptdist = ptdist+min(dist1,dist2);
                }
            }
            /*

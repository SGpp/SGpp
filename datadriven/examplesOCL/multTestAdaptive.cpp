...
int main(int argc, char** argv) {
  ...
  parameters.addIDAttr("ADAPTIVE_STREAMING_HARD_LIMIT", "10");  // absolute
                                                                // value
  ...
  if (argc >= 4) {
    ...
    std::cout << "level: " << level
              << " numRef: " << adaptConfig.numRefinements_ << std::endl;
  }
  ...
  if (bCompare) {
    ...
    for (size_t i = 0; i < dataSizeVectorResultCompare.getSize(); i++) {
      std::cout << "mine: " << dataSizeVectorResult[i]
    ...
    std::cout << "biggest error i: " << biggestErrorIndex
              << " value: " << biggestError << std::endl;
  }
}

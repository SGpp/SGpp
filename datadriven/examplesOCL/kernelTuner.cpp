...
int main(int argc, char **argv) {
  ...
  std::string
      parameterConfigurationFileName;  // <- includes kernel type, subtype
  ...
  description.add_options()("help", "display help")(
      ...
      "parameterConfiguration", boost::program_options::value<std::string>(
                                    &parameterConfigurationFileName),
      "the parameter configuration file to be used (serialized "
      "StaticParameterTuner)")(
      "kernel", boost::program_options::value<std::string>(&kernelName),
  ...
  // TODO(pfandedd): LOCAL_SIZE should be added
  if (kernelName.compare("StreamingOCLMultiPlatform") == 0) {
    ...
    staticParameterTuner.addParameter("KERNEL_MAX_DIM_UNROLL",
                                      {"4"});  // "1", "8", "16"
    staticParameterTuner.addParameter("LOCAL_SIZE",
                                      {"128", "256"});  // "1", "8", "16"
    ...
    //        staticParameterTuner.addParameter("KERNEL_DATA_BLOCKING_SIZE", {
    //        "1", "2", "4", "8" }); //
    //        staticParameterTuner.addParameter("KERNEL_TRANS_DATA_BLOCK_SIZE",
    //        { "1", "2", "4", "8" }); //
    //        staticParameterTuner.addParameter("KERNEL_TRANS_GRID_BLOCK_SIZE",
    //        { "1", "4", "2", "4", "8" }); //
    //        staticParameterTuner.addParameter("KERNEL_STORE_DATA", {
    //        "register", "array" }); //
    //        staticParameterTuner.addParameter("KERNEL_MAX_DIM_UNROLL", { "4",
    //        "1" }); // "8", "16"
  } else {
    throw;
  }

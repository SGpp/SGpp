...
from sg_op.operation.interpolation_combi import *
...
if __name__ == '__main__':
	...
	x = y 	= np.arange(0., 1. + delta, delta)
	X, Y 	= np.meshgrid(x, y)

	err_interp = np.zeros(X.shape)

	for ii, _x in enumerate(x):
		for jj, _y in enumerate(y):
			...
			for index, multiindex in enumerate(multiindex_set):
				...
				fg_res[index] = Interpolation_obj.eval_full_grid_interpolant(func_eval, grid_points_1D, [_x, _y])

			sg_res = Interpolation_obj.eval_combi_interpolant(fg_res, combi_coeffs)

			err_interp[ii, jj] = np.abs(sg_res - test_function([_x, _y])) 

	c = contourf(X, Y, err_interp)
	colorbar(c)

...
struct FilesNamesAndErrorFixture {
  ...
  std::vector<std::tuple<std::string, double>> fileNamesErrorDouble = {
      std::tuple<std::string, double>("datadriven/tests/data/friedman/friedman2_4d_10000.arff.gz", 1E-18),
      std::tuple<std::string, double>("datadriven/tests/data/friedman/friedman1_10d_2000.arff.gz", 1E-26)};

  std::vector<std::tuple<std::string, double>> fileNamesErrorFloat = {
      std::tuple<std::string, double>("datadriven/tests/data/friedman/friedman2_4d_10000.arff.gz", 1E+5),
      std::tuple<std::string, double>("datadriven/tests/data/friedman/friedman1_10d_2000.arff.gz", 1E-8)};
...
BOOST_AUTO_TEST_CASE(Blocking) {
  ...
  for (json::Node &deviceNode : deviceNodes) {
    deviceNode.replaceIDAttr("KERNEL_USE_LOCAL_MEMORY", false);
    deviceNode.replaceIDAttr("KERNEL_DATA_BLOCK_SIZE", UINT64_C(4));
    deviceNode.replaceIDAttr("KERNEL_TRANS_GRID_BLOCK_SIZE", UINT64_C(4));

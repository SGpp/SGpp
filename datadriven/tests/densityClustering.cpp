...
void multiply_and_test(
  ...
  sgpp::datadriven::DensityOCLMultiPlatform::OperationDensity *mult_operation =
      new sgpp::datadriven::DensityOCLMultiPlatform::
          OperationDensityOCLMultiPlatform<double>(grid, 2, manager, parameters,
                                                   0.001, 0, 0);
...
BOOST_AUTO_TEST_CASE(DensityMultiplicationOpenCL) {
  ...
  std::vector<double> mult_optimal_result;
  std::ifstream mult_in(
      "datadriven/datasets/clustering_test_data/mult_erg_dim2_depth11.txt");
  if (mult_in) {
    double value;
    while (mult_in >> value) mult_optimal_result.push_back(value);
  } else {
    BOOST_THROW_EXCEPTION(
        std::runtime_error("Density multiplication result file is missing!"));
  }
  mult_in.close();
  ...
  sgpp::datadriven::DensityOCLMultiPlatform::OperationDensity::
      load_default_parameters(parameters.get());
  ...
  std::cout << "Testing with preprocessed positions and ignored flags..."
            << std::endl;
  for (std::string &platformName : (*parameters)["PLATFORMS"].keys()) {
    json::Node &platformNode = (*parameters)["PLATFORMS"][platformName];
    for (std::string &deviceName : platformNode["DEVICES"].keys()) {
  ...
  std::cout << "Testing with branchless mutliplication kernel with fabs "
               "modifications..."
            << std::endl;
  for (std::string &platformName : (*parameters)["PLATFORMS"].keys()) {
    json::Node &platformNode = (*parameters)["PLATFORMS"][platformName];
  ...
  multiply_and_test(parameters.get(), mult_optimal_result, manager, *grid);
  std::cout
      << "Testing with branchless mutliplication kernel with level cache..."
      << std::endl;
  for (std::string &platformName : (*parameters)["PLATFORMS"].keys()) {
    json::Node &platformNode = (*parameters)["PLATFORMS"][platformName];
    for (std::string &deviceName : platformNode["DEVICES"].keys()) {
  ...
  std::cout << "Testing with branchless mutliplication kernel with implicit "
               "modifications..."
            << std::endl;
  for (std::string &platformName : (*parameters)["PLATFORMS"].keys()) {
    json::Node &platformNode = (*parameters)["PLATFORMS"][platformName];
  ...
  std::cout << "Testing default multiplcation kernel with fabs modifications..."
            << std::endl;
  for (std::string &platformName : (*parameters)["PLATFORMS"].keys()) {
    json::Node &platformNode = (*parameters)["PLATFORMS"][platformName];
    for (std::string &deviceName : platformNode["DEVICES"].keys()) {
  ...
  std::cout
      << "Testing default multiplcation kernel with implicit modifications..."
      << std::endl;
  for (std::string &platformName : (*parameters)["PLATFORMS"].keys()) {
    json::Node &platformNode = (*parameters)["PLATFORMS"][platformName];
    for (std::string &deviceName : platformNode["DEVICES"].keys()) {
...
BOOST_AUTO_TEST_CASE(DensityAlphaSolver) {
  ...
  std::ifstream rhs_in(
      "datadriven/datasets/clustering_test_data/rhs_erg_dim2_depth11.txt");
  if (rhs_in) {
    double value;
    int counter = 0;
    while (rhs_in >> value) {
    ...
    BOOST_THROW_EXCEPTION(
        std::runtime_error("Density rhs result file is missing!"));
  }
  rhs_in.close();
  ...
  sgpp::datadriven::DensityOCLMultiPlatform::OperationDensity *mult_operation =
      new sgpp::datadriven::DensityOCLMultiPlatform::
          OperationDensityOCLMultiPlatform<double>(
              *grid, 2, manager, parameters.get(), 0.001, 0, 0);

  // Create solver
  sgpp::solver::ConjugateGradients *solver =
      new sgpp::solver::ConjugateGradients(100, 0.001);
  sgpp::base::DataVector alpha(gridsize, 1.0);
...
BOOST_AUTO_TEST_CASE(DensityRHSOpenCL) {
  ...
  sgpp::datadriven::Dataset data =
      sgpp::datadriven::ARFFTools::readARFFFromFile(
          "datadriven/datasets/clustering_test_data/"
          "clustering_testdataset_dim2.arff");
...
BOOST_AUTO_TEST_CASE(KNNGraphOpenCL) {
  ...
  std::ifstream graph_in(
      "datadriven/datasets/clustering_test_data/graph_erg_dim2_depth11.txt");
  if (graph_in) {
    int value;
    while (graph_in >> value) graph_optimal_result.push_back(value);
  } else {
    BOOST_THROW_EXCEPTION(
        std::runtime_error("knn graph result file is missing!"));
  }
  graph_in.close();
  ...
  size_t k = 8;
  sgpp::datadriven::DensityOCLMultiPlatform::OperationCreateGraphOCL
      *operation_graph = new sgpp::datadriven::DensityOCLMultiPlatform::
          OperationCreateGraphOCLSingleDevice<double>(
  ...
  std::cout << "Testing default knn graph kernel with local memory and select "
               "statements..."
            << std::endl;
  for (std::string &platformName : (*parameters)["PLATFORMS"].keys()) {
    json::Node &platformNode = (*parameters)["PLATFORMS"][platformName];
...
BOOST_AUTO_TEST_CASE(KNNPruneGraphOpenCL) {
  ...
  std::ifstream graph_result(
      "datadriven/datasets/clustering_test_data/"
      "graph_pruned_erg_dim2_depth11.txt");
  if (graph_result) {
    int value;
    while (graph_result >> value) graph_optimal_result.push_back(value);
  } else {
    BOOST_THROW_EXCEPTION(
        std::runtime_error("knn pruned graph result file is missing!"));
  }
  graph_result.close();
  ...
  // Create operation
  sgpp::datadriven::DensityOCLMultiPlatform::OperationPruneGraphOCL
      *operation_prune = new sgpp::datadriven::DensityOCLMultiPlatform::
          OperationPruneGraphOCLMultiPlatform<double>(*grid, alpha, dataset, 2,
                                                      manager, parameters.get(),
                                                      0.2, 8, 0, 0);
...
BOOST_AUTO_TEST_CASE(KNNClusterSearch) {
  ...
  if (graph_in) {
    ...
    BOOST_THROW_EXCEPTION(
        std::runtime_error("Pruned knn graph result file is missing!"));
  }
  graph_in.close();

  std::vector<size_t> optimal_cluster_assignement;
  std::ifstream assignement_in(
      "datadriven/datasets/clustering_test_data/cluster_erg.txt");
  if (assignement_in) {
    size_t value;
  ...
  std::vector<size_t> cluster_assignement = sgpp::datadriven::
      DensityOCLMultiPlatform::OperationCreateGraphOCL::find_clusters(graph, 8);

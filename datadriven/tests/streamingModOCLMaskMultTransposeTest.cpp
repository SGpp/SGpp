...
BOOST_AUTO_TEST_CASE(Simple) {
    ...
    std::vector<std::tuple<std::string, double> > fileNamesError = { std::tuple<std::string, double>(
            "datadriven/tests/data/friedman_4d.arff.gz", 1E-12), std::tuple<std::string, double>(
            "datadriven/tests/data/friedman_10d.arff.gz", 1E-12) };
    ...
    for (std::tuple<std::string, double> fileNameError : fileNamesError) {
        double mse = compareToReferenceTranspose(SGPP::base::GridType::ModLinear, std::get<0>(fileNameError), level, configuration);
...
BOOST_AUTO_TEST_CASE(SimpleSinglePrecision) {
    ...
    std::vector<std::tuple<std::string, double> > fileNamesError = { std::tuple<std::string, double>(
            "datadriven/tests/data/friedman_4d.arff.gz", 1E+7), std::tuple<std::string, double>(
            "datadriven/tests/data/friedman_10d.arff.gz", 1E+6) };

...
struct FilesNamesAndErrorFixture {
  ...
  std::vector<std::tuple<std::string, double>> fileNamesErrorFloat = {
      std::tuple<std::string, double>(
          "datadriven/datasets/friedman/friedman2_4d_10000.arff.gz", 10E-6),
      std::tuple<std::string, double>(
          "datadriven/datasets/friedman/friedman1_10d_2000.arff.gz", 10E-4)};
...
BOOST_FIXTURE_TEST_SUITE(
    TestStreamingOCLMultiPlatformMult,
...
BOOST_AUTO_TEST_CASE(SimpleSinglePrecision) {
  ...
  compareDatasets(fileNamesErrorFloat, sgpp::base::GridType::Linear, level,
                  configuration);
}

...
using sgpp::datadriven::RelativeDensityRatioEstimationMinerFactory;
...
double testDistributionRDRE(std::string testCSV, std::string config) {
  // Train model
  RelativeDensityRatioEstimationMinerFactory factory;
...
BOOST_AUTO_TEST_SUITE(testRelativeDensityRatioEstimation)
...
BOOST_AUTO_TEST_CASE(Test_2D_B2_64_B2_2) {
  ...
  stream << "{ \"dataSource\": { \"filePath\" : [\"" << samplesP << "\",\"" << samplesQ << "\"], "
         ...
         << "\"densityEstimationConfig\" : { \"omega\" : 0.5 } } }" << std::endl;

  double mse = testDistributionRDRE(

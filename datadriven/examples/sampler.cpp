...
int main(int argc, char **argv) {
	  ...
	  kernelConfiguration.type = SGPP::datadriven::OperationMultipleEvalType::STREAMING;
	  kernelConfiguration.subType = SGPP::datadriven::OperationMultipleEvalSubType::DEFAULT;
	  ...
	  std::string metaInformation = "refine: " + std::to_string((unsigned long long) adaptConfig.numRefinements_) + " points: " +
			  std::to_string((unsigned long long) adaptConfig.noPoints_) + " iterations: " + std::to_string((unsigned long long) SLESolverConfigRefine.maxIterations_);

	  double lambda = 0.000001;

	  bool verbose = true;
	  SGPP::datadriven::MetaLearner learner(SLESolverConfigRefine, SLESolverConfigFinal, adaptConfig, maxLevel, lambda, verbose);

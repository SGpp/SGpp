...
for i in `seq 0 $((${#walltimes_single_node[@]}-1))`; do
    ...
    ./create-strong-scaling-test.sh $machine $packetsize $max_nodes strong-scaling ${datasetFolder} ${datasets[$i]} ${walltimes_single_node[$i]}
done

walltimes_weak_scaling_single_node=800
dataset_size_single_node=20000
echo "echo \"weak scaling:\"" >> results_aggregator.sh
./create-weak-scaling-test.sh $machine $packetsize $max_nodes weak-scaling ${datasetFolder} "weak_N%i_c10_size%i_dim10.arff" $walltimes_weak_scaling_single_node $dataset_size_single_node

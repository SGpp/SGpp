...
#include "../../../../../datadriven/src/sgpp/datadriven/opencl/OCLStretchedBufferMultiPlatform.hpp"
...
OCLStretchedBufferMultiPlatform::OCLStretchedBufferMultiPlatform(std::shared_ptr<OCLManagerMultiPlatform> manager) :
        manager(manager) {
    initialized = false;
    sizeofType = 0;
    elements = 0;
...
void OCLStretchedBufferMultiPlatform::initializeBuffer(size_t sizeofType, size_t elements) {
    ...
    for (OCLPlatformWrapper &platform : manager->platforms) {
        ...
        CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, sizeofType * elements, nullptr, &err);
        if (err != CL_SUCCESS) {
        ...
        CL_MAP_READ | CL_MAP_WRITE, 0, sizeofType * elements, 0, nullptr, nullptr, &err);
        if (err != CL_SUCCESS) {
            std::stringstream errorString;
...
void OCLStretchedBufferMultiPlatform::copyToOtherHostBuffers(cl_platform_id originPlatformId) {
    if (!this->initialized) {
        std::stringstream errorString;
        errorString << "OCL Error: tried to \"copyToOtherHostBuffers\" with uninitialized buffer" << std::endl;
        throw SGPP::base::operation_exception(errorString.str());
    }
...
//    size_t actionIndex = 0;
    ...
    for (OCLPlatformWrapper &platform : this->manager->platforms) {
        ...
        for (size_t i = 0; i < platform.deviceCount; i++) {
            ...
            if (range != 0) {
                err = clEnqueueReadBuffer(platform.commandQueues[i], platformBufferList[platform.platformId][i],
...
void OCLStretchedBufferMultiPlatform::combineBuffer(std::map<cl_platform_id, size_t *> indexStart,
    ...
    for (OCLPlatformWrapper &platform : this->manager->platforms) {
        ...
        for (size_t deviceIndex = 0; deviceIndex < platform.deviceCount; deviceIndex++) {
            ...
            size_t range = indexEndDevice - indexStartDevice;
            memmove(static_cast<char *>(this->mappedHostBuffer[platformId]) + indexStartDevice * this->sizeofType,
                    static_cast<char *>(this->mappedHostBuffer[platform.platformId])

...
class DMConfigTools {
 public:
  static void dumpToStream(const base::GeneralGridConfiguration& gridConfig,
    ...
    stream_out << "type: \t\t\t"
               << base::GeneralGridTypeParser::toString(gridConfig.generalType_)
               << std::endl;
    stream_out << "type: \t\t\t"
               << base::GridTypeParser::toString(gridConfig.type_) << std::endl;
    stream_out << "dim: \t\t\t" << gridConfig.dim_ << std::endl;
    stream_out << "level: \t\t\t" << gridConfig.level_ << std::endl;
    stream_out << "levelVector: \t";
    for (auto i = gridConfig.levelVector_.begin();
         i != gridConfig.levelVector_.end(); ++i)
      stream_out << *i << ' ';
    stream_out << std::endl;
  ...
  static void dumpToStream(
      const base::AdaptivityConfiguration& adaptivityConfig,
      std::ostream& stream_out = std::cout) {
    stream_out << "numRefinements: \t\t" << adaptivityConfig.numRefinements_
               << std::endl;
    stream_out << "thresholdType: \t\t"
               << base::AdaptivityThresholdTypeParser::toString(
                      adaptivityConfig.thresholdType_)
               << std::endl;
    stream_out << "refinementThreshold: \t"
               << adaptivityConfig.refinementThreshold_ << std::endl;
    stream_out << "refinementThreshold: \t"
               << adaptivityConfig.coarseningThreshold_ << std::endl;
    stream_out << std::boolalpha << "coarsenInitialPoints: \t"
               << adaptivityConfig.coarsenInitialPoints_ << std::endl;
    stream_out << std::boolalpha << "maxLevelType: \t"
               << adaptivityConfig.maxLevelType_ << std::endl;
    stream_out << "numRefinementPoints: \t"
               << adaptivityConfig.numRefinementPoints_ << std::endl;
    stream_out << "numCoarseningPoints: \t"
               << adaptivityConfig.numCoarseningPoints_ << std::endl;
    ...
    stream_out << "errorConvergenceThreshold: \t"
               << adaptivityConfig.errorConvergenceThreshold << std::endl;
    stream_out << "errorBufferSize: \t\t" << adaptivityConfig.errorBufferSize
               << std::endl;
    stream_out << "errorMinInterval: \t\t" << adaptivityConfig.errorMinInterval
               << std::endl;
    stream_out << "refinementPeriod: \t\t" << adaptivityConfig.refinementPeriod
               << std::endl;
    ...
    stream_out << std::boolalpha << "levelPenalize: \t\t"
               << adaptivityConfig.levelPenalize << std::endl;
  ...
  static void dumpToStream(
      ...
      std::ostream& stream_out = std::cout) {
    stream_out << "enable \t\t\t" << std::boolalpha
               << crossvalidationConfig.enable_ << std::endl;
    stream_out << "kfold \t\t\t" << crossvalidationConfig.kfold_ << std::endl;
    stream_out << "seed \t\t\t" << crossvalidationConfig.seed_ << std::endl;
    stream_out << "shuffle \t\t" << std::boolalpha
               << crossvalidationConfig.shuffle_ << std::endl;
    stream_out << "silent \t\t\t" << std::boolalpha
               << crossvalidationConfig.silent_ << std::endl;
    ...
    stream_out << "lambdaSteps \t\t" << crossvalidationConfig.lambdaSteps_
               << std::endl;
    stream_out << "logScale \t\t" << std::boolalpha
               << crossvalidationConfig.logScale_ << std::endl;
  }

  static void dumpToStream(
      const DensityEstimationConfiguration& densityEstimationConfig,
      std::ostream& stream_out = std::cout) {
    stream_out << "type: \t\t\t" << DensityEstimationTypeParser::toString(
    ...
    stream_out << "normalize: \t\t" << std::boolalpha
               << densityEstimationConfig.normalize_ << std::endl;
  ...
  static void dumpToStream(const solver::SLESolverConfiguration& solverConfig,
                           std::ostream& stream_out = std::cout) {
    stream_out << "type: \t\t"
               << solver::SLESolverTypeParser::toString(solverConfig.type_)
               << std::endl;
    stream_out << "eps: \t\t" << solverConfig.eps_ << std::endl;
    stream_out << "maxIterations: \t" << solverConfig.maxIterations_
               << std::endl;
    stream_out << "threshold: \t" << solverConfig.threshold_ << std::endl;
    stream_out << "verbose: \t" << std::boolalpha << solverConfig.verbose_
               << std::endl;
  }
  ...
  static void dumpToStream(
      ...
      std::ostream& stream_out = std::cout) {
    stream_out << "type: \t\t\t"
               << RegularizationTypeParser::toString(regularizationConfig.type_)
               << std::endl;
    stream_out << "lambda: \t\t\t" << regularizationConfig.lambda_ << std::endl;
    stream_out << "l1Ratio: \t\t\t" << regularizationConfig.l1Ratio_
               << std::endl;
    stream_out << "exponentBase: \t\t" << regularizationConfig.exponentBase_
               << std::endl;
    stream_out << "lambda_start: \t\t" << regularizationConfig.lamda_start_
               << std::endl;
    stream_out << "lambda_end: \t\t" << regularizationConfig.lambda_end_
               << std::endl;
    stream_out << "lambda_steps: \t\t" << regularizationConfig.lambda_steps_
               << std::endl;
    stream_out << "lambda_log_scale: \t" << std::boolalpha
               << regularizationConfig.lambda_log_scale_ << std::endl;
    stream_out << "optimizeLambda: \t" << std::boolalpha
               << regularizationConfig.optimizeLambda_ << std::endl;
    stream_out << "optimizerTolerance: \t"
               << regularizationConfig.optimizerTolerance_ << std::endl;
    stream_out << "convergenceThreshold: \t"
               << regularizationConfig.convergenceThreshold_ << std::endl;
    stream_out << "intervalA: \t\t" << regularizationConfig.intervalA_
               << std::endl;
    stream_out << "intervalB: \t\t" << regularizationConfig.intervalB_
               << std::endl;
  }

  static void dumpToStream(const LearnerConfiguration& learnerConfig,
                           std::ostream& stream_out = std::cout) {
    stream_out << "learningRate: \t\t" << learnerConfig.learningRate
               << std::endl;
    stream_out << "usePrior: \t\t" << std::boolalpha << learnerConfig.usePrior
               << std::endl;
  }
  ...
  static void dumpToStream(const ParallelConfiguration& parallelConfig,
    ...
    stream_out << "scalapackEnabled: \t\t" << std::boolalpha
               << parallelConfig.scalapackEnabled_ << std::endl;
    stream_out << "processRows: \t\t" << parallelConfig.processRows_
               << std::endl;
    stream_out << "processCols: \t\t" << parallelConfig.processCols_
               << std::endl;
    stream_out << "rowBlockSize: \t\t" << parallelConfig.rowBlockSize_
               << std::endl;
    stream_out << "columnBlockSize: \t\t" << parallelConfig.columnBlockSize_
               << std::endl;
  }
};

 ...
 * Segments are distributed as equally as possible (the difference between the minimum and maximum
     number of
 * items is at most @a blocksize). When just dividing (integer division) and leaving the rest to
     the last segment, this
 * segment could have twice as much to do as all the others (example: totalSize=127,
     segmentCount = 16, blockSize = 1)
 *
...
#pragma offload_attribute(pop)
   ...
   *   numCunks chunks and
   * fills the two arrays @a sizes and @a offsets with the respective offsets and sizes (both
   *   arrays have to be
   * already allocated and must be of size @a numChunks). When blocksize greater than 1, then
   *   the resulting sizes
   ...
   * @param offsets output array to store resulting distribution offsets (array size must
       be numChunks)

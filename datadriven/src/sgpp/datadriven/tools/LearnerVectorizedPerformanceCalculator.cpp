...
LearnerVectorizedPerformance LearnerVectorizedPerformanceCalculator::getGFlopAndGByte(sg::base::Grid& Grid,
	...
	if (strcmp(Grid.getType(), "modlinear") == 0) {
		for (size_t g = 0; g < Grid.getSize(); g++) {
			...
			for (size_t h = 0; h < nDim; h++) {
				...
				if (level == 1) {
					...
					result.GByte_ += 1e-9 * 4.0 * static_cast<double>(numIterations) * static_cast<double>(sizeDatatype)
							* static_cast<double>(numInstances);
				} else if (index
						== static_cast<sg::base::GridStorage::index_type::index_type>((1
								<< static_cast<sg::base::GridStorage::index_type::index_type>(level)) - 1)) {
					result.GFlop_ += 1e-9 * 10.0 * static_cast<double>(numIterations)
							* static_cast<double>(numInstances);
					result.GByte_ += 1e-9 * 6.0 * static_cast<double>(numIterations) * static_cast<double>(sizeDatatype)
							* static_cast<double>(numInstances);
				} else {
		...
		// GFlops
		result.GFlop_ += 2.0 * 1e-9 * static_cast<double>(nGridsize) * static_cast<double>(numInstances)
				* static_cast<double>(nDim) * 6.0 * static_cast<double>(numIterations);

		// GBytes
		result.GByte_ += 2.0 * 1e-9 * static_cast<double>(nGridsize) * static_cast<double>(numInstances)
				* static_cast<double>(nDim) * 3.0 * static_cast<double>(numIterations)
				* static_cast<double>(sizeDatatype);

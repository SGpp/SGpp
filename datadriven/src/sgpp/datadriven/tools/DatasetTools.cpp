...
void DatasetTools::splitset(base::DataMatrix &dataset, base::DataVector &datasetValues, size_t kFold,
        std::vector<base::DataMatrix> &trainingSets, std::vector<base::DataVector> &trainingSetValues,
        std::vector<base::DataMatrix> &testSets, std::vector<base::DataVector> &testSetValues, bool verbose) {
    ...
    //fill data
    for (size_t i = 0; i < kFold; i++) {
        trainingSets.emplace_back(dataset.getNrows() - partitionSizes[i], dataset.getNcols());

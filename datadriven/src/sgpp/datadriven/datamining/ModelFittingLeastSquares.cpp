...
ModelFittingLeastSquares::createSystemMatrix(base::DataMatrix& trainDataset, float_t lambda) {
...
//  result.GByte_ = 0.0;
  ...
  systemMatrix = std::shared_ptr<datadriven::DMSystemMatrixBase>(
          createSystemMatrix(dataset.getTrainingData(), configuration.lambda));

  if (configuration.solverRefineConfig.type_ == SGPP::solver::SLESolverType::CG) {
    solver = std::make_shared<solver::ConjugateGradients>(
            configuration.solverRefineConfig.maxIterations_, configuration.solverRefineConfig.eps_);
  } else if (configuration.solverRefineConfig.type_ == SGPP::solver::SLESolverType::BiCGSTAB) {
    solver = std::make_shared<solver::BiCGStab>(configuration.solverRefineConfig.maxIterations_,
            configuration.solverRefineConfig.eps_);
  } else {
  ...
  systemMatrix->generateb(dataset.getClasses(), b);

  if (configuration.adaptivityConfig.numRefinements_ == 0) {
    solver->setMaxIterations(configuration.solverFinalConfig.maxIterations_);
    solver->setEpsilon(configuration.solverFinalConfig.eps_);
  }
...
//  auto refinementFunctor = std::make_shared<base::SurplusRefinementFunctor>(alpha,
//          configuration.adaptivityConfig.noPoints_, configuration.adaptivityConfig.threshold_);
...
//    myCG->setMaxIterations(configuration.solverFinalConfig.maxIterations_);
//    myCG->setEpsilon(configuration.solverFinalConfig.eps_);
//  }
}

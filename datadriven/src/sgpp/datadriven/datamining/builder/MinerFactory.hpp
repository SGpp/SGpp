 ...
 * a file. A miner consists of a data source, a fitter and a scorer. A concrete
 * Factory class has to
...
class MinerFactory {
 public:
  ...
  virtual sgpp::datadriven::HyperparameterOptimizer* buildHPO(
 ...
 protected:
  ...
  virtual std::vector<DataSourceSplitting*>
  createDataSourceSplitting_TwoDatasets(
      const DataMiningConfigParser& parser) const {
    throw base::application_exception(
        "This miner only allows a single dataSource instance");
  }
  ...
  /**
   * Build an instance of a #sgpp::datadriven::ModelFittingBase object as
   * specified in the
   * configuration
   * file.
   ...
   * #sgpp::datadriven::ModelFittingBase object) as
   * specified in the
   * configuration file.
   */
  virtual ModelFittingBase* createFitter(
      const DataMiningConfigParser& parser) const = 0;

  virtual FitterFactory* createFitterFactory(
      const DataMiningConfigParser& parser) const = 0;
   ...
   * @return the scorer instance
   */
  virtual Visualizer* createVisualizer(
      const DataMiningConfigParser& parser) const = 0;
};
} /* namespace datadriven */
} /* namespace sgpp */

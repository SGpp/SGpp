...
#pragma omp parallel for schedule(dynamic)
      for (size_t index = 0; index < models->size(); index++) {
        ...
        if (config.getGeneralConfig().crossValidation_) {
          ...
          currentDirectory = config.getGeneralConfig().targetDirectory_ + "/Batch_" +
                             std::to_string(batch) + "/Model_Class_" +
                             std::to_string(static_cast<int>(classes.get(index)));
        }
...
void VisualizerClassification::getHeatmapsClassification(ModelFittingBase &model,
  ...
  if (nDimensions >= 3) {
    std::string outputDir(currentDirectory + "/Classification" + "/Heatmaps/");
...
void VisualizerClassification::getHeatmapMore4DClassification(ModelFittingBase &model,
                                                              std::string currentDirectory,
                                                              DataMatrix &classMatrix) {
  std::string outputDir(currentDirectory + "/Classification" + "/Heatmaps/");
      ...
      for (size_t combination = 0; combination < 3; combination++) {
        ...
        if (iteration == 0) {
          if (config.getGeneralConfig().targetFileType_ == VisualizationFileType::CSV) {
            ...
            storeHeatmapJsonClassification(
                heatMapResults, model, variableColumnIndexes, variableColumnIndexes.at(0),
...
void VisualizerClassification::getHeatmap2DClassification(ModelFittingBase &model,
                                                          std::string currentDirectory,
                                                          DataMatrix &classMatrix) {
  std::string outputDir(currentDirectory + "/Classification/");
  ...
  if (config.getGeneralConfig().targetFileType_ == VisualizationFileType::CSV) {
    ...
    storeHeatmapJsonClassification(heatMapResults, model, outputDir + "ClassificationModel");
  }
}
...
void VisualizerClassification::storeHeatmapJsonClassification(DataMatrix &matrix,
  ...
  jsonOutput["layout"]["title"].addIDAttr("text",
                                          "\"Classification "
                                          "Heatmaps: Variable dimensions: " +
                                              std::to_string(varDim1 + 1) + " and " +
  ...
  for (size_t graphNumber = 0; graphNumber < totalGraphs; graphNumber++) {
    ...
    for (size_t i = 0; i < numberModels; i++) {
      ...
      jsonOutput["data"][graphIndex]["marker"].addIDAttr("size", static_cast<size_t>(10));
      ...
      jsonOutput["data"][graphIndex].addIDAttr("legendgroup", std::to_string(i));
      ...
      if (graphNumber == 0) {
        ...
        jsonOutput["data"][graphIndex].addIDAttr(
            "name", "\"Data Class  " + std::to_string(static_cast<int>(classes.at(i))) + "\"");
      } else {
    ...
    for (size_t i = 0; i < numberModels; i++) {
      ...
      jsonOutput["data"][graphIndex].addIDAttr("legendgroup", std::to_string(i + numberModels));

      jsonOutput["data"][graphIndex].addDictAttr("marker");

      jsonOutput["data"][graphIndex]["marker"].addIDAttr("color", "\"" + colors.at(i) + "\"");

      if (showLegendGroup.at(i) && tempGrid.getNrows() > 0) {
        jsonOutput["data"][graphIndex].addIDAttr("showlegend", true);
        jsonOutput["data"][graphIndex].addIDAttr(
            "name", "\" Grid Class " + std::to_string(static_cast<int>(classes.at(i))) + "\"");

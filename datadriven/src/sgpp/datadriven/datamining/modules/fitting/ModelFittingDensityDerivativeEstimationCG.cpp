...
ModelFittingDensityDerivativeEstimationCG::ModelFittingDensityDerivativeEstimationCG(
...
double ModelFittingDensityDerivativeEstimationCG::evaluate(const DataVector& sample) {
...
// TODO(lettrich): exceptions have to be thrown if not valid.
void ModelFittingDensityDerivativeEstimationCG::evaluate(DataMatrix& samples, DataVector& results) {
  // For density derivative we use b-splines, so we need the EvalNaive instead!
  sgpp::op_factory::createOperationMultipleEvalNaive(*grid, samples)->eval(alpha, results);
}

void ModelFittingDensityDerivativeEstimationCG::fit(Dataset& newDataset) {
  dataset = &newDataset;
  fit(newDataset.getData());
}

void ModelFittingDensityDerivativeEstimationCG::fit(DataMatrix& newDataset) {
  // Reset grid
  reset();
...
bool ModelFittingDensityDerivativeEstimationCG::adapt(size_t newNoPoints,
...
void ModelFittingDensityDerivativeEstimationCG::update(Dataset& newDataset) {
  dataset = &newDataset;
  update(newDataset.getData());
}

base::OperationMatrix* ModelFittingDensityDerivativeEstimationCG::computeRegularizationMatrix(
    base::Grid& grid) {
...
void ModelFittingDensityDerivativeEstimationCG::update(DataMatrix& newDataset) {
  if (grid == nullptr) {
    ...
    datadriven::DensityDerivativeSystemMatrix SMatrix(
        *grid, newDataset, C, regularizationConfig.lambda_,
        this->config->getDensityEstimationConfig().derivDim_);
...
bool ModelFittingDensityDerivativeEstimationCG::isRefinable() { return true; }

void ModelFittingDensityDerivativeEstimationCG::reset() {
  // Clear model
  grid.reset();

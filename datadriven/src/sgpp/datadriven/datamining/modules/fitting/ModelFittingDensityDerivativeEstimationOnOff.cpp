...
ModelFittingDensityDerivativeEstimationOnOff::ModelFittingDensityDerivativeEstimationOnOff(
...
ModelFittingDensityDerivativeEstimationOnOff::ModelFittingDensityDerivativeEstimationOnOff(
    ...
    : ModelFittingDensityDerivativeEstimationOnOff(config) {
  this->objectStore = objectStore;
  this->hasObjectStore = true;
}

// TODO(lettrich): exceptions have to be thrown if not valid.
double ModelFittingDensityDerivativeEstimationOnOff::evaluate(const DataVector& sample) {
  return online->eval(alpha, sample, *grid);
}

// TODO(lettrich): exceptions have to be thrown if not valid.
void ModelFittingDensityDerivativeEstimationOnOff::evaluate(DataMatrix& samples,
                                                            DataVector& results) {
  online->eval(alpha, samples, results, *grid);
}

void ModelFittingDensityDerivativeEstimationOnOff::fit(Dataset& newDataset) {
  dataset = &newDataset;
  fit(newDataset.getData());
}

void ModelFittingDensityDerivativeEstimationOnOff::fit(DataMatrix& newDataset) {
  // Get configurations
...
bool ModelFittingDensityDerivativeEstimationOnOff::adapt(size_t newNoPoints,
...
void ModelFittingDensityDerivativeEstimationOnOff::update(Dataset& newDataset) {
  dataset = &newDataset;
  update(newDataset.getData());
}

void ModelFittingDensityDerivativeEstimationOnOff::update(DataMatrix& newDataset) {
  if (grid == nullptr) {
...
double ModelFittingDensityDerivativeEstimationOnOff::computeResidual(
    DataMatrix& validationData) const {
  DataVector bValidation = online->computeWeightedDerivativeBFromBatch(
...
void ModelFittingDensityDerivativeEstimationOnOff::updateRegularization(double lambda) {
  if (grid != nullptr) {
    auto& densityEstimationConfig = this->config->getDensityEstimationConfig();
...
bool ModelFittingDensityDerivativeEstimationOnOff::isRefinable() {
  if (grid != nullptr) {
...
void ModelFittingDensityDerivativeEstimationOnOff::reset() {
  grid.reset();
  online.reset();
  refinementsPerformed = 0;
}

void ModelFittingDensityDerivativeEstimationOnOff::resetTraining() {
  if (grid != nullptr) {
    alpha = DataVector(grid->getSize());

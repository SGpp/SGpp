...
#include "CategoricalParameter.hpp"
...
void CategoricalParameter::makeConfigBits(std::list<std::unique_ptr<ConfigurationBit>>& allbits){
	int i = 1;
	int c = 2;
	while(c < max-min+1){
		i++;
...
int CategoricalParameter::getValue(){
	return value;
}

int CategoricalParameter::getNOptions() {
  return max-min+1;
}

void CategoricalParameter::setBO(int option){
  value = min+option;
}
void CategoricalParameter::setHarmonica(){

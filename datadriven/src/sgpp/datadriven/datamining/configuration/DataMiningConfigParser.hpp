...
class DataMiningConfigParser {
 public:
  ...
  std::vector<int64_t> parseIntArray(json::JSON::DictNode &dict,
  ...
  /**
   * Checks whether the fitter configuration contains a cross validation
   * configuration
   * @return if the fitter configuration contains a cross validation
   * configuration
   */
   ...
   * used as a template for filling in the parameters for all dataSource
   * instances.
   */
  bool getMultiDataSourceConfig(
   ...
   */
  bool getScorerConfig(ScorerConfiguration &config,
                       const ScorerConfiguration &defaults) const;

  bool getFitterConfigType(FitterType &fitter,
                           const FitterType &defaults) const;

  bool getFitterGridConfig(
      base::GeneralGridConfiguration &config,
      const base::GeneralGridConfiguration &defaults) const;

  bool getFitterAdaptivityConfig(
      base::AdaptivityConfiguration &config,
      const base::AdaptivityConfiguration &defaults) const;

  bool getFitterCrossvalidationConfig(
      CrossvalidationConfiguration &config,
      const CrossvalidationConfiguration &defaults) const;
  bool getFitterDensityEstimationConfig(
      DensityEstimationConfiguration &config,
      const DensityEstimationConfiguration &defaults) const;
  bool getFitterSolverRefineConfig(
      solver::SLESolverConfiguration &config,
      const solver::SLESolverConfiguration &defaults) const;
  bool getFitterSolverFinalConfig(
      solver::SLESolverConfiguration &config,
      const solver::SLESolverConfiguration &defaults) const;
  bool getFitterRegularizationConfig(
   ...
   * @param defaults default values if the fitter does not contain a database
   * configuration
   * @return whether the fitter contains a database configuration
   */
  bool getFitterDatabaseConfig(
   ...
   * @param defaults default values if the fitter config does not contain a
   * matching entry
   * @return whether the configuration contains a learner configuration
   */
  bool getFitterLearnerConfig(
   ...
   * @param config the configuration instance that will be initialized
   * @param defaults default values if the parallel config does not contain a
   * matching entry
   * @return whether the configuration contains a parallel configuration
   */
  bool getFitterParallelConfig(
   ...
   */
  bool getVisualizationGeneralConfig(
      datadriven::VisualizationGeneralConfig &config,
   ...
   * @return wether the configuration contains a visualization configuration
   */
  bool getVisualizationParameters(
      datadriven::VisualizationParameters &config,
 ...
 private:
  ...
  std::string parseString(json::JSON::DictNode &dict, const std::string &key,
                          ...
                          const std::string &parentNode) const;
  double parseDouble(json::JSON::DictNode &dict, const std::string &key,
                     double defaultValue, const std::string &parentNode) const;
  std::vector<double> parseDoubleArray(json::JSON::DictNode &dict,
                                       const std::string &key,
  ...
  std::vector<std::string> parseStringArray(
      ...
      const std::string &parentNode) const;
  std::vector<size_t> parseUIntArray(json::JSON::DictNode &dict,
                                     const std::string &key,
                                     std::vector<size_t> defaultValue,
                                     const std::string &parentNode) const;
  size_t parseUInt(json::JSON::DictNode &dict, const std::string &key,
                   size_t defaultValue, const std::string &parentNode) const;
  int64_t parseInt(json::JSON::DictNode &dict, const std::string &key,
                   int64_t defaultValue, const std::string &parentNode) const;
  bool parseBool(json::JSON::DictNode &dict, const std::string &key,
                 bool defaultValue, const std::string &parentNode) const;

  void parseSLESolverConfig(json::JSON::DictNode &dict,
                            solver::SLESolverConfiguration &config,
                            const solver::SLESolverConfiguration &defaults,
                            const std::string &parentNode) const;

  void parseDataTransformationConfig(json::JSON::DictNode &dict,
                                     DataTransformationConfig &config,
                                     const DataTransformationConfig &defaults,
                                     const std::string &parentNode) const;

  void parseRosenblattTransformationConfig(
      json::JSON::DictNode &dict, RosenblattTransformationConfig &config,

...
DataMiningConfigParser::DataMiningConfigParser(const std::string &filepath)
    : configFile(nullptr) {
  try {
...
bool DataMiningConfigParser::hasDataTransformationConfig() const {
  bool hasDataTransformationConfig =
      hasDataSourceConfig()
          ? (*configFile)[dataSource].contains("dataTransformation")
          : false;
  return hasDataTransformationConfig;
}

bool DataMiningConfigParser::hasScorerConfig() const {
  return configFile->contains(scorer);
}

bool DataMiningConfigParser::hasFitterConfig() const {
  return configFile->contains(fitter);
}

bool DataMiningConfigParser::hasParallelConfig() const {
  bool hasParallelConfig =
      hasFitterConfig() ? (*configFile)[fitter].contains("parallelConfig")
                        : false;
...
bool DataMiningConfigParser::hasFitterConfigCrossValidation() const {
  bool hasFitterCrossValidationConfig =
      hasFitterConfig() ? (*configFile)[fitter].contains("crossValidation")
                        : false;
...
bool DataMiningConfigParser::hasVisualizationGeneralConfig() const {
  bool hasVisualizationParameters =
      hasVisualizationConfig()
          ? (*configFile)[visualization].contains("generalConfig")
          : false;
...
bool DataMiningConfigParser::hasVisualizationParametersConfig() const {
  bool hasVisualizationParameters =
      hasVisualizationConfig()
          ? (*configFile)[visualization].contains("parameters")
          : false;
  return hasVisualizationParameters;
}

bool DataMiningConfigParser::getDataSourceConfig(
    DataSourceConfig &config, const DataSourceConfig &defaults) const {
...
bool DataMiningConfigParser::getMultiDataSourceConfig(
    std::vector<DataSourceConfig> &config,
    const std::vector<DataSourceConfig> &defaults) const {
  bool hasDataSource = hasDataSourceConfig();

  if (hasDataSource) {
    auto dataSourceConfig =
        dynamic_cast<json::DictNode *>(&(*configFile)[dataSource]);

    // Fill in all parameters for first dataset (except the filePath)
    config[0].isCompressed = parseBool(*dataSourceConfig, "compression",
                                       defaults[0].isCompressed, "dataSource");
    config[0].numBatches = parseUInt(*dataSourceConfig, "numBatches",
                                     defaults[0].numBatches, "dataSource");
    config[0].batchSize = parseUInt(*dataSourceConfig, "batchSize",
                                    defaults[0].batchSize, "dataSource");
    config[0].hasTargets = parseBool(*dataSourceConfig, "hasTargets",
                                     defaults[0].hasTargets, "dataSource");
    config[0].validationPortion =
        parseDouble(*dataSourceConfig, "validationPortion",
                    defaults[0].validationPortion, "dataSource");
    // if negative we want UINT_MAX here, so all should be fine
    config[0].readinCutoff =
        static_cast<size_t>(parseInt(*dataSourceConfig, "readinCutoff",
                                     defaults[0].readinCutoff, "dataSource"));
    config[0].readinClasses =
        parseDoubleArray(*dataSourceConfig, "readinClasses",
                         defaults[0].readinClasses, "dataSource");
    config[0].readinColumns =
        parseUIntArray(*dataSourceConfig, "readinColumns",
                       defaults[0].readinColumns, "dataSource");

    // parse file type
    if (dataSourceConfig->contains("fileType")) {
      config[0].fileType = DataSourceFileTypeParser::parse(
          (*dataSourceConfig)["fileType"].get());
    } else {
      std::cout << "# Did not find " << dataSource
                << "[fileType]. Setting default value "
                << DataSourceFileTypeParser::toString(defaults[0].fileType)
                << "." << std::endl;
    ...
    if (hasDataTransformation) {
      auto dataTransformationConfig = static_cast<DictNode *>(
          &(*configFile)[dataSource]["dataTransformation"]);
      parseDataTransformationConfig(
          *dataTransformationConfig, config[0].dataTransformationConfig,
    ...
    // parse the shuffling
    if (dataSourceConfig->contains("shuffling")) {
      config[0].shuffling = DataSourceShufflingTypeParser::parse(
          (*dataSourceConfig)["shuffling"].get());
    } else {
      std::cout
          << "# Did not find dataSource[shuffling]. Setting default value "
    ...
    config[0].randomSeed = parseUInt(*dataSourceConfig, "randomSeed",
                                     defaults[0].randomSeed, "dataSource");
    config[0].epochs = parseUInt(*dataSourceConfig, "epochs",
                                 defaults[0].epochs, "dataSource");

    // Parse the filePath field for all config instances
    json::ListNode &paths =
        dynamic_cast<json::ListNode &>((*dataSourceConfig)["filePath"]);
...
bool DataMiningConfigParser::getScorerConfig(
  ...
  if (hasScorer) {
    ...
    std::cout << "# Could not find specification  of scorer. Falling Back to "
                 "default values."
              << std::endl;
    config = defaults;
  }
...
bool DataMiningConfigParser::getFitterConfigType(
    FitterType &config, const FitterType &defaults) const {
  ...
  if (hasFitterConfig) {
    ...
    if (fitterConfig->contains("type")) {
      ...
      std::cout << "# Could not find specification  of fitter[type]. Falling "
                   "Back to default "
                   "values."
                << std::endl;
      config = defaults;
...
bool DataMiningConfigParser::getFitterGridConfig(
    GeneralGridConfiguration &config,
  ...
  if (hasFitterGridConfig) {
    auto fitterConfig =
        static_cast<DictNode *>(&(*configFile)[fitter]["gridConfig"]);
    config.dim_ = parseUInt(*fitterConfig, "dim", defaults.dim_, "gridConfig");
    config.level_ = static_cast<int>(
        parseInt(*fitterConfig, "level", defaults.level_, "gridConfig"));
    ...
    config.maxDegree_ = parseUInt(*fitterConfig, "maxDegree",
                                  defaults.maxDegree_, "gridConfig");
    config.boundaryLevel_ = static_cast<unsigned int>(parseUInt(
        *fitterConfig, "boundaryLevel", defaults.boundaryLevel_, "gridConfig"));
    config.filename_ = parseString(*fitterConfig, "fileName",
                                   defaults.filename_, "gridConfig");
    ...
    if (fitterConfig->contains("generalGridType")) {
      ...
      std::cout << "# Did not find gridConfig[generalGridType]. Setting "
                   "default value."
                << std::endl;
    }
    ...
    std::cout << "# Could not find specification  of fitter[gridConfig]. "
...
bool DataMiningConfigParser::getFitterAdaptivityConfig(
  ...
  bool hasFitterAdaptivityConfig =
      hasFitterConfig() ? (*configFile)[fitter].contains("adaptivityConfig")
                        : false;

  if (hasFitterAdaptivityConfig) {
    auto adaptivityConfig =
        static_cast<DictNode *>(&(*configFile)[fitter]["adaptivityConfig"]);
...
bool DataMiningConfigParser::getFitterCrossvalidationConfig(
  ...
  if (hasFitterCrossvalidationConfig) {
    ...
    config.enable_ = parseBool(*crossvalidationConfig, "enable",
                               defaults.enable_, "crossValidation");
    config.kfold_ = parseUInt(*crossvalidationConfig, "kFold", defaults.kfold_,
                              "crossValidation");
    config.seed_ =
        static_cast<int>(parseInt(*crossvalidationConfig, "randomSeed",
                                  defaults.seed_, "crossvalidationConfig"));
    config.shuffle_ = parseBool(*crossvalidationConfig, "shuffle",
                                defaults.shuffle_, "crossValidation");
    config.silent_ = parseBool(*crossvalidationConfig, "silent",
                               defaults.silent_, "crossValidation");
    config.lambda_ = parseDouble(*crossvalidationConfig, "lambda",
                                 defaults.lambda_, "crossValidation");
    config.lambdaStart_ = parseDouble(*crossvalidationConfig, "lambdaStart",
                                      defaults.lambdaStart_, "crossValidation");
    config.lambdaEnd_ = parseDouble(*crossvalidationConfig, "lambdaEnd",
                                    defaults.lambdaEnd_, "crossValidation");
    config.lambdaSteps_ = parseUInt(*crossvalidationConfig, "lambdaSteps",
                                    defaults.lambdaSteps_, "crossValidation");
    config.logScale_ = parseBool(*crossvalidationConfig, "logScale",
                                 defaults.logScale_, "crossValidation");
  } else {
    std::cout << "# Could not find specification  of "
                 "fitter[crossvalidationConfig]. Falling "
...
bool DataMiningConfigParser::getFitterDensityEstimationConfig(
    DensityEstimationConfiguration &config,
    const DensityEstimationConfiguration &defaults) const {
  bool hasFitterDensityEstimationConfig =
      hasFitterConfig()
          ? (*configFile)[fitter].contains("densityEstimationConfig")
          : false;

  if (hasFitterDensityEstimationConfig) {
    auto densityEstimationConfig = static_cast<DictNode *>(
        &(*configFile)[fitter]["densityEstimationConfig"]);
    config.iCholSweepsDecompose_ =
        parseUInt(*densityEstimationConfig, "iCholSweepsDecompose",
                  defaults.iCholSweepsDecompose_, "densityEstimationConfig");
    config.iCholSweepsRefine_ =
        parseUInt(*densityEstimationConfig, "iCholSweepsRefine",
    ...
    config.iCholSweepsSolver_ =
        parseUInt(*densityEstimationConfig, "iCholSweepsSolver",
                  defaults.iCholSweepsSolver_, "densityEstimationConfig");

    config.normalize_ =
        parseBool(*densityEstimationConfig, "normalize", defaults.normalize_,
                  "densityEstimationConfig");

    config.useOfflinePermutation =
        parseBool(*densityEstimationConfig, "useOfflinePermutation",
                  defaults.useOfflinePermutation, "densityEstimationConfig");
    ...
    if (densityEstimationConfig->contains("densityEstimationType")) {
      ...
      std::cout
          ...
          << DensityEstimationTypeParser::toString(defaults.type_) << "."
          << std::endl;
      config.type_ = defaults.type_;
    }
    ...
    std::cout << "# Could not find specification  of "
                 "fitter[densityEstimationConfig]. Falling Back to "
                 "default values."
              << std::endl;
    config = defaults;
  }
...
bool DataMiningConfigParser::getFitterSolverRefineConfig(
  ...
  bool hasFitterSolverRefineConfig =
      hasFitterConfig() ? (*configFile)[fitter].contains("solverRefineConfig")
                        : false;

  if (hasFitterSolverRefineConfig) {
    auto solverConfig =
        static_cast<DictNode *>(&(*configFile)[fitter]["solverRefineConfig"]);

    parseSLESolverConfig(*solverConfig, config, defaults, "solverRefineConfig");
  } else {
    std::cout << "# Could not find specification  of "
                 "fitter[solverRefineConfig]. Falling Back to "
                 "default values."
              << std::endl;
    config = defaults;
  }
...
bool DataMiningConfigParser::getFitterSolverFinalConfig(
  ...
  bool hasFitterSolverFinalConfig =
      hasFitterConfig() ? (*configFile)[fitter].contains("solverFinalConfig")
                        : false;

  if (hasFitterSolverFinalConfig) {
    auto solverConfig =
        static_cast<DictNode *>(&(*configFile)[fitter]["solverFinalConfig"]);

    parseSLESolverConfig(*solverConfig, config, defaults, "solverFinalConfig");
  } else {
    std::cout << "# Could not find specification  of "
                 "fitter[solverFinalConfig]. Falling Back to "
                 "default values."
              << std::endl;
    config = defaults;
  }
...
bool DataMiningConfigParser::getFitterRegularizationConfig(
  ...
  bool hasRegularizationConfig =
      hasFitterConfig() ? (*configFile)[fitter].contains("regularizationConfig")
                        : false;
  ...
  if (hasRegularizationConfig) {
    ...
    if (regularizationConfig->contains("regularizationType")) {
      config.type_ = RegularizationTypeParser::parse(
          (*regularizationConfig)["regularizationType"].get());
    } else {
      std::cout << "# Did not find regularizationConfig[regularizationType]. "
                   "Setting default value "
                << RegularizationTypeParser::toString(defaults.type_) << "."
                << std::endl;
      config.type_ = defaults.type_;
    }

    config.lambda_ = parseDouble(*regularizationConfig, "lambda",
                                 defaults.lambda_, "regularizationConfig");

    config.exponentBase_ =
        parseDouble(*regularizationConfig, "exponentBase",
                    defaults.exponentBase_, "regularizationConfig");

    config.l1Ratio_ = parseDouble(*regularizationConfig, "l1Ratio",
                                  defaults.l1Ratio_, "regularizationConfig");

    config.optimizeLambda_ =
        parseBool(*regularizationConfig, "optimizeLambda",
                  defaults.optimizeLambda_, "regularizationConfig");

    config.optimizerTolerance_ =
        parseDouble(*regularizationConfig, "optimizerTolerance",
                    defaults.optimizerTolerance_, "regularizationConfig");

    config.convergenceThreshold_ =
        parseDouble(*regularizationConfig, "convergenceThreshold",
                    defaults.convergenceThreshold_, "regularizationConfig");

    config.intervalA_ =
        parseDouble(*regularizationConfig, "intervalA", defaults.intervalA_,
                    "regularizationConfig");
    config.intervalB_ =
        parseDouble(*regularizationConfig, "intervalB", defaults.intervalB_,
                    "regularizationConfig");

    if (regularizationConfig->contains("regularizationMetric")) {
      config.regularizationMetric_ =
          ScorerMetricTypeParser::parseRegularizationMetric(
...
std::string DataMiningConfigParser::parseString(
    DictNode &dict, const std::string &key, const std::string &defaultValue,
  ...
  if (dict.contains(key)) {
    try {
      ...
      try {
        ...
        std::string errorMsg = "# Failed to parse string " + parentDict + "[" +
                               key +
    ...
    std::cout << "# Did not find " << parentDict << "[" << key
...
double DataMiningConfigParser::parseDouble(
    DictNode &dict, const std::string &key, double defaultValue,
    const std::string &parentDict) const {
  if (dict.contains(key)) {
    try {
      ...
      try {
        ...
        std::string errorMsg = "# Failed to parse double " + parentDict + "[" +
                               key +
                               "] from string";  // + dict[key].get() + ".";
        throw data_exception(errorMsg.c_str());
      }
  ...
  if (dict.contains(key)) {
    try {
      ...
      try {
        ...
        std::string errorMsg = "# Failed to parse unsigned integer " +
                               parentDict + "[" + key +
                               "] from string";  // + dict[key].get() + ".";
        throw data_exception(errorMsg.c_str());
      }
...
bool DataMiningConfigParser::parseBool(DictNode &dict, const std::string &key,
                                       bool defaultValue,
  ...
  if (dict.contains(key)) {
    try {
      ...
      std::string errorMsg = "# Failed to parse bool " + parentNode + "[" +
                             key + "] from string" + dict[key].get() + ".";
      throw data_exception(errorMsg.c_str());
    }
  } else {
  ...
  if (dict.contains(key)) {
    try {
      ...
      try {
        ...
        std::string errorMsg = "# Failed to parse integer " + parentNode + "[" +
                               key +
                               "] from string";  // + dict[key].get() + ".";
        throw data_exception(errorMsg.c_str());
      }
...
std::vector<int64_t> DataMiningConfigParser::parseIntArray(
    DictNode &dict, const std::string &key, std::vector<int64_t> defaultValue,
    const std::string &parentNode) const {
  if (dict.contains(key)) {
    try {
      ...
      std::string errorMsg = "# Failed to parse integer array" + parentNode +
                             "[" + key + "] from string" + dict[key].get() +
                             ".";
      throw data_exception(errorMsg.c_str());
    }
...
std::vector<std::vector<int64_t>> DataMiningConfigParser::parseArrayOfIntArrays(
    DictNode &dict, const std::string &key,
  ...
  if (dict.contains(key)) {
    try {
      ...
      std::string errorMsg = "# Failed to parse array of integer arrays" +
                             parentNode + "[" + key + "] from string" +
                             dict[key].get() + ": " + e.what();
      throw data_exception(errorMsg.c_str());
    }
  } else {
...
std::vector<double> DataMiningConfigParser::parseDoubleArray(
    DictNode &dict, const std::string &key, std::vector<double> defaultValue,
    const std::string &parentNode) const {
  if (dict.contains(key)) {
    try {
      ...
      std::string errorMsg = "# Failed to parse double array" + parentNode +
                             "[" + key + "] from string" + dict[key].get() +
                             ".";
      throw data_exception(errorMsg.c_str());
    }
...
// (Sebastian) Adapted from parseIntArray without much thought
std::vector<size_t> DataMiningConfigParser::parseUIntArray(
    DictNode &dict, const std::string &key, std::vector<size_t> defaultValue,
    const std::string &parentNode) const {
  if (dict.contains(key)) {
    try {
      ...
      std::string errorMsg = "# Failed to parse uint array" + parentNode + "[" +
                             key + "] from string" + dict[key].get() + ".";
      throw data_exception(errorMsg.c_str());
    }
  } else {
...
void DataMiningConfigParser::parseSLESolverConfig(
    DictNode &dict, SLESolverConfiguration &config,
    const SLESolverConfiguration &defaults,
    const std::string &parentNode) const {
  config.eps_ = parseDouble(dict, "eps", defaults.eps_, parentNode);
  config.maxIterations_ =
      parseUInt(dict, "maxIterations", defaults.maxIterations_, parentNode);
  config.threshold_ =
      parseDouble(dict, "threshold", defaults.threshold_, parentNode);
...
void DataMiningConfigParser::getHyperparameters(
  ...
  try {
    if ((*configFile)[fitter]["gridConfig"]["level"]["optimize"].getBool()) {
      int64_t min =
          (*configFile)[fitter]["gridConfig"]["level"]["min"].getInt();
      int64_t max =
          (*configFile)[fitter]["gridConfig"]["level"]["max"].getInt();
      if (max > min) {
        dispar["level"] = DiscreteParameter("level", static_cast<int>(min),
                                            static_cast<int>(max));
      }
    }
  } catch (json_exception &) {
  ...
  try {
    if ((*configFile)[fitter]["gridConfig"]["gridType"]["optimize"].getBool()) {
      ...
      if (nOptions > 1) {
        ...
        catpar["basisFunction"] = DiscreteParameter(
            "basisFunction", 0, static_cast<int>(nOptions - 1));
      }
    }
  } catch (json_exception &) {
  }
  try {
    if ((*configFile)[fitter]["adaptivityConfig"]["noPoints"]["optimize"]
            .getBool()) {
      int64_t min =
          (*configFile)[fitter]["adaptivityConfig"]["noPoints"]["min"].getInt();
      int64_t max =
          (*configFile)[fitter]["adaptivityConfig"]["noPoints"]["max"].getInt();
      if (max > min) {
        dispar["noPoints"] = DiscreteParameter(
            "noPoints", static_cast<int>(min), static_cast<int>(max));
      }
    }
  } catch (json_exception &) {
  }
  try {
    if ((*configFile)[fitter]["adaptivityConfig"]["threshold"]["optimize"]
            .getBool()) {
      double min = (*configFile)[fitter]["adaptivityConfig"]["threshold"]["min"]
                       .getDouble();
      double max = (*configFile)[fitter]["adaptivityConfig"]["threshold"]["max"]
                       .getDouble();
      int64_t bits =
          (*configFile)[fitter]["adaptivityConfig"]["threshold"]["bits"]
              .getInt();
      bool logscale =
          (*configFile)[fitter]["adaptivityConfig"]["threshold"]["logscale"]
              .getBool();
      if (max > min) {
        conpar["threshold"] = ContinuousParameter(
            static_cast<size_t>(bits), "threshold", min, max, logscale);
      }
    }
  } catch (json_exception &) {
  }
  try {
    if ((*configFile)[fitter]["regularizationConfig"]["lambda"]["optimize"]
            .getBool()) {
      double min =
          (*configFile)[fitter]["regularizationConfig"]["lambda"]["min"]
              .getDouble();
      double max =
          (*configFile)[fitter]["regularizationConfig"]["lambda"]["max"]
              .getDouble();
      int64_t bits =
          (*configFile)[fitter]["regularizationConfig"]["lambda"]["bits"]
              .getInt();
      bool logscale =
          (*configFile)[fitter]["adaptivityConfig"]["threshold"]["logscale"]
              .getBool();
...
void DataMiningConfigParser::getHPOConfig(HPOConfig &config) {
  if (configFile->contains("hpo")) {
    ...
    config.setNTrainSamples(
        parseInt(*node, "trainSize", config.getNTrainSamples(), "hpo"));
    if (node->contains("harmonica")) {
      auto harmonica = static_cast<DictNode *>(&(*node)["harmonica"]);
      config.setLambda(
          parseDouble(*harmonica, "lambda", config.getLambda(), "hpo"));
      config.setStages(
          parseIntArray(*harmonica, "stages", config.getStages(), "hpo"));
      config.setConstraints(parseIntArray(*harmonica, "constraints",
                                          config.getConstraints(), "hpo"));
    } else {
      std::cout << "# Could not find specification  of hpo[harmonica]. Falling "
                   "Back to "
                   "default values."
                << std::endl;
    }
    if (node->contains("bayesianOptimization")) {
      ...
      std::cout << "# Could not find specification  of "
                   "hpo[bayesianOptimization]. Falling Back to "
                   "default values."
                << std::endl;
    }
  } else {
...
std::string DataMiningConfigParser::getHPOMethod(
...
bool DataMiningConfigParser::getGeometryConfig(
  ...
  bool hasGeometryConfig =
      hasFitterConfig() ? (*configFile)[fitter].contains("geometryConfig")
                        : false;
...
bool DataMiningConfigParser::getFitterParallelConfig(
  ...
  if (hasParallelConfig) {
    auto parallelConfig =
        static_cast<DictNode *>(&(*configFile)[fitter]["parallelConfig"]);

    config.scalapackEnabled_ = true;

    config.processRows_ =
        static_cast<int>(parseInt(*parallelConfig, "processRows",
                                  defaults.processRows_, "parallelConfig"));
    config.processCols_ =
        static_cast<int>(parseInt(*parallelConfig, "processColumns",
                                  defaults.processCols_, "parallelConfig"));

    config.rowBlockSize_ = parseUInt(*parallelConfig, "rowBlockSize",
                                     defaults.rowBlockSize_, "parallelConfig");
    config.columnBlockSize_ =
        parseUInt(*parallelConfig, "columnBlockSize", defaults.columnBlockSize_,
                  "parallelConfig");
  }

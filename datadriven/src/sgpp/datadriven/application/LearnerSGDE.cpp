...
std::shared_ptr<base::DataVector> LearnerSGDE::getSharedSurpluses() {
  return alpha;
}
...
double LearnerSGDE::optimizeLambdaCV() {
  ...
  for (size_t i = 0; i < crossvalidationConfig.lambdaSteps_; i++) {
    ...
    if (i % static_cast<size_t>(std::max(
                static_cast<double>(crossvalidationConfig.lambdaSteps_) / 10.0,
...
base::OperationMatrix* LearnerSGDE::computeRegularizationMatrix(
    base::Grid& grid) {

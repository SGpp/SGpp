...
float_t LearnerSGDE::pdf(base::DataVector& x) {
  ...
  for (size_t i = 0; i < learnerSGDEConfig.lambdaSteps_; i++) {
    ...
    curLambda = lambdaStart +
                static_cast<float_t>(i) * (lambdaEnd - lambdaStart) /
                    static_cast<float_t>(learnerSGDEConfig.lambdaSteps_ - 1);

    if (learnerSGDEConfig.logScale_) curLambda = exp(curLambda);

    if (i % static_cast<size_t>(
                std::max(static_cast<float_t>(learnerSGDEConfig.lambdaSteps_) / 10.0f,
                         static_cast<float_t>(1.0f))) ==
        0) {
...
float_t LearnerSGDE::computeResidual(base::Grid& grid, base::DataVector& alpha,

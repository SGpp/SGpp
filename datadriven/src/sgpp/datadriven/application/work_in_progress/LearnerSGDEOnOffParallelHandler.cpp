    ...
    namespace datadriven {
        ...
        void LearnerSGDEOnOffParallelHandler::doRefinementForClass(const std::string &refType,
            ...
            for (unsigned long i = 0; i < numberOfNewPoints; i++) {
                ...
                for (unsigned long currentDimension = 0; currentDimension < numDimensions; currentDimension += 1) {
                    uint32_t pointLevel;
                    uint32_t pointIndex;
...
//                              ", index " << pointIndex << std::endl;
        ...
        void LearnerSGDEOnOffParallelHandler::updateClassVariablesAfterRefinement(unsigned long classIndex,
            ...
            if (!MPIMethods::isMaster()) {
                ...
                for (std::vector<LevelIndexPair> &levelIndexVector : refinementResult->addedGridPoints) {
                    unsigned long sizeBeforePoint = grid.getSize();
                    ...
                    for (unsigned long currentDimension = 0; currentDimension < numDimensions; currentDimension++) {
                ...
                if (learnerInstance->getOffline()->isRefineable()) {
                    ...
                    learnerInstance->getScheduler().assignTaskStaticTaskSize(RECOMPUTE_CHOLESKY_DECOMPOSITION,
                                                                             assignTaskResult);
            ...
            // update alpha vector
            unsigned long oldSize = densEst->getAlpha().size();
        ...
        bool LearnerSGDEOnOffParallelHandler::checkRefinementNecessary(const std::string &refMonitor,
            ...
            if (refMonitor == "periodic") {
                ...
                if (learnerInstance->getValidationData() == nullptr) {
                    throw sgpp::base::data_exception("No validation data for checking convergence provided!");
                }
        ...
        LearnerSGDEOnOffParallelHandler::handleDataAndZeroBasedRefinement(bool preCompute,
                                                                          ...
                                                                          unsigned long idx, base::Grid &grid,
        ...
        unsigned long
        LearnerSGDEOnOffParallelHandler::handleSurplusBasedRefinement(DBMatOnlineDE *densEst, base::Grid &grid,
                                                                      base::GridGenerator &gridGen) const {
        ...
        LearnerSGDEOnOffParallelHandler::LearnerSGDEOnOffParallelHandler(LearnerSGDEOnOffParallel *learnerInstance,
                                                                         size_t numClasses) {

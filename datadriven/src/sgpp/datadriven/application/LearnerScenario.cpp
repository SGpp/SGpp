...
LearnerScenario::LearnerScenario() :
        isInitialized(false), lambda(0.0), datasetFileName("") {
}
...
SGPP::solver::SLESolverConfiguration SLESolverConfigFinal, SGPP::base::AdpativityConfiguration adaptConfig) :
        isInitialized(true), lambda(lambda), datasetFileName(datasetFileName), gridConfig(gridConfig), solverConfigRefine(
                SLESolverConfigRefine), solverConfigFinal(SLESolverConfigFinal), adaptConfig(adaptConfig) {
...
double LearnerScenario::getLambda() {
    return this->lambda;
}
...
template<class T> T LearnerScenario::fromString(const std::string& s) {
    ...
    if (file.is_open()) {
        ...
        while (std::getline(file, line)) {
            ...
            if (name.compare("lambda") == 0) {
                ...
                if (value.compare("0") != 0) { //TODO: compare startswith
                    throw;
                }
                gridConfig.dim_ = 0;
                check |= 0x00000004;

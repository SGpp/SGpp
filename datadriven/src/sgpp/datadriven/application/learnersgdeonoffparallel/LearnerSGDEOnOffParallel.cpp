...
LearnerSGDEOnOffParallel::LearnerSGDEOnOffParallel(
    ...
    sgpp::base::DataVector &classLabels, size_t numClassesInit, bool usePrior,
  ...
  for (size_t classIndex = 0; classIndex < numClasses; classIndex++) {
    ...
    offlineCloned->decomposeMatrix(regularizationConfig,
                                   densityEstimationConfig);
    auto densEst =
        std::unique_ptr<DBMatOnlineDE>{DBMatOnlineDEFactory::buildDBMatOnlineDE(
            *offlineCloned, *grid, regularizationConfig.lambda_, beta)};
    densityFunctions.emplace_back(
        std::make_pair(std::move(densEst), classIndex));
...
double LearnerSGDEOnOffParallel::getAccuracy() const {
  ...
  for (size_t i = 0; i < computedLabels.getSize(); i++) {
    ...
    if ((computedLabels.get(i) == -1.0) &&
        (testData.getTargets().get(i) == -1.0)) {
      correctLabel1++;
    } else if ((computedLabels.get(i) == 1.0) &&
               (testData.getTargets().get(i) == 1.0)) {
      correctLabel2++;
    }
  }
...
void LearnerSGDEOnOffParallel::predict(DataMatrix &data,
                                       DataVector &result) const {
  ...
  for (auto &densityFunction : densityFunctions) {
    ...
    densityFunction.first->eval(*(alphas[classIndex]), data,
                                perClassDensities.back(), *(grids[classIndex]),
                                true);
...
void LearnerSGDEOnOffParallel::updateAlpha(size_t classIndex,
                                           std::list<size_t> *deletedPoints,
                                           size_t newPoints) {
  DataVector &alpha = *(alphas[classIndex]);
  if (alpha.getSize() != 0 && deletedPoints != nullptr &&
      !deletedPoints->empty()) {
    std::vector<size_t> vecDeletedPoints{std::begin(*deletedPoints),
...
void LearnerSGDEOnOffParallel::trainParallel(
    size_t batchSize, size_t maxDataPasses, std::string refinementFunctorType,
  ...
  RefinementMonitorConvergence monitor{accDeclineThreshold,
  ...
  while (completedDataPasses < maxDataPasses) {
    ...
    while (processedPoints < trainData.getNumberInstances()) {
      ...
      std::cout << processedPoints << " have already been assigned."
                << std::endl;
      ...
      while (refinementsNecessary > 0) {
        while (!refinementHandler.checkReadyForRefinement()) {
          D(std::cout << "Waiting for " << MPIMethods::getQueueSize()
                      << " queue operations to complete before continuing"
                      << std::endl;)
          MPIMethods::waitForAnyMPIRequestsToComplete();
        }
        ...
        doRefinementForAll(refinementFunctorType, refMonitor, onlineObjects,
                           monitor);
        numberOfCompletedRefinements += 1;
        refinementsNecessary--;
        D(std::cout << "Refinement at " << processedPoints << " complete"
                    << std::endl;)
...
void LearnerSGDEOnOffParallel::printGridSizeStatistics(
  ...
  for (size_t idx = 0; idx < onlineObjects.size(); idx++) {
    ...
    Grid &grid = *(grids[idx]);
    std::cout << messageString << onlineObject.second << ", " << grid.getSize()
              << std::endl;
  }
}
...
void LearnerSGDEOnOffParallel::doRefinementForAll(
  ...
  for (size_t classIndex = 0; classIndex < this->getNumClasses();
       classIndex++) {
  ...
  MPIMethods::waitForIncomingMessageType(
      ...
      [](PendingMPIRequest &request) {
        ...
        D(std::cout << "Test packet grid version "
                    ...
                    << ", update type "
                    << refinementResultNetworkMessage->updateType << std::endl;)
...
void LearnerSGDEOnOffParallel::computeNewSystemMatrixDecomposition(
...
void LearnerSGDEOnOffParallel::assembleNextBatchData(
    Dataset *dataBatch, size_t *batchOffset) const {
  size_t batchSize = dataBatch->getNumberInstances();
  size_t dataDimensionality = dataBatch->getDimension();
  D(std::cout << "Assembling batch of size " << batchSize << " at offset "
...
void LearnerSGDEOnOffParallel::train(Dataset &dataset, bool doCrossValidation) {
  ...
  std::cout << "Starting train cycle (dataset size: "
            << dataset.getNumberInstances() << ")" << std::endl;
...
void LearnerSGDEOnOffParallel::splitBatchIntoClasses(
  ...
  for (size_t i = 0; i < dataset.getNumberInstances(); i++) {
    ...
    auto &classPairDataMatrixDouble =
...
void LearnerSGDEOnOffParallel::train(
  ...
  for (size_t classIndex = 0; classIndex < trainDataClasses.size();
       classIndex++) {
    std::pair<sgpp::base::DataMatrix *, double> p =
        trainDataClasses[classIndex];

    if ((*p.first).getNrows() > 0) {
      // update density function for current class
      RefinementResult &classRefinementResult =
          refinementHandler.getRefinementResult(classIndex);
      std::cout << "Calling compute density function class " << classIndex
                << " (refinement +"
                << classRefinementResult.addedGridPoints.size() << ", -"
                << classRefinementResult.deletedGridPointsIndices.size() << ")"
                << std::endl;
      densityFunctions[classIndex].first->computeDensityFunction(
          *(alphas[classIndex]), *p.first, *(grids[classIndex]),
          densityEstimationConfig, true, doCrossValidation);
      D(std::cout << "Clearing the refinement results class " << classIndex
                  << std::endl;)
      ...
      if (usePrior) {
        ...
        D(std::cout << "Setting prior[" << p.second << "] to " << newPrior
                    << std::endl;)
...
void LearnerSGDEOnOffParallel::workBatch(Dataset dataset, size_t batchOffset,
  ...
  D(std::cout << "Batch " << batchOffset - dataset.getNumberInstances()
              << " completed." << std::endl;)
  for (size_t classIndex = 0; classIndex < getNumClasses(); classIndex++) {
    D(std::cout << "Sending alpha values to master for class " << classIndex
                << " with grid version " << getLocalGridVersion(classIndex)
                << std::endl;)
    DataVector alphaVector = *(alphas[classIndex]);
    MPIMethods::sendMergeGridNetworkMessage(
        classIndex, batchOffset, dataset.getNumberInstances(), alphaVector);

    D(DataVector &dataVector =
          getDensityFunctions()[classIndex].first->getAlpha();
      std::cout << "Local alpha sum " << classIndex << " is now "
                << std::accumulate(dataVector.begin(), dataVector.end(), 0.0)
                << std::endl;)
  }
  D(std::cout << "Completed work batch "
              << batchOffset - dataset.getNumberInstances()
...
void LearnerSGDEOnOffParallel::waitForAllGridsConsistent() {
  ...
  while (classIndex < localGridVersions.size()) {
    ...
    if (!checkGridStateConsistent(classIndex) ||
      ...
      std::cout << "Attempted to train from an inconsistent grid " << classIndex
                << " version " << getLocalGridVersion(classIndex) << std::endl;
...
size_t LearnerSGDEOnOffParallel::assignBatchToWorker(size_t batchOffset,
                                                     bool doCrossValidation) {
  AssignTaskResult assignTaskResult{};
  mpiTaskScheduler.assignTaskVariableTaskSize(TRAIN_FROM_BATCH,
                                              assignTaskResult);

  if (assignTaskResult.taskSize + batchOffset >
      trainData.getNumberInstances()) {
    std::cout << "Shortening last batch." << std::endl;
    assignTaskResult.taskSize = trainData.getNumberInstances() - batchOffset;
  }

  std::cout << "Assigning batch " << batchOffset << " to worker "
            << assignTaskResult.workerID << " with size "
            << assignTaskResult.taskSize << std::endl;
  MPIMethods::assignBatch(assignTaskResult.workerID, batchOffset,
                          assignTaskResult.taskSize, doCrossValidation);
  return assignTaskResult.taskSize;
}

void LearnerSGDEOnOffParallel::mergeAlphaValues(
    size_t classIndex, size_t remoteGridVersion, DataVector dataVector,
  ...
  if (!isVersionConsistent(remoteGridVersion)) {
    std::cout << "Received merge request with inconsistent grid " << classIndex
              << " version " << remoteGridVersion << std::endl;
    throw algorithm_exception(
        "Received a merge request to an inconsistent grid");
  }

  size_t localGridVersion = getLocalGridVersion(classIndex);
  if (isLastPacketInSeries) {
    mpiTaskScheduler.onMergeRequestIncoming(
        batchOffset, batchSize, remoteGridVersion, localGridVersion);
  }
  ...
  if (remoteGridVersion != localGridVersion) {
    D(std::cout << "Received merge grid request with incorrect grid version!"
                ...
                << ", remote: " << remoteGridVersion << std::endl;)
    if (remoteGridVersion + 1 == localGridVersion) {
      RefinementResult &refinementResult =
          refinementHandler.getRefinementResult(classIndex);
      std::list<size_t> &deletedPoints =
          refinementResult.deletedGridPointsIndices;
      std::list<LevelIndexVector> &addedPoints =
          refinementResult.addedGridPoints;

      D(std::cout << "Attempting to automatically compensate for outdated grid."
                  << std::endl
                  << "Refinement result has " << addedPoints.size()
                  << " additions and " << deletedPoints.size() << " deletions"
                  << std::endl
                  << "The original remote size is " << dataVector.size()
                  << std::endl;)
      if (!deletedPoints.empty() || !addedPoints.empty()) {
        ...
        if (!deletedPoints.empty()) {
          D(std::cout << "Copying vector (deleting deleted grid points)."
                      << std::endl;)
          DataVector newAlpha(dataVector.getSize() - deletedPoints.size() +
                              addedPoints.size());
        ...
        D(std::cout << "New alpha vector is now " << dataVector.size()
                    << " elements long." << std::endl;)
      } else {
        std::cout
            << "Refinement data has already been deleted." << std::endl
            << "This is probably because the master is training from batches. "
            << std::endl
            << "Cannot compensate, will now fail." << std::endl;
        throw sgpp::base::algorithm_exception(
            "Missing refinement data for alpha update.");
      }
    } else {
      std::cout << "Merge request " << batchOffset << ", size " << batchSize
                ...
                << std::endl;
      throw sgpp::base::algorithm_exception(
          "Merge request older than one refinement cycle.");
    }
  }

  DataVector &localAlpha = *(alphas[classIndex]);
  if (localAlpha.size() != dataVector.size()) {
    std::cout << "Received merge request with incorrect size (local "
              << localAlpha.size() << ", remote " << dataVector.size()
              << "), local version is " << localGridVersions[classIndex]
              << std::endl;
    throw sgpp::base::algorithm_exception(
        "Merge request with incorrect size received.");
  }

  if (usePrior) {
    throw algorithm_exception("Use prior not implemented");
  } else {
    D(std::cout << "Setting prior [" << classLabels[classIndex] << "] to -1"
                << std::endl;)
...
bool LearnerSGDEOnOffParallel::checkAllGridsConsistent() {
  return std::all_of(
      localGridVersions.begin(), localGridVersions.end(),
...
bool LearnerSGDEOnOffParallel::checkGridStateConsistent(size_t classIndex) {
  if (localGridVersions.size() < classIndex) {
    throw algorithm_exception("Received request for consistency of class " +
                              classIndex);
  }
  return isVersionConsistent(localGridVersions[classIndex]);
}

void LearnerSGDEOnOffParallel::setLocalGridVersion(size_t classIndex,
                                                   size_t gridVersion) {
  D(if (localGridVersions[classIndex] != gridVersion) {
    std::cout << "Grid " << classIndex << " now has version " << gridVersion
              << " (previously " << localGridVersions[classIndex] << ")"
              << std::endl;
  })
  if (!checkGridStateConsistent(classIndex) &&
      isVersionConsistent(gridVersion)) {
    std::cout << "Grid " << classIndex << " has been fully updated to version "

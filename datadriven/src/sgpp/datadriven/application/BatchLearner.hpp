 ...
 *   with different functions. Labels are predicted by using density functions. Adaptivity can
 *   be enabled.
 */
class BatchLearner {
 protected:
  ...
  std::map<int, base::DataMatrix*>
  dataInBatch;  //!< mapping of data in each batch to label
  ...
  alphaStorage;  //!< mapping used to store the previous alphas
  ...
  solver::SLESolver* myCG;  //!< solver

  /**
   * function to apply one of the weighting methods to the alpha provided, returns the weighted
   *   alpha
  ...
  /**
   * function that processes the data provided as string, learns the new data and refines the grids
   *   (if wanted by user)
   * @param workData string containing the items separated by '\n', data is separated by ',',
   *   the last entry per item is the class as int
   */
   ...
   */
  void stringToDataVector(std::string input, base::DataVector& dataFound,
                          int& classFound);

  /**
   * function that parses a string containing many items
   * if (mapData) and maps the data according to class into dataInBatch (will be cleared before
   *   usage, dataFound and classesFound will not be touched)
   * if (!mapData) to a DataMatrix containg the data and a DataVector containing the classes
   *   (dataInBatch will not be touched)
   * @param input many items seperated by '\n'
   * @param dataFound DataMatrix that will contain the found data afterwards (will be cleared
   *   before usage)
   * @param classesFound DataVector that will contain the found classes afterwards (will be
   *   cleared before usage)
   * @param mapData controls whether data is mapped to dataInBatch or saved to dataFound and
   *   classesFound
  */
  void stringToDataMatrix(std::string& input, base::DataMatrix& dataFound,
 ...
 public:
   ...
   * @param updateNorm should the normalization factors be updated before predicting?
         */
  base::DataVector predict(base::DataMatrix& entries,
                                 bool updateNorm);

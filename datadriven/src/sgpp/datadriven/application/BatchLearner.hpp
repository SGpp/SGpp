...
class BatchLearner {
 protected:
  ...
  SGPP::solver::SLESolverConfiguration
  solverConf;//!< configuration for the solver
  SGPP::base::AdpativityConfiguration
  adaptConf;//!< configuration for the adaptivity
  ...
  std::map<int, SGPP::base::DataMatrix*>
  dataInBatch;//!< mapping of data in each batch to label
  std::map<int, SGPP::base::DataVector*>
  ...
  std::map<int, std::deque<SGPP::base::DataVector> >
  ...
  std::string
  batch; //!< because testdata is added to the following batch: save batch global
   ...
   */
  void stringToDataVector(std::string input, SGPP::base::DataVector& dataFound,
                          int& classFound);
  ...
  */
  void stringToDataMatrix(std::string& input, SGPP::base::DataMatrix& dataFound,
                          SGPP::base::DataVector& classesFound, bool mapData);

  bool isFinished =
    false; //!< indicates whether the stream has been read to the end
  float_t acc_global =
    -1.0; //!< accuracy over all predictions done so far (including the ones if batchConfig.testsize > 0 )
 ...
 public:
   ...
   * @param updateNorm should the normalization factors be updated before predicting?
         */
  SGPP::base::DataVector predict(SGPP::base::DataMatrix& entries,
                                 bool updateNorm);

...
namespace SGPP {
  namespace datadriven {
    ...
    void MetaLearner::learnString(SGPP::datadriven::OperationMultipleEvalConfiguration& operationConfiguration,
      ...
      auto myLearner = std::make_shared<LearnerLeastSquaresIdentity>(isRegression, this->verbose);
    ...
    void MetaLearner::learnReference(std::string& datasetFileName, float_t lambda, bool isRegression) {
      std::ifstream t(datasetFileName);
    ...
    void MetaLearner::learnReferenceString(std::string& content, float_t lambda, bool isRegression) {
      ...
      auto referenceLearner = std::make_shared<LearnerLeastSquaresIdentity>(isRegression, this->verbose);
    ...
    void MetaLearner::learnAndTestString(SGPP::datadriven::OperationMultipleEvalConfiguration& operationConfiguration,
                                         std::string& dataContent, std::string& testContent, float_t lambda, bool isRegression) {
    ...
    float_t MetaLearner::learnAndCompare(SGPP::datadriven::OperationMultipleEvalConfiguration& operationConfiguration,
      ...
      buffer << t.rdbuf();
      std::string bufferString = buffer.str();
      return this->learnAndCompareString(operationConfiguration, bufferString, lambda, gridGranularity);
    }
    ...
    void MetaLearner::optimizeLambdaLog(SGPP::base::DataMatrix& dataset, SGPP::base::DataVector& values, size_t kFold,
    ...
    float_t MetaLearner::optimizeLambdaLog(SGPP::base::DataMatrix& dataset, SGPP::base::DataVector& values, size_t kFold,
                                           size_t maxLevel) {
      ...
      datadriven::DatasetTools::splitset(dataset, values, kFold, trainingSets, trainingSetsValues, testSets, testSetsValues);

      //initial values are pure dummy values
      float_t bestLambda = 0.0;
      float_t bestMSE = 0.0;

      this->optimizeLambdaLog_(dataset, values, kFold, maxLevel, trainingSets, trainingSetsValues, testSets,
    ...
    void MetaLearner::optimizeLambdaLog_(SGPP::base::DataMatrix& dataset, SGPP::base::DataVector& datasetValues,
      ...
      if (curLevel < maxLevel) {
        this->optimizeLambdaLog_(dataset, datasetValues, kFold, maxLevel, trainingSets, trainingSetsValues, testSets,
                                 testSetsValues, curLevel + 1, lambdaLogStepSize / 2.0, bestLogLambda, bestMSE);
      }
    }

    void MetaLearner::train(base::DataMatrix& train, base::DataVector& trainValues, float_t lambda,
      ...
      SGPP::datadriven::OperationMultipleEvalConfiguration config(OperationMultipleEvalType::DEFAULT,
          OperationMultipleEvalSubType::DEFAULT);
      myLearner->setImplementation(config);

      timing = myLearner->train(train, trainValues, this->gridConfig, this->solverConfig, this->solverFinalStep,
    ...
    float_t MetaLearner::calculateMSE(base::Grid& grid, base::DataVector& alpha, base::DataMatrix& testSubset,

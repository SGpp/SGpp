...
MetaLearner::MetaLearner(
    ...
    SGPP::base::AdpativityConfiguration adaptivityConfiguration, bool verbose) {
  this->csvSep = "& ";
...
void MetaLearner::learn(SGPP::datadriven::OperationMultipleEvalConfiguration&
                        ...
                        std::string& datasetFileName, float_t lambda,
                        bool isRegression) {
...
void MetaLearner::learnString(
  ...
  LearnerTiming timings = myLearner->train(
      trainingData, classesVector, this->gridConfig, this->solverConfig,
...
void MetaLearner::optimizeLambdaLog(
    SGPP::base::DataMatrix& dataset, SGPP::base::DataVector& values,
    size_t kFold, size_t maxLevel, std::shared_ptr<base::Grid>& bestGrid,
...
void MetaLearner::optimizeLambdaLog_(
    SGPP::base::DataMatrix& dataset, SGPP::base::DataVector& datasetValues,
    ...
    float_t lambdaLogStepSize, float_t& bestLogLambda, float_t& bestMSE) {
  if (verbose) {
    std::cout << "entering level=" << curLevel
              << " with lambda=" << pow(10.0, -bestLogLambda) << std::endl;
  }
  ...
  for (float_t curLogLambda : logLambdaValues) {
    ...
    for (size_t j = 0; j < kFold; j++) {
      ...
      // compute density
      train(trainingSets[j], trainingSetsValues[j], curLambda, grid, alpha,
            timing);

      float_t mse =
          this->calculateMSE(*grid, *alpha, testSets[j], testSetsValues[j]);
      curMeanMSE += mse;
    }
    ...
    if (verbose) {
      std::cout << "# lambda: " << curLambda << " curMeanMSE: " << curMeanMSE
                << " bestLambda: " << pow(10.0, -bestLogLambda)
                << " bestMSE: " << bestMSE
                << " lambdaLogStepSize: " << lambdaLogStepSize << std::endl;
    }
  }

  if (curLevel < maxLevel) {
    this->optimizeLambdaLog_(dataset, datasetValues, kFold, maxLevel,
                             trainingSets, trainingSetsValues, testSets,
                             testSetsValues, curLevel + 1,
                             lambdaLogStepSize / 2.0, bestLogLambda, bestMSE);
  }
}

void MetaLearner::train(base::DataMatrix& train, base::DataVector& trainValues,
                        float_t lambda, std::shared_ptr<base::Grid>& grid,
                        std::shared_ptr<base::DataVector>& alpha,
                        datadriven::LearnerTiming& timing) {
  if (verbose) {
...
float_t MetaLearner::calculateMSE(base::Grid& grid, base::DataVector& alpha,
  ...
  for (size_t i = 0; i < testSubset.getNrows(); i++) {
    ...
    mse += (approximation - valuesTestSubset[i]) *
           (approximation - valuesTestSubset[i]);

    if (verbose && i < 100) {
      std::cout << "mine: " << approximation
                << " reference: " << valuesTestSubset[i] << std::endl;
    }
  }

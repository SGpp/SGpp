 ...
 * Cholesky decomposition (offline step). Then, for each class a density function
...
class LearnerSGDEOnOff : public DBMatOnline {
 public:
   ...
   * @param usePrior Determines if prior probabilities should be used to compute class labels
  ...
  LearnerSGDEOnOff(sgpp::datadriven::DBMatDensityConfiguration& dconf,
                   ...
                   base::DataVector& classLabels, unsigned int classNumber,
   ...
   *        is chosen)
   * @param enableCv Specifies whether to perform cross-validation during training process or not
   ...
   */
  void train(base::DataMatrix& trainData, base::DataVector& classes, bool doCv = false, 
             std::vector<std::pair<std::list<size_t>, unsigned int>>* refineCoarse = nullptr);
	
  /**
   * Trains the learner with the given data batch that is already split up wrt its different 
   * classes.
   *
   * @param trainDataClasses A vector of pairs; Each pair contains the data points that belong to 

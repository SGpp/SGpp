...
PiecewiseConstantSmoothedRegressionMetaLearner::PiecewiseConstantSmoothedRegressionMetaLearner(bool verbose, base::DataMatrix &trainingDataSet,
...
void PiecewiseConstantSmoothedRegressionMetaLearner::optimizeLambdaLog(size_t kFold, size_t maxLevel, float_t fastApproximationMSE,
    ...
    lambdaOpt = this->optimizeLambdaLog(kFold, maxLevel, fastApproximationMSE, fastApproximationMaxLevel);
    this->train(dataset, datasetValues, lambdaOpt, fastApproximationMSE, fastApproximationMaxLevel, bestGrid, bestAlpha);
}

float_t PiecewiseConstantSmoothedRegressionMetaLearner::optimizeLambdaLog(size_t kFold, size_t maxLevel, float_t fastApproximationMSE,
    ...
    DatasetTools::splitset(this->dataset, this->datasetValues, kFold, trainingSets, trainingSetsValues, testSets,
            testSetsValues);
    ...
    this->optimizeLambdaLog_(kFold, maxLevel, fastApproximationMSE, fastApproximationMaxLevel, trainingSets,
            trainingSetsValues, testSets, testSetsValues, 0, 1.0, bestLambda, bestMSE);
...
void PiecewiseConstantSmoothedRegressionMetaLearner::optimizeLambdaLog_(size_t kFold, size_t maxLevel, float_t fastApproximationMSE,
    ...
    if (verbose) {
        std::cout << "entering level=" << curLevel << " with lambda=" << pow(10.0, -bestLogLambda) << std::endl;
    }
...
// cross-validation
        ...
        for (size_t j = 0; j < kFold; j++) {
            ...
            //compute density
            train(trainingSets[j], trainingSetsValues[j], curLambda, fastApproximationMSE, fastApproximationMaxLevel,
                    grid, alpha);
        ...
        if (verbose) {
            std::cout << "# lambda: " << curLambda << " curMeanMSE: " << curMeanMSE << " bestLambda: "
                    << pow(10.0, -bestLogLambda) << " bestMSE: " << bestMSE << " lambdaLogStepSize: "
    ...
    if (curLevel < maxLevel) {
        this->optimizeLambdaLog_(kFold, maxLevel, fastApproximationMSE, fastApproximationMaxLevel, trainingSets,
                trainingSetsValues, testSets, testSetsValues, curLevel + 1, lambdaLogStepSize / 2.0, bestLogLambda,
                bestMSE);
    }
}

void PiecewiseConstantSmoothedRegressionMetaLearner::train(base::DataMatrix &train, base::DataVector &trainValues, float_t lambda,
        float_t fastApproximationMSE, size_t fastApproximationMaxLevel, std::shared_ptr<base::Grid> &grid,
        std::shared_ptr<base::DataVector> &alpha) {

    SGPP::datadriven::OperationPiecewiseConstantRegression piecewiseRegressorOperator(train, trainValues);
...
base::Grid *PiecewiseConstantSmoothedRegressionMetaLearner::createRegularGrid(size_t dim) {
...
float_t PiecewiseConstantSmoothedRegressionMetaLearner::calculateMSE(base::Grid &grid, base::DataVector &alpha,
        base::DataMatrix &testSubset, base::DataVector &valuesTestSubset, bool verbose) {
    float_t mse = 0.0;
    ...
    for (size_t i = 0; i < testSubset.getNrows(); i++) {
        ...
        if (verbose && i < 100) {
            std::cout << "mine: " << approximation << " reference: " << valuesTestSubset[i] << std::endl;
        }
    }
    return mse;
}

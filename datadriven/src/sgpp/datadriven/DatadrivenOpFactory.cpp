...
createOperationDensityMarginalizeKDE(datadriven::GaussianKDE& kde) {
  return new datadriven::OperationDensityMarginalizeKDE(kde);
}

datadriven::OperationDensityConditionalKDE*
createOperationDensityConditionalKDE(datadriven::GaussianKDE& kde) {
    ...
    if (configuration.getType() == datadriven::OperationMultipleEvalType::DEFAULT
...
#else
        throw base::factory_exception("Error creating function: the library wasn't compiled for double precision");
#endif
#else
      ...
      if (configuration.getSubType() ==
        ...
        throw base::factory_exception("Error creating function: the library wasn't compiled with AVX");
               ...
               datadriven::OperationMultipleEvalType::ADAPTIVE) {
      if (configuration.getSubType() ==
          SGPP::datadriven::OperationMultipleEvalSubType::DEFAULT) {
#ifdef USE_OCL
...
#ifdef USE_OCL
        return datadriven::createStreamingModOCLFastMultiPlatformConfigured(grid,
...
#ifdef USE_OCL
        return datadriven::createStreamingModOCLMaskConfigured(grid, dataset,
               configuration);
#else
        throw base::factory_exception(
          "Error creating function: the library wasn't compiled with OpenCL support");

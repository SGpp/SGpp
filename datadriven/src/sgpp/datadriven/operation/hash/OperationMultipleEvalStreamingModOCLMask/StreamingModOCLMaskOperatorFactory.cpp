...
base::OperationMultipleEval* createStreamingModOCLMaskConfigured(
  ...
  if (configuration.getParameters().operator bool()) {
    ...
    parameters =
      std::make_shared<base::OCLOperationConfiguration>("StreamingModOCLMask.cfg");
  ...
  if ((*parameters)["INTERNAL_PRECISION"].get() == "float") {
    return new datadriven::OperationMultiEvalStreamingModOCLMask<float>(grid,
           dataset, parameters);
  } else if ((*parameters)["INTERNAL_PRECISION"].get() == "double") {
    return new datadriven::OperationMultiEvalStreamingModOCLMask<double>(grid,
           dataset, parameters);
  } else {

...
template<typename T>
class OperationMultiEvalStreamingModOCLMask: public
 ...
 public:
  ...
  OperationMultiEvalStreamingModOCLMask(base::Grid& grid,
  ...
  void mult(SGPP::base::DataVector& alpha,
    ...
    this->kernel->mult(this->level, this->index, this->mask, this->offset,
                       this->gridSize, this->kernelDataset, this->datasetSize, alphaArray,
  ...
  void multTranspose(
    ...
    std::chrono::time_point<std::chrono::system_clock> start, end;
    start = std::chrono::system_clock::now();
    this->kernel->multTranspose(this->level, this->index, this->mask, this->offset,
    ...
    for (size_t i = 0; i < this->storage->size(); i++) {
      for (size_t dim = 0; dim < this->dims; dim++) {
        ...
        if (curLevel == 1) {
          ...
          this->level[i * this->dims + dim] = static_cast<T>(-1.0) * static_cast<T>
                                              (1 << curLevel);
          this->index[i * this->dims + dim] = 0.0;

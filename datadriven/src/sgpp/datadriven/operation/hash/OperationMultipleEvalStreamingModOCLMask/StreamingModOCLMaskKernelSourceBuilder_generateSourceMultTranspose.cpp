...
std::string StreamingModOCLMaskKernelSourceBuilder::generateSourceMultTrans() {
  ...
  sourceStream << "__attribute__((reqd_work_group_size(" << localWorkgroupSize <<
               ", 1, 1)))" << std::endl;
  sourceStream << "void multTransOCLMask(__global const " << this->asString() <<
               "* ptrLevel," << std::endl;
  ...
  if (useLocalMemory) {
    sourceStream << "   __local " << this->asString() << " locData[" << dims*
                 localWorkgroupSize << "];"
                 << std::endl;
    sourceStream << "   __local " << this->asString() << " locSource[" <<
                 localWorkgroupSize << "];" << std::endl
                 << std::endl;
  }

  for (size_t d = 0; d < dims; d++) {
    sourceStream << " " << this->asString() << " level_" << d <<
                 " = ptrLevel[(globalIdx*" << dims << ")+" << d
                 << "];" << std::endl;
    sourceStream << " " << this->asString() << " index_" << d <<
                 " = ptrIndex[(globalIdx*" << dims << ")+" << d
                 << "];" << std::endl;
    sourceStream << " " << this->asString() << " mask_" << d <<
                 " = ptrMask[(globalIdx*" << dims << ")+" << d
                 << "];" << std::endl;
    sourceStream << " " << this->asString() << " offset_" << d <<
                 " = ptrOffset[(globalIdx*" << dims << ")+" << d
                 << "];" << std::endl;
  }
  ...
  if (useLocalMemory) {
    sourceStream << " for(int i = start_data; i < end_data; i+=" <<
                 localWorkgroupSize << ")" << std::endl;
    sourceStream << "   {" << std::endl;
    ...
    sourceStream << "       locSource[localIdx] = ptrSource[i+localIdx];" <<
                 std::endl;
  ...
  for (size_t d = 0; d < dims; d++) {
    ...
    sourceStream << "         index_calc = eval - (index_" << d << ");" <<
                 std::endl;
    sourceStream << "         abs = as_" << this->asString() << "(as_" <<
                 this->intAsString()
                 << "(index_calc) | as_" << this->intAsString() << "(mask_" << d << "));" <<
                 std::endl;

...
class StreamingModOCLMaskKernelImpl {
 ...
 public:
  ...
  StreamingModOCLMaskKernelImpl(size_t dims,
                                std::shared_ptr<base::OCLManager> manager,
                                std::shared_ptr<base::OCLOperationConfiguration> parameters) :
    deviceData(manager), deviceLevel(manager), deviceIndex(manager),
    deviceMask(manager), deviceOffset(manager), deviceGrid(
      manager), deviceTemp(manager), kernelSourceBuilder(parameters, dims),
    manager(manager), parameters(
      parameters), multLoadBalancer(manager, this->parameters),
    multTransposeLoadBalancer(manager,
                              this->parameters) {
  ...
  double mult(std::vector<real_type>& level, std::vector<real_type>& index,
              ...
              std::vector<real_type>& offset, size_t gridSize,
              std::vector<real_type>& dataset, size_t datasetSize,
              std::vector<real_type>& alpha, std::vector<real_type>& result,
    ...
    if (kernel_mult[0] == nullptr) {
      this->createMult(this->dims, (*parameters)["LOCAL_SIZE"].getUInt(),
                       manager->context, manager->num_devices,
    ...
    for (size_t i = 0; i < manager->num_devices; i++) {
      ...
      cl_uint gpu_end_data = static_cast<cl_uint>(gpu_end_index_data[i]) /
                             static_cast<cl_uint>(dataBlockingSize);
      //      std::cout << "device: " << i << " start data: " << gpu_start_data << " end data: " << gpu_end_data << std::endl;

      if (gpu_end_data > gpu_start_data) {
        err = clSetKernelArg(kernel_mult[i], 0, sizeof(cl_mem),
        ...
        err = clSetKernelArg(kernel_mult[i], 1, sizeof(cl_mem),
        ...
        err = clSetKernelArg(kernel_mult[i], 2, sizeof(cl_mem),
        ...
        err = clSetKernelArg(kernel_mult[i], 3, sizeof(cl_mem),
        ...
        err = clSetKernelArg(kernel_mult[i], 4, sizeof(cl_mem),
        ...
        err = clSetKernelArg(kernel_mult[i], 5, sizeof(cl_mem),
        ...
        err = clSetKernelArg(kernel_mult[i], 6, sizeof(cl_mem),
        ...
        err = clSetKernelArg(kernel_mult[i], 7, sizeof(cl_uint),
  ...
  double multTranspose(std::vector<real_type>& level,
                       std::vector<real_type>& index, std::vector<real_type>& mask,
                       std::vector<real_type>& offset, size_t gridSize,
                       std::vector<real_type>& dataset, size_t datasetSize,
 ...
 private:
  ...
  void createMultTrans(size_t dims, size_t local_workgroup_size,
    ...
    manager->buildKernel(program_src, "multTransOCLMask", context, num_devices,
                         device_ids, kernel);
  }
  ...
  void createMult(size_t dims, size_t local_workgroup_size, cl_context context,
    ...
    manager->buildKernel(program_src, "multOCLMask", context, num_devices,
                         device_ids, kernel);
  }
  ...
  void initOCLBuffers(std::vector<real_type>& level,
    ...
    if (!this->deviceMask.isInitialized()) {
      this->deviceMask.initializeBuffer(mask.data(), sizeof(real_type),
                                        gridSize * this->dims);
    }

    if (!this->deviceOffset.isInitialized()) {
      this->deviceOffset.initializeBuffer(offset.data(), sizeof(real_type),
                                          gridSize * this->dims);
    }

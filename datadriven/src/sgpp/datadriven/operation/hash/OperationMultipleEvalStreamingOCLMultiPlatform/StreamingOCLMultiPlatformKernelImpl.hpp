...
class StreamingOCLMultiPlatformKernelImpl {
 ...
 public:
  ...
  StreamingOCLMultiPlatformKernelImpl(size_t dims,
                                      std::shared_ptr<base::OCLManagerMultiPlatform> manager,
                                      std::shared_ptr<base::OCLOperationConfiguration> parameters,
                                      json::Node& firstDeviceConfig) :
  ...
  double mult(std::vector<T>& level, std::vector<T>& index, size_t gridSize,
    ...
    T* hostTemp = (T*) deviceTemp.getMappedHostBuffer(
                    manager->platforms[0].platformId);
  ...
  double multTranspose(std::vector<T>& level, std::vector<T>& index,
                       size_t gridSize, std::vector<T>& dataset,
                       size_t datasetSize, std::vector<T>& source, std::vector<T>& result,
    ...
    size_t transGridBlockingSize =
      firstDeviceConfig["KERNEL_TRANS_GRID_BLOCKING_SIZE"].getUInt();
    ...
    multTransposeLoadBalancer.getPartitionSegments(start_index_grid, end_index_grid,
        firstDeviceConfig["LOCAL_SIZE"].getUInt() * transGridBlockingSize,
        gpu_start_index_grid,
        gpu_end_index_grid);
    ...
    for (base::OCLPlatformWrapper& platform : manager->platforms) {
      ...
      for (size_t i = 0; i < platform.getDeviceCount(); i++) {
        ...
        size_t rangeSize = (gpuEndGrid / transGridBlockingSize) -
                           (gpuStartGrid / transGridBlockingSize);

        if (rangeSize > 0) {

          size_t gridOffset = gpu_start_index_grid[platform.platformId][i] /
    ...
    T* hostGrid = (T*) deviceGrid.getMappedHostBuffer(
                    manager->platforms[0].platformId);
 ...
 private:
  ...
  void initOCLBuffers(std::vector<T> level, std::vector<T> index, size_t gridSize,
                      std::vector<T> dataset,
                      size_t datasetSize) {
    if (!this->deviceLevel.isInitialized()) {
      this->deviceLevel.initializeBuffer(level.data(), sizeof(T),
                                         gridSize * this->dims);
    }

    if (!this->deviceIndex.isInitialized()) {
      this->deviceIndex.initializeBuffer(index.data(), sizeof(T),
                                         gridSize * this->dims);
    }

    if (!this->deviceData.isInitialized()) {
      this->deviceData.initializeBuffer(dataset.data(), sizeof(T),
                                        datasetSize * this->dims);
    }
  }

  void initParams(std::vector<T> grid, size_t gridSize, std::vector<T> tmp,
                  size_t datasetSize) {
    ...
    T* hostTemp = (T*) this->deviceTemp.getMappedHostBuffer(
                    manager->platforms[0].platformId);

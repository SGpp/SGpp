...
public:
    ...
    KernelMult(std::shared_ptr<base::OCLDevice> device, size_t dims,
            ...
            device(device), dims(dims), err(CL_SUCCESS), deviceData(device), deviceLevel(
                    ...
                    kernelConfiguration, dims), manager(manager), deviceTimingMult(
                    0.0), kernelConfiguration(kernelConfiguration), queueLoadBalancerMult(
                    queueBalancerMult), buildDuration(0.0) {
...
#pragma omp barrier
        ...
        while (true) {
            ...
            bool segmentAvailable = queueLoadBalancerMult->getNextSegment(
                    scheduleSize, totalBlockSize, kernelStartData,
                    kernelEndData);
            if (!segmentAvailable) {
                break;
            }
            ...
            size_t rangeSizeAfterBlocking = (kernelEndData / dataBlockSize)
                    - (kernelStartData / dataBlockSize);
...
#pragma omp critical(StreamingOCLMultiPlatformKernelMult)
                {
                    std::cout << "device: " << device->deviceName << " ("
                            ...
                            << kernelStartData << " to: " << kernelEndData
            ...
            if (rangeSize > 0) {
                ...
                err = clSetKernelArg(this->kernelMult, 7, sizeof(cl_int),
                        &kernelEndGrid);
                if (err != CL_SUCCESS) {
                    std::stringstream errorString;
                    errorString
                ...
                err = clEnqueueNDRangeKernel(device->commandQueue,
                        this->kernelMult, 1, 0, &rangeSizeAfterBlocking,
...
private:
     ...
     * @param kernelEndData End of the range to be processed by the device
     */
    void initDatasetResultBuffers(size_t kernelStartData,
            size_t kernelEndData) {

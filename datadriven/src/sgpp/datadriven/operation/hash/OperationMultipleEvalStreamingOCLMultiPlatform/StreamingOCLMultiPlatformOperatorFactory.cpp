...
base::OperationMultipleEval* createStreamingOCLMultiPlatformConfigured(
  ...
  json::Node& firstDeviceConfig =
    deviceNode["KERNELS"][StreamingOCLMultiPlatformConfiguration::getKernelName()];

  if (firstDeviceConfig["INTERNAL_PRECISION"].get().compare("float") == 0) {
    return new datadriven::OperationMultiEvalStreamingOCLMultiPlatform<float>(grid,
           dataset, manager, parameters, firstDeviceConfig);
  } else if (firstDeviceConfig["INTERNAL_PRECISION"].get().compare("double") ==
             0) {
    return new datadriven::OperationMultiEvalStreamingOCLMultiPlatform<double>(grid,
           dataset, manager, parameters, firstDeviceConfig);
  } else {

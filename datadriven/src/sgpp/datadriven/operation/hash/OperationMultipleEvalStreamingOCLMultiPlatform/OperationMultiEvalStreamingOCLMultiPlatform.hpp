...
#include <sgpp/datadriven/operation/hash/OperationMultipleEvalStreamingOCLMultiPlatform/StreamingOCLMultiPlatformKernelImpl.hpp>
...
template<typename T>
class OperationMultiEvalStreamingOCLMultiPlatform: public
 ...
 public:
  ...
  OperationMultiEvalStreamingOCLMultiPlatform(base::Grid& grid,
    ...
    this->kernel = std::make_unique<StreamingOCLMultiPlatformKernelImpl<T>>(dims,
  ...
  void mult(SGPP::base::DataVector& alpha,
    ...
    if ((*parameters)["VERBOSE"].getBool()) {
      std::cout << "duration mult streaming oclmp: " << elapsed_seconds.count() <<
                std::endl;
    }
  ...
  void multTranspose(
    ...
    if ((*parameters)["VERBOSE"].getBool()) {
      std::cout << "duration multTranspose streaming oclmp: " <<
 ...
 private:
  ...
  void recalculateLevelAndIndex() {
    ...
    uint32_t localWorkSize = static_cast<uint32_t>
                             (firstDeviceConfig["LOCAL_SIZE"].getUInt()) * static_cast<uint32_t>
                             (blockingPadRequirements);

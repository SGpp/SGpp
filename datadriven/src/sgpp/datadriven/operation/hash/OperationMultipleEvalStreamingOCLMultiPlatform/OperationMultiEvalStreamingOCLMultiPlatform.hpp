...
public:
     ...
     */
    OperationMultiEvalStreamingOCLMultiPlatform(base::Grid &grid,
            base::DataMatrix &dataset,
            std::shared_ptr<base::OCLManagerMultiPlatform> manager,
            std::shared_ptr<base::OCLOperationConfiguration> parameters) :
            OperationMultipleEval(grid, dataset), preparedDataset(dataset), parameters(
                    parameters), myTimer(base::SGppStopwatch()), duration(-1.0), manager(
                    manager), devices(manager->getDevices()) {
        ...
        this->kernelDataset = std::vector<T>(
                this->preparedDataset.getNrows()
    ...
    void mult(base::DataVector& alpha, base::DataVector& result,
        ...
        size_t startIndexDataPadded = this->padIndexDown(startIndexData,
                commonDatasetPadding);
        size_t endIndexDataPadded = this->padIndexUp(endIndexData,
                commonDatasetPadding);
...
#pragma omp parallel
    ...
    void multTranspose(base::DataVector& source, base::DataVector& result,
        ...
        // ensure padding requirements are fulfilled by start and end index
        size_t startIndexGridPadded = this->padIndexDown(startIndexGrid,
                commonGridPadding);
        size_t endIndexGridPadded = this->padIndexUp(endIndexGrid,
                commonGridPadding);
...
#pragma omp parallel
        {
            ...
            try {
                this->multTransposeKernels[threadId].multTranspose(this->level,
                        this->index, this->kernelDataset, sourceArray,
        ...
        if (verbose) {
            std::cout << "duration multTranspose ocl: " << duration
                    << std::endl;
        }
    }
    ...
    size_t calculateCommonGridPadding() {
        ...
        for (size_t deviceIndex = 0; deviceIndex < devices.size();
            ...
            commonPaddingRequirement =
                    std::max(commonPaddingRequirement,

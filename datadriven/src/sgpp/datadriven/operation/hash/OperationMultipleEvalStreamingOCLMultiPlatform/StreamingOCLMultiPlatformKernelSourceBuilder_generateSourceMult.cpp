  ...
  if (useLocalMemory) {
    ...
    if (dims > maxDimUnroll) {
      sourceStream << indent2 << "for (size_t d = 0; d < " << dims << "; d++) {" <<
                   std::endl;
      sourceStream << indent3 << "locLevel[(localIdx*" << dims <<
                   ")+ d] = ptrLevel[((j+localIdx)*" << dims << ")+ d];"
                   << std::endl;
      sourceStream << indent3 << "locIndex[(localIdx*" << dims <<
    ...
    for (size_t i = 0; i < dataBlockSize; i++) {
      sourceStream << indent3 << this->asString() << " curSupport_" << i <<
                   " = locAlpha[k];" << std::endl;
    }

    sourceStream << std::endl;
  } else {
    sourceStream << indent << "for(int k = start_grid; k < end_grid; k++) {" <<
                 std::endl;

    for (size_t i = 0; i < dataBlockSize; i++) {
      sourceStream << indent2 << this->asString() << " curSupport_" << i <<
                   " = ptrAlpha[k];" << std::endl;
    }

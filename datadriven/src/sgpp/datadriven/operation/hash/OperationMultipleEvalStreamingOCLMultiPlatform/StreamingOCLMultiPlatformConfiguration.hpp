...
class StreamingOCLMultiPlatformConfiguration {
 ...
 public:
  ...
  static void augmentDefaultParameters(SGPP::base::OCLOperationConfiguration&
                                       parameters) {
    ...
    for (std::string& platformName : parameters["PLATFORMS"].keys()) {
      ...
      for (std::string& deviceName : platformNode["DEVICES"].keys()) {
        ...
        const std::string& kernelName =
          SGPP::datadriven::StreamingOCLMultiPlatformConfiguration::getKernelName();

        json::Node& kernelNode =
          deviceNode["KERNELS"].contains(kernelName) ?
          deviceNode["KERNELS"][kernelName] : deviceNode["KERNELS"].addDictAttr(
            kernelName);

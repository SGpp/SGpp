 ...
 public:
  ...
  std::string generateSource() {
    ...
    sourceStream << "__attribute__((reqd_work_group_size(" << localWorkgroupSize
                 << ", 1, 1)))" << std::endl;
    sourceStream << "void multOCL(__global const " << this->floatType()
                 << "* ptrLevel," << std::endl;
    ...
    if (useLocalMemory) {
      ...
      sourceStream << this->indent[0]
                   << "int chunkSizeGrid = end_grid - start_grid;" << std::endl;
      sourceStream << this->indent[0]
                   << "int fastChunkSizeGrid = (chunkSizeGrid / "
                   << localWorkgroupSize << ") * " << localWorkgroupSize << ";"
                   << std::endl;
      sourceStream << std::endl;
      sourceStream << this->indent[0]
                   << "for(int gridBlockStart = start_grid; gridBlockStart < "
                      "start_grid + fastChunkSizeGrid; gridBlockStart += "
      ...
      if (dims > maxDimUnroll) {
        ...
        sourceStream
            << this->indent[2]
            << "locLevel[d * groupSize + localIdx] = ptrLevel[gridBlockStart * "
        ...
        sourceStream
            << this->indent[2]
            << "locIndex[d * groupSize + localIdx] = ptrIndex[gridBlockStart * "
            << dims << " +  (groupSize * d) + localIdx];" << std::endl;
        sourceStream << this->indent[1] << "}" << std::endl;
      } else {
        for (size_t d = 0; d < dims; d++) {
          sourceStream
              << this->indent[1] << "locLevel[" << d
      ...
      sourceStream
          << this->indent[1]
          << "locAlpha[localIdx] = ptrAlpha[gridBlockStart + localIdx];"
          << std::endl;
    ...
    if (kernelConfiguration["WRITE_SOURCE"].getBool()) {
      this->writeSource("StreamingOCLMultiPlatform_mult.cl",
                        sourceStream.str());
    }

...
StreamingOCLMultiPlatformKernelSourceBuilder::StreamingOCLMultiPlatformKernelSourceBuilder(
  ...
  dims(dims), indent("    "), indent2("        "), indent3("            "),
  indent4("                "), firstDeviceConfig(
    firstDeviceConfig) {
...
std::string StreamingOCLMultiPlatformKernelSourceBuilder::reuseSource(
  std::string fileName) {
...
void StreamingOCLMultiPlatformKernelSourceBuilder::writeSource(
...
std::string StreamingOCLMultiPlatformKernelSourceBuilder::getData(
...
std::string StreamingOCLMultiPlatformKernelSourceBuilder::getData(size_t dim,
...
std::string StreamingOCLMultiPlatformKernelSourceBuilder::getLevelTrans(
...
std::string StreamingOCLMultiPlatformKernelSourceBuilder::getIndexTrans(
  std::string dim, size_t gridBlockingIndex) {
  std::stringstream output;
...
std::string StreamingOCLMultiPlatformKernelSourceBuilder::getDataTrans(
  std::string dim, size_t dataBlockingIndex) {
  std::stringstream output;
...
std::string
StreamingOCLMultiPlatformKernelSourceBuilder::unrolledBasisFunctionEvalulation(
  ...
  for (size_t d = startDim; d < endDim; d++) {
    ...
    output << indent3 << "dimLevelIndex = " << "(k * " << dims << ") + " <<
           pointerAccess << ";" << std::endl;

    for (size_t i = 0; i < dataBlockSize; i++) {
      output << indent3 << "curSupport_" << i << " *= fmax(1.0" << this->constSuffix()
             << " - fabs((";
      output << levelAccess << " * " << getData(dString,
             i) << ") - " << indexAccess << "), 0.0"
...
std::string
StreamingOCLMultiPlatformKernelSourceBuilder::unrolledBasisFunctionEvalulationTrans(
  size_t dims,
  ...
  for (size_t d = startDim; d < endDim; d++) {
    ...
    for (size_t gridIndex = 0; gridIndex < transGridBlockSize; gridIndex++) {
      output << indent3 << "curSupport_" << gridIndex << " *= fmax(1.0" <<
             this->constSuffix() << " - fabs((";
      output << getLevelTrans(dString, gridIndex) << " * " << getDataTrans(dString,
             0) << ") - "
             << getIndexTrans(dString,
                              gridIndex) << "), 0.0" << this->constSuffix() << ");" << std::endl;
    }
  }

...
public:
    ...
    std::string generateSource() {
        ...
        sourceStream << "// platform: " << device->platformName << " device: " << device->deviceName << std::endl
                << std::endl;
        ...
        sourceStream << "void multOCLMask(__global const " << this->floatType() << "* ptrLevel," << std::endl;
        sourceStream << "           __global const " << this->floatType() << "* ptrIndex," << std::endl;
        sourceStream << "           __global const " << this->floatType() << "* ptrMask," << std::endl; // not needed for this kernel, but there for uniformity
        sourceStream << "           __global const " << this->floatType() << "* ptrOffset," << std::endl; // not needed for this kernel, but there for uniformity
        ...
        if (useLocalMemory) {
            sourceStream << "   __local " << this->floatType() << " locLevel[" << dims * localWorkgroupSize << "];"
                    << std::endl;
            sourceStream << "   __local " << this->floatType() << " locIndex[" << dims * localWorkgroupSize << "];"
                    << std::endl;
            sourceStream << "   __local " << this->floatType() << " locMask[" << dims * localWorkgroupSize << "];"
                    << std::endl;
            sourceStream << "   __local " << this->floatType() << " locOffset[" << dims * localWorkgroupSize << "];"
                    << std::endl;
        ...
        sourceStream << "   " << this->floatType() << " eval, index_calc, abs, last, localSupport, curSupport;"
                << std::endl << std::endl;
        sourceStream << "   " << this->floatType() << " myResult = ptrResult[globalIdx];" << std::endl << std::endl;
        sourceStream << "   // Create registers for the data" << std::endl;

        for (size_t d = 0; d < dims; d++) {
            sourceStream << " " << this->floatType() << " data_" << d << " = ptrData[globalIdx+(resultSize*" << d
                    << ")];" << std::endl;
        }
        ...
        if (useLocalMemory) {
            ...
            for (size_t d = 0; d < dims; d++) {
                ...
                sourceStream << "     locMask[(localIdx*" << dims << ")+" << d << "] = ptrMask[((j+localIdx)*" << dims
                        << ")+" << d << "];" << std::endl;
                sourceStream << "     locOffset[(localIdx*" << dims << ")+" << d << "] = ptrOffset[((j+localIdx)*"
                        << dims << ")+" << d << "];" << std::endl;
            }
            ...
            for (size_t d = 0; d < dims; d++) {
                ...
                sourceStream << "         abs = as_" << this->floatType() << "(as_" << this->intType()
                        << "(index_calc) | as_" << this->intType() << "(locMask[(k*" << dims << ")+" << d << "]));"
                        << std::endl;
            ...
            for (size_t d = 0; d < dims; d++) {
                ...
                sourceStream << "     abs = as_" << this->floatType() << "(as_" << this->intType()
                        << "(index_calc) | as_" << this->intType() << "(ptrMask[(m*" << dims << ")+" << d << "]));"
                        << std::endl;

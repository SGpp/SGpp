...
public:
    ...
    std::string generateSource() {
        ...
        sourceStream << "// platform: " << device->platformName << " device: " << device->deviceName << std::endl << std::endl;
        ...
        sourceStream << "void multTransOCLMask(__global const " << this->floatType() << "* ptrLevel," << std::endl;
    ...
    //    sourceStream << "           uint sourceSize," << std::endl;
        ...
        if (useLocalMemory) {
            sourceStream << "   __local " << this->floatType() << " locData[" << dims * localWorkgroupSize << "];"
                    << std::endl;
            sourceStream << "   __local " << this->floatType() << " locSource[" << localWorkgroupSize << "];" << std::endl
                    << std::endl;
        }

        for (size_t d = 0; d < dims; d++) {
            sourceStream << " " << this->floatType() << " level_" << d << " = ptrLevel[(globalIdx*" << dims << ")+" << d
                    << "];" << std::endl;
            sourceStream << " " << this->floatType() << " index_" << d << " = ptrIndex[(globalIdx*" << dims << ")+" << d
                    << "];" << std::endl;
            sourceStream << " " << this->floatType() << " mask_" << d << " = ptrMask[(globalIdx*" << dims << ")+" << d
                    << "];" << std::endl;
            sourceStream << " " << this->floatType() << " offset_" << d << " = ptrOffset[(globalIdx*" << dims << ")+" << d
                    << "];" << std::endl;
        }
        ...
        for (size_t d = 0; d < dims; d++) {
            ...
            sourceStream << "         abs = as_" << this->floatType() << "(as_" << this->intType()
                    << "(index_calc) | as_" << this->intType() << "(mask_" << d << "));" << std::endl;

 ...
 public:
  ...
  std::string generateSource() {
    ...
    sourceStream << "__attribute__((reqd_work_group_size(" << localWorkgroupSize
                 << ", 1, 1)))" << std::endl;
    sourceStream << "void multTransOCLMask(__global const " << this->floatType()
                 << "* ptrLevel," << std::endl;
    ...
    sourceStream << "   " << this->floatType()
                 ...
                 << std::endl;
    if (useLocalMemory) {
      sourceStream << "   __local " << this->floatType() << " locData["
                   << dims * localWorkgroupSize << "];" << std::endl;
      sourceStream << "   __local " << this->floatType() << " locSource["
                   << localWorkgroupSize << "];" << std::endl
                   << std::endl;
    }

    for (size_t d = 0; d < dims; d++) {
      sourceStream << " " << this->floatType() << " level_" << d
                   << " = ptrLevel[(globalIdx*" << dims << ")+" << d << "];"
                   << std::endl;
      sourceStream << " " << this->floatType() << " index_" << d
                   << " = ptrIndex[(globalIdx*" << dims << ")+" << d << "];"
                   << std::endl;
      sourceStream << " " << this->floatType() << " mask_" << d
    ...
    if (useLocalMemory) {
      ...
      sourceStream << "   for(int k = start_data; k < end_data; k++)"
                   << std::endl;
      sourceStream << "   {" << std::endl;
    ...
    if (kernelConfiguration["WRITE_SOURCE"].getBool()) {
      this->writeSource("streamingModOCLMaskMP_multTranspose.cl",
                        sourceStream.str());
    }

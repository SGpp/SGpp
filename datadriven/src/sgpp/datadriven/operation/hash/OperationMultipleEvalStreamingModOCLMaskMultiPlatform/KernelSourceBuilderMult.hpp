 ...
 public:
  ...
  std::string generateSource() {
    ...
    sourceStream << "__attribute__((reqd_work_group_size(" << localWorkgroupSize
                 << ", 1, 1)))" << std::endl;
    sourceStream << "void multOCLMask(__global const " << this->floatType()
                 << "* ptrLevel," << std::endl;
    sourceStream << "           __global const " << this->floatType()
                 << "* ptrIndex," << std::endl;
    sourceStream
        << "           __global const " << this->floatType() << "* ptrMask,"
        << std::endl;  // not needed for this kernel, but there for uniformity
    sourceStream
        << "           __global const " << this->floatType() << "* ptrOffset,"
        << std::endl;  // not needed for this kernel, but there for uniformity
    sourceStream << "           __global const " << this->floatType()
                 << "* ptrData," << std::endl;
    sourceStream << "           __global const " << this->floatType()
    ...
    if (useLocalMemory) {
      ...
      sourceStream << "   __local " << this->floatType() << " locIndex["
                   << dims * localWorkgroupSize << "];" << std::endl;
      sourceStream << "   __local " << this->floatType() << " locMask["
                   << dims * localWorkgroupSize << "];" << std::endl;
    ...
    if (useLocalMemory) {
      sourceStream
          ...
          << std::endl;
      sourceStream << " uint chunkSizeGrid = end_grid - start_grid;"
                   << std::endl;
      ...
      sourceStream << std::endl;
      sourceStream << "       for(int k = 0; k < " << localWorkgroupSize
                   << "; k++)" << std::endl;
      sourceStream << "       {" << std::endl;
      ...
      for (size_t d = 0; d < dims; d++) {
        ...
        sourceStream << "         abs = as_" << this->floatType() << "(as_"
                     << this->intType() << "(index_calc) | as_"
                     << this->intType() << "(locMask[(k*" << dims << ")+" << d
                     << "]));" << std::endl;
        sourceStream << "         last = locOffset[(k*" << dims << ")+" << d
                     << "] + abs;" << std::endl;
        sourceStream << "         localSupport = fmax(last, 0.0"
      ...
      for (size_t d = 0; d < dims; d++) {
        sourceStream << "     eval = ((ptrLevel[(m*" << dims << ")+" << d
                     << "]) * (data_" << d << "));" << std::endl;
        sourceStream << "     index_calc = eval - (ptrIndex[(m*" << dims << ")+"
                     << d << "]);" << std::endl;
        sourceStream << "     abs = as_" << this->floatType() << "(as_"
                     << this->intType() << "(index_calc) | as_"
                     << this->intType() << "(ptrMask[(m*" << dims << ")+" << d
                     << "]));" << std::endl;
        sourceStream << "     last = ptrOffset[(m*" << dims << ")+" << d
                     << "] + abs;" << std::endl;
        sourceStream << "     localSupport = fmax(last, 0.0"
                     << this->constSuffix() << ");" << std::endl;
        sourceStream << "     curSupport *= localSupport;" << std::endl
      ...
      sourceStream << "   // Iterate over all grid points (without cache)"
                   << std::endl;
      sourceStream << " for(int m = start_grid; m < end_grid; m++)"
                   << std::endl;
      sourceStream << "   {" << std::endl;
      sourceStream << "       curSupport = ptrAlpha[m];" << std::endl
                   << std::endl;

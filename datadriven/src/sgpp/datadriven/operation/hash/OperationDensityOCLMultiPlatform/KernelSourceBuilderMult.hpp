...
public:
    ...
    std::string generateSource() {
		...
		sourceStream << "void kernel multdensity(global const int *starting_points,global const "<<this->floatType()<<" *alpha, global "<<this->floatType()<<" *result, private const int dimensionsize, private const int gridsize,const "<<this->floatType()<<" lambda)" << std::endl;
		...
		sourceStream << "for(private int dim=0;dim<dimensionsize;dim++) {" << std::endl;
		sourceStream << "int index=starting_points[get_global_id(0)*dimensionsize*2+2*dim];" << std::endl;
		sourceStream << "int level=starting_points[get_global_id(0)*dimensionsize*2+2*dim+1];" << std::endl;
		sourceStream << "int index2=starting_points[i*dimensionsize*2+2*dim];" << std::endl;
		sourceStream << "int level2=starting_points[i*dimensionsize*2+2*dim+1];" << std::endl;
		sourceStream << "if(starting_points[get_global_id(0)*dimensionsize*2+2*dim+1]>starting_points[i*dimensionsize*2+2*dim+1]) {" << std::endl;
		sourceStream << "index=starting_points[i*dimensionsize*2+2*dim];" << std::endl;
		sourceStream << "level=starting_points[i*dimensionsize*2+2*dim+1];" << std::endl;
		sourceStream << "index2=starting_points[get_global_id(0)*dimensionsize*2+2*dim];" << std::endl;
		sourceStream << "level2=starting_points[get_global_id(0)*dimensionsize*2+2*dim+1];" << std::endl;
		sourceStream << "}" << std::endl;
		sourceStream << ""<<this->floatType()<<" h=1.0;" << std::endl;
		...
		sourceStream << ""<<this->floatType()<<" uright=1;" << std::endl;
		sourceStream << "for(int z=1;z<=level;z++)" << std::endl;
		sourceStream << "uright*=2;" << std::endl;
		sourceStream << ""<<this->floatType()<<" uleft=uright*grenze1-index;" << std::endl;
		sourceStream << "uright=uright*grenze2-index;" << std::endl;
		...
		sourceStream << "if(starting_points[i*dimensionsize*2+2*dim+1]==starting_points[get_global_id(0)*dimensionsize*2+2*dim+1]) {" << std::endl;
		sourceStream << "zellenintegral*=2.0/3.0*h;" << std::endl;
		sourceStream << "if(starting_points[i*dimensionsize*2+2*dim]!=starting_points[get_global_id(0)*dimensionsize*2+2*dim])" << std::endl;

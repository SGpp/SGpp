...
std::string StreamingModOCLFastMultiPlatformKernelSourceBuilder::generateSourceMultTrans() {
  ...
  sourceStream << "__attribute__((reqd_work_group_size(" << localWorkgroupSize << ", 1, 1)))"
               << std::endl;
  sourceStream << "void multTransOCL(__global const " << this->asString() << "* ptrLevel,"
               << std::endl;
  sourceStream << "                  __global const " << this->asString() << "* ptrIndex,"
               << std::endl;
  sourceStream << "                  __global const " << this->asString() << "* ptrData,"
               << std::endl;
  sourceStream << "                  __global const " << this->asString() << "* ptrSource,"
               << std::endl;
  sourceStream << "                  __global       " << this->asString() << "* ptrResult,"
               << std::endl;
  ...
  sourceStream << indent << "int assignedComponentBase = resultOffset + groupIdx;" << std::endl;
  sourceStream << std::endl;

  // array for local reduction
  sourceStream << indent << "__local " << this->asString() << " resultsTemp[" << localWorkgroupSize
               << "];" << std::endl;
  sourceStream << std::endl;

  // blocked result variables
  for (size_t gridIndex = 0; gridIndex < transGridBlockSize; gridIndex++) {
    sourceStream << indent << this->asString() << " myResult_" << gridIndex << " = 0.0;"
                 << std::endl
                 << std::endl;
  }
  ...
  if (firstDeviceConfig["KERNEL_STORE_DATA"].get().compare("array") == 0) {
    for (size_t gridIndex = 0; gridIndex < transGridBlockSize; gridIndex++) {
      sourceStream << indent << this->asString() << " level_" << gridIndex << "[" << dims << "];"
                   << std::endl;
      sourceStream << indent << this->asString() << " index_" << gridIndex << "[" << dims << "];"
                   << std::endl;

      for (size_t d = 0; d < dims; d++) {
        sourceStream << indent << "level_" << gridIndex << "[" << d << "] = ptrLevel[(("
                     << transGridBlockSize << " * assignedComponentBase + " << gridIndex << ") * "
                     << dims << ") +" << d << "];" << std::endl;
  ...
  for (size_t gridIndex = 0; gridIndex < transGridBlockSize; gridIndex++) {
    if (dims > maxDimUnroll) {
      ...
      sourceStream << this->unrolledBasisFunctionEvalulationTrans(
          dims, 0, std::min(maxDimUnroll, dims), "unrollDim", gridIndex);

      sourceStream << indent2 << "}" << std::endl;
      if (dims % maxDimUnroll != 0) {
        sourceStream << this->unrolledBasisFunctionEvalulationTrans(
            dims, (dims / maxDimUnroll) * maxDimUnroll, dims, "", gridIndex);
      }
    } else {
  ...
  for (size_t gridIndex = 0; gridIndex < transGridBlockSize; gridIndex++) {
    for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
      sourceStream << indent2 << "myResult_" << gridIndex << " += curSupport_" << gridIndex << "_"
                   << dataIndex << ";" << std::endl;
    }
  }
  ...
  for (size_t gridIndex = 0; gridIndex < transGridBlockSize; gridIndex++) {
    ...
    sourceStream << indent << "resultsTemp[localIdx] = myResult_" << gridIndex << ";" << std::endl
                 << std::endl;
    sourceStream << indent << "barrier(CLK_LOCAL_MEM_FENCE);" << std::endl
                 << std::endl;
    ...
    sourceStream << indent2 << "for (int i = 0; i < " << localWorkgroupSize << "; i++) {"
                 << std::endl;
    sourceStream << indent3 << "overallResult += resultsTemp[i];" << std::endl;
    sourceStream << indent2 << "}" << std::endl;
    sourceStream << indent2 << "ptrResult[" << transGridBlockSize << " * assignedComponentBase + "
                 << gridIndex << "] = overallResult;" << std::endl;
    sourceStream << indent << "}" << std::endl;
  }

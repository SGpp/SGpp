...
base::OperationMultipleEval* createStreamingModOCLFastMultiPlatformConfigured(base::Grid& grid,
    ...
    json::Node &firstDeviceConfig = deviceNode["KERNELS"][StreamingModOCLFastMultiPlatformConfiguration::getKernelName()];

    if (firstDeviceConfig["INTERNAL_PRECISION"].get() == "float") {
        return new datadriven::OperationMultiEvalStreamingModOCLFastMultiPlatform<float>(grid, dataset, manager, parameters, firstDeviceConfig);
    } else if (firstDeviceConfig["INTERNAL_PRECISION"].get() == "double") {
        return new datadriven::OperationMultiEvalStreamingModOCLFastMultiPlatform<double>(grid, dataset, manager, parameters, firstDeviceConfig);
    } else {

...
class StreamingModOCLFastMultiPlatformKernelImpl {
 ...
 public:
  ...
  StreamingModOCLFastMultiPlatformKernelImpl(size_t dims,
      std::shared_ptr<base::OCLManagerMultiPlatform> manager,
      std::shared_ptr<base::OCLOperationConfiguration> parameters,
      json::Node& firstDeviceConfig) :
    deviceData(manager), deviceLevel(manager), deviceIndex(manager),
    deviceGrid(manager), deviceTemp(manager), kernelSourceBuilder(
      parameters, dims, firstDeviceConfig), manager(manager), parameters(parameters),
    multLoadBalancer(manager,
    ...
    for (base::OCLPlatformWrapper& platform : manager->platforms) {
      this->deviceTimingsMult[platform.platformId] = new
      double[platform.getDeviceCount()];
      this->deviceTimingsMultTranspose[platform.platformId] = new
      double[platform.getDeviceCount()];
  ...
  double mult(std::vector<real_type>& level, std::vector<real_type>& index,
    ...
    for (base::OCLPlatformWrapper& platform : manager->platforms) {
      gpu_start_index_data[platform.platformId] = new
      size_t[platform.getDeviceCount()];
    }
    ...
    size_t dataBlockingSize =
      firstDeviceConfig["KERNEL_DATA_BLOCKING_SIZE"].getUInt();
    multLoadBalancer.getPartitionSegments(start_index_data, end_index_data,
                                          firstDeviceConfig["LOCAL_SIZE"].getUInt() * dataBlockingSize,
      ...
      for (size_t i = 0; i < platform.getDeviceCount(); i++) {
        ...
        if (gpu_end_data > gpu_start_data) {
          if (clSetKernelArg(kernel, 0, sizeof(cl_mem),
              ...
              clSetKernelArg(kernel, 5, sizeof(cl_uint), &clResultSize)
    ...
    for (base::OCLPlatformWrapper& platform : manager->platforms) {
      ...
      for (size_t i = 0; i < platform.getDeviceCount(); i++) {
        size_t rangeSize = (gpu_end_index_data[platform.platformId][i] /
                            dataBlockingSize)
                           - (gpu_start_index_data[platform.platformId][i] / dataBlockingSize);

        if (rangeSize > 0) {

          size_t dataOffset = gpu_start_index_data[platform.platformId][i] /
                              dataBlockingSize;
          err = clEnqueueNDRangeKernel(platform.commandQueues[i],
                                       kernelsMult[platform.platformId][i], 1,
    ...
    real_type* hostTemp = (real_type*) deviceTemp.getMappedHostBuffer(
                            manager->platforms[0].platformId);
  ...
  double multTranspose(std::vector<real_type>& level,
                       std::vector<real_type>& index, size_t gridSize,
                       std::vector<real_type>& dataset, size_t datasetSize,
    ...
    for (base::OCLPlatformWrapper& platform : manager->platforms) {
      gpu_start_index_grid[platform.platformId] = new
      size_t[platform.getDeviceCount()];
    }
    ...
    multTransposeLoadBalancer.getPartitionSegments(start_index_grid, end_index_grid,
        firstDeviceConfig["LOCAL_SIZE"].getUInt() * gridBlockingSize,
      ...
      for (size_t i = 0; i < platform.getDeviceCount(); i++) {
        ...
        if (gpu_end_grid > gpu_start_grid) {
          size_t resultOffset = gpu_start_index_grid[platform.platformId][i] /
                                (cl_uint) gridBlockingSize; //based on work groups, cannot be transmitted through global_work_offset mechanism
          ...
          if (clSetKernelArg(kernel, 0, sizeof(cl_mem),
              ...
              clSetKernelArg(kernel, 6, sizeof(cl_uint), &clSourceSize)
    ...
    for (base::OCLPlatformWrapper& platform : manager->platforms) {
      ...
      for (size_t i = 0; i < platform.getDeviceCount(); i++) {
        ...
        if (rangeSize > 0) {
          size_t totalWorkItems = rangeSize *
                                  local; //TODO: possible bug -> variable might be reallocated, the kernel uses a reference...
          //TODO: don't forget the "reqd_work_group_size" kernel attribute
          err = clEnqueueNDRangeKernel(platform.commandQueues[i],
                                       kernelsMultTrans[platform.platformId][i], 1,
    ...
    real_type* hostGrid = (real_type*) deviceGrid.getMappedHostBuffer(
                            manager->platforms[0].platformId);
 ...
 private:
  ...
  void initOCLBuffers(std::vector<real_type>& level,
                      std::vector<real_type>& index, size_t gridSize,
                      std::vector<real_type>& dataset, size_t datasetSize) {
    if (!this->deviceLevel.isInitialized()) {
      this->deviceLevel.initializeBuffer(level.data(), sizeof(real_type),
                                         gridSize * this->dims);
    }

    if (!this->deviceIndex.isInitialized()) {
      this->deviceIndex.initializeBuffer(index.data(), sizeof(real_type),
                                         gridSize * this->dims);
    }
  ...
  void initParams(std::vector<real_type>& grid, size_t gridSize,
                  std::vector<real_type>& tmp, size_t datasetSize) {
    ...
    real_type* hostTemp = (real_type*) this->deviceTemp.getMappedHostBuffer(
                            manager->platforms[0].platformId);

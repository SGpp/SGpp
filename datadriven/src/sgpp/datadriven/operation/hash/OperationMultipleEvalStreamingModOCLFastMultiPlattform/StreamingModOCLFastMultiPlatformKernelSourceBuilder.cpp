...
StreamingModOCLFastMultiPlatformKernelSourceBuilder::
  ...
  dataBlockSize = firstDeviceConfig["KERNEL_DATA_BLOCK_SIZE"].getUInt();
  maxDimUnroll = firstDeviceConfig["KERNEL_MAX_DIM_UNROLL"].getUInt();
}

std::string StreamingModOCLFastMultiPlatformKernelSourceBuilder::getData(std::string dim,
...
std::string StreamingModOCLFastMultiPlatformKernelSourceBuilder::getData(size_t dim,
                                                                         size_t dataBlockingIndex) {
...
std::string StreamingModOCLFastMultiPlatformKernelSourceBuilder::unrolledBasisFunctionEvalulation(
    size_t dims, size_t startDim, size_t endDim, std::string unrollVariable) {
  std::stringstream output;
  ...
  for (size_t d = startDim; d < endDim; d++) {
    ...
    output << indent3 << "dimLevelIndex = "
           << "(m * " << dims << ") + " << pointerAccess << ";" << std::endl;

    output << indent3 << "if (" << levelAccess << " == 2.0" << this->constSuffix() << ") {"
           << std::endl;
    output << indent3 << "} else if (" << indexAccess << " == 1.0" << this->constSuffix() << ") {"
           << std::endl;

    for (size_t i = 0; i < dataBlockSize; i++) {
      output << indent4 << "curSupport_" << i << " *= max(2.0" << this->constSuffix() << " - ("
             << levelAccess << " * " << getData(dString, i) << "), 0.0" << this->constSuffix()
             << ") ;" << std::endl;
    }

    output << indent3 << "} else if (" << indexAccess << " == " << levelAccess << " - 1.0"
    ...
    for (size_t i = 0; i < dataBlockSize; i++) {
      output << indent4 << "curSupport_" << i << " *= max(1.0" << this->constSuffix() << " - fabs("
             << levelAccess << " * " << getData(dString, i) << " - " << indexAccess << "), 0.0"
             << this->constSuffix() << ");" << std::endl;
    }
    output << indent3 << "}" << std::endl;
  }
...
StreamingModOCLFastMultiPlatformKernelSourceBuilder::unrolledBasisFunctionEvalulationTrans(
  ...
  for (size_t d = startDim; d < endDim; d++) {
    ...
    output << indent3 << "if (" << getLevelTrans(dString, gridBlockIndex) << " == 2.0"
           << this->constSuffix() << ") {" << std::endl;

    output << indent3 << "} else if (" << getIndexTrans(dString, gridBlockIndex) << " == 1.0"
           << this->constSuffix() << ") {" << std::endl;
    ...
    for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
      output << indent4 << "curSupport_" << gridBlockIndex << "_" << dataIndex << " *= max(1.0"
             ...
             << getDataTrans(dString, dataIndex) << " - " << getIndexTrans(dString, gridBlockIndex)
             << "), 0.0" << this->constSuffix() << ");" << std::endl;
    }
...
void StreamingModOCLFastMultiPlatformKernelSourceBuilder::writeSource(std::string fileName,
                                                                      std::string source) {

...
std::string StreamingModOCLFastMultiPlatformKernelSourceBuilder::generateSourceMult() {
    ...
    for (size_t i = 0; i < dataBlockSize; i++) {
        sourceStream << indent << this->asString() << " curSupport_" << i << ";" << std::endl;
        sourceStream << indent << this->asString() << " myResult_" << i << " = 0.0;" << std::endl << std::endl;
    }
    ...
    if (useLocalMemory) {
        ...
        sourceStream << indent << "__local " << this->asString() << " locIndex[" << dims * localWorkgroupSize << "];"
                << std::endl;
        sourceStream << indent << "__local " << this->asString() << " locAlpha[" << localWorkgroupSize << "];"
                << std::endl;
        sourceStream << std::endl;

        sourceStream << indent << "// grid is always divisible by workgroup size (by padding)" << std::endl;
        ...
        sourceStream << indent << "for(int j = start_grid; j < start_grid + fastChunkSizeGrid; j+="
                << localWorkgroupSize << ") {" << std::endl;
        ...
        sourceStream << std::endl;
        sourceStream << indent2 << "for (size_t m = 0; m < " << localWorkgroupSize << "; m++) {" << std::endl;
        for (size_t i = 0; i < dataBlockSize; i++) {
            sourceStream << indent3 << "curSupport_" << i << " = locAlpha[m];" << std::endl << std::endl;
        }
    } else {
        sourceStream << indent << "// Iterate over all grid points (slow ones, without cache)" << std::endl;
        sourceStream << indent << "for (int m = start_grid; m < end_grid; m++) {" << std::endl;
        for (size_t i = 0; i < dataBlockSize; i++) {
            sourceStream << indent2 << "curSupport_" << i << " = ptrAlpha[m];" << std::endl << std::endl;
        }
    }

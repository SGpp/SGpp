...
#include <sgpp/datadriven/operation/hash/OperationMultipleEvalStreamingModOCLFastMultiPlattform/StreamingModOCLFastMultiPlatformKernelImpl.hpp>

namespace SGPP {
namespace datadriven {

template<typename T>
class OperationMultiEvalStreamingModOCLFastMultiPlatform: public
 ...
 public:
  ...
  OperationMultiEvalStreamingModOCLFastMultiPlatform(base::Grid& grid,
    ...
    parameters(parameters), myTimer(
      SGPP::base::SGppStopwatch()), duration(-1.0), manager(manager),
  ...
  void multTranspose(
    ...
    if ((*parameters)["VERBOSE"].getBool()) {
      std::cout << "duration multTranspose ocl mod fast: " << elapsed_seconds.count()
                << std::endl;
    }
 ...
 private:
  ...
  void recalculateLevelAndIndex() {
    ...
    uint32_t localWorkSize = static_cast<uint32_t>
                             (firstDeviceConfig["LOCAL_SIZE"].getUInt())

...
std::string StreamingModOCLFastMultiPlatformKernelSourceBuilder::generateSourceMult() {
  ...
  if (useLocalMemory) {
    ...
    sourceStream << indent << "// grid is always divisible by workgroup size (by padding)"
                 << std::endl;
    ...
    sourceStream << std::endl;
    sourceStream << indent2 << "for (size_t m = 0; m < " << localWorkgroupSize << "; m++) {"
                 << std::endl;
    for (size_t i = 0; i < dataBlockSize; i++) {
    ...
    sourceStream << indent << "// Iterate over all grid points (slow ones, without cache)"
                 << std::endl;
  ...
  for (size_t i = 0; i < dataBlockSize; i++) {
    sourceStream << indent << "ptrResult[(" << dataBlockSize << " * globalIdx) + " << i

...
#include <sgpp/datadriven/operation/hash/OperationMultipleEvalStreamingMaskOCL/OCLKernelSourceBuilder.hpp>
...
class OCLKernelImpl {
 ...
 public:
  ...
  OCLKernelImpl(size_t dims, base::OCLManager& manager,
                base::ConfigurationParameters parameters) :
  ...
  double mult(real_type* level, real_type* index, size_t gridSize,
    ...
    if (kernel_mult[0] == nullptr) {
      this->createMult(this->dims, parameters.getAsUnsigned("LOCAL_SIZE"), context,
    ...
    for (size_t i = 0; i < num_devices; i++) {
      ...
      if (rangeSize > 0) {
        err = clEnqueueNDRangeKernel(command_queue[i], kernel_mult[i], 1,
                                     &gpu_start_index_data[i], &rangeSize, &local,
  ...
  double multTranspose(real_type* level, real_type* index, size_t gridSize,
    ...
    if (kernel_multTrans[0] == nullptr) {
      this->createMultTrans(this->dims, parameters.getAsUnsigned("LOCAL_SIZE"),
                            context, num_devices, device_ids,
                            kernel_multTrans);
    }
    ...
    multTransposeLoadBalancer.getPartitionSegments(start_index_grid, end_index_grid,
        parameters.getAsUnsigned("LOCAL_SIZE"), gpu_start_index_grid,
        gpu_end_index_grid);

...
namespace SGPP {
  namespace datadriven {
    ...
    base::OperationMultipleEval* createStreamingMaskOCLConfigured(base::Grid& grid, base::DataMatrix& dataset) {
      ...
      base::OpenCLConfigurationParameters parameters("StreamingMaskOCL.cfg", defaultParameter);
      ...
      if (parameters["INTERNAL_PRECISION"] == "float") {
        return new datadriven::OperationMultiEvalStreamingMaskOCL<float>(grid, dataset, parameters);
      } else if (parameters["INTERNAL_PRECISION"] == "double") {
        return new datadriven::OperationMultiEvalStreamingMaskOCL<double>(grid, dataset, parameters);
      } else {

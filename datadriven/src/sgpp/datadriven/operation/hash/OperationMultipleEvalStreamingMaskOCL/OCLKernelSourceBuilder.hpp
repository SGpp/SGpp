 ...
 public:
  ...
  std::string generateSourceMult(size_t dims) {
    ...
    bool useLocalMemory = this->parameters.getAsBoolean(
                            "STREAMING_MASK_OCL_USE_LOCAL_MEMORY");
    uint64_t maxDimUnroll = this->parameters.getAsUnsigned(
                              "STREAMING_MASK_OCL_MAX_DIM_UNROLL");
    ...
    stream_program_src << "           __global const "
                       << getType<real_type>::asString() << "* ptrMask," << std::endl;
    stream_program_src << "           __global const "
                       << getType<real_type>::asString() << "* ptrOffset," << std::endl;
    stream_program_src << "           __global const "
    ...
    if (useLocalMemory) {
      stream_program_src << " __local " << getType<real_type>::asString()
                         << " locLevel[" << dims* localWorkgroupSize << "];"
                         << std::endl;
      stream_program_src << " __local " << getType<real_type>::asString()
                         << " locIndex[" << dims* localWorkgroupSize << "];"
                         << std::endl;
      stream_program_src << "__local " << getType<real_type>::asString()
                         << " locMask[" << dims* local_workgroup_size << "];"
                         << std::endl;
      stream_program_src << "__local " << getType<real_type>::asString()
                         << " locOffset[" << dims* local_workgroup_size << "];"
                         << std::endl;
      stream_program_src << " __local " << getType<real_type>::asString()
                         << " locAlpha[" << localWorkgroupSize << "];" << std::endl;
      stream_program_src << std::endl;
    }
    ...
    if (dims > maxDimUnroll) {
      ...
      stream_program_src << "for (size_t d = 0; d < " << dims
                         << "; d++) {" << std::endl;
      stream_program_src
          << "  data[d] = ptrData[globalIdx+(resultSize * d)];"
          << std::endl;
      stream_program_src << "}" << std::endl;
    } else {
      for (size_t d = 0; d < dims; d++) {
        stream_program_src << " " << getType<real_type>::asString()
                           << " data_" << d << " = ptrData[globalIdx+(resultSize*"
                           << d << ")];" << std::endl;
      }
    }
    ...
    if (useLocalMemory) {
      ...
      if (dims > maxDimUnroll) {
        stream_program_src << "for (size_t d = 0; d < " << dims
                           << "; d++) {" << std::endl;
        stream_program_src << "   locLevel[(localIdx*" << dims
                           << ")+ d] = ptrLevel[((j+localIdx)*" << dims << ")+ d];"
                           << std::endl;
        stream_program_src << "   locIndex[(localIdx*" << dims
                           << ")+ d] = ptrIndex[((j+localIdx)*" << dims << ")+ d];"
                           << std::endl;
        stream_program_src << "   locMask[(localIdx*" << dims
                           << ")+ d = ptrMask[((j+localIdx)*" << dims << ")+ d];"
                           << std::endl;
        stream_program_src << "   locOffset[(localIdx*" << dims
                           << ")+ d = ptrOffset[((j+localIdx)*" << dims << ")+ d];"
                           << std::endl;
        stream_program_src << "}" << std::endl;
      } else {
        for (size_t d = 0; d < dims; d++) {
          stream_program_src << "   locLevel[(localIdx*" << dims
                             << ")+" << d << "] = ptrLevel[((j+localIdx)*"
                             << dims << ")+" << d << "];" << std::endl;
          stream_program_src << "   locIndex[(localIdx*" << dims
                             << ")+" << d << "] = ptrIndex[((j+localIdx)*"
                             << dims << ")+" << d << "];" << std::endl;
        }
      }
      ...
      stream_program_src
          ...
          << std::endl;
      stream_program_src << "   barrier(CLK_LOCAL_MEM_FENCE);"
                         << std::endl;
      stream_program_src << std::endl;
      ...
      if (dims > maxDimUnroll) {
        stream_program_src << "for (size_t d = 0; d < " << dims
                           << "; d++) {" << std::endl;
        stream_program_src << "     eval = ((locLevel[(k*" << dims
                           << ")+ d ]) * (data[d]));" << std::endl;
        stream_program_src << "     index_calc = eval - (locIndex[(k*"
                           << dims << ")+ d]);" << std::endl;
        stream_program_src << "     abs = as_" << getType<real_type>::asString()
        ...
        for (size_t d = 0; d < dims; d++) {
          stream_program_src << "     eval = ((locLevel[(k*" << dims
                             ...
                             << std::endl;
          stream_program_src
              << "     index_calc = eval - (locIndex[(k*" << dims
              << ")+" << d << "]);" << std::endl;
          stream_program_src << "     abs = as_" << getType<real_type>::asString()
                             << "(as_" << getType<real_type>::intAsString()
                             << "(index_calc) | as_" << getType<real_type>::intAsString()
      ...
      if (dims > maxDimUnroll) {
        ...
        stream_program_src << "   eval = ((ptrLevel[(m*" << dims
                           << ")+ d]) * (data[d]));" << std::endl;
        stream_program_src << "   index_calc = eval - (ptrIndex[(m*"
                           << dims << ")+ d]);" << std::endl;
        stream_program_src << "     abs = as_" << getType<real_type>::asString()
                           << "(as_" << getType<real_type>::intAsString()
                           << "(index_calc) | as_" << getType<real_type>::intAsString()
      ...
      stream_program_src
          << " // Iterate over all grid points (without cache)"
          << std::endl;
  ...
  std::string generateSourceMultTrans(size_t dims) {
    ...
    size_t localWorkgroupSize = parameters.getAsUnsigned("LOCAL_SIZE");
    bool useLocalMemory = this->parameters.getAsBoolean(
                            "STREAMING_OCL_USE_LOCAL_MEMORY");
    uint64_t maxDimUnroll = this->parameters.getAsUnsigned(
                              "STREAMING_OCL_MAX_DIM_UNROLL");
    ...
    if (useLocalMemory) {
      stream_program_src << " __local " << getType<real_type>::asString()
                         << " locData[" << dims* localWorkgroupSize << "];"
                         << std::endl;
      stream_program_src << " __local " << getType<real_type>::asString()
                         << " locSource[" << localWorkgroupSize << "];" << std::endl
                         << std::endl;
    }
    ...
    if (dims > maxDimUnroll) {
      ...
      stream_program_src << "for (size_t d = 0; d < " << dims
                         << "; d++) {" << std::endl;
      stream_program_src << "  level[d] = ptrLevel[(globalIdx*" << dims
                         << ") + d];" << std::endl;
      stream_program_src << "  index[d] = ptrIndex[(globalIdx*" << dims
                         << ") + d];" << std::endl;
      stream_program_src << "}" << std::endl;
    } else {
      for (size_t d = 0; d < dims; d++) {
        ...
        stream_program_src << " " << getType<real_type>::asString()
                           ...
                           << ")+" << d << "];" << std::endl;
        stream_program_src << "  " << getType<real_type>::asString()
                           << " mask_" << d << " = ptrMask[(globalIdx*" << dims
                           << ")+" << d << "];" << std::endl;
        stream_program_src << "  " << getType<real_type>::asString()
                           << " offset_" << d << " = ptrOffset[(globalIdx*" << dims
                           << ")+" << d << "];" << std::endl;
      }
    }
    ...
    if (useLocalMemory) {
      ...
      if (dims > maxDimUnroll) {
        ...
        for (size_t d = 0; d < dims; d++) {
          stream_program_src << "   locData[(" << d << "*"
                             << localWorkgroupSize << ")+(localIdx)] = ptrData[("
      ...
      stream_program_src
          << "   for(int k = start_data; k < end_data; k++)"
          << std::endl;
      stream_program_src << "   {" << std::endl;
    ...
    if (dims > maxDimUnroll) {
      ...
      if (useLocalMemory) {
        stream_program_src << "     eval = (level[d] * (locData[(d * "

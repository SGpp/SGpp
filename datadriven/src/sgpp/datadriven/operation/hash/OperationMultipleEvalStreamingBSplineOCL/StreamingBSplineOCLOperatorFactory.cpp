...
base::OperationMultipleEval* createStreamingBSplineOCLConfigured(
  ...
  if (configuration.getParameters().operator bool()) {
    ...
    parameters =
      std::make_shared<base::OCLOperationConfiguration>("StreamingBSplineOCL.cfg");
  ...
  if ((*parameters)["INTERNAL_PRECISION"].get() == "float") {
    return new datadriven::OperationMultiEvalStreamingBSplineOCL<float>(grid,
           dataset, parameters);
  } else if ((*parameters)["INTERNAL_PRECISION"].get() == "double") {
    return new datadriven::OperationMultiEvalStreamingBSplineOCL<double>(grid,
           dataset, parameters);
  } else {

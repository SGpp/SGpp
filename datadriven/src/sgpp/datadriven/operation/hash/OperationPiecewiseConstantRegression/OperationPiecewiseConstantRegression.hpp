...
class OperationPiecewiseConstantRegression {
  ...
  size_t dims;
 public:
  OperationPiecewiseConstantRegression(base::DataMatrix& dataset,
                                       base::DataVector& values) :
    dataset(dataset), values(values), dims(dataset.getNcols()) {
  }

  std::unique_ptr<PiecewiseConstantRegression::Node> hierarchize(
    float_t targetMSE, size_t targetMaxLevel) {
    ...
    std::unique_ptr<PiecewiseConstantRegression::Node> root =
      std::make_unique<PiecewiseConstantRegression::Node>(xRoot, hRoot, rootSupport,
          dataset, values);

    root->hierarchize(targetMSE, targetMaxLevel);

    std::cout << "total node count: " << (root->getChildCount() + 1) << std::endl;
    std::cout << "hierarchization max level: " << root->getHierarchizationMaxLevel()
              << std::endl;

...
namespace SGPP {
  namespace datadriven {
    ...
    void OperationMultipleEvalSubspaceCombined::uncachedMultTransposeInner(size_t dim, const float_t* const datasetPtr, size_t dataIndexBase,
      ...
      for (size_t validIndex = 0; validIndex < validIndicesCount; validIndex += X86COMBINED_VEC_PADDING) {
        ...
        for (size_t innerIndex = 0; innerIndex < 4; innerIndex++) {
          if (!std::isnan(surplus2[innerIndex])) {
            //cout << parallelIndices2[innerIndex] << " -> phi: " << phiEval2[innerIndex] << " surplus: " << surplus2[innerIndex] << " = " << phiEval2[innerIndex] * surplus2[innerIndex] << endl;

            //cout << parallelIndices2[innerIndex] << " ->second phi: " << phiEval2[innerIndex] << " surplus: " << surplus2[innerIndex] << " = " << phiEval2[innerIndex] * surplus2[innerIndex] << endl;
            //nextRoundResults[innerIndex] = phiEval2[innerIndex] * surplus2[innerIndex];

            componentResults[parallelIndices2[innerIndex]] += phiEval2[innerIndex] * surplus2[innerIndex];

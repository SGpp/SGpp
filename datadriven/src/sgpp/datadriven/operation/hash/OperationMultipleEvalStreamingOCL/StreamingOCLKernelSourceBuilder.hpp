...
private:
  ...
  real_type blubb;
public:
  StreamingOCLKernelSourceBuilder(base::ConfigurationParameters parameters) :
  ...
  std::string generateSourceMult(size_t dims) {
    ...
    if (streamingOCL::getType<real_type>::asString() == "double") {
    ...
    stream_program_src << "void multOCL(__global const " << streamingOCL::getType<real_type>::asString() << "* ptrLevel,"
        << std::endl;
    stream_program_src << "           __global const " << streamingOCL::getType<real_type>::asString() << "* ptrIndex," << std::endl;
    stream_program_src << "           __global const " << streamingOCL::getType<real_type>::asString() << "* ptrData," << std::endl;
    stream_program_src << "           __global const " << streamingOCL::getType<real_type>::asString() << "* ptrAlpha," << std::endl;
    stream_program_src << "           __global       " << streamingOCL::getType<real_type>::asString() << "* ptrResult," << std::endl;
    ...
    if (useLocalMemory) {
      stream_program_src << " __local " << streamingOCL::getType<real_type>::asString() << " locLevel[" << dims * localWorkgroupSize
          << "];" << std::endl;
      stream_program_src << " __local " << streamingOCL::getType<real_type>::asString() << " locIndex[" << dims * localWorkgroupSize
          << "];" << std::endl;
      stream_program_src << " __local " << streamingOCL::getType<real_type>::asString() << " locAlpha[" << localWorkgroupSize << "];"
          << std::endl;
      stream_program_src << std::endl;
    }
    stream_program_src << " " << streamingOCL::getType<real_type>::asString()
        << " eval, index_calc, abs, last, localSupport, curSupport;" << std::endl << std::endl;
    stream_program_src << " " << streamingOCL::getType<real_type>::asString() << " myResult = 0.0;" << std::endl << std::endl;
    stream_program_src << " // Create registers for the data" << std::endl;

    if (dims > maxDimUnroll) {
      stream_program_src << " " << streamingOCL::getType<real_type>::asString() << " data[" << dims << "];" << std::endl;
      stream_program_src << "for (size_t d = 0; d < " << dims << "; d++) {" << std::endl;
      stream_program_src << "  data[d] = ptrData[globalIdx+(resultSize * d)];" << std::endl;
      stream_program_src << "}" << std::endl;
    } else {
      for (size_t d = 0; d < dims; d++) {
        stream_program_src << " " << streamingOCL::getType<real_type>::asString() << " data_" << d
    ...
    if (useLocalMemory) {
      ...
      if (dims > maxDimUnroll) {
        ...
        stream_program_src << "     last = 1.0" << streamingOCL::getType<real_type>::constSuffix() << " - abs;" << std::endl;
        stream_program_src << "     localSupport = fmax(last, 0.0" << streamingOCL::getType<real_type>::constSuffix() << ");"
            << std::endl;
  ...
  std::string generateSourceMultTrans(size_t dims) {
    ...
    stream_program_src << "__attribute__((reqd_work_group_size(" << localWorkgroupSize << ", 1, 1)))" << std::endl;
    stream_program_src << "void multTransOCL(__global const " << streamingOCL::getType<real_type>::asString() << "* ptrLevel,"
        << std::endl;
    stream_program_src << "           __global const " << streamingOCL::getType<real_type>::asString() << "* ptrIndex," << std::endl;
    stream_program_src << "           __global const " << streamingOCL::getType<real_type>::asString() << "* ptrData," << std::endl;
    stream_program_src << "           __global const " << streamingOCL::getType<real_type>::asString() << "* ptrSource," << std::endl;
    stream_program_src << "           __global       " << streamingOCL::getType<real_type>::asString() << "* ptrResult," << std::endl;
    ...
    stream_program_src << " " << streamingOCL::getType<real_type>::asString()
        << " eval, index_calc, abs, last, localSupport, curSupport;" << std::endl << std::endl;
    stream_program_src << " " << streamingOCL::getType<real_type>::asString() << " myResult = ptrResult[globalIdx];" << std::endl
        << std::endl;

    if (useLocalMemory) {
      stream_program_src << " __local " << streamingOCL::getType<real_type>::asString() << " locData[" << dims * localWorkgroupSize
          << "];" << std::endl;
      stream_program_src << " __local " << streamingOCL::getType<real_type>::asString() << " locSource[" << localWorkgroupSize << "];"
          << std::endl << std::endl;
    }

    if (dims > maxDimUnroll) {
      stream_program_src << " " << streamingOCL::getType<real_type>::asString() << " level[" << dims << "];" << std::endl;
      stream_program_src << " " << streamingOCL::getType<real_type>::asString() << " index[" << dims << "];" << std::endl;
      stream_program_src << "for (size_t d = 0; d < " << dims << "; d++) {" << std::endl;
      ...
      for (size_t d = 0; d < dims; d++) {
        stream_program_src << " " << streamingOCL::getType<real_type>::asString() << " level_" << d << " = ptrLevel[(globalIdx*"
            << dims << ")+" << d << "];" << std::endl;
        stream_program_src << " " << streamingOCL::getType<real_type>::asString() << " index_" << d << " = ptrIndex[(globalIdx*"
            << dims << ")+" << d << "];" << std::endl;
      }
    }

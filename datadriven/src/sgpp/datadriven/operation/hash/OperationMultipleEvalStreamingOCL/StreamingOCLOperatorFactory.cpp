...
base::OperationMultipleEval* createStreamingOCLConfigured(base::Grid& grid,
  ...
  if (configuration.getParameters().operator bool()) {
    ...
    parameters =
      std::make_shared<base::OCLOperationConfiguration>("StreamingOCL.cfg");
  ...
  if ((*parameters)["VERBOSE"].getBool()) {
    ...
    std::cout << "device type is: " << (*parameters)["DEVICE_TYPE"].get() <<
              std::endl;
    std::cout << "KERNEL_USE_LOCAL_MEMORY: " <<
              (*parameters)["KERNEL_USE_LOCAL_MEMORY"].get() << std::endl;
  }

  if ((*parameters)["INTERNAL_PRECISION"].get() == "float") {
    return new datadriven::OperationMultiEvalStreamingOCL<float>(grid, dataset,
           parameters);
  } else if ((*parameters)["INTERNAL_PRECISION"].get() == "double") {
    return new datadriven::OperationMultiEvalStreamingOCL<double>(grid, dataset,
           parameters);
  } else {

...
class StreamingOCLKernelImpl {
 ...
 public:
  ...
  double mult(std::vector<T>& level, std::vector<T>& index, size_t gridSize,
    ...
    if (kernel_mult[0] == nullptr) {
      ...
      manager->buildKernel(program_src, "multOCL", manager->context,
                           manager->num_devices, manager->device_ids,
                           kernel_mult);
    }
    ...
    multLoadBalancer.getPartitionSegments(start_index_data, end_index_data,
                                          (*parameters)["LOCAL_SIZE"].getUInt() * dataBlockingSize, gpu_start_index_data,
                                          gpu_end_index_data);
  ...
  double multTranspose(std::vector<T>& level, std::vector<T>& index,
                       size_t gridSize, std::vector<T>& dataset, size_t datasetSize,
    ...
    if (kernel_multTrans[0] == nullptr) {
      ...
      manager->buildKernel(program_src, "multTransOCL", manager->context,
                           manager->num_devices,
    ...
    multTransposeLoadBalancer.getPartitionSegments(start_index_grid, end_index_grid,
        (*parameters)["LOCAL_SIZE"].getUInt() * transGridBlockingSize,
 ...
 private:
  ...
  void initOCLBuffers(std::vector<T>& level, std::vector<T>& index,
                      size_t gridSize, std::vector<T>& dataset, size_t datasetSize) {

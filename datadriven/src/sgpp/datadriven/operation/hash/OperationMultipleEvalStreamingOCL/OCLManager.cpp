...
OCLMemory OCLManager::createBuffer(void *initialValues, size_t sizeofType, size_t elements) {
  cl_int err;
  cl_mem *bufferList = new cl_mem[this->num_devices];
  for (size_t i = 0; i < this->num_devices; i++) {
    if (initialValues != nullptr) {
      bufferList[i] = clCreateBuffer(this->context,
  ...
  OCLMemory memory;
  memory.bufferList = bufferList;
  memory.sizeofType = sizeofType;
  memory.elements = elements;
  memory.isMappedMemory = false;
  memory.hostBuffer = nullptr;
  memory.mappedHostBuffer = nullptr;
  return memory;
}

OCLMemory OCLManager::createMappedBuffer(size_t sizeofType, size_t elements) {
  cl_int err;
  //cl buffer that is allocated on the host, but cannot be directly accessed
  cl_mem hostBuffer = clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_ALLOC_HOST_PTR, sizeofType * elements, nullptr, &err);
  if (err != CL_SUCCESS) {
    std::stringstream errorString;
  ...
  cl_mem *bufferList = new cl_mem[this->num_devices];
  for (size_t i = 0; i < num_devices; i++) {
    bufferList[i] = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeofType * elements, nullptr, &err);
    if (err != CL_SUCCESS) {
      std::stringstream errorString;
      errorString << "OCL Error: Could not allocate buffer! Error code: " << err << std::endl;
      throw SGPP::base::operation_exception(errorString.str());
  ...
  void *hostPinnedMemory = clEnqueueMapBuffer(command_queue[0], hostBuffer, CL_TRUE, CL_MAP_READ | CL_MAP_WRITE, 0, sizeofType * elements, 0,
      nullptr, nullptr, &err);
  if (err != CL_SUCCESS) {
    std::stringstream errorString;
  ...
  memory.sizeofType = sizeofType;
  memory.elements = elements;
  memory.isMappedMemory = true;
  memory.hostBuffer = hostBuffer;
  memory.mappedHostBuffer = hostPinnedMemory;
...
void OCLManager::writeToBuffer(OCLMemory buffers, void *hostData, size_t *offsets) {
  cl_int err;
  cl_event* actionDone = new cl_event[num_devices];

  for (size_t i = 0; i < this->num_devices; i++) {
    if (offsets == nullptr) {
      err = clEnqueueWriteBuffer(this->command_queue[i], buffers.bufferList[i],
      CL_FALSE, 0, buffers.sizeofType * buffers.elements, hostData, 0, nullptr, nullptr);
    } else {
      err = clEnqueueWriteBuffer(this->command_queue[i], buffers.bufferList[i],
      CL_FALSE, buffers.sizeofType * offsets[i], buffers.sizeofType * buffers.elements, hostData, 0, nullptr, nullptr);
    }
  ...
  clWaitForEvents((cl_uint) num_devices, actionDone);
}

void OCLManager::readFromBuffer(OCLMemory buffers, void *hostData, size_t *offsets, size_t *ranges) {
  cl_int err;
  cl_event* actionDone = new cl_event[num_devices];
  ...
  for (size_t i = 0; i < num_devices; i++) {
    if (offsets == nullptr) {
    err = clEnqueueReadBuffer(command_queue[i], buffers.bufferList[i],
    CL_FALSE, 0, buffers.sizeofType * buffers.elements, hostData, 0, nullptr,
        &(actionDone[i]));
    } else {
      err = clEnqueueReadBuffer(command_queue[i], buffers.bufferList[i],
      CL_FALSE, buffers.sizeofType * offsets[i], buffers.sizeofType * ranges[i], static_cast<char *>(hostData) + (buffers.sizeofType * offsets[i]), 0, nullptr,
          &(actionDone[i]));
    }
...
void OCLManager::freeBuffer(OCLMemory memory) {
  for (size_t i = 0; i < this->num_devices; i++) {
    if (memory.bufferList[i]) {
      clReleaseMemObject (memory.bufferList[i]);
      memory.bufferList[i] = nullptr;
    }
  }
  delete[] memory.bufferList;
  if (memory.isMappedMemory) {
    clReleaseMemObject(memory.hostBuffer);
  }
}

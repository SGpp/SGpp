...
//    }
		...
		for (size_t i = 0; i < dataBlockSize; i++) {
			for (size_t d = 0; d < dims; d++) {
				stream_program_src << " " << streamingModOCLFast::getType<real_type>::asString() << " data_" << i << "_"
						<< d << " = ptrData[" << i << " + (" << dataBlockSize << " * globalIdx) + (resultSize * " << d
						<< ")];" << std::endl;
//        stream_program_src << "printf(\"%i is working on data_" << i << ": %lf\\n\", globalIdx, data_" << i << "_" << d << ");" << std::endl;
		...
		if (useLocalMemory) {
			...
			stream_program_src << " uint fastChunkSizeGrid = (chunkSizeGrid / " << localWorkgroupSize << ") * "
					<< localWorkgroupSize << ";" << std::endl;
			stream_program_src << " for(int j = start_grid; j < start_grid + fastChunkSizeGrid; j+="
					<< localWorkgroupSize << ")" << std::endl;
			stream_program_src << "   {" << std::endl;

			for (size_t d = 0; d < dims; d++) {
				stream_program_src << "     locLevel[(localIdx*" << dims << ")+" << d << "] = ptrLevel[((j+localIdx)*"
						<< dims << ")+" << d << "];" << std::endl;
				stream_program_src << "     locIndex[(localIdx*" << dims << ")+" << d << "] = ptrIndex[((j+localIdx)*"
						<< dims << ")+" << d << "];" << std::endl;
			}
...
//            << ";" << std::endl;
				...
				for (size_t i = 0; i < dataBlockSize; i++) {
					stream_program_src << "             curSupport_" << i << " *= max(2.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << " - ( (locLevel[(k*" << dims
							<< ")+" << d << "]) * (data_" << i << "_" << d << ") ), 0.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << ") ;" << std::endl;
				}

				stream_program_src << "         }" << std::endl;
				stream_program_src << "         else if ((locIndex[(k*" << dims << ")+" << d << "]) == ((locLevel[(k*"
						<< dims << ")+" << d << "]) - 1.0" << streamingModOCLFast::getType<real_type>::constSuffix()
						<< ") )" << std::endl;
				stream_program_src << "         {" << std::endl;

				for (size_t i = 0; i < dataBlockSize; i++) {
					stream_program_src << "             curSupport_" << i << " *= max(( (locLevel[(k*" << dims << ")+"
							<< d << "]) * (data_" << i << "_" << d << ") ) - (locIndex[(k*" << dims << ")+" << d
							<< "]) + 1.0" << streamingModOCLFast::getType<real_type>::constSuffix() << ", 0.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << ");" << std::endl;
				}
				...
				for (size_t i = 0; i < dataBlockSize; i++) {
					stream_program_src << "             curSupport_" << i << " *= max(1.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << " - fabs( ( (locLevel[(k*"
							<< dims << ")+" << d << "]) * (data_" << i << "_" << d << ") ) - (locIndex[(k*" << dims
							<< ")+" << d << "]) ), 0.0" << streamingModOCLFast::getType<real_type>::constSuffix()
							<< ");" << std::endl;
				}
...
//            << std::endl;
				...
				for (size_t i = 0; i < dataBlockSize; i++) {
					stream_program_src << "         curSupport_" << i << " *= max(2.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << " - ( (ptrLevel[(m*" << dims
							<< ")+" << d << "]) * (data_" << i << "_" << d << ") ), 0.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << ") ;" << std::endl;
				}

				stream_program_src << "     }" << std::endl;
				stream_program_src << "     else if ((ptrIndex[(m*" << dims << ")+" << d << "]) == ((ptrLevel[(m*"
						<< dims << ")+" << d << "]) - 1.0" << streamingModOCLFast::getType<real_type>::constSuffix()
						<< ") )" << std::endl;
				stream_program_src << "     {" << std::endl;
				...
				for (size_t i = 0; i < dataBlockSize; i++) {
					stream_program_src << "         curSupport_" << i << " *= max(1.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << " - fabs( ( (ptrLevel[(m*"
							<< dims << ")+" << d << "]) * (data_" << i << "_" << d << ") ) - (ptrIndex[(m*" << dims
							<< ")+" << d << "]) ), 0.0" << streamingModOCLFast::getType<real_type>::constSuffix()
							<< ");" << std::endl;
				}
...
//          << ");" << std::endl;
	...
	std::string generateSourceMultTrans(size_t dims) {
		...
		size_t localWorkgroupSize = parameters.getAsUnsigned("LOCAL_SIZE");
		size_t gridBlockSize = this->parameters.getAsUnsigned("KERNEL_TRANS_GRID_BLOCK_SIZE");
		size_t transDataBlockSize = this->parameters.getAsUnsigned("KERNEL_TRANS_DATA_BLOCK_SIZE");
		bool unroll1DBasisFunctions = this->parameters.getAsBoolean("KERNEL_TRANS_UNROLL_1D");
...
//		stream_program_src << "   int globalSize = get_global_size(0);" << std::endl;
		...
		for (size_t gridIndex = 0; gridIndex < gridBlockSize; gridIndex++) {
			stream_program_src << "   " << streamingModOCLFast::getType<real_type>::asString() << " myResult_"
					<< gridIndex << " = 0.0;" << std::endl << std::endl;
		}

		if (unroll1DBasisFunctions) {
			for (size_t gridIndex = 0; gridIndex < gridBlockSize; gridIndex++) {
				for (size_t d = 0; d < dims; d++) {
					stream_program_src << " " << streamingModOCLFast::getType<real_type>::asString() << " level_"
							<< gridIndex << "_" << d << " = ptrLevel[((" << gridBlockSize << " * groupIdx + "
							<< gridIndex << ") * " << dims << ") + " << d << "];" << std::endl;
					stream_program_src << " " << streamingModOCLFast::getType<real_type>::asString() << " index_"
							<< gridIndex << "_" << d << " = ptrIndex[((" << gridBlockSize << " * groupIdx + "
							<< gridIndex << ") * " << dims << ") + " << d << "];" << std::endl;
				}
			}
		} else {
			for (size_t gridIndex = 0; gridIndex < gridBlockSize; gridIndex++) {
				stream_program_src << "   " << streamingModOCLFast::getType<real_type>::asString() << " level_"
						<< gridIndex << "[" << dims << "];" << std::endl;
				stream_program_src << "   " << streamingModOCLFast::getType<real_type>::asString() << " index_"
						<< gridIndex << "[" << dims << "];" << std::endl;

				for (size_t d = 0; d < dims; d++) {
					stream_program_src << "   level_" << gridIndex << "[" << d << "] = ptrLevel[((" << gridBlockSize
							<< " * groupIdx + " << gridIndex << ") * " << dims << ") +" << d << "];" << std::endl;
					stream_program_src << "   index_" << gridIndex << "[" << d << "] = ptrIndex[((" << gridBlockSize
							<< " * groupIdx + " << gridIndex << ") * " << dims << ") +" << d << "];" << std::endl;
				}
				stream_program_src << std::endl;
			}
		...
		for (size_t gridIndex = 0; gridIndex < gridBlockSize; gridIndex++) {
			for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
				stream_program_src << "     " << streamingModOCLFast::getType<real_type>::asString() << " curSupport_"
						<< gridIndex << "_" << dataIndex << " = ptrSource[k + " << (localWorkgroupSize * dataIndex)
						<< "];" << std::endl;
			}
		}
		stream_program_src << std::endl;
		...
		if (unroll1DBasisFunctions) {
			for (size_t gridIndex = 0; gridIndex < gridBlockSize; gridIndex++) {
				for (size_t d = 0; d < dims; d++) {
					stream_program_src << "         if ((level_" << gridIndex << "_" << d << ") == 2.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << ") {" << std::endl;

					stream_program_src << "         } else if ((index_" << gridIndex << "_" << d << ") == 1.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << ") {" << std::endl;

					for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
						stream_program_src << "             curSupport_" << gridIndex << "_" << dataIndex
								<< " *= max(2.0" << streamingModOCLFast::getType<real_type>::constSuffix()
								<< " - ( (level_" << gridIndex << "_" << d << ") * (ptrData[(" << d
								<< "*sourceSize)+k + " << (localWorkgroupSize * dataIndex) << "]) ), 0.0"
								<< streamingModOCLFast::getType<real_type>::constSuffix() << ");" << std::endl;
					}

					stream_program_src << "         } else if ((index_" << gridIndex << "_" << d << ") == ((level_"
							<< gridIndex << "_" << d << ") - 1.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << ") )" << std::endl;
					stream_program_src << "         {" << std::endl;
					...
					for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
						stream_program_src << "             curSupport_" << gridIndex << "_" << dataIndex
								<< " *= max(( (level_" << gridIndex << "_" << d << ") * (ptrData[(" << d
								<< " * sourceSize)+k + " << (localWorkgroupSize * dataIndex) << "]) ) - (index_"
								<< gridIndex << "_" << d << ") + 1.0, 0.0);" << std::endl;
					}
					...
					for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
						stream_program_src << "             curSupport_" << gridIndex << "_" << dataIndex
								<< " *= max(1.0" << streamingModOCLFast::getType<real_type>::constSuffix()
								<< " - fabs( ( (level_" << gridIndex << "_" << d << ") * (ptrData[(" << d
								<< "*sourceSize)+k + " << (localWorkgroupSize * dataIndex) << "]) ) - (index_"
								<< gridIndex << "_" << d << ") ), 0.0"
								<< streamingModOCLFast::getType<real_type>::constSuffix() << ");" << std::endl;
					}
			...
			for (size_t gridIndex = 0; gridIndex < gridBlockSize; gridIndex++) {
				...
				stream_program_src << "         if ((level_" << gridIndex << "[d]) == 2.0"
						<< streamingModOCLFast::getType<real_type>::constSuffix() << ") {" << std::endl;

				stream_program_src << "         } else if ((index_" << gridIndex << "[d]) == 1.0"
						<< streamingModOCLFast::getType<real_type>::constSuffix() << ") {" << std::endl;

				for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
					stream_program_src << "             curSupport_" << gridIndex << "_" << dataIndex << " *= max(2.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << " - ( (level_" << gridIndex
							<< "[d]) * (ptrData[(d * sourceSize) + k + " << (localWorkgroupSize * dataIndex)
							<< "]) ), 0.0" << streamingModOCLFast::getType<real_type>::constSuffix() << ") ;"
							<< std::endl;
				}

				stream_program_src << "         } else if (index_" << gridIndex << "[d] == (level_" << gridIndex
						<< "[d] - 1.0" << streamingModOCLFast::getType<real_type>::constSuffix() << ")) {" << std::endl;

				for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
					stream_program_src << "             curSupport_" << gridIndex << "_" << dataIndex
							<< " *= max(( level_" << gridIndex << "[d] * (ptrData[(d * sourceSize) + k + "
							<< (localWorkgroupSize * dataIndex) << "]) ) - (index_" << gridIndex << "[d]) + 1.0, 0.0);"
							<< std::endl;
				}
				...
				for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
					stream_program_src << "             curSupport_" << gridIndex << "_" << dataIndex << " *= max(1.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << " - fabs( ( level_"
							<< gridIndex << "[d] * (ptrData[(d * sourceSize) + k + " << (localWorkgroupSize * dataIndex)
							<< "]) ) - index_" << gridIndex << "[d] ), 0.0"
							<< streamingModOCLFast::getType<real_type>::constSuffix() << ");" << std::endl;
				}
		...
		for (size_t gridIndex = 0; gridIndex < gridBlockSize; gridIndex++) {
			for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
				stream_program_src << "      myResult_" << gridIndex << " += curSupport_" << gridIndex << "_"
						<< dataIndex << ";" << std::endl;
			}
		}
		...
		for (size_t gridIndex = 0; gridIndex < gridBlockSize; gridIndex++) {
			...
			stream_program_src << "   resultsTemp[localIdx] = myResult_" << gridIndex << ";" << std::endl << std::endl;
			stream_program_src << "   barrier(CLK_LOCAL_MEM_FENCE);" << std::endl << std::endl;
			...
			stream_program_src << "     }" << std::endl;
			stream_program_src << "     ptrResult[" << gridBlockSize << " * groupIdx + " << gridIndex

...
StreamingModOCLFastKernelSourceBuilder::StreamingModOCLFastKernelSourceBuilder(
  std::shared_ptr<base::OCLOperationConfiguration> parameters,
  size_t dims) :
  parameters(parameters), dims(dims), indent("    "), indent2("        "),
  indent3("            "), indent4(
    "                ") {
...
std::string StreamingModOCLFastKernelSourceBuilder::getData(std::string dim,
...
std::string StreamingModOCLFastKernelSourceBuilder::getData(size_t dim,
    size_t dataBlockingIndex) {
...
std::string
StreamingModOCLFastKernelSourceBuilder::unrolledBasisFunctionEvalulation(
  size_t dims, size_t startDim,
  ...
  for (size_t d = startDim; d < endDim; d++) {
    ...
    output << indent3 << "dimLevelIndex = " << "(m * " << dims << ") + " << dString
           << ";" << std::endl;

    output << indent3 << "if (" << levelAccess << " == 2.0" << this->constSuffix()
           << ") {" << std::endl;
    output << indent3 << "} else if (" << indexAccess << " == 1.0" <<
           this->constSuffix() << ") {" << std::endl;

    for (size_t i = 0; i < dataBlockSize; i++) {
      output << indent4 << "curSupport_" << i << " *= max(2.0" << this->constSuffix()
             << " - (" << levelAccess
             << " * " << getData(dString,
                                 i) << "), 0.0" << this->constSuffix() << ") ;" << std::endl;
    }

    output << indent3 << "} else if (" << indexAccess << " == " << levelAccess <<
           " - 1.0" << this->constSuffix()
           << ")" << std::endl;
    output << indent3 << "{" << std::endl;

    for (size_t i = 0; i < dataBlockSize; i++) {
      output << indent4 << "curSupport_" << i << " *= max((" << levelAccess << " * "
             << getData(dString, i)
             << ") - " << indexAccess << " + 1.0" << this->constSuffix() << ", 0.0" <<
             this->constSuffix()
             << ");" << std::endl;
    }
    ...
    for (size_t i = 0; i < dataBlockSize; i++) {
      output << indent4 << "curSupport_" << i << " *= max(1.0" << this->constSuffix()
             << " - fabs(" << levelAccess
             << " * " << getData(dString,
                                 i) << " - " << indexAccess << "), 0.0" << this->constSuffix() << ");"
             << std::endl;
    }
...
std::string StreamingModOCLFastKernelSourceBuilder::getDataTrans(
  std::string dim, size_t dataBlockingIndex) {
  std::stringstream output;
...
std::string StreamingModOCLFastKernelSourceBuilder::getLevelTrans(
  std::string dim, size_t gridBlockingIndex) {
  std::stringstream output;
...
std::string StreamingModOCLFastKernelSourceBuilder::getIndexTrans(
  std::string dim, size_t gridBlockingIndex) {
  std::stringstream output;
...
std::string
StreamingModOCLFastKernelSourceBuilder::unrolledBasisFunctionEvalulationTrans(
  size_t dims, size_t startDim,
  size_t endDim, std::string unrollVariable, size_t gridBlockIndex) {

  size_t transDataBlockSize =
    (*parameters)["KERNEL_TRANS_DATA_BLOCK_SIZE"].getUInt();
  size_t transGridBlockSize =
    (*parameters)["KERNEL_TRANS_GRID_BLOCK_SIZE"].getUInt();
  ...
  for (size_t d = startDim; d < endDim; d++) {
    ...
    output << indent3 << "dimDataIndex = " << "(" << dString <<
           " * sourceSize) + k;" << std::endl;

    if ((*parameters)["KERNEL_STORE_DATA"].get().compare("pointer") == 0) {
      output << indent3 << "dimLevelIndex = " << "((" << transGridBlockSize <<
             " * groupIdx + " << gridBlockIndex
             << ") * " << dims << ") +" << dString << ";" << std::endl;
    }

    output << indent3 << "if (" << getLevelTrans(dString,
           gridBlockIndex) << " == 2.0" << this->constSuffix()
           << ") {" << std::endl;

    output << indent3 << "} else if (" << getIndexTrans(dString,
           gridBlockIndex) << " == 1.0" << this->constSuffix()
           << ") {" << std::endl;

    for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
      output << indent4 << "curSupport_" << gridBlockIndex << "_" << dataIndex <<
             " *= max(2.0"
             << this->constSuffix() << " - (" << getLevelTrans(dString,
                 gridBlockIndex) << " * "
             << getDataTrans(dString,
                             dataIndex) << "), 0.0" << this->constSuffix() << ") ;" << std::endl;
    }

    output << indent3 << "} else if (" << getIndexTrans(dString,
           gridBlockIndex) << " == ("
           << getLevelTrans(dString,
                            gridBlockIndex) << " - 1.0" << this->constSuffix() << ")) {" << std::endl;

    for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
      output << indent4 << "curSupport_" << gridBlockIndex << "_" << dataIndex <<
             " *= max(("
             << getLevelTrans(dString, gridBlockIndex) << " * " << getDataTrans(dString,
                 dataIndex) << ") - "
    ...
    for (size_t dataIndex = 0; dataIndex < transDataBlockSize; dataIndex++) {
      output << indent4 << "curSupport_" << gridBlockIndex << "_" << dataIndex <<
             " *= max(1.0"
             << this->constSuffix() << " - fabs(" << getLevelTrans(dString,
                 gridBlockIndex) << " * "
             << getDataTrans(dString, dataIndex) << " - " << getIndexTrans(dString,
                 gridBlockIndex) << "), 0.0"
             << this->constSuffix() << ");" << std::endl;
    }
...
std::string StreamingModOCLFastKernelSourceBuilder::reuseSource(
  std::string fileName) {
...
void StreamingModOCLFastKernelSourceBuilder::writeSource(std::string fileName,
    std::string source) {

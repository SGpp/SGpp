...
//    std::cout << "start data: " << gpu_start_data << " end data: " << gpu_end_data << std::endl;
    ...
    for (size_t i = 0; i < num_devices; i++) {
      cl_uint gpu_start_grid = (cl_uint) gpu_start_index_grid[i] / gridBlockingSize;
      cl_uint gpu_end_grid = (cl_uint) gpu_end_index_grid[i] / gridBlockingSize;
//      std::cout << "device: " << i << " start grid: " << gpu_start_grid << " end grid: " << gpu_end_grid << std::endl;
    ...
    for (size_t i = 0; i < num_devices; i++) {
      size_t rangeSize = (gpu_end_index_grid[i] / gridBlockingSize) - (gpu_start_index_grid[i] / gridBlockingSize);
...
//    	std::cout << "enqueuing device: " << i << std::endl;
        ...
        err = clEnqueueNDRangeKernel(command_queue[i], kernel_multTrans[i], 1, &gpu_start_index_grid[i], &totalWorkItems,

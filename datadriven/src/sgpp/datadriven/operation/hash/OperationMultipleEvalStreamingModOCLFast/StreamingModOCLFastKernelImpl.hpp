    ...
    class StreamingModOCLFastKernelImpl {
      ...
      public:
        ...
        double multTranspose(real_type* level, real_type* index, size_t gridSize, real_type* dataset, size_t datasetSize,
          ...
          for (size_t i = 0; i < num_devices; i++) {
            cl_uint gpu_start_grid = (cl_uint) gpu_start_index_grid[i] / (cl_uint) gridBlockingSize;
            cl_uint gpu_end_grid = (cl_uint) gpu_end_index_grid[i] / (cl_uint) gridBlockingSize;

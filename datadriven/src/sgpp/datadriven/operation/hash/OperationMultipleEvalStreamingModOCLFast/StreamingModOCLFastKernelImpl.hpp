...
class StreamingModOCLFastKernelImpl {
 private:
  size_t dims;
    ...
    for (size_t i = 0; i < num_devices; i++) {
      ...
      if (rangeSize > 0) {
        ...
        err = clEnqueueNDRangeKernel(manager->command_queue[i], kernel_multTrans[i], 1,
                                     ...
                                     &totalWorkItems, &local, 0, nullptr, &(clTimings[i]));

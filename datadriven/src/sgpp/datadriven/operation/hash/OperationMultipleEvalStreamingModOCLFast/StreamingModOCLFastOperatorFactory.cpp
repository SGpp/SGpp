...
base::OperationMultipleEval* createStreamingModOCLFastConfigured(
  ...
  if (configuration.getParameters().operator bool()) {
    ...
    parameters =
      std::make_shared<base::OCLOperationConfiguration>("StreamingModOCLFast.cfg");
    ...
    if ((*parameters).contains("KERNEL_TRANS_UNROLL_1D") == false) {
      (*parameters).addTextAttr("KERNEL_TRANS_UNROLL_1D", "true");
    }
  ...
  if ((*parameters)["INTERNAL_PRECISION"].get() == "float") {
    return new datadriven::OperationMultiEvalStreamingModOCLFast<float>(grid,
           dataset, parameters);
  } else if ((*parameters)["INTERNAL_PRECISION"].get() == "double") {
    return new datadriven::OperationMultiEvalStreamingModOCLFast<double>(grid,
           dataset, parameters);
  } else {

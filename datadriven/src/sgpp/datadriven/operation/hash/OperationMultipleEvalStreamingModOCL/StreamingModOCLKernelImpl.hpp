...
#include <sgpp/datadriven/operation/hash/OperationMultipleEvalStreamingModOCL/StreamingModOCLKernelImpl.hpp>
...
#include <sgpp/datadriven/operation/hash/OperationMultipleEvalStreamingModOCL/StreamingModOCLKernelSourceBuilder.hpp>

namespace SGPP {
namespace datadriven {

template<typename real_type>
class StreamingModOCLKernelImpl {
 private:
  size_t dims;
 ...
 public:
  ...
  StreamingModOCLKernelImpl(size_t dims,
                            std::shared_ptr<base::OCLManager> manager,
                            std::shared_ptr<base::OCLOperationConfiguration> parameters) :
    ...
    for (size_t i = 0; i < num_devices; i++) {
      ...
      if (gpu_end_data > gpu_start_data) {
        if (clSetKernelArg(kernel_mult[i], 0, sizeof(cl_mem),
            ...
            clSetKernelArg(kernel_mult[i], 5, sizeof(cl_uint), &clResultSize)
      ...
      if (gpu_end_grid > gpu_start_grid) {
        if (clSetKernelArg(kernel_multTrans[i], 0, sizeof(cl_mem),
            ...
            clSetKernelArg(kernel_multTrans[i], 4, sizeof(cl_mem),
                           this->deviceGrid.getBuffer(i)) ||
            clSetKernelArg(kernel_multTrans[i], 5, sizeof(cl_uint), &clSourceSize)
 ...
 private:
  ...
  void initOCLBuffers(real_type* level, real_type* index, size_t gridSize,
    ...
    if (level != nullptr && !this->deviceLevel.isInitialized()) {
      this->deviceLevel.initializeBuffer(level, sizeof(real_type),
                                         gridSize * this->dims);
    }

    if (index != nullptr && !this->deviceIndex.isInitialized()) {
      this->deviceIndex.initializeBuffer(index, sizeof(real_type),
                                         gridSize * this->dims);
    }

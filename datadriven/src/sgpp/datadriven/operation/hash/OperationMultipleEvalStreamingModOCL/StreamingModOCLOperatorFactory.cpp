...
base::OperationMultipleEval* createStreamingModOCLConfigured(base::Grid& grid,
  ...
  if (configuration.getParameters().operator bool()) {
    ...
    parameters =
      std::make_shared<base::OCLOperationConfiguration>("StreamingModOCL.cfg");
  ...
  if ((*parameters)["INTERNAL_PRECISION"].get() == "float") {
    return new datadriven::OperationMultiEvalStreamingModOCL<float>(grid, dataset,
           parameters);
  } else if ((*parameters)["INTERNAL_PRECISION"].get() == "double") {
    return new datadriven::OperationMultiEvalStreamingModOCL<double>(grid, dataset,
           parameters);
  } else {

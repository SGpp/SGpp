...
class OperationMultiEvalStreamingModOCL: public base::OperationMultipleEval {
 ...
 public:
  ...
  OperationMultiEvalStreamingModOCL(base::Grid& grid, base::DataMatrix& dataset,
                                    std::shared_ptr<base::OCLOperationConfiguration> parameters) :
    ...
    this->kernel = std::unique_ptr<StreamingModOCLKernelImpl<T>>
                   (new StreamingModOCLKernelImpl<T>(dims, this->manager, parameters));

...
class StreamingModOCLKernelSourceBuilder {
 ...
 public:
  StreamingModOCLKernelSourceBuilder(
    std::shared_ptr<base::OCLOperationConfiguration> parameters) :
  ...
  std::string generateSourceMult(size_t dims) {
    ...
    stream_program_src << "void multOCL(__global const " <<
                       streamingModOCL::getType<real_type>::asString()
                       << "* ptrLevel," << std::endl;
    ...
    if (useLocalMemory) {
      stream_program_src << "   __local " <<
                         streamingModOCL::getType<real_type>::asString() << " locLevel["
                         << dims* localWorkgroupSize << "];" << std::endl;
      stream_program_src << "   __local " <<
                         streamingModOCL::getType<real_type>::asString() << " locIndex["
                         << dims* localWorkgroupSize << "];" << std::endl;
      stream_program_src << "   __local " <<
                         streamingModOCL::getType<real_type>::asString() << " locAlpha["
    ...
    if (useLocalMemory) {
      stream_program_src <<
                         "   // Iterate over all grid points (fast ones, with cache)" << std::endl;
      ...
      for (size_t d = 0; d < dims; d++) {
        stream_program_src << "     locLevel[(localIdx*" << dims << ")+" << d <<
                           "] = ptrLevel[((j+localIdx)*" << dims
                           << ")+" << d << "];" << std::endl;
        stream_program_src << "     locIndex[(localIdx*" << dims << ")+" << d <<
                           "] = ptrIndex[((j+localIdx)*" << dims
                           << ")+" << d << "];" << std::endl;
      }
      ...
      stream_program_src << std::endl;
      stream_program_src << "       for(int k = 0; k < " << localWorkgroupSize <<
                         "; k++)" << std::endl;
      stream_program_src << "       {" << std::endl;

      for (size_t i = 0; i < dataBlockSize; i++) {
        stream_program_src << "           curSupport_" << i << " = locAlpha[k];" <<
                           std::endl << std::endl;
      }
      ...
      for (size_t d = 0; d < dims; d++) {
        ...
        for (size_t i = 0; i < dataBlockSize; i++) {
          stream_program_src << "             curSupport_" << i << " *= max(2.0"
                             << streamingModOCL::getType<real_type>::constSuffix() << " - ( (locLevel[(k*" <<
                             dims << ")+" << d
                             << "]) * (data_" << i << "_" << d << ") ), 0.0" <<
                             streamingModOCL::getType<real_type>::constSuffix()
                             << ") ;" << std::endl;
        }
        ...
        stream_program_src << "         else if ((locIndex[(k*" << dims << ")+" << d <<
                           "]) == ((locLevel[(k*" << dims
                           << ")+" << d << "]) - 1.0" << streamingModOCL::getType<real_type>::constSuffix()
                           << ") )" << std::endl;
        stream_program_src << "         {" << std::endl;

        for (size_t i = 0; i < dataBlockSize; i++) {
          stream_program_src << "             curSupport_" << i <<
                             " *= max(( (locLevel[(k*" << dims << ")+" << d
                             << "]) * (data_" << i << "_" << d << ") ) - (locIndex[(k*" << dims << ")+" << d
                             << "]) + 1.0"
      ...
      for (size_t i = 0; i < dataBlockSize; i++) {
        stream_program_src << "           myResult_" << i << " += curSupport_" << i <<
                           ";" << std::endl;
      }
      ...
      for (size_t i = 0; i < dataBlockSize; i++) {
        stream_program_src << "       curSupport_" << i << " = ptrAlpha[m];" <<
                           std::endl << std::endl;
      }

      for (size_t d = 0; d < dims; d++) {
        stream_program_src << "     if ((ptrLevel[(m*" << dims << ")+" << d <<
                           "]) == 2.0"
                           << streamingModOCL::getType<real_type>::constSuffix() << ")" << std::endl;
        stream_program_src << "     {" << std::endl;
        //        stream_program_src << "         curSupport *= 1.0" << streamingModOCL::getType<real_type>::constSuffix() << ";"
        ...
        for (size_t i = 0; i < dataBlockSize; i++) {
          stream_program_src << "         curSupport_" << i << " *= max(2.0"
                             << streamingModOCL::getType<real_type>::constSuffix() << " - ( (ptrLevel[(m*" <<
                             dims << ")+" << d
                             << "]) * (data_" << i << "_" << d << ") ), 0.0" <<
                             streamingModOCL::getType<real_type>::constSuffix()
                             << ") ;" << std::endl;
        ...
        for (size_t i = 0; i < dataBlockSize; i++) {
          stream_program_src << "         curSupport_" << i << " *= max(( (ptrLevel[(m*"
                             << dims << ")+" << d
    ...
    for (size_t i = 0; i < dataBlockSize; i++) {
      ...
      stream_program_src << "   ptrResult[(" << dataBlockSize << " * globalIdx) + " <<
                         i << "] = myResult_" << i << ";"
                         << std::endl;
    }
  ...
  std::string generateSourceMultTrans(size_t dims) {
    ...
    if (useLocalMemory) {
      stream_program_src << "   __local " <<
                         streamingModOCL::getType<real_type>::asString() << " locData["
                         << dims* localWorkgroupSize << "];" << std::endl;
      stream_program_src << "   __local " <<
                         streamingModOCL::getType<real_type>::asString() << " locSource["
    ...
    for (size_t d = 0; d < dims; d++) {
      ...
      if (useLocalMemory) {
        stream_program_src << "             curSupport *= max(2.0" <<
                           streamingModOCL::getType<real_type>::constSuffix()
                           << " - ( (level_" << d << ") * (locData[(" << d << "*" << localWorkgroupSize <<
                           ")+k]) ), 0.0"
                           << streamingModOCL::getType<real_type>::constSuffix() << ") ;" << std::endl;
      } else {
        stream_program_src << "             curSupport *= max(2.0" <<
      ...
      if (useLocalMemory) {
        stream_program_src << "             curSupport *= max(1.0" <<
                           streamingModOCL::getType<real_type>::constSuffix()
                           << " - fabs( ( (level_" << d << ") * (locData[(" << d << "*" <<
                           localWorkgroupSize << ")+k]) ) - (index_"
                           << d << ") ), 0.0" << streamingModOCL::getType<real_type>::constSuffix() << ");"
                           << std::endl;
      } else {
        stream_program_src << "             curSupport *= max(1.0" <<
                           streamingModOCL::getType<real_type>::constSuffix()
                           << " - fabs( ( (level_" << d << ") * (ptrData[(" << d <<
                           "*sourceSize)+k]) ) - (index_" << d << ") ), 0.0"
                           << streamingModOCL::getType<real_type>::constSuffix() << ");" << std::endl;
      }

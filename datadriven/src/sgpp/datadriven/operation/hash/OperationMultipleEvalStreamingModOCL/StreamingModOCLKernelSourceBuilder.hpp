...
public:
  ...
  std::string generateSourceMult(size_t dims) {
    ...
    if (streamingModOCL::getType<real_type>::asString() == "double") {
    ...
    stream_program_src << "void multOCL(__global const " << streamingModOCL::getType<real_type>::asString() << "* ptrLevel,"
        << std::endl;
    stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString() << "* ptrIndex," << std::endl;
//    stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString() << "* ptrMask," << std::endl; // not needed for this kernel, but there for uniformity
//    stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString() << "* ptrOffset," << std::endl; // not needed for this kernel, but there for uniformity
    stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString() << "* ptrData," << std::endl;
    stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString() << "* ptrAlpha," << std::endl;
    stream_program_src << "           __global       " << streamingModOCL::getType<real_type>::asString() << "* ptrResult," << std::endl;
    ...
    stream_program_src << std::endl;
    if (useLocalMemory) {
      stream_program_src << "   __local " << streamingModOCL::getType<real_type>::asString() << " locLevel[" << dims * localWorkgroupSize
          << "];" << std::endl;
      stream_program_src << "   __local " << streamingModOCL::getType<real_type>::asString() << " locIndex[" << dims * localWorkgroupSize
          << "];" << std::endl;
      stream_program_src << "   __local " << streamingModOCL::getType<real_type>::asString() << " locAlpha[" << localWorkgroupSize
          << "];" << std::endl;
      stream_program_src << std::endl;
    }
    stream_program_src << "   " << streamingModOCL::getType<real_type>::asString()
        << " eval, index_calc, abs, last, localSupport, curSupport;" << std::endl << std::endl;
    stream_program_src << "   " << streamingModOCL::getType<real_type>::asString() << " myResult = ptrResult[globalIdx];" << std::endl
        << std::endl;
    stream_program_src << "   // Create registers for the data" << std::endl;

    for (size_t d = 0; d < dims; d++) {
      stream_program_src << " " << streamingModOCL::getType<real_type>::asString() << " data_" << d
    ...
    if (useLocalMemory) {
      ...
      for (size_t d = 0; d < dims; d++) {
        stream_program_src << "         if ((locLevel[(k*" << dims << ")+" << d << "]) == 2.0"
            << streamingModOCL::getType<real_type>::constSuffix() << ")" << std::endl;
        stream_program_src << "         {" << std::endl;
        stream_program_src << "             curSupport *= 1.0" << streamingModOCL::getType<real_type>::constSuffix() << ";" << std::endl;
        stream_program_src << "         }" << std::endl;
        stream_program_src << "         else if ((locIndex[(k*" << dims << ")+" << d << "]) == 1.0"
            << streamingModOCL::getType<real_type>::constSuffix() << ")" << std::endl;
        stream_program_src << "         {" << std::endl;
        stream_program_src << "             curSupport *= max(2.0" << streamingModOCL::getType<real_type>::constSuffix()
            << " - ( (locLevel[(k*" << dims << ")+" << d << "]) * (data_" << d << ") ), 0.0"
            << streamingModOCL::getType<real_type>::constSuffix() << ") ;" << std::endl;
        stream_program_src << "         }" << std::endl;
        stream_program_src << "         else if ((locIndex[(k*" << dims << ")+" << d << "]) == ((locLevel[(k*" << dims
            << ")+" << d << "]) - 1.0" << streamingModOCL::getType<real_type>::constSuffix() << ") )" << std::endl;
        stream_program_src << "         {" << std::endl;
        stream_program_src << "             curSupport *= max(( (locLevel[(k*" << dims << ")+" << d << "]) * (data_"
            << d << ") ) - (locIndex[(k*" << dims << ")+" << d << "]) + 1.0" << streamingModOCL::getType<real_type>::constSuffix()
            << ", 0.0" << streamingModOCL::getType<real_type>::constSuffix() << ");" << std::endl;
        stream_program_src << "         }" << std::endl;
        stream_program_src << "         else " << std::endl;
        stream_program_src << "         {" << std::endl;
        stream_program_src << "             curSupport *= max(1.0" << streamingModOCL::getType<real_type>::constSuffix()
            << " - fabs( ( (locLevel[(k*" << dims << ")+" << d << "]) * (data_" << d << ") ) - (locIndex[(k*" << dims
            << ")+" << d << "]) ), 0.0" << streamingModOCL::getType<real_type>::constSuffix() << ");" << std::endl;
        stream_program_src << "         }" << std::endl;
      }
      ...
      for (size_t d = 0; d < dims; d++) {
        ...
        stream_program_src << "         curSupport *= max(( (ptrLevel[(m*" << dims << ")+" << d << "]) * (data_" << d
            << ") ) - (ptrIndex[(m*" << dims << ")+" << d << "]) + 1.0" << streamingModOCL::getType<real_type>::constSuffix() << ", 0.0"
            << streamingModOCL::getType<real_type>::constSuffix() << ");" << std::endl;
        stream_program_src << "     }" << std::endl;
        stream_program_src << "     else " << std::endl;
        stream_program_src << "     {" << std::endl;
    ...
    multFile.open("multKernelMod_tmp.cl");
  ...
  std::string generateSourceMultTrans(size_t dims) {
    ...
    stream_program_src << "__kernel" << std::endl;
    stream_program_src << "__attribute__((reqd_work_group_size(" << localWorkgroupSize << ", 1, 1)))" << std::endl;
    stream_program_src << "void multTransOCL(__global const " << streamingModOCL::getType<real_type>::asString() << "* ptrLevel,"
        << std::endl;
    stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString() << "* ptrIndex," << std::endl;
//    stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString() << "* ptrMask," << std::endl; // not needed for this kernel, but there for uniformity
//    stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString() << "* ptrOffset," << std::endl; // not needed for this kernel, but there for uniformity
    stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString() << "* ptrData," << std::endl;
    stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString() << "* ptrSource," << std::endl;
    stream_program_src << "           __global       " << streamingModOCL::getType<real_type>::asString() << "* ptrResult," << std::endl;
    ...
    stream_program_src << "   " << streamingModOCL::getType<real_type>::asString() << " myResult = ptrResult[globalIdx];" << std::endl
        << std::endl;
    if (useLocalMemory) {
      stream_program_src << "   __local " << streamingModOCL::getType<real_type>::asString() << " locData[" << dims * localWorkgroupSize
          << "];" << std::endl;
      stream_program_src << "   __local " << streamingModOCL::getType<real_type>::asString() << " locSource[" << localWorkgroupSize
          << "];" << std::endl << std::endl;
    }

    for (size_t d = 0; d < dims; d++) {
      stream_program_src << " " << streamingModOCL::getType<real_type>::asString() << " level_" << d << " = ptrLevel[(globalIdx*" << dims
          << ")+" << d << "];" << std::endl;
      stream_program_src << " " << streamingModOCL::getType<real_type>::asString() << " index_" << d << " = ptrIndex[(globalIdx*" << dims
          << ")+" << d << "];" << std::endl;
    }
    ...
    for (size_t d = 0; d < dims; d++) {
      stream_program_src << "         if ((level_" << d << ") == 2.0" << streamingModOCL::getType<real_type>::constSuffix() << ")"
          << std::endl;
      stream_program_src << "         {" << std::endl;
      stream_program_src << "             curSupport *= 1.0" << streamingModOCL::getType<real_type>::constSuffix() << ";" << std::endl;
      stream_program_src << "         }" << std::endl;
      stream_program_src << "         else if ((index_" << d << ") == 1.0" << streamingModOCL::getType<real_type>::constSuffix() << ")"
          << std::endl;
      stream_program_src << "         {" << std::endl;
      if (useLocalMemory) {
        stream_program_src << "             curSupport *= max(2.0" << streamingModOCL::getType<real_type>::constSuffix()
      ...
      if (useLocalMemory) {
        stream_program_src << "             curSupport *= max(( (level_" << d << ") * (locData[(" << d << "*"
            << localWorkgroupSize << ")+k]) ) - (index_" << d << ") + 1.0" << streamingModOCL::getType<real_type>::constSuffix()
            << ", 0.0" << streamingModOCL::getType<real_type>::constSuffix() << ");" << std::endl;
      } else {
    ...
    multTransFile.open("multTransKernelMod_tmp.cl");

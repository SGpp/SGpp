...
namespace SGPP {
  namespace datadriven {
    ...
    class StreamingModOCLKernelSourceBuilder {
      ...
      public:
        StreamingModOCLKernelSourceBuilder(std::shared_ptr<base::OCLOperationConfiguration> parameters) :
        ...
        std::string generateSourceMult(size_t dims) {
          ...
          stream_program_src << "void multOCL(__global const " << streamingModOCL::getType<real_type>::asString()
                             << "* ptrLevel," << std::endl;
          stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString()
                             << "* ptrIndex," << std::endl;
          ...
          stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString()
                             << "* ptrAlpha," << std::endl;
          stream_program_src << "           __global       " << streamingModOCL::getType<real_type>::asString()
                             << "* ptrResult," << std::endl;
          ...
          if (useLocalMemory) {
            stream_program_src << "   __local " << streamingModOCL::getType<real_type>::asString() << " locLevel["
                               << dims* localWorkgroupSize << "];" << std::endl;
            stream_program_src << "   __local " << streamingModOCL::getType<real_type>::asString() << " locIndex["
                               << dims* localWorkgroupSize << "];" << std::endl;
            stream_program_src << "   __local " << streamingModOCL::getType<real_type>::asString() << " locAlpha["
          ...
          for (size_t i = 0; i < dataBlockSize; i++) {
            stream_program_src << "   " << streamingModOCL::getType<real_type>::asString() << " curSupport_" << i << ";"
                               << std::endl;
            //      stream_program_src << "   " << streamingModOCL::getType<real_type>::asString() << " myResult_" << i
            //          << " = ptrResult[globalIdx];" << std::endl << std::endl;
            stream_program_src << "   " << streamingModOCL::getType<real_type>::asString() << " myResult_" << i << " = 0.0;"
                               << std::endl << std::endl;
          }
          ...
          for (size_t i = 0; i < dataBlockSize; i++) {
            for (size_t d = 0; d < dims; d++) {
              stream_program_src << " " << streamingModOCL::getType<real_type>::asString() << " data_" << i << "_" << d
          ...
          if (useLocalMemory) {
            ...
            for (size_t d = 0; d < dims; d++) {
              ...
              stream_program_src << "         {" << std::endl;
              //        stream_program_src << "             curSupport *= 1.0" << streamingModOCL::getType<real_type>::constSuffix()
              //            << ";" << std::endl;
              stream_program_src << "         }" << std::endl;
              ...
              for (size_t i = 0; i < dataBlockSize; i++) {
                stream_program_src << "             curSupport_" << i << " *= max(2.0"
                                   << streamingModOCL::getType<real_type>::constSuffix() << " - ( (locLevel[(k*" << dims << ")+" << d
                                   << "]) * (data_" << i << "_" << d << ") ), 0.0" << streamingModOCL::getType<real_type>::constSuffix()
                                   << ") ;" << std::endl;
              }
              ...
              for (size_t i = 0; i < dataBlockSize; i++) {
                stream_program_src << "             curSupport_" << i << " *= max(( (locLevel[(k*" << dims << ")+" << d
                                   ...
                                   << streamingModOCL::getType<real_type>::constSuffix() << ", 0.0"
                                   << streamingModOCL::getType<real_type>::constSuffix() << ");" << std::endl;
              }
              ...
              for (size_t i = 0; i < dataBlockSize; i++) {
                stream_program_src << "             curSupport_" << i << " *= max(1.0"
                                   << streamingModOCL::getType<real_type>::constSuffix() << " - fabs( ( (locLevel[(k*" << dims << ")+" << d
                                   << "]) * (data_" << i << "_" << d << ") ) - (locIndex[(k*" << dims << ")+" << d << "]) ), 0.0"
                                   << streamingModOCL::getType<real_type>::constSuffix() << ");" << std::endl;
              }
            ...
            for (size_t d = 0; d < dims; d++) {
              ...
              for (size_t i = 0; i < dataBlockSize; i++) {
                stream_program_src << "         curSupport_" << i << " *= max(2.0"
                                   << streamingModOCL::getType<real_type>::constSuffix() << " - ( (ptrLevel[(m*" << dims << ")+" << d
                                   << "]) * (data_" << i << "_" << d << ") ), 0.0" << streamingModOCL::getType<real_type>::constSuffix()
                                   << ") ;" << std::endl;
              }
              ...
              for (size_t i = 0; i < dataBlockSize; i++) {
                stream_program_src << "         curSupport_" << i << " *= max(1.0"
                                   << streamingModOCL::getType<real_type>::constSuffix() << " - fabs( ( (ptrLevel[(m*" << dims << ")+" << d
                                   << "]) * (data_" << i << "_" << d << ") ) - (ptrIndex[(m*" << dims << ")+" << d << "]) ), 0.0"
                                   << streamingModOCL::getType<real_type>::constSuffix() << ");" << std::endl;
              }
        ...
        std::string generateSourceMultTrans(size_t dims) {
          ...
          stream_program_src << "__kernel" << std::endl;
          stream_program_src << "__attribute__((reqd_work_group_size(" << localWorkgroupSize << ", 1, 1)))" << std::endl;
          stream_program_src << "void multTransOCL(__global const " << streamingModOCL::getType<real_type>::asString()
                             << "* ptrLevel," << std::endl;
          stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString()
                             << "* ptrIndex," << std::endl;
          //    stream_program_src << "           __global const " << streamingModOCL::getType<real_type>::asString() << "* ptrMask," << std::endl; // not needed for this kernel, but there for uniformity
          ...
          if (useLocalMemory) {
            stream_program_src << "   __local " << streamingModOCL::getType<real_type>::asString() << " locData["
                               << dims* localWorkgroupSize << "];" << std::endl;
            stream_program_src << "   __local " << streamingModOCL::getType<real_type>::asString() << " locSource["
                               << localWorkgroupSize << "];" << std::endl << std::endl;
          }

          for (size_t d = 0; d < dims; d++) {
            stream_program_src << " " << streamingModOCL::getType<real_type>::asString() << " level_" << d
                               << " = ptrLevel[(globalIdx*" << dims << ")+" << d << "];" << std::endl;
            stream_program_src << " " << streamingModOCL::getType<real_type>::asString() << " index_" << d
          ...
          for (size_t d = 0; d < dims; d++) {
            ...
            if (useLocalMemory) {
              stream_program_src << "             curSupport *= max(( (level_" << d << ") * (locData[(" << d << "*"
                                 << localWorkgroupSize << ")+k]) ) - (index_" << d << ") + 1.0"
                                 << streamingModOCL::getType<real_type>::constSuffix() << ", 0.0"
                                 << streamingModOCL::getType<real_type>::constSuffix() << ");" << std::endl;
            } else {

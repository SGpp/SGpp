...
void OperationDensitySampling1DLinear::doSampling1D(base::DataVector* alpha,
    size_t num_samples, base::DataVector*& samples, unsigned int* seedp) {
  ...
  for (unsigned int i = 0; i < gs->size(); i++) {
    ...
    coord_pdf.insert( std::pair<float_t, float_t>(coord[0], opEval->eval(*alpha,
                      coord)) );

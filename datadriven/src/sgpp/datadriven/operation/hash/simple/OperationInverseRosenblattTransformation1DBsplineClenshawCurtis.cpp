 ...
 */
OperationInverseRosenblattTransformation1DBsplineClenshawCurtis::
    OperationInverseRosenblattTransformation1DBsplineClenshawCurtis(
        base::Grid* grid)
...
void OperationInverseRosenblattTransformation1DBsplineClenshawCurtis::init(
  ...
  double area = 0.0;
  double right_coord = 0, right_function_value = 0;
  size_t p =
      dynamic_cast<sgpp::base::BsplineClenshawCurtisGrid*>(grid)->getDegree();
  quadOrder = (p + 1) / 2 + 2;
  ...
  for (size_t i = 1; i < ordered_grid_points.size(); i++) {
    ...
    // this will always be initialized to false except if we are in the first
    // patch
...
double OperationInverseRosenblattTransformation1DBsplineClenshawCurtis::sample(
...
double OperationInverseRosenblattTransformation1DBsplineClenshawCurtis::

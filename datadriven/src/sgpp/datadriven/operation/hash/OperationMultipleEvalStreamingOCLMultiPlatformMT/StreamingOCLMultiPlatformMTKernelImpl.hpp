...
template<typename T>
class StreamingOCLMultiPlatformMTKernelImpl {
private:
    size_t dims;
...
//    std::vector<T> hostTemp;
    ...
    StreamingOCLMultiPlatformMTKernelSourceBuilder kernelSourceBuilder;
...
public:
    ...
    StreamingOCLMultiPlatformMTKernelImpl(size_t dims, std::shared_ptr<base::OCLManagerMultiPlatform> manager,
            std::shared_ptr<base::OCLConfigurationParameters> parameters) :
...
//        }
    ...
    ~StreamingOCLMultiPlatformMTKernelImpl() {
        for (base::OCLPlatformWrapper &platform : manager->platforms) {

...
class AdaptiveOCLKernelSourceBuilder {
 ...
 public:
  explicit AdaptiveOCLKernelSourceBuilder(
      std::shared_ptr<base::OCLOperationConfiguration> parameters)
  ...
  std::string generateSourceMult(size_t dims) {
    ...
    stream_program_src << "__attribute__((reqd_work_group_size(" << localWorkgroupSize
                       << ", 1, 1)))" << std::endl;
    stream_program_src << "void multOCL(__global const " << strType << "* ptrStreamingArray, "
                       << std::endl;
    stream_program_src << "           __global const " << strType << "* ptrSubspaceArray,"
                       << std::endl;
    ...
    stream_program_src << "  " << strType << " index_calc, abs, last, localSupport, curSupport;"
                       << std::endl
                       << std::endl;
    ...
    if (dims > maxDimUnroll) {
      ...
      for (size_t d = 0; d < dims; d++) {
        stream_program_src << "    " << strType << " level_" << d << " = ptrMetaInfo[l*metaSize + "
                           << d << " + 3];" << std::endl;
        stream_program_src << "    " << strType << " eval_" << d << " = (pow(2.0"
                           << AdaptiveOCL::getType<real_type>::constSuffix() << ",level_" << d
                           << ") * (data_" << d << "));" << std::endl;
      }
    }
    ...
    stream_program_src << "                index_calc = eval[d] - "
                          "ptrStreamingArray[streamElementSize*(startIndex + i) + d];" << std::endl;
    ...
    stream_program_src << "            floored = (int)(data[d]*pow(2.0"
                       << AdaptiveOCL::getType<real_type>::constSuffix() << ",level[d]));"
                       << std::endl;
    stream_program_src << "            index[d] = (" << strType << ")(floored | 0x1);" << std::endl;
    stream_program_src << "        }" << std::endl;
    // LINEAR INDEX
    stream_program_src << "        //calc linear index of gridpoint (relative to subspace)"
                       << std::endl;
  ...
  std::string generateSourceMultTrans(size_t dims) {
    ...
    if (strPrecisionType == "double") {
      stream_program_src << "#pragma OPENCL EXTENSION cl_khr_fp64 : enable" << std::endl;
      stream_program_src << "#pragma OPENCL EXTENSION cl_khr_int64_base_atomics : enable"
                         << std::endl
                         << std::endl;
      ...
      stream_program_src << "  } while (atom_cmpxchg((volatile __global long *)source, "
                            "prevVal.intVal, newVal.intVal) != prevVal.intVal);" << std::endl;
      stream_program_src << "}" << std::endl
                         << std::endl;
    } else {
      stream_program_src << "#pragma OPENCL EXTENSION cl_khr_fp64 : enable" << std::endl;
      stream_program_src << "#pragma OPENCL EXTENSION cl_khr_global_int32_base_atomics : enable"
                         << std::endl
                         << std::endl;
      ...
      stream_program_src << "    prevVal.floatVal = *source;" << std::endl;
      stream_program_src << "    newVal.floatVal = prevVal.floatVal + operand;" << std::endl;
      stream_program_src << "  } while (atomic_cmpxchg((volatile __global uint *)source, "
                            "prevVal.intVal, newVal.intVal) != prevVal.intVal);" << std::endl;
      stream_program_src << "}" << std::endl
                         << std::endl;
    }
    ...
    stream_program_src << "           __global const " << strPrecisionType << "* ptrSubspaceArray,"
                       << std::endl;
    stream_program_src << "           __global const uint* ptrMetaInfo," << std::endl;
    stream_program_src << "           __global const " << strPrecisionType << "* ptrData,"
                       << std::endl;
    stream_program_src << "           __global const " << strPrecisionType << "* ptrSource,"
                       << std::endl;
    stream_program_src << "           __global       " << strPrecisionType << "* ptrResult,"
                       << std::endl;
    ...
    stream_program_src << "    " << strPrecisionType
                       << " index_calc, abs, last, localSupport, curSupport;" << std::endl
                       << std::endl;
    ...
    if (dims > maxDimUnroll) {
      ...
      for (size_t d = 0; d < dims; d++) {
        ...
        stream_program_src << "    " << strPrecisionType << " eval_" << d << " = (pow(2,level_" << d
                           << ") * (data_" << d << "));" << std::endl;
      }
    }
    ...
    stream_program_src << "                floored = (int)(data[d]*pow(2.0"
                       << AdaptiveOCL::getType<real_type>::constSuffix() << ",level[d]));"
                       << std::endl;
    stream_program_src << "                index[d] = (" << strPrecisionType << ")(floored | 0x1);"
                       << std::endl;
    stream_program_src << "            }" << std::endl;
    // LINEAR INDEX
    stream_program_src << "            //calc linear index of gridpoint (relative to subspace)"
    ...
    stream_program_src << "            " << strPrecisionType << " level_calc = 0.0"
                       << AdaptiveOCL::getType<real_type>::constSuffix() << ";" << std::endl;
    stream_program_src << "            for (size_t d = 0; d < " << dims - 1 << "; d++) {"
                       << std::endl;
    stream_program_src << "                index_half = (int)index[d] >> 1;" << std::endl;
    stream_program_src << "                level_calc = pow(2.0"
                       << AdaptiveOCL::getType<real_type>::constSuffix() << ", level[d+1]-1);"

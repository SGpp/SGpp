...
class AdaptiveOCLKernelImpl {
 ...
 public:
  ...
  AdaptiveOCLKernelImpl(size_t dims, std::shared_ptr<base::OCLManager> manager,
                        std::shared_ptr<base::OCLOperationConfiguration> parameters) :
    deviceData(manager), deviceStreamArray(manager), deviceSubspaceArray(manager),
    deviceMetaInfo(manager), deviceAlpha(manager), deviceTemp(manager),
    kernelSourceBuilder(
      parameters), manager(manager), parameters(parameters), multLoadBalancer(manager,
          this->parameters), multTransposeLoadBalancer(
  ...
  double mult(real_type* streamingArray, real_type* subspaceArray,
              uint32_t* metaInfo, size_t gridSize, real_type* dataset, size_t datasetSize,
              real_type* alpha, real_type* result, const size_t start_index_grid,
    ...
    if (kernel_mult[0] == nullptr) {
      this->createMult(this->dims, (*parameters)["LOCAL_SIZE"].getUInt(),
                       manager->context, num_devices, device_ids,
                       kernel_mult);
    }

    initOCLBuffers(streamingArray, subspaceArray, metaInfo, gridSize, dataset,
                   datasetSize, numSubspaces, numStreamingSubs, numSubSubs);
    ...
    for (size_t i = 0; i < num_devices; i++) {
      ...
      if (gpu_end_data > gpu_start_data) {
        if (clSetKernelArg(kernel_mult[i], 0, sizeof(cl_mem),
            ...
            clSetKernelArg(kernel_mult[i], 4, sizeof(cl_mem),
                           this->deviceAlpha.getBuffer(i)) ||
            clSetKernelArg(kernel_mult[i], 5, sizeof(cl_mem),
                           this->deviceTemp.getBuffer(i)) ||
            clSetKernelArg(kernel_mult[i], 6, sizeof(cl_uint), &clResultSize)
            || // resultsize == number of entries in dataset
            clSetKernelArg(kernel_mult[i], 7, sizeof(cl_uint),
                           &clNumSubspaces) != CL_SUCCESS) {
    ...
    for (size_t i = 0; i < num_devices; i++) {
      ...
      if (rangeSize > 0) {
        err = clEnqueueNDRangeKernel(manager->command_queue[i], kernel_mult[i], 1,
                                     &gpu_start_index_data[i], &rangeSize,
  ...
  double multTranspose(real_type* streamingArray, real_type* subspaceArray,
                       uint32_t* metaInfo, size_t gridSize, real_type* dataset, size_t datasetSize,
                       real_type* source, real_type* result, const size_t start_index_grid,
                       const size_t end_index_grid,
                       const size_t start_index_data, const size_t end_index_data, size_t numSubspaces,
                       uint32_t numStreamingSubs, uint32_t numSubSubs) {
    ...
    if (kernel_multTrans[0] == nullptr) {
      this->createMultTrans(this->dims, (*parameters)["LOCAL_SIZE"].getUInt(),
                            manager->context, num_devices, device_ids,
                            kernel_multTrans);
    }
      ...
      if (gpu_end_data > gpu_start_data) {
        if (clSetKernelArg(kernel_multTrans[i], 0, sizeof(cl_mem),
            ...
            clSetKernelArg(kernel_multTrans[i], 2, sizeof(cl_mem),
                           this->deviceMetaInfo.getBuffer(i)) ||
            clSetKernelArg(kernel_multTrans[i], 3, sizeof(cl_mem),
                           this->deviceData.getBuffer(i)) ||
            clSetKernelArg(kernel_multTrans[i], 4, sizeof(cl_mem),
                           this->deviceTemp.getBuffer(i)) ||
            clSetKernelArg(kernel_multTrans[i], 5, sizeof(cl_mem),
                           this->deviceAlpha.getBuffer(i)) ||
            clSetKernelArg(kernel_multTrans[i], 6, sizeof(cl_uint), &clSourceSize)
            || // sourceSize == number of entries in dataset
            clSetKernelArg(kernel_multTrans[i], 7, sizeof(cl_uint),
                           &numSubspaces) != CL_SUCCESS) {
 ...
 private:
  ...
  void createMultTrans(size_t dims, size_t local_workgroup_size,
    ...
    manager->buildKernel(program_src, "multTransOCL", context, num_devices,
                         device_ids, kernel);
  }

  void createMult(size_t dims, size_t local_workgroup_size, cl_context context,
                  size_t num_devices,
                  cl_device_id* device_ids, cl_kernel* kernel) {
    std::string program_src = kernelSourceBuilder.generateSourceMult(dims);
    manager->buildKernel(program_src, "multOCL", context, num_devices, device_ids,
                         kernel);
  }
  ...
  void initOCLBuffers(real_type* streamingArray, real_type* subspaceArray,
                      uint32_t* metaInfo, size_t gridSize, real_type* dataset, size_t datasetSize,
    ...
    else if (streamingArray != nullptr
             && !this->deviceStreamArray.isInitialized()) {
      this->deviceStreamArray.initializeBuffer(streamingArray, sizeof(real_type),
          numStreamSubs * (this->dims + 1), true);
    }

    if ( numSubSubs == 0 && subspaceArray != nullptr
         && !this->deviceSubspaceArray.isInitialized()) {
    ...
    if (!this->deviceData.isInitialized()) {
      this->deviceData.initializeBuffer(dataset, sizeof(real_type),
                                        datasetSize * this->dims, true);
    }
  ...
  void initParams(real_type* alpha, size_t alphaSize, real_type* tmp,
                  size_t datasetSize) {

...
class OperationMultiEvalAdaptiveOCL : public base::OperationMultipleEval {
 protected:
  ...
  uint32_t* m_metaInfo = nullptr;
  T* m_streamingArray = nullptr;
  T* m_subspaceArray = nullptr;
  size_t m_streamingCounter = 0;
  size_t m_subspaceCounter = 0;
 ...
 public:
  OperationMultiEvalAdaptiveOCL(base::Grid& grid, base::DataMatrix& dataset,
    ...
    // TODO(leiterrl): iteration one -> not a subspace level list, but instead a
    // (level ->
    // points to start of first surplus in surplus array)
 ...
 private:
  ...
  uint32_t getSubspaceSize(uint32_t* level) {
    ...
    for (size_t d = 0; d < m_dims; d++) {
      if (level[d] < 1) throw std::runtime_error("Negative level value.");
  ...
  void buildDatastructure() {
    ...
    // vector used to preserve subspace order as std::map silently sorts item by
    // key value
    ...
    for (size_t gridIndex = 0; gridIndex < this->gridSize; gridIndex++) {
      ...
      if (flatLevelEntry == flatLevelList.end()) {
        ...
        /*printf("level: %i \t %i \t %i \t %i \n", flatLevelEntry->second[0],
         flatLevelEntry->second[1], flatLevelEntry->second[2],

...
base::OperationMultipleEval* createAdaptiveOCLConfigured(base::Grid& grid,
  ...
  if (configuration.getParameters().operator bool()) {
    ...
    parameters =
      std::make_shared<base::OCLOperationConfiguration>("AdaptiveOCL.cfg");
  ...
  if ((*parameters)["VERBOSE"].getBool()) {
    std::cout << "are optimizations on: " <<
              (*parameters)["ENABLE_OPTIMIZATIONS"].getBool() << std::endl;
    std::cout << "is local memory on: " <<
              (*parameters)["KERNEL_USE_LOCAL_MEMORY"].getBool() << std::endl;
    std::cout << "local size: " << (*parameters)["LOCAL_SIZE"].getUInt() <<
              std::endl;
    std::cout << "internal precision: " << (*parameters)["INTERNAL_PRECISION"].get()
              << std::endl;
    std::cout << "platform is: " << (*parameters)["PLATFORM"].get() << std::endl;
    std::cout << "device type is: " << (*parameters)["DEVICE_TYPE"].get() <<
              std::endl;
    std::cout << "hard limit: " <<
              (*parameters)["ADAPTIVE_STREAMING_HARD_LIMIT"].get() << std::endl;
    std::cout << "soft limit: " << (*parameters)["ADAPTIVE_STREAMING_DENSITY"].get()
              << std::endl;
  }

  if ((*parameters)["INTERNAL_PRECISION"].get() == "float") {
    return new datadriven::OperationMultiEvalAdaptiveOCL<float>(grid, dataset,
           parameters);
  } else if ((*parameters)["INTERNAL_PRECISION"].get() == "double") {
    return new datadriven::OperationMultiEvalAdaptiveOCL<double>(grid, dataset,
           parameters);
  } else {

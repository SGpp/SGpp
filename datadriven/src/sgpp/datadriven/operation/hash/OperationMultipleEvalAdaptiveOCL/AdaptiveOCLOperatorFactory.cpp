...
base::OperationMultipleEval *createAdaptiveOCLConfigured(
  ...
  if (configuration.getParameters().operator bool()) {
    ...
    if ((*parameters).contains("LOCAL_SIZE") == false) {
      (*parameters).addIDAttr("LOCAL_SIZE", UINT64_C(128));
    }
    ...
    if ((*parameters).contains("KERNEL_TRANS_DATA_BLOCK_SIZE") == false) {
      (*parameters).addIDAttr("KERNEL_TRANS_DATA_BLOCK_SIZE", UINT64_C(1));
    }

    if ((*parameters).contains("ADAPTIVE_STREAMING_HARD_LIMIT") == false) {
      (*parameters).addIDAttr("ADAPTIVE_STREAMING_HARD_LIMIT", UINT64_C(10));  // absolute value
    }

    if ((*parameters).contains("ADAPTIVE_STREAMING_DENSITY") == false) {
      (*parameters).addIDAttr("ADAPTIVE_STREAMING_DENSITY", UINT64_C(5));  // In percent
    }
  }
  ...
  if ((*parameters)["VERBOSE"].getBool()) {
    ...
    std::cout << "hard limit: " << (*parameters)["ADAPTIVE_STREAMING_HARD_LIMIT"].get()
              << std::endl;
  ...
  if ((*parameters)["INTERNAL_PRECISION"].get() == "float") {
    ...
    throw base::factory_exception(
        "Error creating operation\"OperationMultiEvalAdaptiveOCL\": invalid value for parameter "
        "\"INTERNAL_PRECISION\"");
  }
  ...
  /*std::shared_ptr<base::OCLOperationConfiguration> parameters;
   ...
   //  std::cout << "are optimizations on: " <<
   (*parameters)["STREAMING_OCL_ENABLE_OPTIMIZATIONS"].getBool() << std::endl;
   //  std::cout << "is local memory on: " <<
   (*parameters)["STREAMING_OCL_USE_LOCAL_MEMORY"].getBool() << std::endl;
   //  std::cout << "local size: " << (*parameters)["STREAMING_OCL_LOCAL_SIZE"].getUInt() <<
   std::endl;
   //  std::cout << "max dim unroll: " << (*parameters)["STREAMING_OCL_MAX_DIM_UNROLL"].getUInt() <<
   std::endl;
   //  std::cout << "internal precision: " <<
   (*parameters)["STREAMING_OCL_INTERNAL_PRECISION"].get() << std::endl;
   //  std::cout << "platform is: " << (*parameters)["STREAMING_OCL_PLATFORM"].get() << std::endl;
   //  std::cout << "device type is: " << (*parameters)["STREAMING_OCL_DEVICE_TYPE"].get() <<
   std::endl;

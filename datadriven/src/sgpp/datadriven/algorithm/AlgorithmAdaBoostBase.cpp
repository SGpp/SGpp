...
AlgorithmAdaBoostBase::AlgorithmAdaBoostBase(SGPP::base::Grid& SparseGrid,
    size_t gridType, SGPP::base::HashGenerator::level_t gridLevel,
  ...
  if (searchNum == 1)
    ...
    this->lambStepsize = (log((float_t)maxLambda) - log((float_t)minLambda)) / ((
                           float_t)searchNum - 1);
...
void AlgorithmAdaBoostBase::doDiscreteAdaBoost(SGPP::base::DataVector&
    hypoWeight, SGPP::base::DataVector& weightError,
    SGPP::base::DataMatrix& weights, SGPP::base::DataMatrix& decision,
  ...
  for (size_t count = 0; count < this->numBaseLearners; count++) {
    ...
    std::cout << std::endl;
    std::cout << "This is the " << this->actualBaseLearners << "th weak learner." <<
              std::endl;
    std::cout << std::endl;
    ...
    if (!this->refinement) {
      if (count == 0)
        ...
        this->sumGridPoint->set(count,
                                this->sumGridPoint->get(count - 1) + (float_t)gridPoint);
    }
    ...
    if (weightError.get(count) >= 0.50) {
      std::cout << std::endl << "The training error rate exceeds 0.5 after " << count
...
void AlgorithmAdaBoostBase::doRealAdaBoost(SGPP::base::DataMatrix& weights,
    SGPP::base::DataMatrix& testData, SGPP::base::DataMatrix& algorithmValueTrain,
    SGPP::base::DataMatrix& algorithmValueTest) {
  SGPP::base::DataVector weight(this->numData);
  weight.setAll(1.0 / float_t(this->numData));
    ...
    for (size_t i = 0; i < numData; i++) {
      ...
      if (this->numBaseLearners == 1)
        algorithmValueTrain.set(i, count,
                                0.5 * value_train); // 0.5 as the coefficient (the original algorithm)
      else if (count == 0)
        algorithmValueTrain.set(i, count, 0.5 * value_train);
      // each column is the sum value of baselearner respect to the column index
      else
        algorithmValueTrain.set(i, count, algorithmValueTrain.get(i,
                                count - 1) + 0.5 * value_train);
    }
...
void AlgorithmAdaBoostBase::doAdaBoostR2(SGPP::base::DataMatrix& weights,
    SGPP::base::DataMatrix& testData, SGPP::base::DataMatrix& algorithmValueTrain,
  ...
  for (size_t count = 0; count < this->numBaseLearners; count++) {
    ...
    if (meanloss >= 0.50) {
      std::cout << std::endl << "The average loss exceeds 0.5 after " << count + 1 <<
                " iterations" << std::endl;
      (this->actualBaseLearners)--;
      break;
    }
...
void AlgorithmAdaBoostBase::doAdaBoostRT(SGPP::base::DataMatrix& weights,
    SGPP::base::DataMatrix& testData, SGPP::base::DataMatrix& algorithmValueTrain,
...
void AlgorithmAdaBoostBase::eval(SGPP::base::DataMatrix& testData,
  ...
  if (this->boostMode == 1) {
    ...
    doDiscreteAdaBoost(theHypoWeight, theWeightError, weightsMatrix, decisionMatrix,
                       testData, algorithmValueTrain, algorithmValueTest);
  } else if (this->boostMode == 2)
    doRealAdaBoost(weightsMatrix, testData, algorithmValueTrain,
                   algorithmValueTest);
  else {
...
void AlgorithmAdaBoostBase::classif(SGPP::base::DataMatrix& testData,
...
void AlgorithmAdaBoostBase::getAccuracy(SGPP::base::DataMatrix& testData,
  ...
  SGPP::base::DataMatrix valueTest(testDataClass.getSize(),
...
void AlgorithmAdaBoostBase::getROC(SGPP::base::DataMatrix& validationData,
  ...
  SGPP::base::DataMatrix valueTrain(this->numData, this->numBaseLearners);
  SGPP::base::DataMatrix valueValidation(validationDataClass.getSize(),
                                         this->numBaseLearners);
...
void AlgorithmAdaBoostBase::getAccuracyBL(SGPP::base::DataMatrix& testData,
...
void AlgorithmAdaBoostBase::doRefinement(SGPP::base::DataVector& alpha_ada,
    SGPP::base::DataVector& weight_ada, size_t curBaseLearner) {
  bool final_ada = false;

  for (size_t adaptiveStep = 1; adaptiveStep <= this->refineTimes;
       adaptiveStep++) {
    ...
    SGPP::base::SurplusRefinementFunctor(&alpha_ada, refineNumber, 0.0);
    ...
    if (adaptiveStep == this->refineTimes) {
      ...
      if (curBaseLearner == 1) {
        ...
        this->sumGridPoint->set(curBaseLearner - 1,
                                this->sumGridPoint->get(curBaseLearner - 2) + (float_t)gridPts);
      }
    }

...
public:
	 ...
	 * @param save_b Indicates whether the old right hand side should be saved and combined with the new right hand side (aka streaming)
	 ...
	 */
	virtual void computeDensityFunction(sgpp::base::DataMatrix& m, bool save_b = false, bool do_cv = false, std::list<size_t>* deletedPoints = NULL, unsigned int newPoints = 0);
	 ...
	 * @param test_cc The results of the points in the test matrix. If NULL, the value old is not overwritten.
...
protected:
	...
	double beta_;
	unsigned long long int total_points;
	bool can_cv;
	int lambda_step_;

...
DBMatOffline::DBMatOffline(sgpp::datadriven::DBMatDensityConfiguration& oc) :
  config_(&oc), lhsMatrix_(nullptr), constructed_(false), decomposed_(false),
  ownsConfig_(false), perm_(nullptr), grid_(nullptr) {}

DBMatOffline::DBMatOffline () :
  config_(nullptr), lhsMatrix_(nullptr), constructed_(false), decomposed_(false),
  ownsConfig_(false), perm_(nullptr), grid_(nullptr) {}

DBMatOffline::DBMatOffline(string fname) :
  config_(nullptr), lhsMatrix_(nullptr), constructed_(false),
  decomposed_(false), ownsConfig_(true), perm_(nullptr), grid_(nullptr) {
  ...
  sgpp::base::RegularGridConfiguration gconf = { grid_type, grid_dim,
						 grid_level };
  config_ = new sgpp::datadriven::DBMatDensityConfiguration(&gconf, nullptr, reg, lambda,
							    decomp);
...
DBMatOffline::DBMatOffline(const DBMatOffline &old) :
  config_(nullptr), lhsMatrix_(nullptr), constructed_(true),
  decomposed_(true), ownsConfig_(true), perm_(nullptr), grid_(nullptr) {
  ...
  sgpp::base::RegularGridConfiguration gconf =
    {old.config_->grid_type_, old.config_->grid_dim_, old.config_->grid_level_};
  ...
  config_ = new sgpp::datadriven::DBMatDensityConfiguration(
    &gconf, &adaptConfig, old.config_->regularization_,
    old.config_->lambda_, old.config_->decomp_type_);
...
DBMatOffline::~DBMatOffline() {
  ...
  /*if (fullgrid_ != nullptr)
    delete fullgrid_;*/
  if (ownsConfig_ && config_ != nullptr)
    delete config_;
}
...
void DBMatOffline::buildMatrix() {
  ...
  if (config_->decomp_type_ == DBMatDecompLU || config_->decomp_type_ == DBMatDecompChol) {
...
void DBMatOffline::Tokenize(string& str, vector<string>& tokens, string& delimiters) {
...
void DBMatOffline::choleskyModification(
  int newPoints,std::list<size_t> deletedPoints, double lambda){
  ...
  if(deletedPoints.size() > 0){
	...
	new sgpp::base::DataMatrix(lhsMatrix_->getNrows(), lhsMatrix_->getNcols());
      ...
      update_matrix->resizeToSubMatrix(
				       coarseCount_1 + 1, 1, lhsMatrix_->getNrows(), coarseCount_1);
      //Resize current Cholesky factor to required submatrix
      //for necessary rank one updates
      lhsMatrix_->resizeToSubMatrix(
        coarseCount_1 + 1, coarseCount_1 + 1, lhsMatrix_->getNrows(), lhsMatrix_->getNrows());
...
void DBMatOffline::choleskyAddPoint(sgpp::base::DataVector* newCol, size_t size){	
  ...
  if(size_up != (size+1)){
    throw sgpp::base::data_exception(
      "Size of update vector newCol needs to be 1 dim larger than the underlying decomposed matrix!");
  }

 ...
 *
 * DensityDifferenceSystemMatrix.cpp
 *
 * Author: Paul Sarbu
 */
...
void DensityDifferenceSystemMatrix::generateb(sgpp::base::DataVector& rhs) {
  sgpp::base::DataVector y(numSamples_P);
  y.setAll(1.0);
  // Bt_p * 1
  B_p->multTranspose(y, rhs);
  // (1 / M_q) * Bt_p * 1
  rhs.mult(1. / static_cast<double>(numSamples_Q));

  sgpp::base::DataVector rhs2(rhs.size());
  // Bt_q * 1
  B_q->multTranspose(y, rhs2);
  // (1 / M_q) * Bt_p * 1 - (1 / M_p) * Bt_q * 1
  rhs.axpy(-1. / static_cast<double>(numSamples_P), rhs2);
}

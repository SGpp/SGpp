...
class JsonReader(object):
    ...
    def _readNumber(self):
        ...
        result = self._next()
        peek = self._peek()
        while peek is not None and (peek.isdigit() or peek in [".", "e", "+", "-"]):
            isfloat = isfloat or peek == "." or peek == "e" # last two items were added by Valeriy
    ...
    def _hexDigitToInt(self, ch):
        try:
            ...
            try:
                result = int(ch)
            except ValueError: raise ReadException( "The character %s is not a hex digit." % ch )
        return result

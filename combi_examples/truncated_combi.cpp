 ...
 *      Author: Aliz Nagy
 *
 *      A program presenting some examples with the basic functions introduced with the combination technique, and the new ModTrapezoidGrid(modified combigrid)
 *      The concepts exemplified in this piece of code include the construction of a modified combigrid(with the createSuperTrapezoidGrid method of the grid class),
 *      generation of a regular sparse grid, assignment of function values to the gridpoints, the construction of a set of fullgrids corresponding to
...
int main()
{
	...
	int level=4;
	int l_user=2;
	/***Create a new ModTrapezoid Boundary Grid-doesn't contain the fullgrids with levels<l_user*/
	Grid* grid = Grid::createTruncatedTrapezoidGrid(dim);
    GridStorage* gridStorage = grid->getStorage();
    GridGenerator* gridGen = grid->createGridGenerator();
    /***Generate a new regular grid structure corresponding to the type of the chosen grid*/
    gridGen->modified(level,l_user);
    ...
    /**
     * Create the set of fullgrids into which the grid decomposes, since the only grid with a 3rd parameter behind dim and level is the modified combi grid
     * we no longer need the type, and it will be replaced by the user level
     * */
    ...
    cout<<"f("<<p[0]<<","<<p[1]<<","<<p[2]<<")="<<f(p[0],p[1],p[2])<<endl;    delete grid;
    delete gridGen;
    delete opEval;
    delete oh;

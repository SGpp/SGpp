    ...
    def testConstrainedOptimizers(self):
        ...
        for i in range(2):
            ...
            for optimizer in optimizers:
                ...
                self.assertAlmostEqual(fopt, fopt_real, places=3)
            
            # test AugmentedLagrangian::findFeasiblePoint
            optimizer = optimizers[-1]
            x = optimizer.findFeasiblePoint()
            g_x = pysgpp.DataVector(g.getNumberOfConstraints())
            h_x = pysgpp.DataVector(h.getNumberOfConstraints())
            g.eval(x, g_x)
            h.eval(x, h_x)
            
            for i in range(len(g_x)):
              self.assertLess(g_x[i], 1e-6)
            for i in range(len(h_x)):
              self.assertLess(abs(h_x[i]), 1e-6)

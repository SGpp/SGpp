...
class G6ConstraintGradient(pysgpp.OptConstraintGradient):
    ...
    def eval(self, x, value, gradient):
        ...
        if all([0.0 <= x[t] <= 1.0 for t in range(2)]):
            ...
            value[0] = -(x0 - 5.0)**2 - (x1 - 5.0)**2 + 100.0
            value[1] =  (x0 - 6.0)**2 + (x1 - 5.0)**2 - 82.81
            gradient.set(0, 0, -2.0 * (x0 - 5.0) * (100.0 - 13.0))
            gradient.set(0, 1, -2.0 * (x1 - 5.0) * (100.0 -  0.0))
            gradient.set(1, 0,  2.0 * (x0 - 6.0) * (100.0 - 13.0))
            gradient.set(1, 1,  2.0 * (x1 - 5.0) * (100.0 -  0.0))
        else:
            value[0] = float("inf")
            value[1] = float("inf")

  ...
  namespace optimization {
    namespace sle {
      namespace solver {
        ...
        bool Auto::solve(system::System& system, const std::vector<float_t>& b,
        ...
        bool Auto::solve(system::System& system, const std::vector<std::vector<float_t> >& B,
          ...
          if (supports[&solverUmfpack] || supports[&solverGmmpp]) {
            ...
            size_t inc = static_cast<size_t>(
                           ESTIMATE_NNZ_ROWS_SAMPLE_SIZE * static_cast<float_t>(n)) + 1;
            ...
            float_t nnzRatio = static_cast<float_t>(nnz) /
                               (static_cast<float_t>(nrows) * static_cast<float_t>(n));
            ...
            if ((n > MAX_DIM_FOR_FULL) || (nnzRatio <= MAX_NNZ_RATIO_FOR_GMMPP)) {
              if (B.size() == 1) {
                // prefer Gmm++ over UMFPACK
                addSolver(&solverGmmpp, solvers, supports);
                addSolver(&solverUmfpack, solvers, supports);
              } else {
          ...
          addSolver(&solverArmadillo, solvers, supports);
          addSolver(&solverEigen, solvers, supports);
          addSolver(&solverGmmpp, solvers, supports);
          addSolver(&solverUmfpack, solvers, supports);
          addSolver(&solverBicgstab, solvers, supports);
          addSolver(&solverGaussianElimination, solvers, supports);

...
#include "OperationMultipleHierarchisationNakBspline.hpp"
...
OperationMultipleHierarchisationNakBspline::OperationMultipleHierarchisationNakBspline(
    base::NakBsplineGrid& grid)
    : grid(grid) {}

OperationMultipleHierarchisationNakBspline::~OperationMultipleHierarchisationNakBspline() {}

bool OperationMultipleHierarchisationNakBspline::doHierarchisation(base::DataVector& nodeValues) {
...
void OperationMultipleHierarchisationNakBspline::doDehierarchisation(base::DataVector& alpha) {
  base::GridStorage& storage = grid.getStorage();
  const size_t d = storage.getDimension();
  base::OperationEvalNakBsplineNaive opNaiveEval(storage, grid.getDegree());
...
bool OperationMultipleHierarchisationNakBspline::doHierarchisation(base::DataMatrix& nodeValues) {
  base::HierarchisationSLE system(grid);
  base::sle_solver::Auto solver;
...
void OperationMultipleHierarchisationNakBspline::doDehierarchisation(base::DataMatrix& alpha) {
  base::GridStorage& storage = grid.getStorage();
  const size_t d = storage.getDimension();
  base::OperationEvalNakBsplineNaive opNaiveEval(storage, grid.getDegree());

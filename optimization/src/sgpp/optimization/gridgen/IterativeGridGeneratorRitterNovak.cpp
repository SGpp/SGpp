  ...
  namespace optimization {
    namespace gridgen {
      ...
      bool IterativeGridGeneratorRitterNovak::generate() {
        ...
        {
          tools::Permuter<float_t> permuter1(fX);
          std::sort(fXOrder.begin(), fXOrder.end(), permuter1);

          tools::Permuter<size_t> permuter2(fXOrder);
        ...
        while (currentN < N) {
          ...
          for (size_t i = 0; i < currentN; i++) {
            ...
            if (powMethod == STD_POW) {
              g = std::pow(static_cast<float_t>(levelSum[i] + degree[i]) + 1.0, alpha) *
                  std::pow(static_cast<float_t>(rank[i]) + 1.0, 1.0 - alpha);
            } else {
              g = fastPow(static_cast<float_t>(levelSum[i] + degree[i]) + 1.0, alpha) *
          ...
          #pragma omp parallel shared(fX, currentN, newN, gridStorage) default(none)
          {
            std::vector<float_t> x(d, 0.0);
            function::Objective* curFPtr = &f;
#ifdef _OPENMP

...
TestScalarFunction& G05::getObjectiveFunction() {
  return f;
}

TestVectorFunction& G05::getInequalityConstraintFunction() {
  return g;
}

TestVectorFunction& G05::getEqualityConstraintFunction() {
  return h;
}
...
float_t G05Objective::evalUndisplaced(
  ...
  return 3.0 * x1 + 0.000001 * std::pow(x1, 3.0) + 2.0 * x2 +
         (0.000002 / 3.0) * std::pow(x2, 3.0);
}

void G05Objective::clone(
  std::unique_ptr<ScalarFunction>& clone) const {
  clone = std::unique_ptr<ScalarFunction>(
            new G05Objective(*this));
}



G05InequalityConstraint::G05InequalityConstraint() :
  TestVectorFunction(4, 2) {
}

G05InequalityConstraint::~G05InequalityConstraint() {
}

void G05InequalityConstraint::evalUndisplaced(
  const base::DataVector& x,
...
void G05InequalityConstraint::clone(
  std::unique_ptr<VectorFunction>& clone) const {
  clone = std::unique_ptr<VectorFunction>(
            new G05InequalityConstraint(*this));
}



G05EqualityConstraint::G05EqualityConstraint() :
  TestVectorFunction(4, 3) {
}

G05EqualityConstraint::~G05EqualityConstraint() {
}

void G05EqualityConstraint::evalUndisplaced(
  const base::DataVector& x,
  base::DataVector& value) {
  const float_t x1 = 1200.0 * x[0];
  const float_t x2 = 1200.0 * x[1];
...
void G05EqualityConstraint::clone(
  std::unique_ptr<VectorFunction>& clone) const {
  clone = std::unique_ptr<VectorFunction>(
            new G05EqualityConstraint(*this));
}

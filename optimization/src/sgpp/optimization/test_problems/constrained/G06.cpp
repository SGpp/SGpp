...
TestScalarFunction& G06::getObjectiveFunction() {
  return f;
}

TestVectorFunction& G06::getInequalityConstraintFunction() {
  return g;
}

TestVectorFunction& G06::getEqualityConstraintFunction() {
  return h;
}
...
float_t G06Objective::evalUndisplaced(
...
void G06InequalityConstraint::evalUndisplaced(
  const base::DataVector& x,
...
void G06EqualityConstraint::clone(
  std::unique_ptr<VectorFunction>& clone) const {
  clone = std::unique_ptr<VectorFunction>(
            new G06EqualityConstraint(*this));
}

...
namespace SGPP {
  namespace optimization {
    namespace test_problems {
      ...
      TestScalarFunction& Soland::getObjectiveFunction() {
        return f;
      }

      TestVectorFunction& Soland::getInequalityConstraintFunction() {
        return g;
      }

      TestVectorFunction& Soland::getEqualityConstraintFunction() {
        return h;
      }
      ...
      float_t SolandObjective::evalUndisplaced(
      ...
      void SolandObjective::clone(
        std::unique_ptr<ScalarFunction>& clone) const {
        clone = std::unique_ptr<ScalarFunction>(
                  new SolandObjective(*this));
      }
      ...
      void SolandInequalityConstraint::evalUndisplaced(
        const base::DataVector& x,
        base::DataVector& value) {
      }

      void SolandInequalityConstraint::clone(
        std::unique_ptr<VectorFunction>& clone) const {
        clone = std::unique_ptr<VectorFunction>(
                  new SolandInequalityConstraint(*this));
      }
      ...
      void SolandEqualityConstraint::evalUndisplaced(
        const base::DataVector& x,
        base::DataVector& value) {
        const float_t x1 = 2.0 * x[0];
        const float_t x2 = 3.0 * x[1];
      ...
      void SolandEqualityConstraint::clone(
        std::unique_ptr<VectorFunction>& clone) const {
        clone = std::unique_ptr<VectorFunction>(
                  new SolandEqualityConstraint(*this));
      }

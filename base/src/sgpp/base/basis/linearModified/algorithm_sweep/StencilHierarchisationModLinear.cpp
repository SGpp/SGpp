...
#include <sgpp/base/basis/linearModified/algorithm_sweep/StencilHierarchisationModLinear.hpp>
...
namespace SGPP {
  namespace base {
    ...
    void StencilHierarchisationModLinear::operator()(DataVector& source, DataVector& result, grid_iterator& index, size_t dim) {
      rec(source, result, index, dim, -1, -1);
    }

    void StencilHierarchisationModLinear::rec(DataVector& source, DataVector& result, grid_iterator& index, size_t dim, int seql, int seqr) {

...
namespace SGPP {
  namespace base {
    ...
    void OCLStretchedBuffer::initializeBuffer(size_t sizeofType, size_t elements) {
      ...
      void* hostPinnedMemory = clEnqueueMapBuffer(manager->command_queue[0], hostBuffer, CL_TRUE,
    ...
    void OCLStretchedBuffer::freeBuffer() {
      ...
      if (this->mappedHostBuffer == nullptr) {
        std::stringstream errorString;
        errorString << "OCL Error: OCLStretchedBuffer in partially initialized state: mappedHostBuffer is null"
                    << std::endl;
        throw SGPP::base::operation_exception(errorString.str());
      }
      ...
      if (this->hostBuffer == nullptr) {
        std::stringstream errorString;
        errorString << "OCL Error: OCLStretchedBuffer in partially initialized state: hostBuffer is null" << std::endl;
        throw SGPP::base::operation_exception(errorString.str());
      }

      cl_int err = clEnqueueUnmapMemObject(manager->command_queue[0], hostBuffer, this->mappedHostBuffer, 0, nullptr, nullptr);
      ...
      if (this->bufferList == nullptr) {
        std::stringstream errorString;
        errorString << "OCL Error: OCLStretchedBuffer in partially initialized state: buffer list is null" << std::endl;
        throw SGPP::base::operation_exception(errorString.str());
      }
      ...
      for (size_t i = 0; i < this->manager->num_devices; i++) {
        if (this->bufferList[i] != nullptr) {
          ...
          std::stringstream errorString;
          errorString << "OCL Error: OCLStretchedBuffer in partially initialized state: device buffer is null"
                      << std::endl;
          throw SGPP::base::operation_exception(errorString.str());
        }
      }
      ...
      if (this->isMappedMemory) {
        if (this->hostBuffer != nullptr) {
          ...
          std::stringstream errorString;
          errorString << "OCL Error: OCLStretchedBuffer in partially initialized state: host buffer is null"
                      << std::endl;
          throw SGPP::base::operation_exception(errorString.str());
        }
      }

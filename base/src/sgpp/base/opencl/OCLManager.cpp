...
OCLManager::OCLManager(std::shared_ptr<base::OCLOperationConfiguration>
  ...
  if (err != CL_SUCCESS) {
    ...
    errorString <<
                "OCL Error: Unable to get number of OpenCL platforms. Error Code: " << err <<
                std::endl;
  ...
  for (cl_uint ui = 0; ui < num_platforms; ui++) {
    ...
    err = clGetPlatformInfo(platform_ids[ui], CL_PLATFORM_NAME, 128 * sizeof(char),
  ...
  if ((*parameters)["DEVICE_TYPE"].get() == "CL_DEVICE_TYPE_CPU") {
    ...
    // get the number of devices
    err = clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_ACCELERATOR, 0, nullptr,
                         &num_devices);
    device_ids = new cl_device_id[num_devices];
    // get the device ids
    err = clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_ACCELERATOR, num_devices,
                         device_ids, nullptr);
    ...
    device_ids = new cl_device_id[num_devices];
    // get the device ids
    err = clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_ALL, num_devices, device_ids,
                         nullptr);
  } else {
  ...
  if ((*parameters).contains("SELECT_SPECIFIC_DEVICE")) {
    ...
    if (selectedDevice > num_devices) {
      std::stringstream errorString;
      errorString << "OCL Error: Illegal value set for \"SELECT_SPECIFIC_DEVICE\"" <<
                  std::endl;
      throw SGPP::base::operation_exception(errorString.str());
    }
  ...
  for (size_t i = 0; i < num_devices; i++) {
    ...
    if (err != CL_SUCCESS) {
      std::stringstream errorString;
      errorString << "OCL Error: Failed to read the device name for device: " << i <<
                  std::endl;
      throw SGPP::base::operation_exception(errorString.str());
    }
...
void OCLManager::buildKernel(const std::string& program_src,
                             const char* kernel_name, cl_context context,
  ...
  if ((*parameters)["ENABLE_OPTIMIZATIONS"].getBool()) {
    ...
    build_opts =
      (*parameters)["OPTIMIZATION_FLAGS"].get(); // -O5  -cl-mad-enable -cl-denorms-are-zero -cl-no-signed-zeros -cl-unsafe-math-optimizations -cl-finite-math-only -cl-fast-relaxed-math
  } else {
  ...
  if ((*parameters)["SHOW_BUILD_LOG"].getBool()) {
    // get the build log
    size_t len;
    clGetProgramBuildInfo(program, device_ids[0], CL_PROGRAM_BUILD_LOG, 0, NULL,
                          &len);

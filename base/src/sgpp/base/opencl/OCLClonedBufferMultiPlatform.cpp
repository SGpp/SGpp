...
OCLClonedBufferMultiPlatform::OCLClonedBufferMultiPlatform(OCLManagerMultiPlatform& manager) :
        manager(manager) {
    initialized = false;
    sizeofType = 0;
    elements = 0;
    ...
    for (OCLPlatformWrapper &platform : this->manager.platforms) {
        ...
        for (size_t i = 0; i < platform.deviceCount; i++) {
            if (offsets == nullptr) {
                ...
                err = clEnqueueWriteBuffer(platform.commandQueues[i], this->platformBufferList[platform.platformId][i],
                CL_FALSE, this->sizeofType * offsets[i], this->sizeofType * this->elements, hostData, 0, nullptr,
                        &(platformActionEvents[platform.platformId][i]));
            }
    ...
    for (OCLPlatformWrapper &platform : this->manager.platforms) {
        ...
        for (size_t i = 0; i < platform.deviceCount; i++) {
            if (offsets == nullptr) {
                ...
                CL_FALSE, 0, this->sizeofType * this->elements, hostData, 0, nullptr, &(platformActionEvents[platform.platformId][i]));
            } else {
                err = clEnqueueReadBuffer(platform.commandQueues[i], this->platformBufferList[platform.platformId][i],
...
void OCLClonedBufferMultiPlatform::initializeBuffer(void* initialValues, size_t sizeofType, size_t elements) {
    cl_int err;

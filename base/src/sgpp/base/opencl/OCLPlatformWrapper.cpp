...
OCLPlatformWrapper::OCLPlatformWrapper(cl_platform_id platformId, char (&platformName)[128],
        std::vector<cl_device_id> &deviceIds, std::vector<std::string> &deviceNames) :
    ...
    this->context = clCreateContext(properties, (cl_uint) this->deviceIds.size(), this->deviceIds.data(), nullptr,
            nullptr, &err);
    ...
    for (uint32_t i = 0; i < this->deviceIds.size(); i++) {
        this->commandQueues.push_back(clCreateCommandQueue(this->context, this->deviceIds[i],
        ...
        err = clGetDeviceInfo(this->deviceIds[i],
        CL_DEVICE_NAME, 128 * sizeof(char), &deviceName, nullptr);

        if (err != CL_SUCCESS) {
            std::stringstream errorString;
            errorString << "OCL Error: Failed to read the device name for device: " << this->deviceIds[i] << std::endl;
            throw SGPP::base::operation_exception(errorString.str());
        }

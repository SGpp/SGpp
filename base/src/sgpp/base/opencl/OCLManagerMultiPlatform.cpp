...
namespace SGPP {
  namespace base {
    ...
    void OCLManagerMultiPlatform::configure(base::OCLOperationConfiguration& configuration, bool useConfiguration) {
    ...
    void OCLManagerMultiPlatform::configurePlatform(cl_platform_id platformId,
      ...
      if (err != CL_SUCCESS) {
        std::stringstream errorString;
        errorString << "OCL Error: Unable to get device id for platform \"" << platformName << "\". Error Code: " << err
                    << std::endl;
        throw SGPP::base::operation_exception(errorString.str());
      }
      ...
      for (cl_device_id deviceId : deviceIds) {
        //filter device ids
        this->configureDevice(deviceId, devicesNode, filteredDeviceIds, countLimitMap, useConfiguration);
      }
    ...
    void OCLManagerMultiPlatform::configureDevice(cl_device_id deviceId, json::Node& devicesNode,
        std::vector<cl_device_id>& filteredDeviceIds, std::map<std::string, size_t>& countLimitMap,
      ...
      if (err != CL_SUCCESS) {
        std::stringstream errorString;
        errorString << "OCL Error: Failed to read the device name for device: " << deviceId << std::endl;
        throw SGPP::base::operation_exception(errorString.str());
      }

...
float_t StdNormalDistribution::getDensity(const float_t x) {
  ...
  const float_t secondTerm = -( (x - mean) * (x - mean) /
                                (2.0 * stddev * stddev) );
...
float_t StdNormalDistribution::getDensity(const float_t x, const float_t mu,
    const float_t sigma) {
...
float_t StdNormalDistribution::getNormedDensity(const float_t x,
    const float_t mu, const float_t sigma) {
  const float_t mean = mu;
  const float_t stddev = sigma;

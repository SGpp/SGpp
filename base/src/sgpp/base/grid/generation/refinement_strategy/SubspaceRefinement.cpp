  ...
  namespace base {
    ...
    void SubspaceRefinement::addElementToCollection(
      ...
      for (AbstractRefinement::refinement_pair_type key_value : current_value_list) {
        ...
        auto predicat = [key_value](const AbstractRefinement::refinement_pair_type & element) {
          bool result = true;

          for (size_t d = 0; d < key_value.first->getIndex().dim(); d++) {
            result = result && (key_value.first->getIndex().getLevel(d) == element.first->getIndex().getLevel(d));
          }

          return result;
        };

        AbstractRefinement::refinement_container_type::iterator iter = std::find_if(collection.begin(),
    ...
    void SubspaceRefinement::free_refine(GridStorage* storage, RefinementFunctor* functor) {
      if (storage->size() == 0) {

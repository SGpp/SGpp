...
class HashGenerator {
 public:
  ...
  /**
   * Generates a regular sparse grid with anisotropic weights and level, without boundaries
   *
   * @param storage Hashmap that stores the grid points
   ...
   */
  void regular(GridStorage& storage, std::vector<size_t>& anisotropic_weights, level_t level) {
    if (storage.getSize() > 0) {
      throw generation_exception("storage not empty");
    }

    auto max = std::max_element(std::begin(anisotropic_weights), std::end(anisotropic_weights));
    ...
    this->regular_rec(storage, point, dim - 1, anisotropic_weights,
                      (level + static_cast<level_t>(dim) - 1) * static_cast<level_t>(*max));
  }
  ...
  */
  ...
  void full(GridStorage& storage, std::vector<size_t>& anisotropic_weights, level_t level) {
      ...
      this->createFullGridIterative(storage, anisotropic_weights, level * static_cast<level_t>(*max));
    }
  ...
  void fullWithBoundary(GridStorage& storage,
        ...
        this->createFullGridTruncatedIterative(storage, anisotropic_weights,
                                               level * static_cast<level_t>(*max));
    }
   ...
   *                      inserted
   */
  void regularWithBoundaries(GridStorage& storage, std::vector<size_t>& anisotropic_weights,
                             level_t level, bool boundaryLevel = 1) {
    if (storage.getSize() > 0) {
      throw generation_exception("storage not empty");
    }
    auto max = std::max_element(std::begin(anisotropic_weights), std::end(anisotropic_weights));
    ...
    if (boundaryLevel == 1) {
      ...
      this->boundaries_truncated_rec(
          storage, point, dim - 1, anisotropic_weights,
          (level + static_cast<level_t>(dim) - 1) * static_cast<level_t>(*max), boundaryLevel);
    } else {
      /* new grid generation
       *
       * for all level the same calculation of the level sum is implemented:
 ...
 protected:
  ...
  void createFullGridIterative(GridStorage& storage,
    ...
    size_t weighted_level_sum;
    size_t max_level_sum = 0;
    for (size_t d = 0; d < storage.getDimension(); d++) {
      max_level_sum += (n / static_cast<level_t>(anisotropic_weights[d]))
    ...
    // Generate 1D grid in first dimension
    for (level_t l = 1; l * static_cast<level_t>(anisotropic_weights[0]) <= n; l++) {
      for (index_t i = 1; i < static_cast<index_t>(1 << l); i += 2) {
        if (l == (n / static_cast<level_t>(anisotropic_weights[0]))) {
          idx_1d.push(0, l, i, true);
        } else {
          idx_1d.push(0, l, i, false);
        }
    ...
    for (size_t d = 1; d < storage.getDimension(); d++) {
      ...
      for (size_t g = 0; g < grid_size; g++) {
        ...
        GridPoint idx(storage.getPoint(g));
        // add remaining level-index pairs in current dimension d
        for (level_t l = 1; l * static_cast<level_t>(anisotropic_weights[d]) <= n ; l++) {
          ...
          for (index_t i = 1; i < static_cast<index_t>(1 << l); i += 2) {
            ...
            if (first == false) {
              ...
              if (weighted_level_sum == max_level_sum) {
                idx.push(d, l, i, true);
              } else {
                idx.push(d, l, i, false);
              }
  ...
  //     */
  //    void regular_rec(GridStorage& storage, GridPoint& index, size_t current_dim,
  //            level_t current_level, level_t level) {
  //        mexPrintf("\n");
  //        if (current_dim == 0) {
  //            mexPrintf("Inserting all points in dim 0\n");
  ...
  //            mexPrintf("Current dim is %d\n", current_dim);
  //            mexPrintf("Current level is %d\n", current_level);
  //
  //            mexPrintf("Input index is %s\n", index.toString().c_str());
  //
  ...
  //
  //                // d-1 recursion
  //                this->regular_rec(storage, index, current_dim - 1,
  //                        current_level, level);
  //                mexPrintf("Now the index is %s\n", index.toString().c_str());
  //
  //            }
  //
  //            if (current_level < level) {
  ...
  //                mexPrintf("New index is %s\n", index.toString().c_str());
  //                this->regular_rec(storage, index, current_dim,
  ...
  void regular_rec(GridStorage& storage, GridPoint& index, size_t current_dim,
                   std::vector<size_t>& anisotropic_weights, level_t level) {
    if (current_dim == 0) {
      regular_rec_1d(storage, index, anisotropic_weights, level);
    } else {
      index_t source_index;
      ...
      current_level = 0;
      for (size_t d = 0; d < storage.getDimension(); d++) {
        current_level += static_cast<level_t>(anisotropic_weights[d]) * index.getLevel(d);
      }
      ...
      if (current_level <= level) {
        ...
        // d-1 recursion
        this->regular_rec(storage, index, current_dim - 1, anisotropic_weights, level);
      }

      if (current_level < level) {
        // current_level + 1 recursion
        index.push(current_dim, source_level + 1, 2 * source_index - 1);
        this->regular_rec(storage, index, current_dim, anisotropic_weights, level);
  ...
  void regular_rec_1d(GridStorage& storage, GridPoint& index,
    ...
    for (level_t l = 1; l * aw + current_level - aw <= level; l++) {
  ...
  void boundaries_truncated_rec(GridStorage& storage, GridPoint& index, size_t current_dim,
                                ...
                                bool bLevelZero) {
    if (current_dim == 0) {
      boundaries_Truncated_rec_1d(storage, index, anisotropic_weights, level, bLevelZero);
    } else {
      index_t source_index;
      level_t source_level, current_level;
      ...
      if (current_level <= level) {
        ...
        if (source_level == 1) {
          index.push(current_dim, 0, 0, false);
          this->boundaries_truncated_rec(storage, index, current_dim - 1, anisotropic_weights,
                                         level, true);
        ...
        // d-1 recursion
        index.setLeaf(bLeafProperty);
        this->boundaries_truncated_rec(storage, index, current_dim - 1, anisotropic_weights, level,
                                       bLevelZero);
      }

      if (current_level < level) {
        index.push(current_dim, source_level + 1, 2 * source_index - 1);
        this->boundaries_truncated_rec(storage, index, current_dim, anisotropic_weights, level,
                                       bLevelZero);
  ...
  /**
   * recursive construction of the spare grid with boundaries, classic level 0 approach, only for level
   * 0 and 1
   *
   * @param storage hashmap that stores the grid points
   * @param index point's index
  ...
  void boundaries_rec(GridStorage& storage, GridPoint& index, size_t current_dim,
    ...
    if (current_level <= level) {
      ...
      if (source_level == 0) {
        if (current_dim == 0) {
          ...
          index.push(current_dim, 0, 0, bLeafProperty);
          this->boundaries_rec(storage, index, current_dim - 1, anisotropic_weights, level);
    ...
    if (current_level < level) {
      if (source_level == 0 && source_index == 0) {
        index.push(current_dim, source_level + 1, 1);
        this->boundaries_rec(storage, index, current_dim, anisotropic_weights, level);
      } else {
        index.push(current_dim, source_level + 1, 2 * source_index - 1);
        this->boundaries_rec(storage, index, current_dim, anisotropic_weights, level);

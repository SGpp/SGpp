...
class HashGenerator {
 public:
     ...
     */
    void regular(GridStorage& storage, std::vector<size_t>& anisotropic_weights,
            level_t level) {
        if (storage.getSize() > 0) {
            throw generation_exception("storage not empty");
        }
     ...
     *                      inserted
     */
    void regularWithBoundaries(GridStorage& storage, level_t level,
    ...
    void squareRoot(GridStorage& storage, level_t level) {
        ...
        this->square_rec(storage, point, storage.getDimension() - 1, level,
                small_level, false, 0);
    }
    /**
    ...
    void regular_iter(GridStorage& storage, level_t n) {
        ...
        for (size_t d = 1; d < storage.getDimension(); d++) {
            ...
            for (size_t g = 0; g < grid_size; g++) {
                ...
                for (level_t l = 1;
                        l + level_sum <= n + storage.getDimension() - 1; l++) {
                    for (index_t i = 1; i < static_cast<index_t>(1 << l); i +=
                            2) {
                        // first grid point is updated, all others inserted
                        if (first == false) {
                            // is leaf?
                            if ((l + level_sum)
                                    == n + storage.getDimension() - 1) {
                                idx.push(d, l, i, true);
                            } else {
    ...
    void cliques_iter(GridStorage& storage, level_t n, size_t clique_size) {
        ...
        for (size_t d = 1; d < storage.getDimension(); d++) {
            ...
            for (size_t g = 0; g < grid_size; g++) {
                ...
                for (size_t dt = 0; dt < clique_size * clique_num && dt < d;
                        dt++) {
            ...
            for (size_t g = 0; g < gridSize; g++) {
                ...
                if (numberOfZeroLevels > 0) {
                    ...
                    if (n + curDim < boundaryLevel + numberOfZeroLevels) {
                        ...
                        upperBound = n + curDim - numberOfZeroLevels
                                - boundaryLevel;
                    }
                } else {
...
//    }
            ...
            if (current_level <= level) {
                ...
                if (source_level == 1) {
                    ...
                    this->boundaries_truncated_rec(storage, index,
                            current_dim - 1, current_level, level, true);
                ...
                this->boundaries_truncated_rec(storage, index, current_dim - 1,
                        current_level, level, bLevelZero);
            }

            if (current_level < level) {
                index.push(current_dim, source_level + 1, 2 * source_index - 1);
                this->boundaries_truncated_rec(storage, index, current_dim,
                        current_level + 1, level, bLevelZero);
        ...
        for (level_t l = 0; l <= level - current_level + 1; l++) {
            if (l == level - current_level + 1) {
                if (l == 0) {
                    ...
                    for (index_t i = 1;
                            static_cast<int>(i)
                                    <= static_cast<int>(1 << (l - 1)); i++) {
                        index.push(0, l, 2 * i - 1,
                                (true && bLevelGreaterZero));
                        storage.insert(index);
                    }
                }
            } else {
     ...
     */
    void square_rec(GridStorage& storage, GridPoint& index, size_t current_dim,
            level_t level, level_t small_level, bool tail, size_t sum) {
        index_t source_index;
        level_t source_level;
        ...
        if (source_level == 0) {
            if (current_dim == 0) {
                ...
                this->square_rec(storage, index, current_dim - 1, level,
                        small_level, newtail, sum);
            ...
            if (current_dim == 0) {
                 ...
                 * */
                if (sum
                        == (small_level * (storage.getDimension() - 1) + level)) {
                    index.setLeaf(true);
                } else {
                    index.setLeaf(false);
                }
         ...
         */
        if ((source_level < small_level)
                || ((!tail) && (source_level < level))) {
            if (source_level == 0 && source_index == 0) {
                index.push(current_dim, source_level + 1, 1);
                this->square_rec(storage, index, current_dim, level,
                        small_level, tail, sum + 1);
            } else {
                index.push(current_dim, source_level + 1, 2 * source_index - 1);
                this->square_rec(storage, index, current_dim, level,
                        small_level, tail, sum + 1);
    ...
    void trunc_rec(GridStorage& storage, GridPoint& index, size_t current_dim,
        ...
        if (current_level <= level) {
            ...
            if (source_level == 0) {
                if (current_dim == 0) {
                    ...
                    index.push(current_dim, 0, 0, bLeafProperty);
                    trunc_rec(storage, index, current_dim - 1, current_level,
                            level, minlevel);
        ...
        if (source_level < minlevel) {
            ...
            if (source_level == 0 && source_index == 0) {
                index.push(current_dim, source_level + 1, 1);
                trunc_rec(storage, index, current_dim, current_level, level,
                        minlevel);
            } else {
                index.push(current_dim, source_level + 1, 2 * source_index - 1);
                trunc_rec(storage, index, current_dim, current_level, level,

 ...
 */
class VectorDistributionRefinementFunctor : public RefinementFunctor {
 public:
  /**
   * Constructor.
   *
   ...
   * threshold
   *
   * This functor draws random samples following the pdfs and refines the grid points closest to
   * these samples
   */
  VectorDistributionRefinementFunctor(DataMatrix& alphas, sgpp::base::DistributionsVector pdfs,
                                      size_t refinements_num = 1, double threshold = 0.0);

  /**
   * Destructor
   */
  ~VectorDistributionRefinementFunctor() override;

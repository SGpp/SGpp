...
NotAKnotBsplineBoundaryGrid::NotAKnotBsplineBoundaryGrid(std::istream& istr)
  ...
  istr >> degree;
  istr >> boundaryLevel;
  basis_.reset(new SNotAKnotBsplineBoundaryBase(degree));
  generator.setBoundaryLevel(boundaryLevel);
}

NotAKnotBsplineBoundaryGrid::NotAKnotBsplineBoundaryGrid(size_t dim, size_t degree,
                                                         level_t boundaryLevel)
    : Grid(dim),
      generator(storage, boundaryLevel),
      degree(degree),
      basis_(new SNotAKnotBsplineBoundaryBase(degree)),

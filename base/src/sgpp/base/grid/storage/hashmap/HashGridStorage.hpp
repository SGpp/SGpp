    ...
    class HashGridStorage {
      public:
        ...
        ~HashGridStorage() {
          ...
          for (grid_list_iterator iter = list.begin(); iter != list.end();
               iter++) {
            delete *iter;
          }
          ...
          if (!bUseStretching) {
            ...
            if (*(stretching->getStretchingMode()) == "analytic") {
              ...
              for (size_t i = 0; i < DIM; i++) {
                ...
                ostream << std::scientific << stretchingType << " "
                        << str1d.x_0 << " " << str1d.xsi << std::endl;
        ...
        // valentjn: remove doxygen comment (slash-star-star) at the beginning as it caused doxygen warnings (despite being commented out)
        //         *
        //         * Converts this storage from AOS (array of structures) to SOA (structure of array)
        //         * with modification to speed up iterative function evaluation. The Level
        //         * array won't contain the levels, it contains the level to the power of two
          ...
          for (size_t i = 0; i < list.size(); i++) {
            for (size_t current_dim = 0; current_dim < DIM; current_dim++) {
              ...
              level.set(i, current_dim,
                        pow(2.0, static_cast<int>(-curLevel)));
            }
          }
          ...
          for (size_t i = 0; i < list.size(); i++) {
            for (size_t current_dim = 0; current_dim < DIM; current_dim++) {
              ...
              if (curLevel == 1) {
                ...
                mask.set(i, current_dim,
                         *reinterpret_cast<double*>(&intmask));
                ...
                offset.set(i, current_dim, 2.0);
                         ...
                         == static_cast<typename index_type::level_type>(((1
                             << curLevel) - 1))) {
                level.set(i, current_dim,
                          static_cast<double>(1 << curLevel));
                index.set(i, current_dim, static_cast<double>(curIndex));
                uint64_t intmask = 0x0000000000000000;
                mask.set(i, current_dim,
                         *reinterpret_cast<double*>(&intmask));
        ...
        void getLevelIndexMaskArraysForModEval(DataMatrixSP& level,
                                               DataMatrixSP& index, DataMatrixSP& mask, DataMatrixSP& offset) {
          typename index_type::level_type curLevel;
          typename index_type::level_type curIndex;
          ...
          for (size_t i = 0; i < list.size(); i++) {
            for (size_t current_dim = 0; current_dim < DIM; current_dim++) {
              ...
              if (curLevel == 1) {
                ...
                mask.set(i, current_dim,
                         *reinterpret_cast<float*>(&intmask));
                offset.set(i, current_dim, 1.0f);
              } else if (curIndex == 1) {
                level.set(i, current_dim,
                          (-1.0f) * static_cast<float>(1 << curLevel));
                index.set(i, current_dim, 0.0f);
                unsigned int intmask = 0x00000000;
                mask.set(i, current_dim,
                         *reinterpret_cast<float*>(&intmask));
            ...
            if (useStretching == 0) {
              ...
              for (size_t i = 0; i < DIM; i++) {
                ...
                switch (stretchingType) {
                  ...
                  default:
                    std::cout
                        << "Stretching Type Unknown in parseGridDescription\n";
                    break;
                }
              ...
              for (size_t i = 0; i < DIM; i++) {
                ...
                vectorLength = static_cast<int>(pow(2.0, discreteLevel))
                               + 1;
              ...
              std::cout
                  << "Unknown Container Id Given in parseGridDescription\n";
            }
          }

...
namespace SGPP {
  namespace base {
    ...
    float_t OperationNaiveEvalModFundamentalSpline::eval(const DataVector& alpha, const DataVector& point) {
      const size_t n = storage->size();
      const size_t d = storage->dim();
      float_t result = 0.0;

      #pragma omp parallel for shared(alpha, point, result) default(none)

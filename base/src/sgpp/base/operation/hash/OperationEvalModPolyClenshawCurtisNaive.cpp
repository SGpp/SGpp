...
#include "OperationEvalModPolyClenshawCurtisNaive.hpp"

namespace sgpp {
namespace base {

double OperationEvalModPolyClenshawCurtisNaive::eval(const DataVector& alpha,
...
void OperationEvalModPolyClenshawCurtisNaive::eval(const DataMatrix& alpha, const DataVector& point,
                                                   DataVector& value) {
  const size_t n = storage.getSize();
  const size_t d = storage.getDimension();

...
double OperationSecondMomentModBspline::doQuadrature(DataVector& alpha, DataMatrix* bounds) {
  // handle bounds
  ...
  if (bounds != nullptr && (bounds->getNcols() != 2 || bounds->getNrows() != numDims)) {
    throw application_exception(
        "OperationSecondMomentModBspline::doQuadrature - bounds matrix has the wrong shape");
  }

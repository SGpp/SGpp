...
double OperationSecondMomentPoly::doQuadrature(DataVector& alpha, DataMatrix* bounds) {
  // handle bounds
  ...
  const size_t quadOrder =  static_cast<size_t>(
    ceil(static_cast<double>(dynamic_cast<sgpp::base::PolyGrid*>(grid)->getDegree()) / 2.))
    + 2;

...
double OperationEvalPolyClenshawCurtisNaive::eval(const DataVector& alpha,
...
void OperationEvalPolyClenshawCurtisNaive::eval(const DataMatrix& alpha, const DataVector& point,
                                                DataVector& value) {
  const size_t n = storage.getSize();
  const size_t d = storage.getDimension();

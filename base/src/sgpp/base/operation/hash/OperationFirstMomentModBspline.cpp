...
double OperationFirstMomentModBspline::doQuadrature(const DataVector& alpha, DataMatrix* bounds) {
  // handle bounds
  ...
  if (bounds != nullptr && (bounds->getNcols() != 2 || bounds->getNrows() != numDims)) {
    throw application_exception(
        "OperationFirstMomentModBspline::doQuadrature - bounds matrix has the wrong shape");
  }
  ...
  for (GridStorage::grid_map_iterator iter = storage.begin(); iter != storage.end(); iter++) {
    ...
    for (size_t dim = 0; dim < storage.getDimension(); dim++) {
      ...
      double scaling = 1./hInv;  // for a single ModBspline section
      double sum_1d = 0.;

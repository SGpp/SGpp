...
#ifndef LAGRANGE_NOTAKNOT_SPLINE_BASE_DERIV2_HPP
#define LAGRANGE_NOTAKNOT_SPLINE_BASE_DERIV2_HPP
...
/**
 * Lagrange not-a-knot spline basis (2nd derivative).
 */
template <class LT, class IT>
class LagrangeNotAKnotSplineBasisDeriv2: public Basis<LT, IT> {
 public:
  /**
   * Default constructor.
   */
  LagrangeNotAKnotSplineBasisDeriv2() :
    lagrangeSplineBasisDeriv2(LagrangeSplineBasisDeriv2<LT, IT>()) {
  }
   ...
   */
  explicit LagrangeNotAKnotSplineBasisDeriv2(size_t degree) :
        lagrangeSplineBasisDeriv2(LagrangeSplineBasisDeriv2<LT, IT>(degree)) {
    if (getDegree() > 7) {
   ...
   * Destructor.
   */
  ~LagrangeNotAKnotSplineBasisDeriv2() override {
  }
    ...
    switch (getDegree()) {
      ...
      case 3:
        if (l == 0) {
          ...
          // l >= 3, 3 < i < 2^l - 3
          return lagrangeSplineBasisDeriv2.eval(l, i, x);
        } else {
          if (i > hInv / 2) {
            i = hInv - i;
            t *= -1.0;
  ...
  inline size_t getDegree() const {
    return lagrangeSplineBasisDeriv2.getDegree();
  }

 protected:
  /// B-spline basis for B-spline evaluation
  LagrangeSplineBasisDeriv2<LT, IT> lagrangeSplineBasisDeriv2;
};

// default type-def (unsigned int for level and index)
typedef LagrangeNotAKnotSplineBasisDeriv2<unsigned int, unsigned int>
SLagrangeNotAKnotSplineBaseDeriv2;

}  // namespace base
}  // namespace sgpp

#endif /* LAGRANGE_NOTAKNOT_SPLINE_BASE_DERIV2_HPP */

...
#ifndef LAGRANGE_NOTAKNOT_SPLINE_MODIFIED_BASE_HPP
#define LAGRANGE_NOTAKNOT_SPLINE_MODIFIED_BASE_HPP
...
class LagrangeNotAKnotSplineModifiedBasis: public Basis<LT, IT> {
 public:
   ...
   * Default constructor.
   */
  LagrangeNotAKnotSplineModifiedBasis() :
    lagrangeNotAKnotSplineBasis(LagrangeNotAKnotSplineBasis<LT, IT>()) {
  }
   ...
   */
  explicit LagrangeNotAKnotSplineModifiedBasis(size_t degree) :
    lagrangeNotAKnotSplineBasis(LagrangeNotAKnotSplineBasis<LT, IT>(degree)) {
    if (getDegree() > 7) {
   ...
   * Destructor.
   */
  ~LagrangeNotAKnotSplineModifiedBasis() override {
  }
    ...
    switch (getDegree()) {
      ...
      case 3:
        if (l == 1) {
          ...
          // l >= 3, 1 < i < 2^l - 1
          return lagrangeNotAKnotSplineBasis.eval(l, i, x);
        } else {
          if (i > hInv / 2) {
            i = hInv - i;
            t *= -1.0;
  ...
  inline size_t getDegree() const {
    return lagrangeNotAKnotSplineBasis.getDegree();
  }

 protected:
  /// Unmodified basis
  LagrangeNotAKnotSplineBasis<LT, IT> lagrangeNotAKnotSplineBasis;
};

// default type-def (unsigned int for level and index)
typedef LagrangeNotAKnotSplineModifiedBasis<unsigned int, unsigned int>
SLagrangeNotAKnotSplineModifiedBase;

}  // namespace base
}  // namespace sgpp

#endif /* LAGRANGE_NOTAKNOT_SPLINE_MODIFIED_BASE_HPP */

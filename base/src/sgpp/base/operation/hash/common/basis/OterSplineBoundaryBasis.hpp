...
class OterSplineBoundaryBasis : public Basis<LT, IT> {
 public:
  /**
   * Default constructor.
   */
  OterSplineBoundaryBasis() : nakBsplineBasis(NakBsplineBasis<LT, IT>()) {}
   ...
   */
  explicit OterSplineBoundaryBasis(size_t degree)
      : nakBsplineBasis(NakBsplineBasis<LT, IT>(degree)) {
    if (getDegree() > 7) {
      throw std::runtime_error("OterSplineBoundaryBasis: Unsupported B-spline degree.");
    }
  }
   ...
   * Destructor.
   */
  ~OterSplineBoundaryBasis() override {}
  ...
  inline double getMean(LT l, IT i, std::shared_ptr<sgpp::base::Distribution> pdf,
    ...
    if ((degree != 1) && (degree != 3) && (degree != 5)) {
      throw std::runtime_error(
          "OterSplineBoundaryBasis: only B spline degrees 1, 3 and 5 are "
          "supported.");
    }
...
typedef OterSplineBoundaryBasis<unsigned int, unsigned int> SOterSplineBoundaryBase;

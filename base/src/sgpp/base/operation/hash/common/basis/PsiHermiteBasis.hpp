...
/**
 * Psi basis function for Hermite Interpolation
    f(0)=1
    f'(0)=0
 */
template <class LT, class IT>
class PsiHermiteBasis : public Basis<LT, IT> {
 public:
  /**
   * Destructor.
   */
  ~PsiHermiteBasis() override {}
  ...
  inline double eval(LT l, IT i, double x) override {
    ...
    if (x_uniform > 1 || x_uniform < -1)
      ...
      return -2.0 * pow(x_uniform, 3) + -3.0 * pow(x_uniform, 2) + 1.0;
    } else {
      return 2.0 * pow(x_uniform, 3) - 3.0 * pow(x_uniform, 2) + 1.0;
    }
  }

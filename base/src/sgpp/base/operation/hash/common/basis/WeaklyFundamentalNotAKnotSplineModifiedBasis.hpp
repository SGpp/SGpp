...
#ifndef WEAKLY_FUNDAMENTAL_NOTAKNOT_SPLINE_MODIFIED_BASE_HPP
#define WEAKLY_FUNDAMENTAL_NOTAKNOT_SPLINE_MODIFIED_BASE_HPP
...
class WeaklyFundamentalNotAKnotSplineModifiedBasis: public Basis<LT, IT> {
 public:
  /**
   * Default constructor.
   */
  WeaklyFundamentalNotAKnotSplineModifiedBasis() :
    weaklyFundamentalNotAKnotSplineBasis(WeaklyFundamentalNotAKnotSplineBasis<LT, IT>()) {
  }
   ...
   */
  explicit WeaklyFundamentalNotAKnotSplineModifiedBasis(size_t degree) :
    weaklyFundamentalNotAKnotSplineBasis(WeaklyFundamentalNotAKnotSplineBasis<LT, IT>(degree)) {
    if (getDegree() > 7) {
   ...
   * Destructor.
   */
  ~WeaklyFundamentalNotAKnotSplineModifiedBasis() override {
  }
    ...
    switch (getDegree()) {
      ...
      case 3:
        if (l == 1) {
          ...
          // l >= 3, 1 < i < 2^l - 1
          return weaklyFundamentalNotAKnotSplineBasis.eval(l, i, x);
        } else {
          if (i > hInv / 2) {
            i = hInv - i;
            t *= -1.0;
  ...
  inline size_t getDegree() const {
    return weaklyFundamentalNotAKnotSplineBasis.getDegree();
  }

 protected:
  /// Unmodified basis
  WeaklyFundamentalNotAKnotSplineBasis<LT, IT> weaklyFundamentalNotAKnotSplineBasis;
};

// default type-def (unsigned int for level and index)
typedef WeaklyFundamentalNotAKnotSplineModifiedBasis<unsigned int, unsigned int>
SWeaklyFundamentalNotAKnotSplineModifiedBase;

}  // namespace base
}  // namespace sgpp

#endif /* WEAKLY_FUNDAMENTAL_NOTAKNOT_SPLINE_MODIFIED_BASE_HPP */

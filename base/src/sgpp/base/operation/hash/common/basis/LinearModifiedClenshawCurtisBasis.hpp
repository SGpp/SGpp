...
class LinearModifiedClenshawCurtisBasis : public Basis<LT, IT> {
 protected:
  /// poly basis
 ...
 public:
  ...
  double getIntegral(LT level, IT index) override {
    ...
    if (level == 1) {
      ...
      return linearBasis.getIntegral(level, index);
    }
  }
 ...
 private:
  ...
  double evalBasis(LT level, IT index, double p) {
    ...
    if (level == 1) {
      ...
      // interior basis function
      return linearBasis.eval(level, index, p);
    }
  }
};

// default type-def (unsigned int for level and index)
typedef LinearModifiedClenshawCurtisBasis<unsigned int, unsigned int>

...
class FundamentalSplineBasis : public Basis<LT, IT> {
 public:
   ...
   * Default constructor.
   */
  FundamentalSplineBasis() : bsplineBasis(BsplineBasis<LT, IT>()) {}
   ...
   */
  explicit FundamentalSplineBasis(size_t degree) : bsplineBasis(BsplineBasis<LT, IT>(degree)) {
    switch (bsplineBasis.getDegree()) {
      case 1:
        coefficients = {1.0};
        break;
      case 3:
        coefficients = {1.73205080756887729352744634150587236694280525381038,
        ...
        break;
      case 5:
        coefficients = {2.84217092202162248422242525148418205507454441028187,
        ...
        break;
      case 7:
        coefficients = {4.96473288630168129311811288638564282856915459001201,
        ...
        break;
      case 9:
        coefficients = {9.23670481112063673364106751485158808845875589510713,
        ...
        break;
      case 11:
        coefficients = {18.1022552378485207290090756895209587347655584558549,
  ...
  inline double eval(LT l, IT i, double x) override {
    ...
    const int kMin = std::max(static_cast<int>(std::floor(t)) - static_cast<int>(p + 1) + 1,
    ...
    for (int k = kMin; k <= kMax; k++) {
      y += coefficients[std::abs(k)] * bsplineBasis.uniformBSpline(t - static_cast<double>(k), p);
    }
  ...
  inline double evalDx(LT l, IT i, double x) {
    ...
    for (int k = kMin; k <= kMax; k++) {
      y += coefficients[std::abs(k)] * bsplineBasis.uniformBSplineDx(t - static_cast<double>(k), p);
    }
  ...
  inline double evalDxDx(LT l, IT i, double x) {
    ...
    for (int k = kMin; k <= kMax; k++) {
      y += coefficients[std::abs(k)] *
           bsplineBasis.uniformBSplineDxDx(t - static_cast<double>(k), p);
    }
        ...
        switch (j) {
          ...
          default:
            throw std::invalid_argument(
                "Error FundamentalSplineBasis integralValues. "

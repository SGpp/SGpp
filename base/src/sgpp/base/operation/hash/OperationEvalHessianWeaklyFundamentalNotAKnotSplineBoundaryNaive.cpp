...
double OperationEvalHessianWeaklyFundamentalNotAKnotSplineBoundaryNaive::evalHessian(const DataVector& alpha,
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    for (size_t t = 0; t < d; t++) {
      ...
      const double dx1d = baseDeriv1.eval(gp.getLevel(t), gp.getIndex(t), pointInUnitCube[t]) *
          innerDerivative[t];
      const double dxdx1d = baseDeriv2.eval(gp.getLevel(t), gp.getIndex(t), pointInUnitCube[t]) *
...
void OperationEvalHessianWeaklyFundamentalNotAKnotSplineBoundaryNaive::evalHessian(const DataMatrix& alpha,
                                                             const DataVector& point,
                                                             DataVector& value,
                                                             DataMatrix& gradient,

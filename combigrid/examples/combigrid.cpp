 ...
 *
 * In this example, we use the combigrid module to interpolate a test function
 * on a two-dimensional
 * regular sparse grid with the combination technique and hierarchical
 * B-splines.
 *
...
/**
 * We define some parameters such as dimensionality and level of the regular
 * sparse grid.
 */
int main() {
  // dimensionality
  ...
  /**
   * The basis functions are defined via an sgpp::combigrid::HeterogeneousBasis
   * object. In contrast
   * to sgpp::base::Basis, this allows for different types of basis functions
   * for the different
   * dimensions. However, for this example, we do not need this flexibility, so
   * we use the same
  ...
  /**
   * An sgpp::combigrid::CombinationGrid is a collection of full grids (nodal
   * subspaces) together
   * with scalar-valued coefficients. Here, we construct an
   * sgpp::combigrid::CombinationGrid object
   * for a regular sparse grid via the combination technique.
   */
  sgpp::combigrid::CombinationGrid combiGrid =
      sgpp::combigrid::CombinationGrid::fromRegularSparse(dim, n, basis,
                                                          hasBoundary);

  /**
   * We obtain the grid points of the regular sparse grid by combining the grid
   * points of all
  ...
  /**
   * We now want to perform an operation on each full grid. For this, we
   * distribute the values of
   * the combined grid (sparse grid) to the full grids. The result is a \c
   * std::vector of
   * sgpp::base::DataVector; each \c DataVector contains the values at all grid
   * points for one
   * specific full grid.
   */
  ...
  /**
   * The operation we want to perform on each full grid is hierarchization.
   * Since the grids are
   * full grids, we can use the unidirectional principle for this, which
   * performs 1D hierarchization
  ...
  sgpp::combigrid::OperationPoleHierarchisationGeneral::fromHeterogenerousBasis(
      basis, opPole);
  ...
  /**
   * The resulting surpluses are also a \c std::vector of
   * sgpp::base::DataVector, separated by full
   * grids. We could combine the full grid surpluses via the combination formula
   * to the sparse grid
   ...
   *
   * We evaluate the combined function (combination of all full grid
   * interpolants) at some arbitrary
  ...
  /**
   * Finally, we do the same for one full grid of the combination grid: We
   * evaluate the
   * corresponding interpolant. We extract the surpluses from the already
   * calculated \c vector of
   * \c DataVector. Alternatively, we could also apply
   * sgpp::combigrid::OperationUPFullGrid with
  ...
  const sgpp::combigrid::FullGrid& fullGrid =
      combiGrid.getFullGrids()[fullGridIndex];

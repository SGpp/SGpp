...
combigrid::PoissonOperator::PoissonOperator(const FullGridD* fg ,
    ...
    const combigrid::CallBackRHS* callbackRHS) : combigrid::OperatorFG(fg , 1) ,
  sigma_(sigma) , callbackRHS_(callbackRHS) {
  ...
  for (int d = 0 ; d < dim_ ; d++ ) {
    COMBIGRID_OUT_LEVEL2( verb ,
                          "PoissonOperator::PoissonOperator test level of dimension d = " << d );
    COMBIGRID_ERROR_TEST( getFG()->getLevels()[d] >= 1 ,
                          " Each dimension must have at least level 2 getFG()->getLevels()[" << d << "]="
                          << getFG()->getLevels()[d]);
    COMBIGRID_OUT_LEVEL2( verb ,
                          "PoissonOperator::PoissonOperator test flag = " << d );
  ...
  nrMatrixElem_ = 0;
  COMBIGRID_OUT_LEVEL2( verb ,
                        "PoissonOperator::PoissonOperator dim_=" << dim_ << " , nrElem_=" << nrElem_ );
  COMBIGRID_OUT_LEVEL2( verb ,
                        "PoissonOperator::PoissonOperator count the number of possible non-zero matrix entries");
  int e = -2 , i = -2 , tmp = -2;
  ...
  COMBIGRID_OUT_LEVEL2( verb ,
                        "PoissonOperator::PoissonOperator set up the CRS structure nrMatrixElem_=" <<
                        nrMatrixElem_);
  ...
  for (e = 0 ; e < nrElem_ ; e++) {
    ...
    for (i = dim_ - 1 ; i >= 0 ; i--) {
      axisIndex[i] = tmp / (getFG()->getOffset(i));
      tmp = tmp % getFG()->getOffset(i);
      COMBIGRID_OUT_LEVEL5( verb ,
                            "axisIndex[" << i << "]=" << axisIndex[i] << "  tmp = " << tmp);
    }
    ...
    if (!isBoundaryPoint[e]) {
      for (i = 0 ; i < dim_ ; i++) {
        ...
        COMBIGRID_OUT_LEVEL5( verb ,
                              " i = " << i << " col_ind_[" << actualP << "]=" << col_ind_[actualP] );
        actualP = actualP + 1;
  ...
  COMBIGRID_OUT_LEVEL2( verb ,
                        "PoissonOperator::PoissonOperator  add the laplace ");
  ...
  for ( e = 0 ; e < nrElem_ ; e++) {
    ...
    if (isBoundaryPoint[e]) {
      ...
      for (i = dim_ - 1 ; i >= 0 ; i--) {
        ...
        COMBIGRID_OUT_LEVEL5( verb ,
                              "axisIndex[" << i << "]=" << axisIndex[i] << " , getFG()->length(i)=" <<
                              getFG()->length(i));
        ...
        COMBIGRID_OUT_LEVEL5( verb ,
                              " add diffusion , sigma_[" << i << "]=" << sigma_[i] << " , h0=" << h0
                              << " , h1=" << h1 << " , p_e_m=" << p_e_m << " , p_e=" << p_e << " , p_e_p=" <<
                              p_e_p);
        ...
        matrixVal_[p_e_m] = matrixVal_[p_e_m] - sigma_[i] * h1 / (0.5 * h0 * h1 *
                            (h0 + h1));
        matrixVal_[p_e]   = matrixVal_[p_e] + sigma_[i] * (h0 + h1) / (0.5 * h0 * h1 *
                            (h0 + h1));
        matrixVal_[p_e_p] = matrixVal_[p_e_p] - sigma_[i] * h0 / (0.5 * h0 * h1 *
                            (h0 + h1));
      }
    }
  }
...
combigrid::OperatorFG* combigrid::PoissonOperator::factory(
  const FullGridD* fg) const {
  return (new combigrid::PoissonOperator( fg , sigma_ , callbackRHS_ ));
}

void combigrid::PoissonOperator::getRHS(std::vector<double>& rhs ,
                                        int& nrSpace) const {
...
void combigrid::PoissonOperator::multiplyVector(std::vector<double>& inVect ,
...
void combigrid::PoissonOperator::doSmoothing(int nrIt ,
  ...
  double tmp = 0.0;
  COMBIGRID_OUT_LEVEL2( verb ,
                        "PoissonOperator::doSmoothing ... START it:" << nrIt);

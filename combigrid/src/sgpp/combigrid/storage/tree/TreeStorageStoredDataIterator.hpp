/*
 * TreeStorageStoredDataIterator.hpp
 *
 ...
 */
template<typename T> class TreeStorageStoredDataIterator: public AbstractMultiStorageIterator<T> {
	...
	int moveToNextImpl() {
		...
		// in all other cases, we are at the end in the last dimension, so we have to advance in another dimension
		lowestIndex = 0;
		...
		if (dim < numInternalDimensions) {
			internalNodes[dim] = (InternalTreeStorageNode<T> *) internalNodes[d]->children[firstValue].get();

			++dim;
			while (dim < numInternalDimensions) {
				internalNodes[dim] = (InternalTreeStorageNode<T> *) internalNodes[dim - 1]->children[0].get();
				++dim;
			}
		...
		lowestNode = (LowestTreeStorageNode<T> *) internalNodes[lastInternalDim]->children[firstValue].get();
		/*if (lowestNode->elements.size() == 0) {
			// it is not valid to have an empty node for efficiency reasons except if the tree is completely empty
...
public:
	TreeStorageStoredDataIterator(AbstractTreeStorageNode<T> *root, size_t numDimensions) :
			internalIndex(numDimensions - 1, 0), internalNodes(), lowestNode(nullptr), lowestIndex(0), valid(true) {
		auto current = root;
	...
	virtual int moveToNext() {
		...
		} while(lowestIndex >= lowestNode->statusVector.size() || lowestNode->statusVector[lowestIndex] != StorageStatus::STORED);

		return h;
	}

	virtual T &value() {
		return lowestNode->elements[lowestIndex];
	}

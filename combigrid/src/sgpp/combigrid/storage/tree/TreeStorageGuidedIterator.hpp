...
#include "../../common/MultiIndexIterator.hpp"
 ...
 */
template<typename T> class TreeStorageGuidedIterator: public AbstractMultiStorageIterator<T> {
	...
	AbstractTreeStorageNode<T> *getChild(InternalTreeStorageNode<T> *node, size_t depth, size_t index, MultiIndex const &multiIndex) {
		if (index >= node->children.size()) {
			...
			if (remainingDimensions >= 2) {
				while (index >= node->children.size()) {
					CGLOG("TreeStorageGuidedIterator::getChild(): create internal tree storage node at depth " << depth);
					node->children.emplace_back(
							new InternalTreeStorageNode<T>(node->context, remainingDimensions - 1, multiIndex, depth + 1, depth, true));
				}
			} else {
				while (index >= node->children.size()) {
					CGLOG("TreeStorageGuidedIterator::getChild(): create lowest tree storage node at depth " << depth);
					node->children.emplace_back(new LowestTreeStorageNode<T>(node->context, multiIndex, depth + 1, depth, true));
				}
			}
		}
...
public:
	...
	TreeStorageGuidedIterator(IterationPolicy const &policy, AbstractTreeStorageNode<T> *root, size_t numDimensions, MultiIndexIterator &iterator) :
			iterator(iterator), permutedIndex(numDimensions, 0), internalNodes(), lowestNode(nullptr), valid(true), policy(policy) {
	...
	virtual int moveToNext() {
		...
		// we assume dim >= 1 since h < numDimensions (otherwise the iteration would stop and then h == -1)
		...
		if (dim < numInternalDimensions) {
			internalNodes[dim] = (InternalTreeStorageNode<T> *) getChild(internalNodes[d], d, nextValue, permutedIndex);
			nextValue = policy.resetAndGetValue(dim, 0);
			permutedIndex[dim] = nextValue;

			++dim;
			while (dim < numInternalDimensions) {
				internalNodes[dim] = (InternalTreeStorageNode<T> *) getChild(internalNodes[dim - 1], dim - 1, nextValue, permutedIndex);
				nextValue = policy.resetAndGetValue(dim, 0);
				permutedIndex[dim] = nextValue;
		...
		lowestNode = (LowestTreeStorageNode<T> *) getChild(internalNodes[lastInternalDim], lastInternalDim, nextValue, permutedIndex);
	...
	virtual size_t indexAt(size_t d) const {
		return iterator.indexAt(d);
	}

	virtual MultiIndex getMultiIndex() const {
		return iterator.getMultiIndex();
	}

	virtual bool computationRequested() {
		size_t lastDim = internalNodes.size();
		size_t permutedLowestIndex = policy.value(lastDim, iterator.indexAt(lastDim));
		return permutedLowestIndex < lowestNode->statusVector.size() && lowestNode->statusVector[permutedLowestIndex] >= StorageStatus::REQUESTED;
	}
	...
	virtual std::function<T()> requestComputationTask() {
		...
		return [myLowestNode, myPermutedIndex]() {
			return myLowestNode->context.func(myPermutedIndex);
		};
	}
};

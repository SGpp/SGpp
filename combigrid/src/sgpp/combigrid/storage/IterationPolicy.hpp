...
/**
 * With the IterationPolicy class, an AbstractMultiStorage can be configured to traverse its entries
 * in a different order than the identity order.
 * Therefore, an IterationPolicy can store a AbstractPermutationIterator for each dimension, which,
 * if available, may define an other order in this dimension.
 */
class IterationPolicy {
 ...
 public:
   ...
   */
  bool hasCustomIterator(size_t d) const { return d < iterators.size() && iterators[d]; }

  /**
   * @return Returns an iterator if there is one and nullptr if there is no custom iterator for this
   * dimension.
   */
  ...
  /**
   * Advances the iterator in dimension d, if available, to the next position and returns the
   * permuted index at this position.
  ...
  /**
   * @return Returns the value in dimension d if there is a custom iterator, or else the default
   * value.
   */
  ...
  /**
   * Resets the iterator in dimension d and returns its value, if available. Returns the default
   * value otherwise.
   */

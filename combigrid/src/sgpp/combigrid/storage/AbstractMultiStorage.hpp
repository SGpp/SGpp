...
/**
 * Interface for storage classes which store values of type T. These values can be accessed via a multi-index.
 * The (efficiency-optimized) TreeStorage class poses certain restrictions on which values are automatically included into the storage if other values are included.
 */
template<typename T> class AbstractMultiStorage {
public:
	virtual ~AbstractMultiStorage(){}

	// virtual std::string serialize() const = 0; // TODO: add SerializationStrategy
	// virtual void deserialize(std::string const &) = 0;

	/**
	 * @return Returns a reference to the element stored at the given MultiIndex. If the element was not yet in the storage, it is created.
	 */
	...
	/**
	 * @return Returns an iterator that iterates over all values that are already stored in the storage.
	 */
	 ...
	 * @return Returns an iterator that iterates over all values that indexIter iterates over, possibly creating them on-the-fly.
	 * Depending on the given policy, the values are not traversed in their natural order but in an order given by the iterators in the policy.
	 */

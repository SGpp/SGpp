...
/**
 * FullGridTensorEvaluator can be used to combine one-dimensional evaluation methods to a multi-dimensional evaluation method on a full grid.
 * For every grid point, the evaluators in every dimension are consulted to get basis coefficients for this dimension.
 * These coefficients are multiplied together and multiplied with the function value at that grid point.
 * The values generated for each grid point in this way are then summed up to compute the resulting value.
 * The concrete grid is given by PointHierarchy-Objects for each dimension and a level-multi-index encoding a level for each dimension.
 */
template<typename V> class FullGridTensorEvaluator: public AbstractFullGridEvaluator<V> {
	// partialProducts[i] stores the product of the first i basis values (corresponding to the current multiindex)
	// , i. e. partialProducts[0] = 1
	// partialProducts has Size numDimensions, since the product partialProducts[numDimensions] is only used once and does not have to be stored
...
public:
	 ...
	 * @param evaluatorPrototypes prototype objects for the evaluators that are cloned to get an evaluator for each dimension and each level.
	 * @param pointHierarchies PointHierarchy objects for each dimension providing the points for each level and information about their ordering.
	 */
	FullGridTensorEvaluator(std::shared_ptr<AbstractCombigridStorage> storage,
			std::vector<std::shared_ptr<AbstractLinearEvaluator<V>>> evaluatorPrototypes,
			std::vector<std::shared_ptr<AbstractPointHierarchy>> pointHierarchies)
	...
	std::vector<ThreadPool::Task> getLevelTasks(MultiIndex const &level, ThreadPool::Task callback, std::mutex &lock) {
		...
		if(computationTasks.empty()) {
			...
			for (size_t i = 0; i < computationTasks.size(); ++i) {
				...
				tasks.push_back([compTask, index, counter, &lock, callback, this, level](){
	...
	virtual V eval(MultiIndex const &level) {
		...
		// if not already stored, the evaluators for the given level are cloned and their parameter, if needed, is set
		// the ordering configuration is created - it stores a boolean for each dimension expressing whether the corresponding evaluator needs sorted (ascending) points
		...
		// for efficient computation, the products over the first i evaluator coefficients are stored for all i up to n-1.
		// This way, we only have to multiply them with the values for the changing indices at each iteration step.
	...
	/**
	 * Sets the parameters for the evaluators. Each dimension in which the evaluator does not need a parameter is skipped.
	 * So if only the evaluators at dimensions 1 and 3 need a parameter, params.size() should be 2 (or at least 2)
	 */
	void setParameters(std::vector<V> const &params) {
		...
		// we can't just set the parameters to the prototypes because the prototypes might be identical (the pointer to one prototype might be duplicated)
		for(size_t d = 0; d < numDimensions; ++d) {

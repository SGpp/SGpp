...
void LevelManager::tryAddLevel(const MultiIndex& level) {
	...
	for (auto &predLevel : predecessors) {
		...
		} else if (predInfo->computationStage == ComputationStage::STARTED || predInfo->computationStage == ComputationStage::TERMINATED) {
			--levelInfo->numNotStartedPredecessors;
		}
	}
...
void LevelManager::addToQueue(const MultiIndex& level, std::shared_ptr<LevelInfo> levelInfo) {
	float_t priority = computePriority(level);
...
void LevelManager::predecessorsCompleted(const MultiIndex& level) {
	...
	for (auto &succLevel : successors) {
		...
		if (succInfo->computationStage == ComputationStage::TERMINATED && succInfo->numNotCompletedPredecessors == 0) {
...
void LevelManager::tryAddSuccessorsToQueue(MultiIndex const &index) {
...
void LevelManager::tryAddIndexToQueue(MultiIndex const &nextIndex) {
	if (combiEval->containsLevel(nextIndex)) {
		return;
	}
	...
	for (size_t prevDim = 0; prevDim < numDimensions; ++prevDim) {
		if (nextIndex[prevDim] > 0) {
			...
			if (combiEval->containsLevel(prevIndex)) {
				float_t norm = combiEval->getDifferenceNorm(prevIndex);
	...
	if (hasAllPredecessors) {
		size_t maxNewPoints = combiEval->maxNewPoints(nextIndex);
...
void LevelManager::setRefinementStrategy(AdaptiveRefinementStrategy strategy) {
...
void LevelManager::precomputeLevelsParallel(const std::vector<MultiIndex>& levels, size_t numThreads) {
...
void LevelManager::addLevelsAdaptive(size_t maxNumPoints) {
	...
	while (!queue.empty()) {
		...
		afterComputation(entry.level); // the actual computation is done here, in addLevel() and the successors are updated here
	}
}

...
public:
	CombigridMultiOperationImpl(std::vector<std::shared_ptr<AbstractPointHierarchy> > pointHierarchies,
			std::vector<std::shared_ptr<AbstractLinearEvaluator<FloatArrayVector> > > evaluatorPrototypes,
			std::shared_ptr<AbstractCombigridStorage> storage) :
			storage(storage), fullGridEval(new FullGridTensorEvaluator<FloatArrayVector>(storage, evaluatorPrototypes, pointHierarchies)), combiEval(
					new CombigridEvaluator<FloatArrayVector>(pointHierarchies.size(), fullGridEval)) {
...
CombigridMultiOperation::CombigridMultiOperation(std::vector<std::shared_ptr<AbstractPointHierarchy> > pointHierarchies,
		std::vector<std::shared_ptr<AbstractLinearEvaluator<FloatArrayVector> > > evaluatorPrototypes,
		MultiFunction func) :
		impl(new CombigridMultiOperationImpl(pointHierarchies, evaluatorPrototypes, std::shared_ptr<AbstractCombigridStorage>(new CombigridTreeStorage(pointHierarchies, func)))) {
}
...
base::DataVector CombigridMultiOperation::evaluate(size_t q, std::vector<base::DataVector> const &params) {
	// TODO: check dimensionalities and sizes...
...
base::DataVector CombigridMultiOperation::evaluateAdaptive(size_t maxNumPoints, std::vector<base::DataVector> const &params) {
	// TODO: check dimensionalities and sizes...
	std::vector<FloatArrayVector> vecs(params[0].getSize());
...
std::shared_ptr<CombigridMultiOperation> CombigridMultiOperation::createExpClenshawCurtisPolynomialInterpolation(size_t numDimensions,
		MultiFunction func) {
	return std::make_shared<CombigridMultiOperation>(
			...
			std::vector<std::shared_ptr<AbstractLinearEvaluator<FloatArrayVector> > >(numDimensions,
				std::make_shared<ArrayEvaluator<BarycentricInterpolationEvaluator>>(true)),
			func);
}

std::shared_ptr<CombigridMultiOperation> CombigridMultiOperation::createLinearClenshawCurtisPolynomialInterpolation(size_t numDimensions,
		MultiFunction func) {
	return std::make_shared<CombigridMultiOperation>(
			std::vector<std::shared_ptr<AbstractPointHierarchy> >(numDimensions,
...
std::shared_ptr<CombigridMultiOperation> CombigridMultiOperation::createExpLejaPolynomialInterpolation(
...
std::shared_ptr<CombigridMultiOperation> CombigridMultiOperation::createLinearLejaPolynomialInterpolation(
...
std::shared_ptr<CombigridMultiOperation> CombigridMultiOperation::createLinearLejaQuadrature(size_t numDimensions, MultiFunction func,
		size_t growthFactor) {
	return std::make_shared<CombigridMultiOperation>(
					std::vector<std::shared_ptr<AbstractPointHierarchy> >(numDimensions,
						std::make_shared<NestedPointHierarchy>(std::make_shared<LejaPointDistribution>(), std::make_shared<IdentityPointOrdering>(std::make_shared<LinearGrowthStrategy>(growthFactor), false))),
					std::vector<std::shared_ptr<AbstractLinearEvaluator<FloatArrayVector> > >(numDimensions,
						std::make_shared<ArrayEvaluator<QuadratureEvaluator>>(false)),
					func);
}

std::shared_ptr<CombigridMultiOperation> CombigridMultiOperation::createExpUniformLinearInterpolation(size_t numDimensions,
		MultiFunction func) {
	return std::make_shared<CombigridMultiOperation>(
			std::vector<std::shared_ptr<AbstractPointHierarchy> >(numDimensions,
			...
			std::vector<std::shared_ptr<AbstractLinearEvaluator<FloatArrayVector> > >(numDimensions,
				std::make_shared<ArrayEvaluator<LinearInterpolationEvaluator>>(true)),
			func);
}

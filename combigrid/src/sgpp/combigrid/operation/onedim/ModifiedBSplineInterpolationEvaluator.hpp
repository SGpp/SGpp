...
namespace combigrid {
/**
 * This evaluator calculates the modified B spline evaluations b_i(evaluatioinPoint) for modified B
 * splines b_i of degree 'degree' and saves these in basisValues
 */

class ModifiedBSplineInterpolationEvaluator : public AbstractLinearEvaluator<FloatScalarVector> {
 public:
  ModifiedBSplineInterpolationEvaluator();
  explicit ModifiedBSplineInterpolationEvaluator(size_t degree);
  virtual ~ModifiedBSplineInterpolationEvaluator();
  ModifiedBSplineInterpolationEvaluator(ModifiedBSplineInterpolationEvaluator const &other);

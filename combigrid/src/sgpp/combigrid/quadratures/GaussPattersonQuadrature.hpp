...
class GaussPattersonQuadrature: public AbstractQuadratureRule<_Tp> {
 ...
 public:
  ...
  /**
     * A constructor for the current quadrature class. When the constructor is called,
     * it pre-loads all coefficients for the 1-dimensional Gauss-Patterson quadrature up to a
     * specified maximal resolution
     *Due to the high numerical accuracy of the GP quadrature rule, we have precomputed the abscissas and the
     * quadrature coefficients up to a maximum of level 9, i.e. 513 grid points.
     *
     * For an Npoint quadrature formula , where N = 2^level + 1, the polynomial degree of accuracy
  ...
  /**
   * Overwrites the inherited "integrate" function from QuadratureRule. This "GausPattersonQuadrature" class encapsulates the
   * implementation of, well, the Gaus-Patterson quadrature rule on a combigrid. Polynomial degree of exactness
  ...
  /**
   *  A public static class method that retrieves the Clenshaw-Curtis' quadrature coefficients for level in_level, and stores them
 ...
 private:
  ...
  /*dim-dimensional FULL GRID implementation of the GAUSS-PATTERSON quadrature rule.
   *
   */

  _Tp gauss_patterson_fullGrid(int dim,

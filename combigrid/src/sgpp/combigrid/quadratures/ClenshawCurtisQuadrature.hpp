...
class ClenshawCurtisQuadrature: public AbstractQuadratureRule<_Tp> {
 ...
 public:
  ...
  /**
   * A constructor for the ClenshawCurtisQuadrature class. When the constructor is called,
   * it pre-computes all coefficients for the 1-dimensional Clenshaw-Curtis quadrature up to a
  ...
  /**
   * Overwrites the inherited "integrate" function from QuadratureRule. This "ClenshawCurtisQuadrature" class encapsulates the
   * implementation of, well, the Clenshaw-Curtis quadrature rule on a combigrid.
   *
   * @param grids - an implementation of the tamplate Combigrid containing the data we wish to integrate on
   ...
   * of the integrate function, the quadrature rule is applied to whatever information is stored on the points of the
 ...
 private:
   ...
   * of size 2^(d+1) + 1 containing the clenshaw-curtis coefficients for a 1-D grid of size 2^(d+1) + 1*/
  _Tp** coefficients;

...
_Tp combigrid::ClenshawCurtisQuadrature<_Tp>::integrate(CombiGrid<_Tp>* grids,
	...
	/**
	 * this flag (badstretching) specifies if the clenshaw_fullGridIntegration function should
	 * interpolate the abscissas it uses for computing the integral or
	 * simply probe the data stored in the underlying full grid.
	 *
	 *	Interpolate will be true if the combigrids' stretching type IS NOT CHEBYSHEV, as required by the
	 *	quadrature rule.
	 */
...
#pragma omp for reduction(+:result) schedule(dynamic,1)
		for (int j = 0; j < nr_fullgrids; j++) {
			...
			if (grids->getFullGrid(j)->isActive()) {
				if ((int) grids->getFullGrid(j)->getMaxLevel() > MAX_LEVELS) // problem with parallelization - if this error occurs the
						// end result might not be set
						{
					COMBIGRID_OUT_ERR(
							"CLENSHAW-CURTIS QUADRATURE FAILED: Active full-grid has level greater than the currently "
					...
					result += (_Tp) grids->getCoef(j)
							* clenshaw_curtis_fullgrid(dim, f,
...
template<typename _Tp>
_Tp combigrid::ClenshawCurtisQuadrature<_Tp>::clenshaw_curtis_fullgrid(int dim,

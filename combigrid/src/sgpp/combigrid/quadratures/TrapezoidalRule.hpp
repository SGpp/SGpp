...
class TrapezoidalRule: public AbstractQuadratureRule<_Tp> {
 ...
 public:
  ...
  /**
   * Overwrites the inherited "integrate" function from QuadratureRule. This "TrapezoidalRule" class encapsulates the
   * implementation of, well, the Trapezoidal Rule quadrature rule on a combigrid.
   *
   *  Note that calling the constructor initializes the pre-computation of the trapezoidal rule
   *  coeffs on the interval [-1;1] !!! This is done so
   *  to maintain consistency with the quadrature coefficients for the Clenshaw-Curtis and Gauss-Patterson rules.
   *  if you wish to integrate over an arbitrary interval, PLEASE setup a CombiEquidistantStretching class
   *  for the GridDomain, as this class implements all relevant transformations, in effect allowing numerical integration
   *  over arbitrary finite, semi-infinite or infinite intervals...
   *
   *
   * @param grids - an implementation of the Combigrid template containing the data we wish to intergrate on
   ...
   */
  _Tp integrate( CombiGrid<_Tp>* grids,
                 _Tp (*f)(std::vector<double>) = NULL);


  /**
  ** pre-computes the trapezoidal rule coeffs on the interval [-1;1]!!! This is done so
  *  to maintain consistency with the quadrature coefficients for the Clenshaw-Curtis and Gauss Patterson rules.
  *  if you wish to integrate over an arbitrary interval, PLEASE setup a CombiEquidistantStretching class
  *  for the GridDomain, as this class implements all relevant transformations, in effect allowing numerical
  *  over arbitrary finite, semi-infinite or infinite intervals....
  *
  *  @param in_level - an integer specifying the level for which this quadrature's coefficients shall be calculated.

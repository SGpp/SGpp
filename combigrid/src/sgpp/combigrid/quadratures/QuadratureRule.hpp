...
// need to include these headers to reference
// the corresponding "calculateCoefficients" static methods!
...
class QuadratureRule : public AbstractQuadratureRule<_Tp> {
 public:
  /**
   * A general quadrature rule class, performing integration over a combigrid
   *with arbitrary stretching along different directions.
   * For example, integration for a 2-D grid with equidistant stretching along
   *x-direction and say Chebyshev-stretched grid along y, will be
   * calculated with the coefficients for the trapezoidal rule and the
   *clenshaw-curtis rule respectively.
   * Based on the underlying stretching type, the class AUTOMATICALLY chooses
   *the most optimal set of coefficients according to the following
   * mapping:
   *
   ...
   * @return an instance of the class QuadratureRule with pre-compute
   *coefficients for trapezoidal, clenshaw-curtis, Gauss-Patterson,
   * and Basu quadrature rules up to (including) level = max_lvl
   *
   */
  explicit QuadratureRule(int max_lvl = 9);

  /***
   * @param max_lvls an integer vector setting the maximum level to pre-compute
   *the coefficients for each of the (currently supported)
   * 4 basic quadrature rules ( keeping the ordering ) {TrapezoidalRule,
   *Clenshaw-Curtis,Gauss-Patterson, Basu}.
   ...
   *
   */
  explicit QuadratureRule(std::vector<int> max_lvls);

...
class AbstractQuadratureRule {
 public:
  /**
   * Pure virtual, interface function which is inherited from all quadrature
   *rules
   * residing in the "quadrature" folder. Multi-dimensional quadratures on
   *combi-grid
   * are implemented employing the "smolyaks'" construction, where the numerical
   *integral
   * on the combigrid is simply a linear combination of the numerical integral
   *values of the
   * underlying fullgrids, where the coefficients are namely the stadnard
   ...
   */
  virtual _Tp integrate(CombiGrid<_Tp>* grids, _Tp (*f)(std::vector<double>) = NULL) = 0;

  virtual ~AbstractQuadratureRule() { ; }

 protected:
  void getGridValues(FullGrid<_Tp>* grid, bool badstretching, AbstractStretchingMaker* stretching,
                     std::vector<_Tp>* f_values, _Tp (*f)(std::vector<double>)) {
    ...
    // precomupte the function values depending on whether or not the user wants
    ...
    if (f == NULL) {
      if (badstretching) {
        std::vector<std::vector<double> > stretchings(dim, std::vector<double>());
        std::vector<std::vector<double> > jacobs(dim, std::vector<double>());

        for (int d = 0; d < dim; d++)
          stretching->get1DStretching(grid->getLevels()[d], domain->getMin()[d],
                                      domain->getMax()[d], &(stretchings[d]), &(jacobs[d]));
        ...
        for (unsigned int j = 0; j < num_elem; j++) {
          ...
          /***
           * Here we add the jacobian in the whole computation... as for
           * infinite and semi-infinite intervals we are

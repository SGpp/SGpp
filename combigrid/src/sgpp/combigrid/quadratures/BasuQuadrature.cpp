		...
		for (int j = 0; j < nr_fullgrids; j++) {
			...
			if (grids->getFullGrid(j)->isActive()) {
				if ((int) grids->getFullGrid(j)->getMaxLevel() > MAX_LEVELS) // problem with parallelization - if this error occurs the
						...
						{
					COMBIGRID_OUT_ERR(
							"BASU QUADRATURE FAILED: Active full-grid has level greater than the currently "
					...
					result += (_Tp) grids->getCoef(j)
							* basu_full_grid(dim, f, grids->getFullGrid(j),
									badstretching);
			}
		}
	}
...
template<typename _Tp>
_Tp combigrid::BasuQuadrature<_Tp>::basu_full_grid(int dim,
...
void combigrid::BasuQuadrature<_Tp>::calculateCoefficients(int in_level,
	...
	*out_coefs = (_Tp*) malloc((N + 1) * sizeof(_Tp));
	...
	for (int s = 0; s <= N; s++) {
		...
		// we need to add this term as to correct for the fact that we are integrating a function in the general

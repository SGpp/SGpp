...
// at the end of the tuples array and modify the calculate coefficients method accordingly
#define NUM_QUADS 4
combigrid::QRtouple tuples[NUM_QUADS] = { { combigrid::EQUIDISTANT, 0 }, { combigrid::CHEBYSHEV, 1 }, { combigrid::LEGENDRE,
		2 }, { combigrid::BASU, 3 } };
...
#pragma omp parallel for schedule(dynamic,1)
	for (int j = 0; j < NUM_QUADS; j++) {
		coefficients[j] = (_Tp **) malloc(sizeof(_Tp*) * MAX_LEVELS[j]);
		for (int d = 1; d <= MAX_LEVELS[j]; d++) {
			/**
			 * For each of the quadrature rules (from 0 to NUM_QUADS)
			 *
			 * - allocate memory and pre-calculate values of the coefficients from levels 1,2... MAX_LEVELS's
			 *
			 * */
			QuadratureRule<_Tp>::calculateCoefficients(d,
					coefficients[j] + d - 1, tuples[j]);
		}
	}
}
...
_Tp combigrid::QuadratureRule<_Tp>::integrate(CombiGrid<_Tp>* grids,
	 ...
	 * simply do a straightforward evaluation of the data stored in the underlying full grid.
	 *
	 *	Interpolate will be true if the user has NOT explicitly specified a function pointer "f"
		...
		for (int j = 0; j < nr_fullgrids; j++) {
			...
			if (grids->getFullGrid(j)->isActive()) {
					result += (_Tp) grids->getCoef(j)
							* quadrature_full_grid(dim, f,
									grids->getFullGrid(j), interpolate);
			}
		}
	}
...
template<typename _Tp>
_Tp combigrid::QuadratureRule<_Tp>::quadrature_full_grid(int dim,
	...
	/** select which coefficient matrix shall we choose, depending on the stretching
...
// check for fully infinite integration domain.
	...
	if (domain != NULL) {
		...
		for (int d = 0; d < dim; d++) {
			...
			coef_selector[d] = stretchingToIdx(dom_1d.getStretchingType(),
					tuples,
					NUM_QUADS);
			if(dom_1d.getLevel() > MAX_LEVELS[coef_selector[d]]){
					COMBIGRID_OUT_ERR(
											" QUADRATURE FAILED: Active full-grid has level greater than the currently "
													"supported MAX level for the current quadrature! Will return 0.0!", __FILE__,
											__LINE__);
					return (_Tp)0.0;
			}
	...
	/**
	 *
	 *	I.2 to the function value at point j, superimpose the jacobian of the domain at that point...
	 */
...
// by the function pointer or he/she wants to sample the values stored on the grid...
	...
	if (f == NULL) //i.e. if we are not explicitly evaluating a function but rather "probing the values on the grid"
	{
		if (interpolate) {
			...
			for (unsigned int j = 0; j < num_elem; j++) {
				...
				f_values[j] = grid->getElementVector()[j] * jacobian_j; // simply pick out the value on the grid...
			}
	} else { // i.e. if the user has provided an external function for evaluation, perform the evaluation for now..
	...
	for (unsigned int j = 0; j < num_elem; j++) {
		...
		for (unsigned int d = 0; d < indices.size(); d++) {
			...
			if (idxToStretching(coef_selector[d], tuples, NUM_QUADS) == BASU
				...
				if (idx_d <= N_0)
					W_j *= (coefficients[coef_selector[d]])[level - 2][N_0
							- idx_d];
				else
					W_j *= (coefficients[coef_selector[d]])[level - 2][idx_d
							- N_0];
			} else
...
template<typename _Tp>
combigrid::Stretching combigrid::QuadratureRule<_Tp>::idxToStretching(int idx,
		QRtouple* qrtuples, int size) {
...
void combigrid::QuadratureRule<_Tp>::calculateCoefficients(int in_level,
	...
	case CHEBYSHEV: // clenshaw-curtis
		ClenshawCurtisQuadrature<_Tp>::calculateCoefficients(in_level,
				out_coefs);
		break;

	case LEGENDRE: // gauss-patterson
		GaussPattersonQuadrature<_Tp>::calculateCoefficients(in_level,
				out_coefs);
		break;
	...
	case ATAN:
		//TODO that might not yield good results
	case TAN:
		//TODO that might not yield good results

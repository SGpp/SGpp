...
class BasuQuadrature: public AbstractQuadratureRule<_Tp> {
 ...
 public:
  ...
  /**
   *  A public static class method that retrieves the BasuQuadrature's coefficients for level in_level, and stores them
 ...
 private:
   ...
   * of size 2^(d+1) + 1 containing the basu coefficients for a 1-D grid of size 2^(d+1) + 1*/
  _Tp** coefficients;

...
void combigrid::SerialCombiGrid::setDomainAllFG( combigrid::GridDomain*
    gridDomain ) const {
...
double combigrid::SerialCombiGrid::eval( std::vector<double>& coords ) const {
  ...
  if (gridDomain_ != NULL ) {
    gridDomain_->transformRealToUnit( coords , combischeme_->getMaxLevel() ,
                                      this->getBoundaryFlags() );
  }
  ...
  for ( int i = 0 ; i < combikernel_->getNrFullGrids() ; i++) {
    ...
    if (coef != 0.0) {
      result = result + combikernel_->getCoef(i) * combikernel_->getFullGrid(i)->eval(
                 coords_tmp);
...
double combigrid::SerialCombiGrid::evalSingleGrid(int index,
...
void combigrid::SerialCombiGrid::eval( std::vector< std::vector<double> >&
                                       coords , std::vector<double>& results ) const {
...
SGPP::base::GridStorage* combigrid::SerialCombiGrid::createSGppGridStorage()
const {
  SGPP::base::GridStorage* gridStoreSGpp = new SGPP::base::GridStorage(
    combikernel_->getDim() );
  combigrid::CombiSGppConverter::createSGpp( gridStoreSGpp , combikernel_ );
  return gridStoreSGpp;
}

void combigrid::SerialCombiGrid::reCompose(SGPP::base::GridStorage*
    gridstorageSGpp , SGPP::base::DataVector* alpha,
  ...
  if (minAlpha != NULL && maxAlpha != NULL) {
    ...
    for ( int i = 0 ; i < combikernel_->getNrFullGrids() ; i++) {
      ...
      combigrid::CombiSGppConverter::FullGridToSGpp( fg , combikernel_->getCoef(i) ,
          gridstorageSGpp , alpha , minAlpha , maxAlpha );
    }
  } else {
    ...
    for ( int i = 0 ; i < combikernel_->getNrFullGrids() ; i++) {
      const FullGridD* fg = combikernel_->getFullGrid(i);
      combigrid::CombiSGppConverter::FullGridToSGpp( fg , combikernel_->getCoef(i) ,
          gridstorageSGpp , alpha );
    }
  }
}

void combigrid::SerialCombiGrid::deCompose(SGPP::base::GridStorage*
    gridstorageSGpp , SGPP::base::DataVector* alpha) {
  for ( int i = 0 ; i < combikernel_->getNrFullGrids() ; i++) {

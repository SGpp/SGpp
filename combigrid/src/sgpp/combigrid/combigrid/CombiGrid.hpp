...
class CombiGrid {
 public:
   ...
   */
  virtual ~CombiGrid() { deleteFullGrids(); }
  ...
  /** adds a full grid with the specified level (dimension was specified in the
   * Ctor) and boundary
   ...
   * !! The actual computation of the hierarchical spaces and the combigrid
   *coefficients
   ...
   * is delegated to the underlying combischeme object!!!
   *
   *  re_init - a method similar to init(), but this one allows data
   *reusability!!
   *  Again the computation of the spaces
   *  and coefficients is delegated to the underlying combischeme, which  will
   *  examine the already existing full grids and as a result decide which of
   *them to
  ...
  const std::vector<bool>& getBoundaryFlags() const { return _hasBoundaryPts; }
   ...
   *
   * @param min - a vector of length dim, which contains the left boundaries of
   *the
   * domain in each direction (dimension)
     * @param max - a vector of length dim, which contains the right boundaries
   *of the
   * domain in each direction (dimension)
  ...
  /**
   * Iterates over all active grids of the combigrid and sets the grid domain to
   *dom
   *
   ...
   *application logic
   * (evaluation, fullgrid initialization, coefficient computation etc.) while
   *the combigrid serves
   * only as a data storage entity. All of the aforementioned operations that
   *have to be performed on a combigrid,
  ...
  unsigned int getNrFullGrids() const { return (unsigned int)_fgrids.size(); }
  /**
  ...
  std::vector<double> getDomainMin() { return _domainMin; }

  std::vector<double> getDomainMax() { return _domainMax; }
  ...
  std::vector<FGridContainer<_Tp>*> getFullgrids() { return _fgrids; }

...
class CombiS_CT : public AbstractCombiScheme<_Tp> {
 public:
  explicit CombiS_CT(std::vector<int> levels, int trunc_levels = 1);
 ...
 private:
   ...
   **/
  void getTrapezoidsums(std::vector<int>& v, size_t dim, int sum, std::vector<double>& ratio_,
                        std::vector<int>& l_user_,
                        std::vector<std::vector<int> >& out_levels_vector);

  void applyScheme(int in_dim, const std::vector<int>& in_levels, std::vector<double>& in_ratio_,
                   std::vector<int>& in_l_user_, std::vector<std::vector<int> >& out_levels_vector,
                   std::vector<_Tp>& out_coefs);
};
}  // namespace combigrid

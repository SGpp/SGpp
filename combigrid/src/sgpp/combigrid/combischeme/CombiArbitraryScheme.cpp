...
template<typename _Tp>
void combigrid::CombiArbitraryScheme<_Tp>::initCombiGrid(int in_dim,
	...
	out_coefs.clear();
	for (unsigned int i = 0; i < tmp_coef.size(); i++)
		out_coefs.push_back((_Tp) tmp_coef[i]);

}
template<typename _Tp>
void combigrid::CombiArbitraryScheme<_Tp>::re_initCombiGrid(int in_dim,
...
template<typename _Tp>
void combigrid::CombiArbitraryScheme<_Tp>::recomputeCoefficients(int in_dim,
		std::vector<FGridContainer<_Tp>*>& out_fgrids) {
	std::cout
			<< "TODO: IMPLEMENT CombiArbitraryScheme's recomputeCoefficients method \n";
}

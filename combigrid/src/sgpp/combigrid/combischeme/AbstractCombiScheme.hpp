...
class AbstractCombiScheme {
 ...
 public:
   ...
   * @param levels - a vector(list) of integers specifying the max level for
   *each dimension
   *
   */
  void setLevels(std::vector<int> levels) { _levels = levels; }
   ...
   * @param out_coefs (out) a vector containing the interpolation coeficients
   *for each level...
   *
   * */
   ...
   * @param in_fgrids - a list of FGridContainers, to be evaluated onto the
   *in_coords
...
// spawn a parallel region for for loop parallelization - due to the different
// workload for grid evaluation
// set the scheduling to dynamic with chunk size 1 in effect implementing
...
#pragma omp for reduction(+ : result) schedule(guided, 1)
      ...
      for (unsigned int i = 0; i < in_fgrids.size(); i++) {
        // we evaluate each full grid and multiply with the coefficient, and sum
        // the result up
        if (in_fgrids[i]->isActive()) {  // evaluate only if grid is active!
          ...
          result += coef * in_fgrids[i]->fg()->eval(coords_tmp);
        }
      }
    }
   ...
   * @param in_fgrids - a list of FGridContainers, the index-th element of which

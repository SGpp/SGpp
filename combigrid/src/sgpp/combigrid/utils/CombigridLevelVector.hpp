...
class CombigridLevelVector {
 public:
  ...
  explicit CombigridLevelVector(std::vector<int> level);
  ...
  explicit CombigridLevelVector(int dim);
  ...
  CombigridLevelVector(std::vector<std::vector<int> > in, std::vector<double> coef);
  ...
  int getDim() const { return static_cast<int>(levelVec_[0].size()); }

  /** Returns the number of full grids */
  int getN() const { return static_cast<int>(levelVec_.size()); }
  ...
  /** Operator unifying two CombigridLevelVectors. Full Grids of the same size

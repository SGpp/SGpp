...
combigrid::Domain1D::Domain1D(int level, double min, double max,
  ...
  if (level < 0) {
    std::cout
        << "Unsupported Operation Exception -> Cannot initialize a Domain with less than 1 point!";
...
void combigrid::Domain1D::transformRealToUnit(double coordReal,
    double& coordUnit, int level_in, bool noBoundary) const {
  ...
  if (_isStretched) {
    ...
    coordUnit = (((double) endInd) * combigrid::oneOverPowOfTwo[level_diff]
                 - intersec) / combigrid::powerOfTwo[_level - level_diff];
    ...
    if (noBoundary) {
      ...
      if (startInd == 0) {
        double h = _stretching[endInd + offs]
                   - _stretching[startInd + offs];
        intersec = (_stretching[endInd] - coordReal) / h;
        coordUnit = (((double) endInd)
                     * combigrid::oneOverPowOfTwo[level_diff] - intersec)
...
void combigrid::Domain1D::transformUnitToReal(int level, int index,
  ...
  if (_isStretched) {
    ...
    // no stretching , just simple scaling
    realCoord = _min
                + (_max - _min) * ((double) index) * oneOverPowOfTwo[level];
  }
}
...
void combigrid::Domain1D::findEntry(double coordReal, int level_in,
  ...
  if (_isStretched) {
    ...
    startIndex = static_cast<int>(::floor(
                                    (double) combigrid::powerOfTwo[level_in] * unitC));
    startIndex = (startIndex < 0) ? 0 : startIndex;
    startIndex =
      (startIndex >= combigrid::powerOfTwo[level_in] - 1) ?
...
void combigrid::Domain1D::getMeshWidth(int index, int level_in, double& h0,
  ...
  if (_isStretched) {
    ...
    h1 = h0 = (_max - _min)
              * (1 / (double) combigrid::powerOfTwo[level_in]);
  }
}

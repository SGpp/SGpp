 ...
 * 	 y_j = 0.5*(b+a) - 0.5*(b-a)*cos(Pi*j/(N-1)); , where N is the nr of points on the discrete grid...
 *	The class is called ChebyshevStretching, well because it results in a non-equidistant or "stretched" grid, and Chebyshev, because
 *	the points:
 *	cos(Pi*j/(N)) are the extrema of Chebyshev polynomial T_N(x), defined beautifully as  T_N(x) = cos(N*arcos(x))
 **/

void combigrid::CombiChebyshevStretching::get1DStretching(int level, double min,
	...
	{
		// transform the stretching with respect to the integration interval...n save the transformation jacobian...

...
class FullGrid {
 ...
 public:
  ...
  FullGrid( int dim , const std::vector<int>& levels,
            const std::vector<bool>& hasBdrPoints, const BasisFunctionBasis* basis = NULL) {
    // set the basis function for the full grid
    if (basis == NULL) basis_ = LinearBasisFunction::getDefaultBasis();
    else basis_ = basis;
  ...
  FG_ELEMENT eval(std::vector<double>& coords) const {
    ...
    if (gridDomain_ != NULL ) {
      for (ii = dim_ - 1 ; ii >= 0; ii--) {
        (gridDomain_->get1DDomain(ii)).transformRealToUnit( coords[ii] , coords[ii] ,
            levels_[ii] , (hasBoundaryPoints_[ii] == false));
      }
    }
    ...
    for ( ii = dim_ - 1 ; ii >= 0; ii--) {
      ...
      if (hasBoundaryPoints_[ii] == true ) {
        ...
        aindex[ii] = (aindex[ii] >= (int)(nrPoints_[ii] - 1)) ? (int)(
                       nrPoints_[ii] - 2) : aindex[ii];
        //calculate the coordinates
        normcoord =  normcoord - (double)aindex[ii];
      } else {
        aindex[ii] = (aindex[ii] < 0) ? 0 : aindex[ii];
        aindex[ii] = (aindex[ii] >= (int)(nrPoints_[ii])) ? (int)(
                       nrPoints_[ii]) : aindex[ii];
        normcoord =  normcoord - (double)aindex[ii];
        ...
        if (aindex[ii] <= 0) {
          ...
          if (aindex[ii] >= (int)nrPoints_[ii] - 1) {
            ...
            normcoord = coords[ii] * powerOfTwo[levels_[ii]] - (double)
                        aindex[ii]; // this should be 1 + ...
    ...
    for (ii = 0; ii < nr; ii++) {
      ...
      ret_val = (ii == 0) ? (baseVal * fullgridVector_[i]) : (ret_val + baseVal *
                fullgridVector_[i]);
    }
  ...
  void getCoords(int elemIndex , std::vector<double>& coords) const {
    ...
    if (gridDomain_ == NULL) {
      ...
      for (int j = 0 ; j < dim_ ; j++) {
        ...
        // set the coordinate based on if we have boundary points
        tmp_add = (hasBoundaryPoints_[j] == true) ? (0) : (1);
        (gridDomain_->get1DDomain(j)).transformUnitToReal( levels_[j] , ind + tmp_add ,
            coords[j] );
        //COMBIGRID_OUT_LEVEL3( verb , "FullGrid::getCoords j:" << j << " , coords[j]:" << coords[j]);
      }
    }

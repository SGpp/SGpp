...
class FullGrid {
 ...
 public:
  ...
  FG_ELEMENT eval(std::vector<double>& coords) const {
    ...
    if (gridDomain_ != NULL) {
      for (ii = dim_ - 1; ii >= 0; ii--) {
        (gridDomain_->get1DDomain(ii)).transformRealToUnit(coords[ii],
            coords[ii], levels_[ii],
    ...
    for (ii = dim_ - 1; ii >= 0; ii--) {
      ...
      if (hasBoundaryPoints_[ii] == true) {
        ...
        aindex[ii] =
          ...
          (int) (nrPoints_[ii] - 2) : aindex[ii];
        //calculate the coordinates
        normcoord = normcoord - (double) aindex[ii];
      } else {
        aindex[ii] = (aindex[ii] < 0) ? 0 : aindex[ii];
        aindex[ii] =
          (aindex[ii] >= (int) (nrPoints_[ii])) ?
          (int) (nrPoints_[ii]) : aindex[ii];
        normcoord = normcoord - (double) aindex[ii];
  ...
  /**
   *  PeTz:Implementation of the prolongation/reduction operations for a d-dimensional FullGrid!
   *  The function:
   *
   *     transform_gird(std::vector<int> new_levels ) : FullGrid<FG_ELEMENT>* ;
   *
   *  returns an extension (G_new) of the current grid (G) with levels vector new_levels and grid values which
   *  are linearly interpolated from G
   *
   * @param new_levels an integer vector of size "dim" specifying the desired levels for eahc of the dimensions of the new grid.
   * @return returns a FullGrid<FG_ELEMENT>* with levels vector equal to "new_levels" and linearly interpolated grid values
   *
   *
   *  Computational complexity is O( N 2^dim) where N is the total number of points in the new grid!
   */
  ...
  FullGrid<FG_ELEMENT>* transform_grid(std::vector<int> new_levels) {
    ...
    if (new_levels.size() != levels_.size()) {
      COMBIGRID_OUT_ERR(
        "Newly requested grid dimensions incompatible with existing grid's dimensions! Please reconsider your inputs and"
        "try again. Returning NULL!", __FILE__, __LINE__)
      return NULL;
    }
    ...
    for (int i = 0; i < new_levels.size(); i++) {
      if (new_levels[i] < 0) {
        COMBIGRID_OUT_ERR(
          "Not all newly requested grid levels are non-negative.! Please reconsider your inputs and"
          "try again. Returning NULL!", __FILE__,
          __LINE__)
        return NULL;
      }
    ...
    // if input is all right create new grid instance and interpolate elements....
    FullGrid<FG_ELEMENT>* new_grid = new FullGrid<FG_ELEMENT>(dim_,
        new_levels, hasBoundaryPoints_, basis_);
    ...
    if (gridDomain_ != NULL) {
      ...
      for (int d = 0 ; d < dim_; d++)
        stretchings[d] =
          combigrid::createStretchingMaker(gridDomain_->get1DDomain(
                                             d).getStretchingType());

      std::vector<double> min = gridDomain_->getMin();
      std::vector<double> max = gridDomain_->getMax();
      GridDomain* new_domain = new GridDomain(dim_, new_levels, min, max,
                                              stretchings);
    ...
    for (int j_k = 0; j_k < nr_elems; j_k++) {
      ...
      //calculate and store the interpolation coeffs for F[e] in each separate dimension
      ...
      for (int d = 0; d < dim_; d++) {
        ...
        interpol_coeffs[d * 2] = calculate_interpolation_coeff(x_d0,
                                 x_d1, k_coords[d], -1);
        interpol_coeffs[d * 2 + 1] = calculate_interpolation_coeff(x_d0,
                                     x_d1, k_coords[d], +1);
      ...
      for (int j = 0; j < powerOfTwo[dim_]; j++) {
        ...
        for (int d = 0; d < dim_; d++) {
          int b_d_j = (j & (1 << d)) >>
                      d; // get the d-th bit of j with a 000010000 mask (1 is at d-th position).
  ...
  /**
   *  PeTz: A quick and dirty function mapping the j_k-th index of a 1D grid of level k( denoted G_k)
   *
   ...
   *    mod(j_k,2^(k-l)) == 0
   *
   *  Even though the above condition is sometimes violated , this "transform_idx" function
   *  always returns the rounded value of j_k/2^(k-l), in effect mapping the j_k-th point to the left boundary of
  ...
  inline int bb_left(int j_k, int l, int k) const {
    ...
    int dL = k - l;
    return (dL >= 0 ? (j_k / powerOfTwo[dL]) : j_k * powerOfTwo[-dL]) - j_k /
           powerOfTwo[k];
    // at the end we subtract j_k/powerOfTwo[k] to ensure that points on the right boundary (i.e. when j_k = 2^k
    // are mapped onto the point 2^l-1, instead of 2^l!!! this is done because bb_left is supposed to give us
    // the index of the left boundary of the bounding box of the coordinate x_{j_k}...
  }
  ...
  /** returns the vector index for one linear index! Please note that irrespective of whether or not
   * the full grid data storage includes the boundary points, the vector indices ALWAYS assume that the
   * the abscissa point lying on the left boundary has index 0 and the one lying on the right boundary
   * has index 2^level + 1.
   *
   * Naturally the getVectorIndex method follows that convention. I.e. for a full grid of say 2 dimensions
   * and for e.g. 3 points per dimension,
   *  the linear index enumeration when bdries are included will look as follows:
   *      _____
   *  2  |6 7 8|
   *  1  |3 4 5|
   *  0  |0 1 2|
   *
   *     0 1 2
   *   for the above construction, a call to getVectorIndex(4,vector) shall return the vector (1,1)
   *
  ...
  inline void setDomain(GridDomain* gridDomain) {
 ...
 private:
   ...
   * PeTz:
   *
   * a simple  function returning the 1-D interpolation coefficient at point x within the interval [x_0;x_1]
   ...
   * @param direction - an integer specifying the boundary point we need the interpolation coefficient of. if direction is a negative number
   * , the method returns the interpolation coefficient for the left and if positive, for right boundary.
   * @return a double value in the range [0.0;1.0] specifying the 1D linear interpolation coefficient in the respective direction
   *
   * Needless to say, for the interpolation to make sense we need to ensure that
   * x_0 <= x <= x_1 and that direction != 0
   *
   * In case of one of those condition being violated, the method prints a warning and returns 0.0.  !!!!
   *
   */
  ...
  double calculate_interpolation_coeff(double x_0, double x_1, double x,
    ...
    if (direction == 0) {
      COMBIGRID_OUT_WRN(
        "interpolation direciton parameter = 0. Returning 0.0!!!",
        __FILE__, __LINE__)
      return 0.0;
    }
    ...
    if (x_0 >= x_1 || x < x_0 || x > x_1) {
      COMBIGRID_OUT_WRN(
        " Invalid interpolation parameters! Please reconsider your input and try again."
        "Returning 0.0!!!", __FILE__, __LINE__)
      return 0.0;
    }

    // now calculate the interpolation coefficient according to the linear interpolation formula...
    double coef = 0.0;
   ...
   * PeTz:
   *
   * this method finds the bounding box of the coordinate x, expressed as the d-dimensional cube 4
   * [x_i1;x_{i1+1}] x [x_i2;x_{i2+1}] ... [x_id;x_{id+1}]
   * where i1, i2 ... id are indices of the current grid's points.
   *
   * The method returns {i1,i2 ... id} stored in the out-vector "bb_idx" as well as the physical
   * coordinates {x_i1, x_i2 ... x_id} stored in the out-vector "bb_coord".
   *
   * @param k_idx - an int vector (of size dim) specifying the multi-index of the sample point
   * @param new_levels - an int vector , also of size dim, specifying the new grid's levels vector
   * @param bb_idx - (out param) an int vector (of size dim) containing the computed bounding box indices.
   * @param bb_coord - (out param) a double vector containing the  2*d points' coordinates specifying the bounding box of x!
   *
   *
   */

  void bounding_box(std::vector<int> k_idx, std::vector<int> new_levels,
                    std::vector<int>& bb_idx, std::vector<double>& bb_coord) const {

    if (k_idx.size() != dim_ || bb_idx.size() != dim_) {
      COMBIGRID_OUT_WRN(
        "Cannot find current point's bounding box! Invalid input parameters. ",
        __FILE__, __LINE__)
      return;
    }
    ...
    if (gridDomain_ == NULL) {
      /**
       *  No grid domain means that the grid consists of equally distributed points in the interval [0;1]
       *
       */
      for (int d = 0; d < dim_; d++) {
        // calculate the interval width in the original level-l grid including the boundaries !!!
        // here l = levels_[d];
      ...
      /**
       *  Having grid domain means that we can extract the d_axis arrays from each 1D domain separately...
       *
       */
      for (int d = 0; d < dim_; d++) {
        // as simple as ...
        std::vector<double> d_axis =
          gridDomain_->get1DDomain(d).axisScaling();
        bb_idx[d] = bb_left(k_idx[d], this->levels_[d], new_levels[d]);
  ...
  /***
   * PeTz: A standard binary search algorithm searching through the ordered double array "grid_line"
   * for the position of x in the ordering, i.e. effectively finds the index of the smallest interval containing x.
   *
   * @param x - the point we are trying to find the surrounding interval of
   * @param grid_line - the grid axis we are looking in
   * @return the index of the largest element of grid_line, which is smaller or equal to x.
   *
   * O( log(N)) operations ...
   *
   *

 ...
 *
 *
 * Simple class that encapsulates the combigrid relevant full grid data. Those
 *are:
...
class FGridContainer {
 private:
  /***
   * levels vector (specifying level depth for each dimension) for the
   * corresponding grid!
   */
  ...
  /**
   *
   * flag determining whether or not this grid values hsould be included in the
   *evaluation
   *
   */
  bool _active;
 ...
 public:
  void setCoef(_Tp newCoef) { _coefficient = newCoef; }

  /**
   * Returns the coefficient of the underlying fullgrid
   *
   */
  _Tp getCoef() { return _coefficient; }

  void setFGLevels(std::vector<int> levels) { _fullgrid_levels = levels; }

  /***
   * Returns the levels vector of this fullgrid
   */
  std::vector<int> getFGLevels() { return _fullgrid_levels; }
   ...
   *
   */
  FullGrid<_Tp>* fg() { return _fullgrid; }

  void activateGrid() { _active = true; }
  void deactivateGrid() { _active = false; }
  bool isActive() { return _active; }

  /**
   * FGridContainer constructor. No empty, default, constructors allowed!
   *
   * @param levels an integer list that specifies the hierarchical level depth
   *for each dimension.
   * @param hasBoundaryPts : a boolean list with flags specifying if a
   *particular dimension has boundary points included!
   * @param coef: well the combigrid coefficient
   * @param basis: the basis function for the underlying fullgrid! This
   *parameter has a default value NULL, in which case
  ...
  /**
   *
   * Allocate actual memory for the fullgrid elements! A simple call to

...
BOOST_AUTO_TEST_CASE(testL2LejaPoints) {
  ...
  //      0.994366,  0.174955,    0.793351,    0.00414731, 0.605696,  0.930801,  0.0569999,
  //      0.229032,
  ...
  //      0.630539,  0.361311,    0.000183025, 0.990173,   0.855379,  0.525842,  0.0684998,
  //      0.727994,
  ...
  //      0.0627104, 0.999946,    0.431229,    0.165108,   0.740011,  0.936467,  0.00568931,
  //      0.592392,
  //      0.294422,  0.883438,    0.0846337,   0.982736};

  for (size_t i = 0; i < 300; i++) {
    double x = l2leja.compute(i, i);
    //    std::cout << i << ": " << points[i] << " ~ " << x << " (err=" << std::abs(x - points[i])
    //    << ")"
    //              << std::endl;
    //    points.push_back(x);

...
#include "../src/sgpp/combigrid/operation/onedim/QuadratureEvaluator.hpp"
...
BOOST_AUTO_TEST_CASE(testIntegration) {
	...
	BOOST_CHECK_CLOSE(eval.getBasisCoefficients()[0].getValue(), 1.0,
			tolerance);
	...
	BOOST_CHECK_CLOSE(eval.getBasisCoefficients()[0].getValue(), 0.5,
			tolerance);
	BOOST_CHECK_CLOSE(eval.getBasisCoefficients()[1].getValue(), 0.5,
			tolerance);
}
...
void testIntegrationWithPolynom() {
	SGPP::combigrid::QuadratureEvaluator eval;
	 ...
	 BOOST_CHECK_CLOSE(eval.getBasisCoefficients()[0].getValue(), 1.0/6,
	 tolerance);
	 BOOST_CHECK_CLOSE(eval.getBasisCoefficients()[1].getValue(), 2.0/3,
	 tolerance);
	 BOOST_CHECK_CLOSE(eval.getBasisCoefficients()[2].getValue(), 1.0/6,
	 tolerance);

	 double accurate_solution = 1.0 / 3;

	 BOOST_CHECK_CLOSE(eval.eval(function_points).getValue(), accurate_solution,
	 tolerance);

...
OperationMatrixLTwoDotExplicitModifiedLinear::OperationMatrixLTwoDotExplicitModifiedLinear(sgpp::base::Grid* grid)
    : ownsMatrix_(true) {
        ...
        if(lik == 2.){
          ...
          if (std::max((iik - 1) / lik, (ijk - 1) / ljk) >=
            ...
            if (lik > ljk) {                            // Phi_i_k is the "smaller" ansatz function
              if(ljk -1 == int(ijk) || ijk == 1.){ //"larger" function is modified
                double ldiff = lik/ljk;
                if(lik -1 == int(iik) || iik == 1.){ //"smaller" function is modified too
              ...
              if(lik -1 == int(iik) || iik == 1.){ //"larger" function is modified
                double ldiff = ljk/lik;
                if(ljk -1 == int(ijk) || ijk == 1.){ //"smaller" function is modified too

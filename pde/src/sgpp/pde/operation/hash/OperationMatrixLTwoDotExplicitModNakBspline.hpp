 ...
 */
class OperationMatrixLTwoDotExplicitModNakBspline : public sgpp::base::OperationMatrix {
 public:
  /**
   * Constructor that only builds the object, without initialization
   */
  OperationMatrixLTwoDotExplicitModNakBspline();
  /**
   ...
   */
  OperationMatrixLTwoDotExplicitModNakBspline(sgpp::base::DataMatrix* m, sgpp::base::Grid* grid);
  /**
   ...
   * @param grid the sparse grid
   */
  explicit OperationMatrixLTwoDotExplicitModNakBspline(sgpp::base::Grid* grid);

  /**
   * Destructor
   */
  virtual ~OperationMatrixLTwoDotExplicitModNakBspline();
  ...
  inline void buildMatrixWithBounds(sgpp::base::DataMatrix* mat, sgpp::base::Grid* grid,
    ...
    const size_t p = dynamic_cast<sgpp::base::ModNakBsplineGrid*>(grid)->getDegree();
    const size_t pp1h = (p + 1) / 2;
    const double pp1hDbl = static_cast<double>(pp1h);
    const size_t quadOrder = p + 1;

    base::SNakBsplineModifiedBase& basis = const_cast<base::SNakBsplineModifiedBase&>(
        dynamic_cast<const base::SNakBsplineModifiedBase&>(grid->getBasis()));

 ...
 */
class OperationMatrixLTwoDotExplicitPolyClenshawCurtis : public sgpp::base::OperationMatrix {
 public:
  /**
   * Constructor that uses a external matrix pointer to construct the matrix,
   * i.e. matrix is NOT destroyed by the destructor of OperationMatrixLTwoDotExplicitPolyClenshawCurtisFullGrid
   *
   * @param m pointer to datamatrix of size (number of grid point) x (number of grid points)
   * @param grid the sparse grid
   */
  OperationMatrixLTwoDotExplicitPolyClenshawCurtis(sgpp::base::DataMatrix* m,
                                                   sgpp::base::Grid* grid);
  /**
   * Constructor that creates an own matrix
   * i.e. matrix is destroyed by the destructor of OperationMatrixLTwoDotExplicitPolyClenshawCurtisFullGrid
   *
   * @param grid the sparse grid
   */
  explicit OperationMatrixLTwoDotExplicitPolyClenshawCurtis(sgpp::base::Grid* grid);

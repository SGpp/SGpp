...
LaplaceEnhancedUpBBLinearBoundary::LaplaceEnhancedUpBBLinearBoundary(
  SGPP::base::GridStorage* storage) : LaplaceEnhancedUpBBLinear(storage) {
}

LaplaceEnhancedUpBBLinearBoundary::~LaplaceEnhancedUpBBLinearBoundary() {
}

void LaplaceEnhancedUpBBLinearBoundary::operator()(SGPP::base::DataMatrix&
...
void LaplaceEnhancedUpBBLinearBoundary::calcL2Boundary(float_t fl, float_t fr,
  ...
  if (this->boundingBox->hasDirichletBoundaryLeft(algo_dim))
    ...
    ptr_result_[(seq_left * this->numAlgoDims_) + dim] =  fl + ((
          1.0 / 6.0) * ptr_source_[(seq_right * this->numAlgoDims_) + dim] * q);
...
void LaplaceEnhancedUpBBLinearBoundary::calcGradBoundary(float_t fl, float_t fr,
    size_t seq_left, size_t seq_right, size_t dim, size_t algo_dim,
    float_t q_reci) {
  if (this->boundingBox->hasDirichletBoundaryLeft(algo_dim))
    ptr_result_[(seq_left * this->numAlgoDims_) + dim] = 0.0;
  else
    ptr_result_[(seq_left * this->numAlgoDims_) + dim] = ((-1.0 * q_reci) *
        ptr_source_[(seq_right * this->numAlgoDims_) + dim]);

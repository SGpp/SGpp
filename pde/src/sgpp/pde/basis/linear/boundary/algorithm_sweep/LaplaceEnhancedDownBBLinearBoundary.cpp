...
namespace SGPP {
  namespace pde {
    ...
    void LaplaceEnhancedDownBBLinearBoundary::calcL2Boundary(float_t fl, float_t fr, size_t seq_left, size_t seq_right, size_t dim, size_t algo_dim, float_t q) {
    ...
    void LaplaceEnhancedDownBBLinearBoundary::calcGradBoundary(float_t fl, float_t fr, size_t seq_left, size_t seq_right, size_t dim, size_t algo_dim, float_t q_reci) {
      if (this->boundingBox->hasDirichletBoundaryLeft(algo_dim))
        ptr_result_[(seq_left * this->numAlgoDims_) + dim] = 0.0;
      else

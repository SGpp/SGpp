...
#ifndef XDPHIPHIUPLINEARBOUNDARYUSCALED_HPP
#define XDPHIPHIUPLINEARBOUNDARYUSCALED_HPP
 ...
 */
class XdPhiPhiUpLinearBoundaryUScaled
{
protected:
...
public:
	 ...
	 */
	XdPhiPhiUpLinearBoundaryUScaled(GridStorage* storage) : storage(storage)
	{
	}
	 ...
	 * Destructor
	 */
	~XdPhiPhiUpLinearBoundaryUScaled()
	{
	}
...
protected:
	...
	void rec(DataVector& source, DataVector& result, grid_iterator& index, size_t dim, double& fl, double& fr)
	{
		...
		if (current_level > 0)
		{
			...
			double bhelp = (1/pow(2.0, static_cast<int>(current_level+1)));
			double i_dbl = static_cast<double>(current_index);

			// transposed operations:
			result[seq] = pow(2.0, static_cast<int>(current_level))*(fml - fmr);

			fl = fm/2.0 + (alpha_value*(bhelp*i_dbl - bhelp)) + fl;
			fr = fm/2.0 - (alpha_value*(bhelp*i_dbl + bhelp)) + fr;
		}
		else
		{
			size_t seq_left;
...
#endif /* XDPHIPHIUPLINEARBOUNDARYUSCALED_HPP */

...
OperationBIterativeSPAVXLinear::OperationBIterativeSPAVXLinear(GridStorage* storage) : storage(storage)
{
...
OperationBIterativeSPAVXLinear::~OperationBIterativeSPAVXLinear()
{
	delete Level;
	delete Index;
	delete myTimer;
...
void OperationBIterativeSPAVXLinear::rebuildLevelAndIndex()
{
	delete Level;
	delete Index;
...
double OperationBIterativeSPAVXLinear::multVectorized(DataVectorSP& alpha, DataMatrixSP& data, DataVectorSP& result)
{
    ...
    if (data.getNcols() % 8 != 0 || source_size != data.getNcols())
    {
    ...
    #pragma omp parallel
	{
		...
		if (chunksize % 8 != 0)
		{
			size_t remainder = chunksize % 8;
			size_t patch = 8 - remainder;
			chunksize += patch;
		}
...
double OperationBIterativeSPAVXLinear::multTransposeVectorized(DataVectorSP& alpha, DataMatrixSP& data, DataVectorSP& result)
{

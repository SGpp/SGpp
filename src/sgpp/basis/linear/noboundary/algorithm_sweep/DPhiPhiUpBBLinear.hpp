...
public:
	 ...
	 */
	DPhiPhiUpBBLinear(GridStorage* storage) : storage(storage), boundingBox(storage->getBoundingBox()), q(1.0), t(0.0)
	{
	}
...
protected:
	...
	void rec(DataVector& source, DataVector& result, grid_iterator& index, size_t dim, double& fl, double& fr)
	{
		...
		fl = (fm/2.0) + ((alpha_value*helper) + fl);
		fr = (fm/2.0) + ((alpha_value*((-1.0)*helper)) + fr);
	}

...
template<KernelType T, typename C> struct rebuild { rebuild(C *op);};
template<typename C> struct rebuild<Standard, C> { rebuild(C *op);};
template<typename C> struct rebuild<Mask, C> { rebuild(C *op);};

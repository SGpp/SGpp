...
#include "parallel/datadriven/application/LearnerBlockJacobi.hpp"
...
LearnerTiming LearnerBlockJacobi::train(sg::base::DataMatrix& trainDataset, sg::base::DataVector& classes,
  ...
  while (true) {
    ...
    if (rank == 0 && (admm_iteration < 100 || admm_iteration % 100 == 0)) std::cout << "iteration  " << admm_iteration << " new prediction error " << prediction_error << std::endl;

    if (prediction_error < convergence_threshold || prev_prediction_error == prediction_error) break;
  ...
  if (rank == 0) std::cout << "average iteration time " << (iterStopwatch->stop() / admm_iteration) << " seconds" << std::endl;

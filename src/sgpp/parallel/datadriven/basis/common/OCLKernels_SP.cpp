...
double OCLKernels::multTransSPOCL(float* ptrSource, float* ptrData, float* ptrLevel, float* ptrIndex, float* ptrGlobalResult, size_t sourceSize, size_t storageSize, size_t dims, size_t gpu_partition) {
...
#ifndef NO_OCL_OPTS
    err = clBuildProgram(program_multTransSP, 0, NULL, "-cl-finite-math-only -cl-fast-relaxed-math -cl-single-precision-constant", NULL, NULL);
#else
  ...
  for (size_t i = 0; i < num_devices; i++) {
    if (global[i] > 0) {
      err = clEnqueueNDRangeKernel(command_queue[i], kernel_multTransSP[i], 1, NULL, &(global[i]), &local, 0, NULL, &(clTimings[i]));
...
double OCLKernels::multSPOCL(float* ptrAlpha, float* ptrData, float* ptrLevel, float* ptrIndex, float* ptrResult, size_t result_size, size_t storageSize, size_t dims, size_t gpu_partition) {
  ...
  if (isFirstTimeMultSP) {
    ...
    // compiling the program
#ifndef NO_OCL_OPTS
    err = clBuildProgram(program_multSP, 0, NULL, "-cl-finite-math-only -cl-fast-relaxed-math -cl-single-precision-constant", NULL, NULL);
#else
  ...
  for (size_t i = 0; i < num_devices; i++) {
    ...
    if (global[i] > 0) {
      ...
      if ( clSetKernelArg(kernel_multSP[i], 0, sizeof(cl_mem), &clAlpha[i]) ||
           ...
           clSetKernelArg(kernel_multSP[i], 7, sizeof(cl_uint), &clOffsets[i]) ||
           clSetKernelArg(kernel_multSP[i], 8, sizeof(cl_uint), &clResultSize) != CL_SUCCESS) {
  ...
  for (size_t i = 0; i < num_devices; i++) {
    if (global[i] > 0) {
      err = clEnqueueNDRangeKernel(command_queue[i], kernel_multSP[i], 1, NULL, &(global[i]), &local, 0, NULL, &(clTimings[i]));

...
SPX86SimdLinearMultTranspose::SPX86SimdLinearMultTranspose(base::DataMatrixSP *level, base::DataMatrixSP *index, base::DataMatrixSP *dataset, base::DataVectorSP &source, base::DataVectorSP &result):
...
void SPX86SimdLinearMultTranspose::operator ()(size_t start_index_grid, size_t end_index_grid)
{
	float* ptrLevel = _level->getPointer();
	float* ptrIndex = _index->getPointer();
	float* ptrSource = _source.getPointer();
	...
	for(size_t k = start_index_grid; k < end; k+=std::min<size_t>(CHUNKGRIDPOINTS_SP_X86, (end-k)))
	{

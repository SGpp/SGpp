...
X86SimdLinearMultTranspose::X86SimdLinearMultTranspose(base::DataMatrix *level, base::DataMatrix *index, base::DataMatrix *dataset, base::DataVector &source, base::DataVector &result):
	chunkDataPoints(CHUNKDATAPOINTS_X86),
	chunkGridPoints(CHUNKGRIDPOINTS_X86),
	_level(level),
	_index(index),
	_dataset(dataset),
	_source(source),
	_result(result)
{
	if (this->_dataset->getNcols() % chunkDataPoints != 0 || _source.getSize() != this->_dataset->getNcols())
	{
...
void X86SimdLinearMultTranspose::operator ()(size_t start_index_grid, size_t end_index_grid)
{
	double* ptrLevel = _level->getPointer();
	double* ptrIndex = _index->getPointer();
	double* ptrSource = _source.getPointer();
	double* ptrData = _dataset->getPointer();
	double* ptrResult = _result.getPointer();

	size_t source_size = _source.getSize();
	size_t dims = _dataset->getNrows();

	size_t end = end_index_grid;

	for(size_t k = start_index_grid; k < end; k+=std::min<size_t>(CHUNKGRIDPOINTS_X86, (end-k)))
	{

...
X86SimdLinearMult::X86SimdLinearMult(sg::base::DataMatrix *level, sg::base::DataMatrix *index, sg::base::DataMatrix *dataset, sg::base::DataVector &alpha, sg::base::DataVector &result):
...
void X86SimdLinearMult::operator ()(size_t start_index_data, size_t end_index_data)
{
	double* ptrLevel = _level->getPointer();
	double* ptrIndex = _index->getPointer();
	double* ptrAlpha = _alpha.getPointer();
	...
	for(size_t c = start_index_data; c < end; c+=std::min<size_t>(CHUNKDATAPOINTS_X86, (end-c)))
	{

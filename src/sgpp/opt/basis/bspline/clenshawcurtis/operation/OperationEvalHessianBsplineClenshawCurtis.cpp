...
double OperationEvalHessianBsplineClenshawCurtis::evalHessian(
        base::DataVector &alpha, const std::vector<double> &point,
        base::DataVector &gradient, base::DataMatrix &hessian)
{
    ...
    hessian = base::DataMatrix(d, d);
    hessian.setAll(0.0);
    
    base::DataVector cur_gradient(d);
    base::DataMatrix cur_hessian(d, d);
    
    for (size_t i = 0; i < n; i++)
    {
        const base::GridIndex *gp = storage->get(i);
        double cur_val = 1.0;

...
void OperationMultipleEvalSubspaceSimple::multImpl(sg::base::DataVector &alpha, sg::base::DataVector &result,
...
#pragma omp barrier
	...
	for (size_t dataIndex = start_index_data; dataIndex < end_index_data; dataIndex++) {
		...
		while (levelIndex < maxIndex) {
			...
			std::cout << "hInversePtr: ";
			for (size_t i = 0; i < dim; i++) {
				if (i > 0)
					std::cout << ", ";
				std::cout << hInversePtr[i];
			}
			std::cout << " ";
			...
			if (!std::isnan(surplus)) {
				...
				for (size_t i = 0; i < dim; i++) {
					...
					if (i > 0)
						std::cout << " * ";
					std::cout << "phi_1d(" << dataTuplePtr[i] << ")";
				...
				std::cout << alpha[dataIndex] << std::endl;

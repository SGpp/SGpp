 ...
 */
void X86Accelerator::prepareSubspaceIterator() {
    ...
    for (size_t gridIndex = 0; gridIndex < this->storage->size(); gridIndex++) {
        ...
        uint32_t flatLevel = X86Accelerator::flattenLevel(this->dim, maxLevel, level);
        ...
        if (it == this->allLevelsIndexMap.end()) {
            ...
            this->allSubspaceNodes.emplace_back(X86AcceleratorSubspaceNode(level, flatLevel, maxIndex, index));

            X86AcceleratorSubspaceNode &subspace = this->allSubspaceNodes[this->subspaceCount];
    ...
    std::sort(this->allSubspaceNodes.begin(), this->allSubspaceNodes.end(),
            X86AcceleratorSubspaceNode::subspaceCompare);
...
//    }
    ...
    //this->allSubspaceNodes.emplace_back(X86AcceleratorSubspaceNode(this->dim, subspaceCount));
    ...
    for (size_t i = subspaceCount - 1; i > 0; i--) {
        X86AcceleratorSubspaceNode &currentNode = this->allSubspaceNodes[i];
        X86AcceleratorSubspaceNode &previousNode = this->allSubspaceNodes[i - 1];

        //number of dimension for which this subspace is responsible
        uint32_t recomputeComponentPreviousNode = X86AcceleratorSubspaceNode::compareLexicographically(currentNode,
                previousNode);
    ...
    X86AcceleratorSubspaceNode &firstNode = this->allSubspaceNodes[0];

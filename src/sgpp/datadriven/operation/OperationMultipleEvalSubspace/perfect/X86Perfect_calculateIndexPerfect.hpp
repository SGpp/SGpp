// static inline void calculateIndexPerfect(size_t dim, size_t nextIterationToRecalc, 
...
static inline void calculateIndexPerfect(size_t dim, size_t nextIterationToRecalc,
  ...
  //for (size_t outer = nextIterationToRecalc; outer < dim; outer += 4) {
    ...
    //for (size_t i = outer; i < min(outer + 4, dim); i++) {
      ...
      //flatten index
      //TODO: can be precomputed as array -> not only that, can all be replaced by a single shift h_i = 2^{l_i} -> actual points mult << l_i - 1
  ...
  //}
  // times 2 to support surplus-isLeaf tuples
  indexFlatReg = _mm_slli_epi32(indexFlatReg, 1);
...
/*
static inline void calculateIndexPerfect2(size_t dim, size_t nextIterationToRecalc, 
					     //rep
					     const double * const (&dataTuplePtr)[4], 
...
static inline void calculateIndexPerfect6(size_t dim, size_t nextIterationToRecalc, 
					     //rep
					     const double * const (&dataTuplePtr)[4], 
					     const double * const (&dataTuplePtr2)[4], 

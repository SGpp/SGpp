	   ...
	   if (hasDomain){
		   ...
		   for (i = 0 ; i < dim ; i++){
			...
			for (i = 0 ; i < dim ; i++){
				...
				if ( boundaryFlag[i] && ( axisIndexF[i] == fgFine->length(i)-1) ) { axisDiv[i] = 1.0; }
				if ( (!boundaryFlag[i]) && ( axisIndexF[i] == fgFine->length(i)-2) ) { axisDiv[i] = 1.0; }
				if ( (!boundaryFlag[i]) && ( axisIndexF[i] == fgFine->length(i)-1) ) { axisDiv[i] = 1.5; }
				if ( (!boundaryFlag[i]) && ( axisIndexF[i] <= 0 )) { axisDiv[i] = -0.5; }
				// limit the value
				axisDiv[i] = COMBIGRID_DMAX( COMBIGRID_DMIN(axisDiv[i],1.0) , 0.0);
			}
		}

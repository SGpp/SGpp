...
void SortedGridObjectContainer::addErrorObject(
		RefinementDecorator::index_type* index,
		dimensionType dimension,
		RefinementFunctor::value_type errorEstimateLeftChild,
		RefinementFunctor::value_type errorEstimateRightChild) {

	GridPointErrorContainer errorContainer(index,dimension,errorEstimateLeftChild,errorEstimateRightChild);
	//fill the pair with key and value;
	this->addErrorObject(errorContainer);
}

void SortedGridObjectContainer::addErrorObject(GridPointErrorContainer& container)
{
	//create a new key - value pair to add to the Sorted Grid.
	GridObjectErrorObject errorMapObject;
	//fill the pair with key and value;
	errorMapObject.first = container.largestErrorValue();
	errorMapObject.second = container;

	//add it to the error Map.	//add it to the error Map.
	gridObjectsByError.insert(errorMapObject);
}

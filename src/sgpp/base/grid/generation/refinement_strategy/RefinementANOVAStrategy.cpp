...
IndexDimension RefinementANOVAStrategy::createIndexDimensionItem(
        HashRefinementAbstract::index_type* index, size_t d) {
    IndexDimension item;
    HashRefinementAbstract::index_type i =
            new HashRefinementAbstract::index_type(index);
    item.d = d;
    item.index = &i;
    return item;
}
            ...
            for (size_t d = 0; d < storage->dim(); d++) {
                ...
                if (child_iter == end_iter) {
                    index_vector.push_back(
                            new HashRefinementAbstract::index_type(
                                    index.getPointer()));

...
#include "pde/basis/linear/noboundary/algorithm_sweep/LaplaceEnhancedDownBBLinear.hpp"
...
void LaplaceEnhancedDownBBLinear::rec_grad(size_t dim, grid_iterator& index)
{
    ...
    ptr_result_[(seq*this->numAlgoDims_) + dim] = (static_cast<double>(1<<(l+1)) * alpha_value);
...
#else
void LaplaceEnhancedDownBBLinear::rec_LL(double fl, double fr, double fl2, double fr2, size_t dim, grid_iterator& index)
#endif
{
    size_t seq = index.seq();
    sg::base::GridStorage::index_type::level_type l;
...
void LaplaceEnhancedDownBBLinear::rec_LG(double fl, double fr, size_t dim, grid_iterator& index)
{
    ...
    ptr_result_[(seq*this->numAlgoDims_) + dim+1] = (static_cast<double>(1<<(l+1)) * alpha_value2);
...
void LaplaceEnhancedDownBBLinear::recBB_LG(double fl, double fr, size_t dim, grid_iterator& index)
{
    ...
    ptr_result_[(seq*this->numAlgoDims_) + dim] = (((h * tmp_m) + (((2.0/3.0) * h) * alpha_value)) * q_);
    // Gradient in second dimension
    ptr_result_[(seq*this->numAlgoDims_) + dim+1] = ((static_cast<double>(1<<(l+1))/q_) * alpha_value2);
...
void LaplaceEnhancedDownBBLinear::recBB_GL(double fl, double fr, size_t dim, grid_iterator& index)
{
    ...
    double tmp_m = ((fl+fr)*0.5);
    // Gradient in second dimension
    ptr_result_[(seq*this->numAlgoDims_) + dim] = ((static_cast<double>(1<<(l+1))/q_) * alpha_value);
    // L2 scalar product

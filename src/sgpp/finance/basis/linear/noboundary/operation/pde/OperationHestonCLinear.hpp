...
/**
 * Implements the Heston C-Operation (corresponds to matrix C in Master's thesis), that is needed
 * the solve the multidimensional Heston
 * equation, on grids with fix Dirichlet-0-Boundaries.
 *
 * @version $HEAD$
 */
class OperationHestonCLinear : public sg::pde::UpDownOneOpDim
{
public:
	/**
	 * Constructor
	 ...
	 */
	OperationHestonCLinear(sg::base::GridStorage* storage, sg::base::DataVector& coef);

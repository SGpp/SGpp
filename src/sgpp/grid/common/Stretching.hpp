...
public:
	 ...
	 * @param BB BoundingBox to get the boundary values for Stretching
	 * @param type type of Stretching
	 */
	Stretching(size_t dim, DimensionBoundary* boundaries, Stretching1D* type);
...
struct Stretching1D{
	std::string type;
	double x_0,xsi;
	double lookup[LOOKUPSIZE];
};

*
 *
 * Stretching can be done in three different ways:
 *
 * 1. No Stretching operation is done
 * 2. Stretching is done via log/exp calculation
 * 3. Sinh Stretching
...
private:
	...
	void stretchingXform(Stretching1D& str1D,size_t dimension);


	 * The main Stretching function, overridden for levels greater than 10.
	 *
	 * Calculates the streched coordinate with respect to the given parameters
	 *Note to self: may take other than double, take care
	 * @param level level of the node
	...
	void parseVectorToLookupTable(std::vector<double> vec, Stretching1D &stretch1d, size_t dim);
...
public:
	...
	*
	...
	void getAdjacentPositions(int level, int index, size_t dimension, double & hl, double &hr);

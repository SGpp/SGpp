...
int main(int argc, char *argv[])
{
	...
	myGrid = new sg::LinearStretchedTrapezoidBoundaryGrid(dim);
	...
	myStretching->setBoundary(0, myInterval);

	std::cout << "Changed Stretching to: " << myInterval.leftBoundary << " to " << myInterval.rightBoundary << std::endl;*/
...
//				outfile1 << std::endl;
	...
	myUpDown1 = myGrid1->createOperationUpDownTest();
	myDelta1 = myGrid1->createOperationDelta(deltaCoef);
	myGamma1 = myGrid1->createOperationGamma(gammaCoef);
	myLaplace1 = myGrid1->createOperationLaplace();
...
/*	std::cout << "Do a Test multiplication:" << std::endl << std::endl;
	...
	myUpDown2 = myGrid2->createOperationUpDownTest();

...
do
	...
	do
		for grid in $GRIDS
		do
			STATSFILE="${TRAINDATA}.${grid}.level_${level}"
			...
			case "$grid" in
  				...
  				"border") python classifier.py --data $TRAINDATA --test $TESTDATA --level $level --zeh laplace --mode test --lambda $lambda --$grid -i 2000 --stats $STATSFILE;;
  			esac
		done
	done
done